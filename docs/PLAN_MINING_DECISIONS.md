...
- **Dependency text.** A Dependencies section never marks an existing edge as proposed, in either direction: a stale marker on an Unblocks entry lives on the parent's side of the edge and is just as wrong. Adding a proposed edge to the graph and correcting the sentence that says it does not exist are one action, not two. Each dependency claim also stands alone as its own bullet: a line that says both "used without an edge" and "blocked by" may be entirely true and still cannot be checked mechanically, because a reader or a script cannot tell which bead each clause is about. The stale "proposed" markers on existing edges were removed; fix any that remain.
...
- **An app bead never appears in a website bead's Unblocks list.** `am-app-native-payloads-bued` really does depend on `am-cm-schemas-argument-llm`, and `br` records that edge, but AGENTS.md rules that app work never delays a website batch and the `am-app-*` beads belong to the iPhone app plan and its own session. A website bead therefore lists only its website dependents in prose; the app plan owns the relationship from its side.
