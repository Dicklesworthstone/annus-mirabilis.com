...
- `s0-p9-s4`: "Wir können also setzen:" (deductive permission).
...
- *kinetische Energie* (kinetic energy): first used in `s0-p9-s2` ("...die kinetische Energie des Körpers...", denoted by $K$).
...
- $K$ denotes kinetic energy (`s0-p9-s2`, `s0-p9-s4`), NOT force (*Kraft*), NOT Boltzmann's constant.
...
- $C$ denotes an arbitrary additive constant in the relation between total energy difference $H - E$ and kinetic energy $K$ (`s0-p9-s3`, `s0-p9-s4`). It cancels exactly upon taking differences.
...
- $K_0, K_1$: kinetic energies before and after emission, first used in `s0-p9-s4`.
- $C$: arbitrary additive constant, first used in `s0-p9-s3`.
...
3. `energy-balances` (`s0-p7`, `s0-p8`, `eq-s0-d2`, `eq-s0-d3`):
...
4. `subtraction-kinetic-energy` (`s0-p8`, `s0-p9`, `s0-p10`, `eq-s0-d4`, `eq-s0-d5`, `eq-s0-d6`):
...
- **Page 639 (5 paragraphs, 8 sentences)**:
  ...
  - `s0-p3`: 1 sentence (`s0-p3-s1`, "Die Gesetze, nach denen sich die Zustände... (Relativitätsprinzip)."). Formatted as an indented block quotation in the original print.
