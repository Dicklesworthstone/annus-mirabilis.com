...
- **Scope and Layer Dispositions:**
  ...
  9. **Images and figures:** Authored diagrams and SVGs are licensed under MIT License with OpenAI/Anthropic Rider. Public-domain photographs (including Lucien Chavan's ca. 1905 portrait of Einstein from ETH-Bibliothek Zürich) are in the public domain and carry source credits. Note added 2026-09-24 by agent:NavyKite: ETH-Bibliothek Zürich's own record for this portrait (Bildarchiv, Portr_05937, doi:10.3932/ethz-a-000495740) gives the photographer as unknown, so the site credits the library's record and names Chavan only as an attribution.
...
- **Why this entry exists:** because the previous owner ruling in this area was never written down, and the orchestrator then cited it wrongly to two panes — see `D-2026-09-21-facsimile-text-layer-stays-forbidden`. Every owner decision is now written to `DECISIONS.md` and to a bead comment in the tick it is obtained, **and to `AGENTS.md` where the decision supersedes something `AGENTS.md` states** (`am-c6ca`, added 2026-09-22). That third destination is not redundancy. A decision recorded here and contradicted in `AGENTS.md` does not leave a gap, it leaves a **false authority**: `AGENTS.md` is the file every agent reads at session start, so the more carefully it is read the more confidently the superseded rule is followed. Measured that day: the OCR paragraph still granted reading the embedded text layer eighteen ticks after `D-2026-09-21` forbade it, and item 2 of "Product Shape & Tech Stack" still said "Restrained Tailwind" ten days after `D-2026-09-17` chose semantic CSS. **Amend in place, and say in the text that it said the opposite until the date** — a law file corrected without a trace gives the next reader no warning that anyone ever believed otherwise.
