...
## D-2026-09-17-voice-rules-theater-allowlist

- **Question:** The `theater` rule in `content/editorial/voice-rules.yaml` flags gamification and scoring vocabulary ("streak", "badge", "points", "grade", etc.). It fired on the word "points" regardless of phrase, causing false positives on legitimate scientific plotting and sampling terms like "data points" (e.g. `aria-label="Plot all data points"` in `TableToPlotBuilder.tsx`). Should technical phrases be allowlisted or should authors be required to rephrase?
- **Options:**
  - **1. Leave as is:** Require authors to rephrase technical terminology (e.g. "Plot every measurement in the table" instead of "Plot all data points").
  - **2. Add a phrase-level allowlist:** Allowlist technical phrases ("data points", "plot points", "sample points", "grid points" and their singular forms) in the `theater` rule while preserving detection of genuine gamification scoring uses ("earn points", "You scored 10 points", "collect points").
- **Choice:** **Option 2 (Phrase-level allowlist)**.
- **Reason:** Precision, not relaxation. "Data points", "plot points", "sample points", and "grid points" are standard technical mathematical and scientific vocabulary throughout the papers and interactive instruments. Forcing artificial rephrasing pushes prose away from natural technical terminology. The `allowlist` mechanism is already first-class in the voice engine (used identically by the `mockery` rule for "naive estimate").
- **Decider:** `agent:DustyIbis` implementing `am-a33s`.
- **Status:** RATIFIED & APPLIED 2026-09-17.
- **Evidence:** Planted negative test in `src/content/checks/voice/voice.test.ts` proving that genuine scoring uses ("earn points for each passage", "You scored 10 points", "collect points") continue to fail with error while technical plotting phrases pass cleanly.
- **Beads Resolved:** `am-a33s`.
- **Revisit Trigger:** If genuine gamification language bypasses the rule or additional legitimate scientific compound phrases with "points" are encountered.
