...
|---|---|---|---|---|---|---|---|---|
| `src/components/ui/LatexRenderer.tsx` | `src/equations/render/LatexRenderer.tsx` (`am-scaf-extract-ui-components-c31`) | KaTeX math rendering (inline, display, block) with error handling, accessible fallback, and HudText | Permissive trust callback; unescaped macro expansions; unrestricted protocol handlers. Restricted KaTeX trust callback rejecting \href, \url, \includegraphics, \htmlClass, \htmlId, \htmlStyle, \htmlData | `am-bm-slice-e2e-sbqu` | 1 | a8e7b0d | header | Restricts KaTeX trust callback to fail-closed behavior |
| `src/components/ui/colorPalette.ts` | `src/equations/colorPalette.ts` (`am-scaf-extract-ui-components-c31`) | Color palette tokens for mathematical expressions, syntax highlighting, and semantic equation roles | Donor patent color roles and styling classes; replaced with Annalen, Kramgasse Night, and Slate theme-aligned tokens | `am-bm-slice-e2e-sbqu` | 1 | a8e7b0d | header | All color combinations verified at WCAG AA contrast ratio (>= 4.5:1) |
| `src/components/ui/equationValueFormatting.ts` | `src/equations/valueFormatting.ts` (`am-scaf-extract-ui-components-c31`) | Dynamic numerical formatting, SI prefix scaling, and precision formatting for live equation values | Patent-specific parameter formats; unified with canonical physical dimensions | `am-bm-slice-e2e-sbqu` | 1 | a8e7b0d | header | Reusable live equation value formatter |
| `src/components/ui/ColorizedEquation.tsx` | `src/equations/legacy/ColorizedEquation.tsx` (`am-scaf-extract-ui-components-c31`) | Legacy AST-driven colorized equation component preserving token highlighting structure | Direct patent physics hooks, Lucide icons, and module-global telemetry maps; marked @deprecated | none | 2 | a8e7b0d | header | legacy; quarantined under src/equations/legacy/; forbidden from src/app/ and src/reader/ imports |
| `src/types/equation.ts` | `src/equations/legacy/equationTypes.ts` (`am-scaf-extract-ui-components-c31`) | Equation AST token interfaces, equation structure types, and derivation step definitions | Patent-specific claim references and parameter binding shapes; marked @deprecated | none | 2 | a8e7b0d | header | legacy; quarantined under src/equations/legacy/; forbidden from src/app/ and src/reader/ imports |
| `src/components/patents/PinnedPdfFacsimile.tsx` | `src/reader/facsimile/PinnedPdfFacsimile.tsx` (`am-scaf-extract-ui-components-c31`) | PDF facsimile viewer with canvas rendering, zoom controls, page jumping, and loading states | Patent-specific PDF download URLs and claim overlays; replaced with paper facsimile manifest bindings | `am-read-facsimile-face-er0` | 1 | a8e7b0d | header | Uses dynamic import of pdfjs to ensure initial route bundle isolation |
| `src/components/patents/usePinnedPdfFacsimile.ts` | `src/reader/facsimile/usePinnedPdfFacsimile.ts` (`am-scaf-extract-ui-components-c31`) | Facsimile viewport state management, page navigation, scale/zoom calculation, and canvas rendering loop | Patent facsimile route parameters; generalized to paper page maps | `am-read-facsimile-face-er0` | 1 | a8e7b0d | header | Dynamic pdfjs loading; accompanied by pure state machine in pinnedPdfFacsimileState.ts |
| `src/components/patents/pinnedPdfFacsimileState.ts` | `src/reader/facsimile/pinnedPdfFacsimileState.ts` (`am-scaf-extract-ui-components-c31`) | Pure reducer state machine, page clamping, and worker URL constants for facsimile viewing | Patent document dimensions and route parameter assumptions | `am-read-facsimile-face-er0` | 1 | a8e7b0d | header | Pure functional reducer extracted alongside hook |
| `public/pdfjs` | `public/pdfjs` (`am-scaf-extract-ui-components-c31`) | Vendored PDF.js library assets, worker script, and CMaps for local first-party PDF rendering without external CDN requests | None | `am-read-facsimile-face-er0` | 1 | a8e7b0d | NOTICE.md | Vendored under Apache-2.0 with NOTICE.md; zero external CDN or origin requests |
| `src/components/patents/visuals/three/ThreeStudioScene.ts` | `src/visuals/three/ThreeStudioScene.ts` (`am-scaf-extract-ui-components-c31`) | Direct Three.js scene management, camera setup, particle cloud rendering, animation loop, and resource disposal | Wright Flyer and patent flight-dynamics models; inlined deterministic LCG generator; decoupled from patent parameters | `am-inst-three-studio-y0y0` | 1 | a8e7b0d | header | Direct Three.js with clean disposal; dynamic import isolation |
| `src/components/patents/visuals/three/StudioKernelChips.tsx` | `src/visuals/three/StudioKernelChips.tsx` (`am-scaf-extract-ui-components-c31`) | Visual simulation kernel selector chips with active state indicators and telemetry tags | Patent-specific simulation kernels (aerodynamic, mechanical); updated with physical simulation kernels | `am-inst-three-studio-y0y0` | 1 | a8e7b0d | header | Accessible interactive kernel chips with keyboard and ARIA support |
| `src/components/layout/PatentSearchPalette.tsx` | `src/search/CommandPalette.tsx` (`am-scaf-extract-ui-components-c31`) | Global command palette with ⌘K / Ctrl+K keyboard shortcuts, search filtering, category grouping, and action execution | Patent claim and patent document search indices; stripped all donor identity strings; ready for build-time index | `am-scaf-extract-ui-components-c31` | 1 | a8e7b0d | header | Client-side dialog with focus management and keyboard shortcuts |
| `src/app/opengraph-image.tsx` | `src/app/_opengraph-image.tsx` (`am-scaf-extract-ui-components-c31`) | Dynamic Open Graph card generator with title, subtitle, and metadata badges using Next.js ImageResponse | Remote Google Fonts CDN fetching; donor patent title branding; uses local CSS styling and self-hosted fonts | `am-scaf-extract-ui-components-c31` | 1 | a8e7b0d | header | Generates OG image using local styles with zero third-party font requests |
| `src/components/patents/patentViewMode.ts` | `src/reader/viewMode.ts` (`am-scaf-extract-ui-components-c31`) | Pure `?view=` and `?panes=` URL search parameter parsing and serialization helpers | Decoupled from patent view states; adapted to Einstein paper reader faces (`reading`, `facsimile`, `translation`, etc.). Notice form is owner-blocked on Rider applicability to short rewrites | `am-read-shell-routes-3ua` | 1 | none | owner-blocked | Pure function rewrite; Rider applicability on short rewrites is an unresolved owner decision (reported as owner-blocked) |
| `src/components/patents/DualProjectionViewer.tsx` | None (`am-scaf-extract-ui-components-c31`, intentionally omitted) | None (interaction ideas adapted, monolith omitted) | Monolithic dual-projection viewer tightly coupled to patent claim IDs and synchronous physics; replaced by independent reader shell projections (`am-read-shell-routes-3ua`) | None | 1 | none | none | Monolith omitted; multi-panel projection decomposed into independent views. See Section 2.3 and Finding 4.5 |
| `src/components/patents/ArchaicGlossaryModal.tsx` | None (`am-scaf-extract-ui-components-c31`, intentionally omitted) | None (redesign input) | Replaced by section-scoped notation concordance (`am-not-concordance-model-uag`); donor used global archaic glossary modal keyed by spelling | None | 1 | none | none | Replaced by section-scoped notation concordance; spelling dictionary omitted. See Section 2.2 |
| `src/data/esotericPatentTerms.ts` | None (`am-scaf-extract-ui-components-c31`, intentionally omitted) | None (domain-specific donor data) | Patent-specific archaic legal and mechanical terms; replaced by section-scoped notation concordance (`am-not-concordance-model-uag`) | None | 1 | none | none | Domain-specific donor terminology left behind. See Section 2.2 |
| `src/components/patents/PatentClaimsList.tsx` | None (`am-scaf-extract-ui-components-c31`, intentionally omitted) | None (domain-specific donor features) | Patent claim dispute trees, patent classification systems, historical era sliders, audio narration, and broadside print mode | None | 1 | none | none | Domain-specific patent legal and presentation features left behind. See Section 2.3 and Section 6 |

#### `am-scaf-extract-runtime-utilities-99y`

| `src/physics/controlTape.ts` | `src/experiments/tape/controlTape.ts` (`am-scaf-extract-runtime-utilities-99y`) | Deterministic recording, scrubbing, checkpointing, and quantization of parameter streams (3600-tick bounded window) | Replaced patentId with instance-scoped experimentId; removed patent-specific control schemes | `am-bm-01-tracer-ensemble-hdly` | 1 | 31d51d0 | header | Pinned 32-bit seed coercion trap in characterization test |
| `src/physics/tickScheduler.ts` | `src/experiments/scheduler/tickScheduler.ts` (`am-scaf-extract-runtime-utilities-99y`) | Host-pumped time accumulation, studio clock state machine, fixed-timestep scheduler with bounded catch-up | Decoupled from global requestAnimationFrame assumptions and patent-specific frame limits | `am-bm-01-tracer-ensemble-hdly` | 1 | 31d51d0 | header | Pinned tick drop past maxCatchup trap in characterization test |
| `src/physics/transport.ts` | `src/workers/transport.ts` (`am-scaf-extract-runtime-utilities-99y`) | Bounded buffer pool with lease tracking and postMessage transport bus for worker offloading | SharedArrayBuffer / shared-memory mode disabled with typed error; removed patent-specific message envelopes | `am-bm-01-tracer-ensemble-hdly` | 1 | 31d51d0 | header | Pinned oldest-lease eviction trap and typed SharedMemoryDisabledError |
| `src/physics/paramAliases.ts` | `src/experiments/paramAliases.ts` (`am-scaf-extract-runtime-utilities-99y`) | Canonical parameter aliasing, expansion, and scaling transformations | Stripped all donor patent strings; keyed strictly by canonical quantity IDs | `am-bm-01-tracer-ensemble-hdly` | 1 | 31d51d0 | header | Pinned rejection of glyph/unit lookups in characterization test. Consumer: `am-linked-experiment-groups-5t5s` owns cross-instrument parameter identity and consumes this module as the alias-resolution seam; this extraction supplies the resolution shape only, no donor alias entries and no linkability policy, because two controls are not linkable because both are named $v$, both carry speed units, or both appear in the relativity paper. Particle velocity, mirror velocity, detector motion, and observer-frame velocity are physically different controls binding different canonical quantity IDs of `am-not-quantity-registry-2f7` (`particleVelocity`, `mirrorSpeed`, `frameSpeed` among them): a moving mirror is a setup of the world, a frame speed is an observer description of it, and linking the two would teach a reader that changing how you look at something moves a mirror. |
| `src/physics/genericWasm.ts` | `src/workers/genericWasm.ts` (`am-scaf-extract-runtime-utilities-99y`) | WASM module loader, fallback numerical kernels (heat, wave, fluid, laplacian, cyclic), source state tracking | Decoupled module loading state from per-snapshot execution provenance | `am-bm-01-tracer-ensemble-hdly` | 1 | 31d51d0 | header | Pinned module-global source trap in characterization test |
| `src/physics/useGenericWasmSource.ts` | `src/workers/useGenericWasmSource.ts` (`am-scaf-extract-runtime-utilities-99y`) | React hook subscribing to WASM engine loading status via useSyncExternalStore | Removed patent-specific visual dispatcher ties | `am-bm-01-tracer-ensemble-hdly` | 1 | 31d51d0 | header | Client hook for WASM source status |
...
| `src/physics/usePatentPhysics.ts` | None (`am-scaf-extract-runtime-utilities-99y`, intentionally omitted) | None (redesign input) | Replaced by instance-scoped snapshot stores (`am-rt-snapshot-store-aft`); donor used module-global maps keyed by patentId and a UI-event tick | None | 1 | none | none | Redesign input; never extracted. Replaced by instance-scoped snapshot stores. See Section 11.3 |
| `src/physics/telemetryData.ts` | None (`am-scaf-extract-runtime-utilities-99y`, intentionally omitted) | None (redesign input) | Replaced by instance-scoped snapshot stores (`am-rt-snapshot-store-aft`); donor used module-global UI tick counter and patent-scoped telemetry maps | None | 1 | none | none | Redesign input; never extracted. Replaced by instance-scoped snapshot stores. See Section 11.4 |
| `src/physics/lie.ts` | None (`am-scaf-extract-runtime-utilities-99y`, intentionally omitted) | None (domain-specific donor code) | Implements SO(3) aircraft rigid-body flight dynamics for Wright flyer; does not implement Lorentz boosts or relativistic velocity composition | None | 1 | none | none | Left behind; relativistic transformations belong in FrankenSim and paper-specific reference evaluators. See Section 11.4 |
