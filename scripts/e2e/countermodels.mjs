...
try {
  for (const width of [1440, 320]) {
    ...
    const errors = [], interactionRequests = [];
    ...
    try {
      ...
      await page.waitForFunction(() => document.querySelectorAll('[data-countermodel-case][data-ready="true"]').length === 2);
      ...
      const version = await a.getAttribute("data-snapshot-version"), run = await a.getAttribute("data-run-id");
      ...
      assert.ok((await a.locator('[data-candidate="galilean"] [data-candidate-conclusion]').innerText()).includes("None of the selected"));
      ...
      await input.fill("0.9"); await input.press("Enter");
      ...
      results.push({ width, passed: true, checks: ["computed-matrix", "keyboard-selection", "zero-toggle-evaluations", "observer-publication", "invalid-input-retention", "full-evidence", "independent-placements", "no-overflow", "no-interaction-network"] });
    } catch (error) {
