...
const base = { paper: "brownian-motion", section: "s5", lang: "en", terms: [], route: "/papers/brownian-motion/",
  anchor: "arg-bm-observable", face: "reading", scopeLabel: "Authored explanatory preview" };
const documents = [
  { ...base, id: "rms", type: "argument", title: "Mean square displacement", text: "How far a particle wanders" },
  { ...base, id: "spread", type: "equation", title: "Mean square model", text: "Compare the diffusion model", terms: ["λ_x"] },
  { ...base, id: "clock", type: "instrument", paper: "special-relativity", title: "Clock synchronization", text: "Compare distant clocks",
    route: "/lab/sr-01/", anchor: "", face: "" },
  { ...base, id: "html", type: "glossary", title: '<img src="x" onerror="alert(1)">', text: "Hostile fixture text, rendered literally" },
];
const bundle = packageSearchIndex(documents, [{ phrase: "clocks disagree", target: "clock", label: "search aid" }], "f".repeat(64), "scaffold");
const files = new Map([["/search/index-manifest.json", bundle.manifestText],
...
for (const name of ["core", "protocol", "loadIndex", "CommandPalette", "launcher"]) {
  ...
  files.set(`/modules/${name}.js`, compiled.outputText.replace(/(from\s*["']|import\(["'])(\.\/[^"']+)\.ts(["'])/gu, "$1$2.js$3"));
}
...
const server = createServer((request, response) => {
  ...
  if (value !== undefined) {
    const type = path.endsWith(".js") ? "text/javascript" : path.endsWith(".css") ? "text/css" : "application/json";
...
async function check(name, callback) { await callback(); checks.push({ name, outcome: "passed" }); }
try {
  const context = await browser.newContext({ viewport: { width: 1100, height: 900 } }); contexts.push(context);
  ...
  await check("lazy initial route", async () => {
    ...
    assert.ok(!requests.some((path) => path.includes("CommandPalette") || path.includes("loadIndex")));
  });
  await check("editable fields keep keyboard shortcuts", async () => {
    await page.locator("#unrelated").focus(); await page.keyboard.press("Control+k");
    assert.equal(await page.locator("dialog").count(), 0);
    await page.locator("#editable").focus(); await page.keyboard.press("Control+k");
    assert.equal(await page.locator("dialog").count(), 0);
  });
  await page.locator("#open").focus(); await page.keyboard.press("Control+k");
  const query = page.getByRole("combobox", { name: "Words, symbols, or a laboratory ID" });
  await query.waitFor(); await page.getByText("4 entries available.", { exact: false }).waitFor();
  await check("modal focus and result semantics", async () => {
    ...
    await query.fill("mean square"); await page.getByText("2 results shown.", { exact: true }).waitFor();
  ...
  await check("non-wrapping keyboard selection and stale-result clearing", async () => {
    ...
    await query.press("ArrowUp"); assert.equal(await query.getAttribute("aria-activedescendant"), first);
    await query.press("ArrowDown"); const last = await query.getAttribute("aria-activedescendant");
    assert.notEqual(first, last); await query.press("ArrowDown");
    assert.equal(await query.getAttribute("aria-activedescendant"), last);
    await query.fill("clocks disagree"); assert.equal(await query.getAttribute("aria-activedescendant"), null);
  ...
  await check("Escape returns focus and reopening uses the cached index", async () => {
    await query.press("Escape"); assert.equal(await page.locator("dialog").count(), 0);
    ...
    await page.locator("#open").click(); await query.waitFor();
    await query.fill("mean square"); await page.getByText("2 results shown.", { exact: true }).waitFor();
  ...
  await check("offline queries and paper filters", async () => {
    ...
    await query.fill("clocks disagree"); await page.getByText("1 result shown.", { exact: true }).waitFor();
  ...
  await check("untrusted titles stay text and queries never reach the network", async () => {
    await query.fill("hostile"); await page.getByText("1 result shown.", { exact: true }).waitFor();
    ...
    await query.fill("private-query-sentinel"); await page.getByText("No matching entries", { exact: false }).waitFor();
  ...
  await check("Enter follows the actual reading face and content anchor", async () => {
    await query.fill("mean square displacement"); await page.getByText("1 result shown.", { exact: true }).waitFor();
    await query.press("Enter"); await page.waitForURL("**/papers/brownian-motion/?view=reading#arg-bm-observable");
  ...
  await check("320px reflow, repeated close, and listener disposal", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await page.locator("#open").click(); await query.waitFor();
      await query.fill("mean square"); await page.getByText("2 results shown.", { exact: true }).waitFor();
      assert.equal(await page.locator("dialog").evaluate((node) => node.scrollWidth <= node.clientWidth + 1), true);
  ...
  await check("no-script fallback remains a real outline link", async () => {
    const nojs = await browser.newContext({ javaScriptEnabled: false }); contexts.push(nojs);
    const tab = await nojs.newPage(); await tab.goto(url);
  ...
  await check("corrupt shard produces a recoverable error, never partial results", async () => {
    const failureContext = await browser.newContext(); contexts.push(failureContext);
    const tab = await failureContext.newPage(); corrupt = true;
    await tab.goto(url); await tab.waitForFunction(() => window.fixtureReady); await tab.locator("#open").click();
    ...
    corrupt = false; await tab.getByRole("button", { name: "Retry loading search" }).click();
  ...
  console.log(JSON.stringify({ suite: "search-browser", browser: browser.version(), checks }, null, 2));
} finally {
  await writeFile(resolve(artifactDir, "search-browser-results.json"), JSON.stringify({ browser: browser.version(), checks }, null, 2));
