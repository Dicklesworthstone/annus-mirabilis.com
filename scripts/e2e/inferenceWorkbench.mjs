...
const url = new URL("/lab/what-can-you-infer/", process.env.BASE_URL ?? "http://127.0.0.1:3000").href;
...
try {
  for (const width of [1440, 320]) {
    ...
    try {
      ...
      const radius = page.locator('[data-infer-case="radius"]'), camera = page.locator('[data-infer-case="camera"]');
      const a = radius.locator("[data-infer-results]"), b = camera.locator("[data-infer-results]");
      const run = await radius.getAttribute("data-run-id"), digest = await radius.getAttribute("data-observation-digest");
      assert.equal(await a.locator('[data-output="molecularInterval"]').getAttribute("data-result-status"), "underdetermined");
      await radius.getByRole("button", { name: "Use the synthetic worked radius", exact: true }).click();
      assert.notEqual(await a.locator('[data-output="molecularInterval"]').getAttribute("data-lower"), null);
      ...
      await radius.getByRole("button", { name: "Apply independent radius", exact: true }).click();
      assert.equal(await a.locator('[data-output="molecularInterval"]').getAttribute("data-result-status"), "not-applicable");
      ...
      assert.notEqual(await b.locator('[data-output="diffusionEstimate"]').getAttribute("data-value"), null);
      assert.equal(await b.locator('[data-output="diffusionConfidenceInterval"]').getAttribute("data-result-status"), "not-applicable");
      await camera.getByLabel("Add variance at twice the spacing", { exact: true }).check();
      assert.notEqual(await b.locator('[data-output="secondVariance"]').getAttribute("data-value"), null);
      assert.equal(await b.locator('[data-output="sampleVariance"]').getAttribute("data-value"), variance);
      ...
      results.push({ width, passed: true, checks: ["fixed-data", "conditional-interval", "refusal-retention", "coverage-not-inflated", "covariance", "second-spacing", "keyboard", "table", "no-overflow", "no-interaction-network"] });
    } catch (error) {
  ...
  try {
    ...
    assert.equal(await page.getByRole("button", { name: "Apply independent radius", exact: true }).isDisabled(), true);
    ...
    assert.equal(await page.locator('[data-static-camera] [data-output="diffusionEstimate"]').isVisible(), true);
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
