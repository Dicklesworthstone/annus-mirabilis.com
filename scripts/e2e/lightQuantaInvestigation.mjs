...
try {
  for (const width of [1440, 320]) {
    ...
    try {
      ...
      await page.waitForFunction(() => document.querySelector("[data-light-investigation]")?.getAttribute("data-ready") === "true");
      const value = (quantity, column) => root.locator(`[data-comparison-quantity="${quantity}"] td`).nth(column).locator("[data-value]").getAttribute("data-value");
      const version = await root.getAttribute("data-snapshot-version");
      page.on("request", request => requests.push(request.url()));
      await root.getByLabel("Optional prediction: what will change, and why?").fill("Power changes rate, not the energy per emitted electron.");
      await root.getByLabel("Investigate independent radiation quanta as a further physical hypothesis.").check();
      assert.equal(await root.getAttribute("data-snapshot-version"), version);
      const oldEnergy = await value("maxKineticEnergy",1), oldRate=Number(await value("emissionRate",1));
      await root.getByRole("button",{name:"Double the accepted optical power",exact:true}).click();
      ...
      assert.equal(await root.locator("[data-changed-settings]").innerText(),"Changed settings: Incident optical power.");
      ...
      assert.equal(await root.getByRole("button",{name:"Double the accepted optical power",exact:true}).isDisabled(),true);
      ...
      assert.equal(await root.locator('[data-comparison-quantity="effectiveIndependentCount"] td').last().locator('[data-result-status="outside-domain"]').count(),1);
      assert.equal(await root.locator('[data-comparison-quantity="maxKineticEnergy"] td').last().locator('[data-result-status="value"]').count(),1);
      await root.getByRole("button",{name:"Restore worked settings",exact:true}).click();
      await root.getByLabel("Exit cost (eV)",{exact:true}).fill("6");
      await root.getByRole("button",{name:"Apply investigation settings",exact:true}).click();
      assert.equal(await root.locator('[data-comparison-quantity="maxKineticEnergy"] td').last().locator('[data-result-status="not-applicable"]').count(),1);
      await root.getByRole("button",{name:"Restore worked settings",exact:true}).click();
  ...
  try {
    ...
    assert.equal(await page.locator('[data-light-investigation]').getAttribute('data-execution-label'),'static');
    assert.equal(await page.locator('[data-comparison-quantity]').count(),12);
    assert.equal(await page.getByRole('button',{name:'Apply investigation settings',exact:true}).isDisabled(),true);
    await page.getByText('Read the explanations without making a prediction',{exact:true}).click();
    assert.equal(await page.getByText('Partial transfer would instead',{exact:false}).isVisible(),true);
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
