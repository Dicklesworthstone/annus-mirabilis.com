...
try {
  ...
  for (const item of EMBED_INSTRUMENTS) {
    errors.length = 0;
    const response = await page.goto(new URL(embedPath(item.id, { theme: "dark", detail: "steps", motion: "reduce" }), origin).href);
    ...
    const mounted = await page.locator('.embed-instrument [data-instrument-id]').evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-instrument-id")));
    assert.ok(mounted.every((id) => id === item.id), `${item.id}: mounted the wrong instrument`);
    ...
    assert.ok((await page.locator('meta[name="robots"]').first().getAttribute("content")).includes("noindex"));
    assert.equal(await page.getByRole("link", { name: "Read the source context in a new tab" }).getAttribute("href"), item.source);
    assert.ok((await page.locator(".embed-instrument").innerText()).length > 80);
    assert.deepEqual(errors, [], `${item.id}: runtime error`);
    console.log(`PASS ${item.id}: real owner view, attribution, presentation and metadata`); passed++;
  }
  ...
  console.log("PASS malformed configuration has visible recovery, not a substituted experiment"); passed++;
  ...
  assert.equal(await page.locator("iframe").count(), 0);
  console.log("PASS builder stages preview changes and never auto-loads an iframe"); passed++;
  ...
  assert.equal(await page.locator("iframe").count(), 0);
  console.log("PASS unknown builder selection cannot produce a plausible wrong embed"); passed++;
  ...
  await page.evaluate((paths) => {
    for (const [name, src] of paths) { const frame = document.createElement("iframe"); frame.name = name; frame.src = src; document.body.append(frame); }
  }, [["left-test", embedPath("sr-04", { ...DEFAULT_EMBED_OPTIONS, theme: "dark" })], ["right-test", embedPath("sr-04", { ...DEFAULT_EMBED_OPTIONS, theme: "light" })]]);
  await page.frameLocator('iframe[name="left-test"]').locator('[data-embed-ready="true"]').waitFor();
  await page.frameLocator('iframe[name="right-test"]').locator('[data-embed-ready="true"]').waitFor();
  const left = page.frameLocator('iframe[name="left-test"]');
  const right = page.frameLocator('iframe[name="right-test"]');
  const untouched = await right.locator('.embed-instrument input[type="number"]').first().inputValue();
  await left.locator('.embed-instrument input[type="number"]').first().fill("0.2");
  assert.equal(await right.locator('.embed-instrument input[type="number"]').first().inputValue(), untouched);
  ...
  console.log("PASS independent frame controls and presentation"); passed++;
  await context.close();
  ...
  for (const item of EMBED_INSTRUMENTS) {
    ...
    assert.ok(await book.locator('.embed-instrument [data-instrument-id]').count() > 0, `${item.id}: no server-rendered example`);
    ...
    console.log(`PASS ${item.id}: no-JavaScript example and explanations`); passed++;
  }
  await noJs.close();
