...
try {
  for (const width of [1280, 320]) {
    ...
    const ratio = async (id) =>
      Number(
        await root.locator(`[data-comparison-output="${id}"] [data-comparison-ratio]`).innerText(),
      );
    async function idle() {
      await root.locator('button:has-text("Apply one change")').waitFor({ state: "visible" });
    ...
    try {
      ...
      assert.equal(await ratio("diffusionCoefficient"), 0.5);
      assert.equal(await ratio("rmsDisplacement1d"), 0.70711);
      ...
      await idle();
      assert.equal(await ratio("diffusionCoefficient"), 0.5);
      assert.equal(await ratio("sampleRms"), 0.70711);
      const previous = await root
        .locator("[data-comparison-results]")
        .getAttribute("data-variant-snapshot");
      ...
      assert.equal(
        await root.locator("[data-comparison-results]").getAttribute("data-variant-snapshot"),
        previous,
      );
      await root
        .getByRole("button", { name: "Pin current result as new baseline", exact: true })
        .click();
      await root.getByLabel("Input to vary", { exact: true }).selectOption("interval");
      await root
        .getByRole("button", { name: "Observe at four times the baseline interval", exact: true })
        .click();
      await idle();
      ...
      assert.equal(
        await pair.getAttribute("data-baseline-run-id"),
        await pair.getAttribute("data-variant-run-id"),
      );
      const points = await root
        .locator("polyline")
      ...
      await input.fill("0.013");
      await root.getByRole("button", { name: "Apply one change", exact: true }).click();
      await idle();
      ...
      await page.screenshot({
        path: resolve(directory, `comparison-${width}.png`),
        fullPage: true,
      });
      results.push({
        width,
  ...
  try {
    ...
    assert.equal(
      await root.getByRole("button", { name: "Start live comparison" }).isDisabled(),
      true,
    );
