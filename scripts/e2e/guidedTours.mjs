...
for (const tour of GUIDED_TOURS) {
  test(`${tour.id}: follow every real destination and finish from the last stop`, {
    timeout: 180000,
  }, async () => {
    ...
    try {
      ...
      for (const [index, stop] of tour.stops.entries()) {
        ...
        const navigation = page.getByRole("navigation", {
          name: "Continue guided reading after this page",
          exact: true,
        });
        await navigation
          .getByRole("link", {
            name: index < tour.stops.length - 1 ? "Next stop" : "Finish this route",
            exact: true,
          })
          .click();
      }
  ...
  test(`${tour.id}: the entire outline and native disclosures work without JavaScript`, async () => {
    ...
    try {
      ...
      for (const stop of tour.stops) {
        ...
        const destination = entry.getByRole("link", {
          name: `Open this stop: ${stop.title}`,
          exact: true,
        });
        assert.equal(
          await destination.getAttribute("href"),
          tourDestination(tourPosition(tour, stop)),
        );
      }
...
test("leaving a guide preserves the open page and non-tour parameters without reloading", async () => {
  ...
  try {
    await page.goto(
      `${base}/lab/me-01/?tour=mass-energy&tourStop=ledgers&tourRevision=1&readerOption=keep#main`,
    );
    const guide = page.locator('[data-guided-tour="mass-energy"]');
    await guide.waitFor();
    await page.evaluate(() => {
      window.__tourVisitToken = "must-survive";
    });
...
test("an invalid revision shows recovery rather than replacing the experiment", async () => {
  ...
  try {
    assert.equal(
      (
        await page.goto(`${base}/lab/me-01/?tour=mass-energy&tourStop=ledgers&tourRevision=999`)
      ).status(),
      200,
    );
...
test("a valid position on a different page offers return instead of advancing", async () => {
  ...
  try {
    ...
    const href = await detour
      .getByRole("link", { name: "Return to Write the same emission twice", exact: true })
      .getAttribute("href");
