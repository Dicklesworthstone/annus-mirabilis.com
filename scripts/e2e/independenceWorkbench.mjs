  ...
  try {
    ...
    if (javaScriptEnabled)
      await page.waitForFunction(
        () => document.querySelector(".occupancy-workbench fieldset")?.disabled === false,
      );
    await run(page);
...
test("mean alone is insufficient; adding variance distinguishes predictions", async () =>
  visit(async (page) => {
...
test("a one-point experiment cannot test dependence between points", async () =>
  visit(async (page) => {
    ...
    assert.equal(
      await page
        .getByRole("region", { name: "Count probability distributions", exact: true })
        .locator("tbody tr")
        .count(),
      2,
    );
...
test("constructed examples distinguish an impossible event from a likelihood preference", async () =>
  visit(async (page) => {
    await openRecord(page);
    await page.getByRole("button", { name: "Illustration: mixed counts", exact: true }).click();
    assert.equal(
      await page.locator("[data-evidence-status]").getAttribute("data-evidence-status"),
      "independent-only",
    );
    await page
      .getByRole("button", { name: "Illustration: all-or-none counts", exact: true })
      .click();
    assert.equal(
      await page.locator("[data-evidence-status]").getAttribute("data-evidence-status"),
      "both-possible",
    ...
    assert.match(
      await page.locator("[data-evidence-status]").innerText(),
      /not a probability that either model is true/u,
    );
  }));

test("a malformed record leaves the accepted record intact", async () =>
  visit(async (page) => {
    await openRecord(page);
    await page.getByRole("button", { name: "Illustration: mixed counts", exact: true }).click();
...
test("unapplied settings cannot relabel evidence; accepting changed settings clears it", async () =>
  visit(async (page) => {
    await openRecord(page);
    await page.getByRole("button", { name: "Illustration: mixed counts", exact: true }).click();
...
test("accepted-settings links exclude private explanations and count records", async () =>
  visit(async (page) => {
    await openRecord(page);
    await page.getByRole("button", { name: "Illustration: mixed counts", exact: true }).click();
    ...
    const link = new URL(
      await page.getByLabel("Accepted-settings link", { exact: true }).inputValue(),
    );
    assert.deepEqual([...link.searchParams.keys()].sort(), ["checks", "n", "occupancy", "q"]);
    assert.ok(!link.href.includes("private-reader-explanation"));
  }));

test("shared settings require explicit acceptance before replacing the worked example", async () =>
  visit(
    async (page) => {
      assert.equal(
        await page.getByLabel("Number of labeled points", { exact: true }).inputValue(),
        "4",
      );
      ...
      assert.equal(
        await page.getByLabel("Fraction of the volume", { exact: true }).inputValue(),
        "3",
      );
      assert.equal(await page.locator("[data-evidence-status]").count(), 0);
    },
...
test("an unsupported link has recovery and keeps the default results", async () =>
  visit(
    async (page) => {
      assert.equal(
        await page.getByLabel("Number of labeled points", { exact: true }).inputValue(),
      ...
      assert.equal(
        await page
          .getByRole("link", { name: "Open the clean example", exact: true })
          .getAttribute("href"),
        route.pathname,
      );
      assert.equal(await outcome(page), "underdetermined");
...
test("the no-JavaScript page retains the worked statistics and probability table", async () =>
  visit(
    async (page) => {
      assert.match(
        await page.locator("[data-occupancy-workbench] noscript").innerText(),
        /full worked comparison/u,
      );
      assert.equal(
        await page
          .getByRole("region", { name: "Model prediction table", exact: true })
          .locator("tbody tr")
          .count(),
        4,
      );
      ...
      assert.equal(
        await page.getByRole("button", { name: "Apply model settings", exact: true }).isDisabled(),
        true,
      );
    },
