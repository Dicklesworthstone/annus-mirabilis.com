...
test("real prepared observations generate deterministically through the real store",async()=>{
 ...
 assert.deepEqual(a,b);assert.equal(a.examples.length,1);
 assert.equal(a.sourceDigest,hashModuleClosure(root,["src/reasoning/infer/session.ts","scripts/generate-inference-workbench.mjs"]));
 for(const evidence of Object.values(a.examples[0])) {
  const {kind,dt,d,exposure,knownDrift,increments}=evidence;
  assert.equal(evidence.observationDigest,createHash("sha256").update(JSON.stringify({kind,dt,d,exposure,knownDrift,increments})).digest("hex"));
 }
});
test("publication profiles exclude the instructional datasets and unknown profiles fail",async()=>{
 for(const profile of ["preview","launch"]){const result=await generateInferenceWorkbench(root,profile,scratch);assert.deepEqual(result.examples,[]);}
