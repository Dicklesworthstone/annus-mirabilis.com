...
import { SR09_DEFAULTS } from "../src/experiments/sr09/definition.ts";
import { snapshotOutputs } from "../src/experiments/sr09/session.ts";
...
export async function evaluatorSources() {
  ...
  await visit("src/experiments/sr09/session.ts");
  seen.set(
    "scripts/generate-sr09.mjs",
    await readFile(resolve(root, "scripts/generate-sr09.mjs"), "utf8"),
  );
  return [...seen].sort(([a], [b]) => a.localeCompare(b, "en"));
}

export async function generateSr09() {
  ...
  const atDefault = snapshotOutputs(SR09_DEFAULTS);
  const atApproaching = snapshotOutputs({ ...SR09_DEFAULTS, propagationAngleDeg: 180 });
  const example = {
    sourceDigest,
  ...
  await writeFile(
    resolve(root, "src/generated/sr09-example.json"),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  console.log(JSON.stringify(await generateSr09()));
