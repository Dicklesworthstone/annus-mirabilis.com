...
try {
  ...
  assert.equal(
    Number(
      await entrance
        .locator('[data-encounter-live] [data-owner-id="massEnergy.subtractionDifference"]')
  ...
  assert.equal(
    await entrance
      .locator("[data-encounter-live] [data-kinetic-status]")
      .getAttribute("data-kinetic-status"),
    "underdetermined",
  );
  ...
  check(
    "the published second paper has eight passages, semantic math and an operable conditional subtraction",
  );
  ...
  check(
    "320-pixel reading has no page overflow and the entrance passes the automated accessibility audit",
  );

  assert.match(
    await entrance.getByRole("link", { name: /Less guidance:/ }).getAttribute("href"),
    /premise=relaxed/,
  );
  assert.equal(
    await entrance.getByRole("link", { name: /Continue with the slow traveler/ }).count(),
    0,
  );
  ...
  assert.match(
    await page.locator('[data-instrument-id="me-02"] [role="alert"]').innerText(),
    /unchanged/,
  );
  ...
  assert.match(
    await staticEntrance.locator(".me-static-solution").innerText(),
    /0\.0005000375031 J/,
  );
  assert.ok(
    await staticEntrance
      .getByRole("button", { name: "Align the accounts", exact: true })
      .isDisabled(),
  );
  assert.ok(
  ...
  check(
    "both worked examples and the complete explanatory paper remain available without JavaScript",
  );
  await noJs.close();
} catch (error) {
  evidence.push({
  ...
  await writeFile(
    "artifacts/browser/mass-energy-checks.json",
