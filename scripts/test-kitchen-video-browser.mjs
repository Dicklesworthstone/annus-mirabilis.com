...
import { VIDEO_FIXTURE_BASE64, VIDEO_FIXTURE_SHA256 } from "../src/testing/fixtures/kitchen/videoFixture.mjs";
...
const root = resolve("out"), evidence = [];
const types = { ".html":"text/html", ".js":"application/javascript", ".css":"text/css", ".json":"application/json", ".woff2":"font/woff2" };
const server = createServer(async (req,res) => {
  try {
    ...
    res.setHeader("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data:; media-src 'self' blob:; worker-src 'self'; connect-src 'self'");
...
try {
  ...
  await ui.getByLabel("Local video",{exact:true}).setInputFiles({name:"private-test-chart.mp4",mimeType:"video/mp4",buffer:bytes});
  ...
  assert.match(await ui.locator("[data-captured-frame]").innerText(),/actual time 0 s.*frame-callback/);
  assert.equal(workers,0);
  check("a selected blob video decodes locally with actual frame timing and no inference worker yet");
  ...
  for(let i=0;i<6;i++) {
    if(i) {
      ...
      await page.waitForFunction(t=>document.querySelector("[data-captured-frame]")?.textContent.includes(`actual time ${t} s`),i*.5);
    }
  ...
  assert.ok(!csv.includes("private-test-chart") && !csv.includes("blob:") && !csv.includes(VIDEO_FIXTURE_BASE64));
  ...
  await page.waitForFunction(()=>!!document.querySelector('[data-instrument-id="bm-07-kitchen"]')?.getAttribute("data-document-digest"));
  assert.ok(workers>0);
  ...
  check("CSV exports preserve capture provenance and failed declarations cannot replace accepted analysis");
  await ui.getByRole("button",{name:"Stop and release video; keep annotations",exact:true}).click();
  assert.equal(await ui.locator("video").getAttribute("src"),null);
  assert.equal(await lab.getAttribute("data-document-digest"),digest);
  assert.match(await ui.getByText(/22 observations/).innerText(),/22 observations/);
  assert.equal(await ui.getByRole("button",{name:"Download raw annotations JSON",exact:true}).isEnabled(),true);
  assert.ok(requests.every(r=>r.method==="GET" && (r.url.startsWith(origin)||r.url.startsWith("blob:"))));
  assert.deepEqual(errors,[]);
  await page.setViewportSize({width:320,height:900});
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
  await page.screenshot({path:"artifacts/browser/kitchen-video-320.png",fullPage:true});
  check("releasing media preserves annotations and accepted results, with no uploads or narrow-screen overflow");
  await page.close();
} finally {
