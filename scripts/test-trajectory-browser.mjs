...
export async function checkTrajectoryBrowser(browser, url, check) {
  ...
  try {
    ...
    assert.ok(
      await page
        .getByRole("button", { name: "Apply observations and assumptions", exact: true })
        .isDisabled(),
    );
    assert.ok(
      await page
        .getByRole("heading", { name: "An interval is not an authenticity certificate" })
        .isVisible(),
    );
  ...
  try {
    ...
    const apply = lab.getByRole("button", {
      name: "Apply observations and assumptions",
      exact: true,
    });
    ...
    assert.equal(
      await lab.locator('[data-accepted-run="1"]').getAttribute("data-result-status"),
      "unavailable",
    );
    ...
    await lab
      .getByLabel("I am explicitly assuming independent, isotropic Gaussian increments.", {
        exact: true,
      })
      .check();
    ...
    await lab
      .getByLabel("Were observations selected, censored or motion-filtered?", { exact: true })
      .selectOption("no");
    await lab
      .getByLabel("Estimator", { exact: true })
      .selectOption("independent-increment-known-zero-drift");
    await apply.click();
    ...
    const report = JSON.parse(
      await downloadText(page, "Download accepted analysis and observations (JSON)"),
    );
    ...
    check(
      "download and refused edits retain accepted observations, parameters and results together",
    );
    ...
    await lab.getByLabel("Local CSV file (maximum 256 KiB)", { exact: true }).setInputFiles({
      name: "trajectory-arithmetic-fixture.csv",
      mimeType: "text/csv",
    ...
    check(
      "all track positions remain inspectable and exportable without selecting or resampling inference data",
    );
    ...
    await page.screenshot({
      path: "artifacts/browser/trajectory-accepted-320.png",
      fullPage: true,
    });
    assert.deepEqual(dataTransfers, []);
    assert.deepEqual(errors, []);
    check(
      "invalid UTF-8 preserves accepted data; the 320px reduced-motion route sends no data requests",
    );
    ...
    try {
      await page.screenshot({ path: "artifacts/browser/trajectory-failure.png", fullPage: true });
    } catch {
      /* Preserve the original acceptance failure even if capture fails. */
    }
    throw error;
  } finally {
    await context.close();
