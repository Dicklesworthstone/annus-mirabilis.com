...
test('actual KaTeX emits both selected subtrees and MathML reproducibly',async()=>{
 ...
 assert.deepEqual(first,second);assert.equal(first.lessons.length,1);
 for(const step of first.lessons[0].steps)for(const html of [step.fromHtml,step.toHtml]) {
  assert.ok(html.includes('<math'));assert.ok(!html.includes('katex-error'));
  for(const id of step.changed)assert.ok(html.includes(`data-expression-id="${id}"`));
 }
 assert.deepEqual(first.lessons[0].cases.map(c=>c.result.meanSquare),[2,4,0]);
});
test('preview and launch do not promote instructional drafts',async()=>{
 for(const profile of ['preview','launch'])assert.equal((await generateMissingSteps(root,profile,output)).lessons.length,0);
});
test('an unknown publication profile fails rather than exposing the draft',async()=>{
 await assert.rejects(()=>generateMissingSteps(root,'unreviewed-public',output),/Unknown missing-step publication profile/);
});
