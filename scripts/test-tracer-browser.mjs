...
export async function checkTracerBrowser(browser, url, check) {
  ...
  try {
    ...
    assert.equal(workers, 0); assert.deepEqual(errors, []);
    ...
    async function accepted(action, target = lab) {
      ...
      await page.waitForFunction(({id, before}) => {
        const node = [...document.querySelectorAll('[data-instrument-id="bm-01"]')].find(n => n.dataset.instanceId === id);
        return node?.dataset.pending === "false" && Number(node.dataset.snapshotVersion) > before;
      }, {id, before}, {timeout:15000});
    }
    await lab.locator('[name="M"]').fill("40"); await lab.locator('[name="H"]').fill("4");
    ...
    const run = await lab.getAttribute("data-run-id"), draws = await lab.getAttribute("data-recording-draws");
    ...
    const versions = await lab.locator("[data-snapshot-version]").evaluateAll(nodes => nodes.map(n => n.dataset.snapshotVersion));
    ...
    await accepted(() => lab.getByRole("button", { name: "Observe at 4 seconds", exact: true }).click());
    assert.equal(await lab.getAttribute("data-run-id"), run); assert.equal(await lab.getAttribute("data-recording-draws"), draws);
    ...
    await lab.locator('[name="d"]').selectOption("3"); await lab.locator('[name="axis"]').selectOption("2");
    await lab.locator('[name="statistic"]').selectOption("apparent"); await accepted(() => apply.click());
    assert.equal(await lab.getAttribute("data-run-id"), run); assert.equal(await lab.getAttribute("data-recording-draws"), draws);
    await lab.locator('[name="d"]').selectOption("1"); await lab.locator('[name="axis"]').selectOption("0");
    await lab.locator('[name="interval"]').fill("1"); await accepted(() => apply.click());
    ...
    assert.equal(await lab.getAttribute("data-snapshot-version"), zoomVersion); assert.equal(await lab.getAttribute("data-recording-draws"), draws);
    assert.equal(await lab.locator('[data-output="sampleMean"]').innerText(), originalMean);
    ...
    check("BM-01: off-grid observation is refused without relabeling the accepted trial, and repair works");

    await lab.locator('[name="interval"]').fill("0"); await accepted(() => apply.click());
    assert.match(await lab.locator('[data-output="apparentSpeed"]').innerText(), /positive interval/);
    ...
    await lab.locator('[name="M"]').fill("1"); await accepted(() => apply.click());
    ...
    await lab.locator('[name="M"]').fill("40"); await lab.locator('[name="interval"]').fill("1");
    await lab.locator('[name="seed"]').fill("9007199254740992"); await accepted(() => apply.click());
    ...
    await lab.locator('[name="seed"]').fill("01"); await apply.click();
    await lab.getByRole("alert").waitFor(); assert.equal(await lab.locator(".accepted-caption").innerText(), acceptedSeed);
    await lab.getByRole("button", { name: "Share accepted trial", exact: true }).click();
    const link = await lab.getByLabel("Accepted trial link").inputValue();
    assert.equal(new URL(link).searchParams.get("seed"), "9007199254740992");
    const shared = await context.newPage(); let sharedWorkers = 0; shared.on("worker", () => sharedWorkers++);
    await shared.goto(link); await shared.getByText("Shared settings are loaded as a draft.", {exact:false}).waitFor();
    assert.equal(await shared.locator('[name="seed"]').inputValue(), "9007199254740992");
    assert.match(await shared.locator(".accepted-caption").innerText(), /seed 1905;/); assert.equal(sharedWorkers,0); await shared.close();
    check("BM-01: neighboring large seeds remain distinct and shared links use accepted settings without autoplay");
    ...
    await second.locator('[name="M"]').fill("20"); await second.locator('[name="H"]').fill("2");
    await second.locator('[name="seed"]').fill("42");
    const firstBefore = await lab.locator(".accepted-caption").innerText();
    await accepted(() => second.getByRole("button", {name:"Apply trial settings",exact:true}).click(), second);
    assert.equal(workers,2); assert.equal(await lab.locator(".accepted-caption").innerText(),firstBefore);
    assert.notEqual(await second.getAttribute("data-instance-id"), await lab.getAttribute("data-instance-id"));
    await page.getByRole("button", {name:"Close the second tracer ensemble",exact:true}).click();
    check("BM-01: separate instances own separate workers and cannot overwrite each other's trials");

    const audit = await new AxeBuilder({page}).include("#main").withTags(["wcag2a","wcag2aa","wcag21aa"]).analyze();
    assert.deepEqual(audit.violations.filter(v=>v.impact==="serious" || v.impact==="critical").map(v=>v.id),[]);
    check("BM-01: automated accessibility supplement",{violations:audit.violations.map(v=>v.id),manualScreenReaderReview:"not performed"});
    await page.setViewportSize({width:320,height:900});
    assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
    await page.screenshot({path:"artifacts/browser/tracers-320.png",fullPage:true});
    assert.deepEqual(errors,[]); check("BM-01: interactive 320px reflow and no uncaught browser errors");
  } catch (error) {
    await page.screenshot({path:"artifacts/browser/tracers-failure.png",fullPage:true});
    console.error("Tracer browser errors:",errors); throw error;
