...
export async function evaluatorSources() {
  ...
  await visit("src/workers/host/sr03Host.ts");
  seen.set(
    "scripts/generate-sr03.mjs",
    await readFile(resolve(root, "scripts/generate-sr03.mjs"), "utf8"),
  );
  return [...seen].sort(([a], [b]) => a.localeCompare(b, "en"));
}

export async function generateSr03() {
  ...
  await writeFile(
    resolve(root, "src/generated/sr03-example.json"),
    `${JSON.stringify(example, null, 2)}\n`,
  );
  await writeFile(
    resolve(root, "src/generated/sr03-provenance.ts"),
    `// Generated by scripts/generate-sr03.mjs; do not edit.\nexport const SR03_SOURCE_DIGEST = ${JSON.stringify(sourceDigest)};\n`,
  );
  await writeFile(
    resolve(root, "src/generated/sr03-sources.json"),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  console.log(JSON.stringify(await generateSr03()));
}
