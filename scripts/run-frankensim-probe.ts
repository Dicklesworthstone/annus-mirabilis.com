...
import { ASUPERSYNC_PIN, FRANKENSIM_PIN, classifyProbeFailure } from "../src/testing/probeCopy.ts";
...
function newLogRunId(): string {
  ...
  const hex = createHash("sha256").update(`${stamp}-${process.pid}-${performance.now()}`).digest("hex").slice(0, 8);
  return `${stamp}-${hex}`;
}
...
function main(): Promise<number> {
  ...
  const buildRoot = process.env.PROBE_BUILD_ROOT ??
    ...
    child.on("close", (code, signal) => {
      ...
      if (exitCode !== 0) {
        ...
        const envLines = Object.keys(env)
          .filter((k) => k.startsWith("RCH_") || k.startsWith("CARGO_") || k.startsWith("RUST") || ["PATH", "PWD", "HOME"].includes(k))
          .sort()
          .map((k) => `${k}=${env[k]}\n`)
          .join("");
      ...
      process.stdout.write(`LOGGED ${testId} outcome=${outcome} exit=${exitCode} durationMs=${durationMs} class=${failureClass}\n`);
      resolvePromise(0);
    });
  });
}
