...
function event(partial: Partial<LogEvent> & Pick<LogEvent, "suite" | "logRunId" | "testId">): LogEvent {
...
test("summarize reports correct counts per suite and outcome for a fixture event set", () => {
  const events: LogEvent[] = [
    event({ suite: "alpha", logRunId: "20260101T000000Z-aaaaaaaa", testId: "t1", outcome: "passed" }),
    event({ suite: "alpha", logRunId: "20260101T000000Z-aaaaaaaa", testId: "t2", outcome: "failed", message: "boom" }),
    event({ suite: "alpha", logRunId: "20260101T000000Z-aaaaaaaa", testId: "t3", outcome: "skipped" }),
    event({ suite: "beta", logRunId: "20260101T000100Z-bbbbbbbb", testId: "t4", outcome: "not-available" }),
    event({ suite: "beta", logRunId: "20260101T000100Z-bbbbbbbb", testId: "t5", outcome: "passed" }),
  ];
...
test("two script processes sharing one toolRunId are grouped under it with both logRunId values listed", () => {
  const events: LogEvent[] = [
    event({ suite: "ocr-ledgers", logRunId: "20260101T000300Z-dddddddd", testId: "chunk-0001", toolRunId: "20260101T000000Z-eeeeeeee", outcome: "passed" }),
    event({ suite: "ocr-ledgers", logRunId: "20260101T000400Z-ffffffff", testId: "chunk-0002", toolRunId: "20260101T000000Z-eeeeeeee", outcome: "passed" }),
    event({ suite: "ocr-ledgers", logRunId: "20260101T000300Z-dddddddd", testId: "chunk-0003", toolRunId: "20260101T000000Z-eeeeeeee", outcome: "failed" }),
  ];
  const summary = summarize(events);
