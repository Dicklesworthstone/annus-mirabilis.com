...
import { LQ06_DEFAULTS } from "../src/experiments/lq06/definition.ts";
import { createLq06Session } from "../src/experiments/lq06/session.ts";
...
export async function evaluatorSources() {
  ...
  await visit("src/experiments/lq06/session.ts");
  seen.set(
    "scripts/generate-lq06.mjs",
    await readFile(resolve(root, "scripts/generate-lq06.mjs"), "utf8"),
  );
  return [...seen].sort(([a], [b]) => a.localeCompare(b, "en"));
}

export async function generateCoefficientMatch() {
  ...
  const session = createLq06Session("lq06-static-example");
  const snapshot = session.getServerSnapshot().accepted;
  if (!snapshot) throw new Error("LQ-06 static worked example failed to initialize.");
  ...
  await writeFile(
    resolve(root, "src/generated/lq06-example.json"),
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  console.log(JSON.stringify(await generateCoefficientMatch()));
