...
export async function generateMassEnergyEntrance() {
  ...
  async function visit(path) {
    ...
    for (const match of text.matchAll(/(?:from\s*|import\s*)["'](\.[^"']+)["']/g)) {
      const target = relative(root, resolve(root, dirname(path), match[1])).split("\\").join("/");
  ...
  const prepared = { schemaVersion: 1, sourceDigest, execution: "prepared-reference-example",
    constantSetId: "modern-si-2019", unit: "J", record,
    scenarios: [prepareMassEnergyScenario("fast"), prepareMassEnergyScenario("slow")] };
  await mkdir(resolve(root, "src/generated"), { recursive: true });
  await writeFile(resolve(root, "src/generated/mass-energy-entrance.json"), `${JSON.stringify(prepared, null, 2)}\n`);
  return prepared;
}
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const prepared = await generateMassEnergyEntrance();
  console.log(JSON.stringify({ event: "mass-energy-entrance-generated", sourceDigest: prepared.sourceDigest, scenarios: prepared.scenarios.length }));
}
