...
import {
  type OfflineBudget,
  type OfflineWorkedExample,
  packageOfflineChapter,
} from "../src/platform/offline/chapter.ts";
import {
...
async function preparedExamples(root: string): Promise<readonly OfflineWorkedExample[]> {
  ...
  const names = (await readdir(directory))
    .filter((name) => /^(?:bm|sr|lq|me)\d{2}-example\.json$/u.test(name))
    .sort();
  ...
  for (const name of names) {
    ...
    for (const encoded of record.results) {
      if (typeof encoded !== "string")
        throw new Error(`Invalid prepared result encoding: ${name}.`);
    ...
    examples.push({
      instrumentId: `${stem.slice(0, 2)}-${stem.slice(2)}`,
...
export async function buildOfflineChapters(
  root = process.cwd(),
  ...
  if (profile === "scaffold") {
    const papers = await Promise.all(
      index.payloads
        ...
        .map(async (entry) => ({
          payload: await loadPaperPayload(entry.id, root),
    ...
    const budgets = JSON.parse(
      await readFile(resolve(root, "perf/offline-chapter-budgets.json"), "utf8"),
    ) as {
      schemaVersion: number;
      default: OfflineBudget;
    };
    if (budgets.schemaVersion !== 1 || !budgets.default)
      throw new Error("Invalid offline budget file.");
    ...
    const citations = new Map(
      papers.flatMap(({ payload }) =>
        payload.citations.map((citation) => [citation.id, citation] as const),
      ),
    );
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(
    JSON.stringify({
      event: "offline-chapters-built",
      profile: manifest.profile,
      chapters: manifest.chapters.length,
      bytes: manifest.chapters.reduce((sum, entry) => sum + entry.bytes, 0),
