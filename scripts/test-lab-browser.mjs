...
const server = createServer(async (req, res) => {
  try {
    ...
    res.setHeader("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data:; worker-src 'self'; connect-src 'self'");
...
const evidence = [];
const check = (name, details = {}) => { const item = { check: name, outcome: "passed", ...details }; evidence.push(item); console.log(JSON.stringify(item)); };
...
try {
  ...
  assert.match(await staticPage.locator('[data-output="diffusionCoefficient"]').first().innerText(), /0\.42944/);
  assert.ok(await staticPage.getByRole("button", { name: "Apply settings", exact: true }).isDisabled());
  ...
  const errors = []; let workerCount = 0;
  ...
  await lab.locator('input[name="T"]').fill("abc"); await apply.click();
  ...
  await lab.getByRole("button", { name: "Try a step that is too large", exact: true }).click(); await apply.click();
  ...
  await lab.locator('input[name="lower"]').fill("0"); await apply.click();
  await page.waitForFunction(() => document.querySelector('[data-output="intervalProbability"]').textContent.includes("35.971"));
  assert.equal(await lab.getAttribute("data-run-id"), runId);
  check("interval remeasurement preserves the identified physical run");

  await page.getByRole("button", { name: "Open an independent second laboratory", exact: true }).click();
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-output="diffusionCoefficient"]')[1].textContent.includes("0.21472"));
  assert.match(await lab.locator('[data-output="diffusionCoefficient"]').innerText(), /0\.42944/);
  ...
  assert.deepEqual(serious.map(v => ({ id: v.id, description: v.description })), []);
  check("automated accessibility supplement", { violations: audit.violations.map(v => v.id), manualScreenReaderReview: "not performed" });
