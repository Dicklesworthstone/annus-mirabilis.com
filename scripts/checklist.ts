...
export function generateChecklistMarkdown(evidence: ReleaseCandidateEvidence): ChecklistOutput {
  ...
  if (evidence.q1.germanSourceReviews.length === 0) {
    ...
    openBlockers.push("Q1: German source review records missing.");
  } else {
  ...
  if (evidence.q2.physicsMathReviews.length === 0) {
    lines.push("  - *None recorded.*");
    openBlockers.push("Q2: Physics/math review records missing.");
  } else {
  ...
  if (evidence.q5.comprehensionRounds.length === 0) {
    lines.push("- *None recorded.*");
    openBlockers.push("Q5: Comprehension study rounds missing.");
  } else {
  ...
  if (evidence.q5.crossProjectionRecords.length === 0) {
    lines.push("- *None recorded.*");
    openBlockers.push("Q5: Cross-projection review records missing.");
  } else {
  ...
  if (openBlockers.length === 0) {
    lines.push(
      "**STATUS**: READY FOR RELEASE. All five questions have sufficient verified evidence.",
    );
  } else {
    lines.push(`**STATUS**: BLOCKED (${openBlockers.length} issues open).`);
    lines.push("");
