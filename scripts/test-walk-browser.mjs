...
export async function checkWalkBrowser(browser, url, check) {
  ...
  await staticPage.goto(`${url}/lab/bm-05/`);
  ...
  check(
    "BM-05: exact coin probabilities, model values, traces and derivation readable without JavaScript at 320px",
  );
  await noJs.close();
  ...
  try {
    await page.goto(`${url}/lab/bm-05/`);
    const lab = page.locator('[data-instrument-id="bm-05"]').first(),
      apply = lab.getByRole("button", { name: "Apply walk settings", exact: true });
    ...
    async function accepted(action, target = lab) {
      ...
      await page.waitForFunction(
        ({ id, before }) => {
          const n = [...document.querySelectorAll('[data-instrument-id="bm-05"]')].find(
            (n) => n.dataset.instanceId === id,
          );
          return n?.dataset.pending === "false" && Number(n.dataset.snapshotVersion) > before;
        },
        { id, before },
        { timeout: 20000 },
      );
    ...
    const run = await lab.getAttribute("data-run-id"),
      ...
      initialTrace = await lab.locator(".walk-trace").first().getAttribute("d");
    const originalMean = await value("sampleMean");
    assert.equal(draws, "64000");
    assert.equal(await value("shapeTerm"), "0.1875");
    const versions = await lab
      .locator("[data-snapshot-version]")
      .evaluateAll((ns) => ns.map((n) => n.dataset.snapshotVersion));
    assert.ok(versions.every((v) => v === versions[0]));
    check(
      "BM-05: one worker publishes the exact four-step law, all-member statistics and a 64-bit seed",
    );
    await accepted(() =>
      lab.getByRole("button", { name: "Observe 400 steps", exact: true }).click(),
    );
    ...
    assert.ok(Math.abs(Number(await value("modelMeanSquare")) * 1e12 - 100) < 1e-12);
    await lab.locator('[name="n"]').fill("43");
    await accepted(() => apply.click());
    assert.equal(await lab.getAttribute("data-run-id"), run);
    assert.ok(Number(await lab.getAttribute("data-replayed-draws")) > 0);
    await accepted(() => lab.getByRole("button", { name: "Observe 4 steps", exact: true }).click());
    assert.equal(await lab.locator(".walk-trace").first().getAttribute("d"), initialTrace);
    assert.equal(await value("sampleMean"), originalMean);
    check(
      "BM-05: cached and replayed observations preserve the identical trial and report replay work honestly",
    );
    for (const kernel of ["uniform", "gaussian"]) {
      await lab.locator('[name="kernel"]').selectOption(kernel);
      await accepted(() => apply.click());
      assert.equal(await value("diffusionCoefficient"), "1.25e-12");
      assert.equal(await value("stepSecondMoment"), "2.5e-13");
      assert.notEqual(await lab.getAttribute("data-run-id"), run);
    ...
    check(
      "BM-05: changing step shape preserves variance and diffusivity while changing the finite-step shape gap",
    );
    const biasRun = await lab.getAttribute("data-run-id");
    await accepted(() =>
      lab
        .getByRole("button", { name: "Restore equal left/right probability", exact: true })
        .click(),
    );
    assert.equal(await lab.getAttribute("data-run-id"), biasRun);
    assert.equal(await lab.getAttribute("data-request-draws"), "0");
    ...
    check(
      "BM-05: analytic drift, infinite-variance and continuum-limit cases do not create unsupported sampled paths",
    );
    await lab.locator('[name="n"]').fill("0");
    await accepted(() => apply.click());
    assert.equal(await value("modelMeanSquare"), "0");
    assert.match(await lab.innerText(), /Before any steps, all walkers are at zero/);
    assert.equal(await lab.locator(".plot").first().locator(".comparison-curve").count(), 0);
    const zeroVersion = await lab.getAttribute("data-snapshot-version");
    await lab.locator('[name="n"]').fill("0.5");
    await apply.click();
    await lab.getByRole("alert").waitFor();
    assert.equal(await lab.getAttribute("data-snapshot-version"), zeroVersion);
    await accepted(() => lab.getByRole("button", { name: "Observe 4 steps", exact: true }).click());
    check("BM-05: zero-step limits and invalid observations never masquerade as a Gaussian result");
    const prior = await lab.getAttribute("data-snapshot-version");
    await lab.locator('[name="walkers"]').fill("10000");
    await lab.locator('[name="runSteps"]').fill("10000");
    await apply.click();
    await lab.getByRole("button", { name: "Restore accepted settings", exact: true }).waitFor();
    assert.equal(await lab.getAttribute("data-snapshot-version"), prior);
    await accepted(() =>
      lab.getByRole("button", { name: "Restore accepted settings", exact: true }).click(),
    );
    await lab.locator('[name="kernel"]').selectOption("uniform");
    await lab.locator('[name="runSteps"]').fill("401");
    ...
    check(
      "BM-05: work and exact-law budgets preserve accepted data and provide a working recovery",
    );
    await lab.locator('[name="seed"]').fill("9007199254740992");
    await accepted(() => apply.click());
    const seedCaption = await lab.locator(".accepted-caption").innerText();
    ...
    await shared.close();
    check(
      "BM-05: invalid seeds cannot overwrite accepted settings and shared links load drafts without autoplay",
    );
    await page
    ...
    const first = await lab.locator(".accepted-caption").innerText();
    await accepted(
      () => second.getByRole("button", { name: "Apply walk settings", exact: true }).click(),
      second,
    );
    assert.equal(workers, 2);
    assert.equal(await lab.locator(".accepted-caption").innerText(), first);
    assert.notEqual(
      await second.getAttribute("data-instance-id"),
      await lab.getAttribute("data-instance-id"),
    );
    await page.getByRole("button", { name: "Close second walk laboratory", exact: true }).click();
    ...
    check("BM-05: automated accessibility supplement", {
      violations: audit.violations.map((v) => v.id),
      manualScreenReaderReview: "not performed",
    });
    await page.screenshot({ path: "artifacts/browser/walks-desktop.png", fullPage: true });
    await page.setViewportSize({ width: 320, height: 900 });
    assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1));
    await page.screenshot({ path: "artifacts/browser/walks-320.png", fullPage: true });
    assert.deepEqual(errors, []);
    check("BM-05: interactive 320px reflow and no uncaught browser errors");
  } catch (error) {
    await page.screenshot({ path: "artifacts/browser/walks-failure.png", fullPage: true });
    console.error("Walk browser errors:", errors);
    throw error;
  } finally {
    await context.close();
  }
