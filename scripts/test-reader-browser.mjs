...
export async function checkReaderBrowser(browser, url, check) {
  ...
  assert.ok(gzipSync(source).length <= 250 * 1024, "Reader HTML exceeds the declared compressed budget.");
  check("reader HTML includes every reading depth within the 250 KiB gzip budget", { gzipBytes: gzipSync(source).length });

  const noJs = await browser.newContext({ javaScriptEnabled: false, viewport: { width: 320, height: 900 } });
  try {
    const page = await noJs.newPage(); await page.goto(url + route);
    ...
    check("reader remains complete without JavaScript, including native derivation disclosures and 320px reflow");
    ...
    await page.goBack(); assert.equal(await page.locator("h1").innerText(), "Mean, variance and RMS");
    check("no-JavaScript foundation links follow complete prerequisite pages and browser Back returns");
  } finally { await noJs.close(); }

  const beforeHydration = await browser.newContext({ viewport: { width: 1000, height: 800 } });
  try {
    await beforeHydration.addInitScript(() => { localStorage.setItem("am:settings:v1:detail", "0"); });
    await beforeHydration.route("**/_next/**/*.js", request => request.abort());
    const page = await beforeHydration.newPage(); await page.goto(url + route + "?detail=steps");
    ...
    await page.goto(url + route); assert.equal(await page.locator("html").getAttribute("data-detail"), "0");
  ...
  try {
    ...
    assert.equal(await page.locator('link[rel="canonical"]').getAttribute("href"), "https://annus-mirabilis.com" + route);
    const passage = page.locator(`#${firstId}`), dialog = page.locator("[data-clarification-dialog]");
    const local = passage.locator(".local-steps"); await local.locator("summary").click();
    const detail = page.locator(".reader-controls [data-detail-control]");
    const historyLength = await page.evaluate(() => history.length);
    await detail.selectOption("0"); assert.ok(await passage.locator('[data-reading="0"]').isVisible());
    await detail.selectOption("2"); assert.ok(await passage.locator('[data-reading="2"]').isVisible());
    assert.notEqual(await local.getAttribute("open"), null);
    await detail.selectOption("1"); assert.equal(await page.evaluate(() => history.length), historyLength);
    ...
    check("three reading depths replace history without resetting local expansion; modern qualifications are independent");
    ...
    await page.waitForFunction(() => document.querySelector('[data-output="diffusionCoefficient"]').textContent.includes("0.21472"));
    const accepted = await lab.evaluate(el => ({ run: el.dataset.runId, version: el.dataset.snapshotVersion, draws: el.dataset.recordingDraws, rms: el.querySelector('[data-output="sampleRms"]').textContent }));
    ...
    await why.click(); await dialog.waitFor({ state: "visible" });
    assert.equal(await dialog.getAttribute("aria-labelledby"), "clarification-mean-variance-rms");
    assert.equal(await page.evaluate(() => document.activeElement.id), "clarification-mean-variance-rms");
    ...
    await page.waitForFunction(() => document.activeElement.id === "clarification-bridge-negative-numbers-direction");
    ...
    await page.goBack(); assert.equal(await page.locator("html").getAttribute("data-reader-view"), "reading");
    assert.ok(await dialog.isVisible());
    await page.keyboard.press("Escape"); await dialog.waitFor({ state: "hidden" });
    const after = await lab.evaluate(el => ({ run: el.dataset.runId, version: el.dataset.snapshotVersion, draws: el.dataset.recordingDraws, rms: el.querySelector('[data-output="sampleRms"]').textContent }));
    ...
    const copied = page.locator("[data-copy-fallback] input"); await copied.waitFor({ state: "visible" });
    const href = new URL(await copied.inputValue());
    assert.equal(href.hash, "#" + firstId); assert.equal(href.pathname, route);
    assert.ok([...href.searchParams.keys()].every(key => ["view", "detail", "lens"].includes(key)));
    assert.ok(!href.href.includes("private"));
    check("unavailable source faces never impersonate a translation, and clipboard fallback exports only public reading axes");

    const mainAudit = await new AxeBuilder({ page }).include("#main").withTags(["wcag2a", "wcag2aa", "wcag21aa"]).analyze();
    assert.deepEqual(mainAudit.violations.filter(v => ["serious", "critical"].includes(v.impact)).map(v => v.id), []);
    await why.click(); await dialog.waitFor({ state: "visible" });
    ...
    check("reader and clarification accessibility, focus containment, mobile layout and print reading", { readerViolations: mainAudit.violations.map(v => v.id), dialogViolations: audit.violations.map(v => v.id), manualScreenReaderReview: "not performed" });
    ...
    await page.goto(`${url}${route}s5/?detail=2&open=foundation:mean-variance-rms#arg-bm-inference`);
