...
import { PhotoelectricRecordError, parseVoltageCsv, type VoltageRecord } from "./record.ts";
...
export const PHOTOELECTRIC_EXAMPLES = Object.freeze([
  Object.freeze({
    id: "linear",
    label: "Constructed straight trend",
    csv: "frequency_THz,stopping_V,sigma_V\n500,0.52,0.04\n600,0.85,0.04\n700,1.36,0.04\n800,1.66,0.04\n900,2.11,0.04\n",
  }),
  Object.freeze({
    id: "curved",
    label: "Constructed curved trend",
    csv: "frequency_THz,stopping_V,sigma_V\n500,0.70,0.03\n600,0.95,0.03\n700,1.30,0.03\n800,1.75,0.03\n900,2.30,0.03\n",
  }),
  Object.freeze({
    id: "unequal",
    label: "Constructed unequal precision",
    csv: "frequency_THz,stopping_V,sigma_V\n500,0.51,0.02\n600,0.89,0.02\n700,1.31,0.02\n800,1.68,0.02\n900,3.10,1.00\n",
  }),
]);
...
export type AnalysisOutcome =
  | Readonly<{ kind: "accepted"; state: AcceptedAnalysis }>
  | Readonly<{ kind: "refused"; message: string; code?: string }>;
...
export function exampleDraft(id = "linear"): AnalysisDraft {
  ...
  if (!example)
    throw new PhotoelectricRecordError("example-unknown", "Unknown constructed example.");
  return {
    csv: example.csv,
    label: example.label,
    weighting: "declared-sigma",
    offsetKind: "unknown",
    offsetVolts: "0",
    offsetSigmaV: "0",
  };
...
export function draftFromAnalysis(state: AcceptedAnalysis): AnalysisDraft {
  return {
    csv: state.csv,
    label: state.label,
    weighting: state.options.weighting,
    offsetKind: state.options.offset.kind,
...
// A record label is one line of text: no character below U+0020 (tab, newline and the rest of C0).
function hasControlCharacter(text: string): boolean {
  for (let i = 0; i < text.length; i += 1) if (text.charCodeAt(i) < 0x20) return true;
  return false;
}

function scalar(text: string): number {
  if (typeof text !== "string" || !/^[+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?$/u.test(text.trim()))
    throw new PhotoelectricRecordError(
      "calibration-not-decimal",
      "Calibration values must be explicit finite decimal numbers; blanks are not zero.",
    );
  const n = Number(text);
  if (!Number.isFinite(n))
    throw new PhotoelectricRecordError(
      "calibration-not-finite",
      "Calibration values must be finite.",
    );
  return n;
}
export function acceptAnalysisDraft(
  draft: AnalysisDraft,
  ...
  try {
    if (!Number.isSafeInteger(revision) || revision < 1)
      throw new PhotoelectricRecordError("analysis-revision-invalid", "Invalid analysis revision.");
    if (
      typeof draft.label !== "string" ||
      draft.label.length > 160 ||
      hasControlCharacter(draft.label)
    )
      throw new PhotoelectricRecordError(
        "analysis-label-invalid",
        "Use a single-line record label of at most 160 characters.",
      );
    if (draft.offsetKind !== "unknown" && draft.offsetKind !== "known")
      throw new PhotoelectricRecordError(
        "analysis-offset-kind-missing",
        "Choose the voltage-offset assumption explicitly.",
      );
    const record = parseVoltageCsv(draft.csv, reference.speedOfLight);
    const options: FitOptions = Object.freeze({
      weighting: draft.weighting,
      offset:
        draft.offsetKind === "unknown"
          ? Object.freeze({ kind: "unknown" as const })
          : Object.freeze({
              kind: "known" as const,
              volts: scalar(draft.offsetVolts),
              sigmaV: scalar(draft.offsetSigmaV),
            }),
    });
    const fit = analyzePhotoelectricData(record.rows, options, reference);
    const source = PHOTOELECTRIC_EXAMPLES.some(
      (e) => e.csv === draft.csv && e.label === draft.label,
    )
      ? "constructed-example"
      : "reader-supplied";
    return {
      ...
      state: Object.freeze({
        revision,
        csv: draft.csv,
        label: draft.label.trim() || "Local record",
        source,
        record,
        options,
        excludedRows: Object.freeze([]),
        fit,
        allRowsFit: fit,
      }),
    };
...
export function refitSelectedRows(
  ...
  try {
    if (
      ...
      excluded.some((id) => !state.record.rows.some((r) => r.row === id))
    )
      throw new PhotoelectricRecordError(
        "selection-rows-invalid",
        "Select only unique observations from the accepted record.",
      );
    const fit = analyzePhotoelectricData(
      state.record.rows.filter((r) => !excluded.includes(r.row)),
      state.options,
      reference,
    );
    return {
      ...
      state: Object.freeze({
        ...
        fit,
        excludedRows: Object.freeze([...excluded].sort((a, b) => a - b)),
      }),
    };
  } catch (error) {
...
export function exportAnalysisReport(
  ...
  return `${JSON.stringify(
    {
      format: "annus-photoelectric-analysis",
      version: 1,
      evaluator: "photoelectric-data-v1",
      reference,
      analysis: state,
      assumptions: [
        "Single surface; detected stopping endpoints only; fitted straight line.",
        "Frequencies treated as known without error; voltage observations independent.",
        "Known offset calibration is independent of the fitted record.",
        "Standard errors are conditional, not complete measurement uncertainties or confidence intervals.",
        "No automatic exclusions, historical authentication or server upload.",
      ],
    },
    null,
    2,
