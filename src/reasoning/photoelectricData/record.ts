...
export type VoltageRow = Readonly<{ row: number; frequencyTHz: number; stoppingV: number; sigmaV?: number }>;
...
export function createVoltageFileReader() {
  ...
  return {
    cancel() { generation++; },
    async read(file: LocalFile): Promise<{ kind: "text"; text: string } | { kind: "invalid"; message: string } | { kind: "stale" }> {
      const token = ++generation;
      if (!Number.isFinite(file.size) || file.size < 0 || file.size > RECORD_BYTE_LIMIT) return { kind: "invalid", message: "The CSV exceeds the 128 KiB local-file limit." };
      try {
        const text = await file.text();
        if (token !== generation) return { kind: "stale" };
        if (text.length > RECORD_BYTE_LIMIT || new TextEncoder().encode(text).byteLength > RECORD_BYTE_LIMIT) return { kind: "invalid", message: "The CSV exceeds the 128 KiB local-file limit." };
        return { kind: "text", text };
      } catch {
        return token !== generation ? { kind: "stale" } : { kind: "invalid", message: "The file could not be read. The accepted analysis is unchanged." };
      }
    },
  };
}
