...
import { renderCameraResults, renderInferenceWorkbench, renderRadiusResults } from "./render.ts";
import { createCameraInferenceSession, createRadiusSession } from "./session.ts";

const read = (id) =>
  JSON.parse(readFileSync(new URL(`../../generated/${id}-example.json`, import.meta.url), "utf8"));
const bm07 = read("bm07"),
  bm08 = read("bm08");
const ideal = evidenceFromPrepared("ideal", bm07, "a".repeat(64)),
  camera = evidenceFromPrepared("camera", bm08, "b".repeat(64));
...
test("both families and optional worked solutions are present before hydration", () => {
  for (const text of [
    "Can displacements tell radius",
    "Motion or localization error",
    "data-static-radius",
    "data-static-camera",
    "Static worked example",
    "data-observation-evidence",
    'data-result-status="underdetermined"',
    'data-result-status="not-applicable"',
  ])
...
test("camera moment solutions never acquire an independent-increment interval", () => {
  ...
  for (const information of ["variance", "covariance", "second-interval"]) {
    session.apply({ information });
    const rendered = renderCameraResults(session.getSnapshot().accepted);
...
test("the original BM-07 family and BM-08 moments use the shared renderer", () => {
  const factory = () => {
    throw new Error("No Worker may be started by a static view.");
  };
  ...
  const family = renderInferenceFamily(oldRadius),
    moments = renderCameraMoments(oldCamera);
  assert.equal((family.match(/data-family-curve/g) || []).length, 1);
  assert.ok(family.includes("not a confidence region"));
  assert.ok(moments.includes("Asymptotic sampling SD"));
  assert.ok(moments.includes('data-output="expectedVariance"'));
  assert.ok(moments.includes('data-output="sdCovariance"'));
  const familyComponent = readFileSync(
    new URL("../../components/lab/InferencePlots.tsx", import.meta.url),
    "utf8",
  );
  const cameraComponent = readFileSync(
    new URL("../../components/lab/CameraLab.tsx", import.meta.url),
    "utf8",
  );
...
test("all tabular scroll regions are named and keyboard focusable", () => {
  const containers = html.match(/<div class="table-scroll"[^>]*>/g) || [];
  assert.ok(containers.length >= 5);
  for (const tag of containers) {
    assert.ok(tag.includes('role="region"'));
    assert.ok(tag.includes("aria-label="));
