...
export function parseInferenceEvidence(input: unknown): InferenceEvidence {
  ...
  if (
    Reflect.ownKeys(data).length !== keys.length ||
    Reflect.ownKeys(data).some((key) => typeof key !== "string" || !keys.includes(key)) ||
  ...
  if (
    ...
    !["ideal", "camera"].includes(String(data.kind)) ||
    typeof data.sourceDigest !== "string" ||
    !/^source:sha256:[a-f0-9]{64}$/u.test(data.sourceDigest) ||
    typeof data.observationDigest !== "string" ||
    !/^[a-f0-9]{64}$/u.test(data.observationDigest) ||
  ...
  if (
    ...
    ![1, 2].includes(data.d) ||
    typeof data.knownDrift !== "number" ||
    !Number.isFinite(data.knownDrift) ||
    typeof data.exposure !== "number" ||
    !Number.isFinite(data.exposure) ||
    data.exposure < 0 ||
    data.exposure > (data.dt as number) ||
    !Array.isArray(data.increments) ||
    data.increments.length < 6 * data.d ||
    data.increments.length > 6000 ||
    data.increments.length % (2 * data.d) !== 0 ||
    Array.from(data.increments).some(
      (value) => typeof value !== "number" || !Number.isFinite(value),
    )
  )
    throw new TypeError(
      "Evidence requires bounded, paired, finite observations on a declared camera grid.",
    );
  ...
  return Object.freeze({
    ...data,
    increments: Object.freeze([...data.increments]),
  }) as InferenceEvidence;
}

/** Build-only projection: explicitly enumerate admitted fields; never spread generator settings. */
export function evidenceFromPrepared(
  kind: InferenceEvidence["kind"],
  source: EncodedEvidenceSource,
  observationDigest: string,
): InferenceEvidence {
  const results = source.results.map(parseResult);
  const matches = results.filter(
    (result) => result.quantityId === (kind === "ideal" ? "observationIncrements" : "increments"),
  );
  ...
  if (
    ...
    !(observed.value instanceof Float64Array) ||
    observed.unit !== "m"
  )
    throw new TypeError(
      "The prepared example has no unique accepted displacement series in metres.",
    );
