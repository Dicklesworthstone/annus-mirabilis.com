...
<section class="laboratory" data-infer-case="camera" data-instrument-id="infer-camera" ${inferenceIdentity(camera)} data-observation-digest="${e(example.camera.observationDigest)}" data-input-revision="${camera.revisions.input}" data-execution-label="static"><header class="lab-heading"><div><p class="eyebrow">One record · different information</p><h2>Motion or localization error?</h2></div><p class="badge" data-infer-label>Static worked example · host calculation</p></header><p>Keep ${example.camera.increments.length / example.camera.d} camera increments in ${example.camera.d} coordinates. Frame spacing ${display(example.camera.dt)} s; known uniform exposure ${display(example.camera.exposure)} s; known drift ${display(example.camera.knownDrift, 1e6)} μm/s. The generator's diffusion coefficient and localization variance are not inference inputs.</p><fieldset disabled data-camera-controls><legend>Which information will you admit?</legend><div class="infer-choices">${methods.map(([value, label]) => `<div><input type="radio" id="${uid}-${value}" name="${uid}-camera-information" value="${value}"${value === "variance" ? " checked" : ""} data-camera-information><label for="${uid}-${value}">${label}</label></div>`).join("")}</div></fieldset><div data-infer-results>${renderCameraResults(camera)}</div><p role="alert" data-infer-error hidden></p><p role="status" aria-live="polite" aria-atomic="true" data-infer-status>Variance alone leaves diffusion and noise unresolved.</p>
