...
<section class="laboratory" data-infer-case="radius" data-instrument-id="infer-ideal" ${inferenceIdentity(radius)} data-observation-digest="${e(example.ideal.observationDigest)}" data-input-revision="${radius.revisions.input}" data-accepted-input-revision="${radius.revisions.input}" data-pending="false" data-execution-label="static"><header class="lab-heading" data-case-heading><div><p class="eyebrow">One relation · two unknowns</p><h2>Can displacements tell radius and molecular number apart?</h2></div><p class="badge" data-infer-label>Static worked example</p></header><p class="fine">Held fixed: ${example.ideal.increments.length / example.ideal.d} synthetic increments in ${example.ideal.d} coordinates, ${display(example.ideal.dt)} s apart, at ${display(example.ideal.T)} K and ${display(example.ideal.eta, 1000)} mPa·s.</p><div class="lab-columns"><div>
