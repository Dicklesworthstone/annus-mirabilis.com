import {
  ...
  validateOccupancySettings, OCCUPANCY_CHECKS,
...
export function parseOccupancyHistogram(text: string, n: number): readonly number[] {
  ...
  if (typeof text !== "string" || text.length > OCCUPANCY_TEXT_LIMIT || !text.trim())
    throw new TypeError(`Enter a frequency record of at most ${OCCUPANCY_TEXT_LIMIT} characters.`);
  ...
  if (tokens.length !== n + 1 || tokens.some((token) => !/^(0|[1-9][0-9]*)$/u.test(token)))
    throw new TypeError(`Enter exactly ${n + 1} nonnegative whole-number frequencies, in order from zero inside to ${n} inside. Use commas or spaces; do not omit empty bins.`);
...
export function illustrativeOccupancyRecord(kind: "mixed" | "all-or-none"): OccupancyState {
  if (kind !== "mixed" && kind !== "all-or-none") throw new TypeError("Unknown illustrative record.");
...
export function decodeOccupancyLink(search: string): DecodedOccupancyLink {
  try {
    if (typeof search !== "string" || search.length > OCCUPANCY_LINK_LIMIT) throw new TypeError("The occupancy link is too long.");
    ...
    if (keys.some((key) => query.getAll(key).length !== 1) || query.get("occupancy") !== "1")
      throw new TypeError("The occupancy link has missing, repeated or unsupported version fields.");
    if (!/^[1-9][0-9]?$/u.test(query.get("n") ?? "") || !/^[0-4]$/u.test(query.get("q") ?? ""))
      throw new TypeError("The occupancy link has invalid point-count or volume fields.");
    ...
    if (checks.some((check) => !(OCCUPANCY_CHECKS as readonly string[]).includes(check)))
      throw new TypeError("The occupancy link names an unknown measurement.");
    ...
    return { kind: "invalid", message: error instanceof Error ? error.message : "The occupancy link could not be read." };
  }
}
