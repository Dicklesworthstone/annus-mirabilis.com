...
export function createOccupancyState(
  input: unknown = DEFAULT_OCCUPANCY_SETTINGS,
  checks = DEFAULT_OCCUPANCY_CHECKS,
): OccupancyState {
...
export function applyOccupancySettings(
  ...
  const same =
    next.comparison.settings.n === current.comparison.settings.n &&
  ...
  return same
    ? Object.freeze({ ...next, evidence: current.evidence, evidenceSource: current.evidenceSource })
    : next;
}

export function selectOccupancyChecks(
  current: OccupancyState,
  checks: readonly OccupancyCheck[],
): OccupancyState {
...
export function parseOccupancyHistogram(text: string, n: number): readonly number[] {
  ...
  if (tokens.length !== n + 1 || tokens.some((token) => !/^(0|[1-9][0-9]*)$/u.test(token)))
    throw new OccupancyInputError(
      "record-frequency-count-mismatch",
      `Enter exactly ${n + 1} nonnegative whole-number frequencies, in order from zero inside to ${n} inside. Use commas or spaces; do not omit empty bins.`,
    );
...
export function analyzeOccupancyRecord(current: OccupancyState, text: string): OccupancyState {
  const evidence = compareOccupancyEvidence(
    current.comparison.settings,
    parseOccupancyHistogram(text, current.comparison.settings.n),
  );
...
export function illustrativeOccupancyRecord(kind: "mixed" | "all-or-none"): OccupancyState {
  if (kind !== "mixed" && kind !== "all-or-none")
    throw new OccupancyInputError("illustrative-record-unknown", "Unknown illustrative record.");
  const state = createOccupancyState(DEFAULT_OCCUPANCY_SETTINGS, [
    "mean-count",
    "all-inside",
    "count-variance",
  ]);
  ...
  return Object.freeze({
    ...state,
    evidence: compareOccupancyEvidence(state.comparison.settings, counts),
    evidenceSource: "illustrative",
  });
}
...
export function encodeOccupancyLink(input: unknown, checks: readonly OccupancyCheck[]): string {
  ...
  return new URLSearchParams({
    occupancy: "1",
    n: String(state.comparison.settings.n),
    q: String(state.comparison.settings.quarters),
...
export function decodeOccupancyLink(search: string): DecodedOccupancyLink {
  try {
    if (typeof search !== "string" || search.length > OCCUPANCY_LINK_LIMIT)
      throw new OccupancyInputError("link-too-long", "The occupancy link is too long.");
    ...
    if (!/^[1-9][0-9]?$/u.test(query.get("n") ?? "") || !/^[0-4]$/u.test(query.get("q") ?? ""))
      throw new OccupancyInputError(
        "link-settings-invalid",
        "The occupancy link has invalid point-count or volume fields.",
      );
    ...
    if (checks.some((check) => !(OCCUPANCY_CHECKS as readonly string[]).includes(check)))
      throw new OccupancyInputError(
        "link-check-unknown",
        "The occupancy link names an unknown measurement.",
      );
    const state = createOccupancyState(
      { n: Number(query.get("n")), quarters: Number(query.get("q")) },
      checks as OccupancyCheck[],
    );
    return { kind: "settings", settings: state.comparison.settings, checks: state.checks };
  } catch (error) {
    const code =
      error instanceof OccupancyInputError || error instanceof OccupancyModelError
        ? error.code
        : "link-unreadable";
    return {
      kind: "invalid",
      message: error instanceof Error ? error.message : "The occupancy link could not be read.",
      code,
    };
  }
}
