...
import { renderCountermodelWorkbench, renderCountermodelResults, escapeWorkbenchText } from "./render.ts";
const examples=CASE_IDS.map(id=>{
 const spec=parseCountermodelCase(parseContentYaml(readFileSync(new URL(`../../../content/reasoning/countermodel/${id}.yaml`,import.meta.url),'utf8')));
 return {case:spec,sourceDigest:`source:sha256:${'a'.repeat(64)}`,caseRevision:'b'.repeat(64),results:evaluateCountermodelCase(spec,spec.defaultBeta).outputs.map(encodeResult)};
});
...
test('deselecting the light-speed requirement changes conclusions without concealing its result',()=>{
 const ex=examples[0],s=createCountermodelSession('case',ex);s.toggle('light-speed',false);
...
test('placement identifiers cannot inject markup, and text escaping handles attribute delimiters',()=>{
 ...
 assert.equal(escapeWorkbenchText('<x a="b"> & \'quote\''),'&lt;x a=&quot;b&quot;&gt; &amp; &#39;quote&#39;');
 const ex=structuredClone(examples[0]);ex.case.title='A "quoted" & named case';
 const html=renderCountermodelWorkbench(ex,'safe');assert.ok(html.includes('A &quot;quoted&quot; &amp; named case'));
});
...
test('rendered explanatory copy passes the shared voice rules',()=>{
 for(const ex of examples){
  ...
  const findings=checkVoice(text,{context:'countermodel-cell'}).filter(f=>f.severity==='error');
