...
export function mountCountermodelWorkbench(root: HTMLElement, example: PreparedCountermodelCase, uid: string): () => void {
  ...
  function refresh() {
    ...
    const state = session.getSnapshot(), snapshot = state.view.accepted;
    if (!snapshot) return;
    const opened = new Set([...results.querySelectorAll<HTMLDetailsElement>("details[open][data-cell-detail]")].map((element) => element.dataset.cellDetail));
    results.innerHTML = renderCountermodelResults(example.case, state, uid);
    results.querySelectorAll<HTMLDetailsElement>("details[data-cell-detail]").forEach((element) => { element.open = opened.has(element.dataset.cellDetail); });
    ...
    label.textContent = state.live ? "Accepted result · ideal model, host calculation" : "Static worked example · ideal model, host calculation";
  ...
  function apply(text: string) {
    ...
    const numeric = /^[+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?$/u.test(trimmed) ? Number(trimmed) : NaN;
    const result = session.apply(numeric);
    error.hidden = result.ok;
    error.textContent = result.ok ? "" : result.message;
    if (result.ok) { input.value = String(numeric); draft.hidden = true; }
  }
  const submit = (event: Event) => { event.preventDefault(); if (input) apply(input.value); };
  const edit = () => { if (draft) draft.hidden = false; };
  ...
  form.addEventListener("submit", submit); input.addEventListener("input", edit);
  reset.addEventListener("click", restore);
  toggles.forEach((toggle) => toggle.addEventListener("change", change));
  return () => {
    unsubscribe(); form.removeEventListener("submit", submit); input.removeEventListener("input", edit);
