...
describe("duplicate-id: the source-block site, not the twelve that already had tests", () => {
  test("(structural.ts:160) two source blocks share an id inside one paper", () => {
    const records = {
...
describe("missing-source-block: the two sites that were not driven", () => {
  test("(structural.ts:410) a paper's orderedBlockIds names a block that does not exist", () => {
    const records = {
      p: {
        kind: "paper",
        id: "brownian-motion",
  ...
  test("(structural.ts:476) an alignment edge points at a source block that does not exist", () => {
    const records = {
      b: { kind: "source-block", id: "s1-p1", paper: "brownian-motion" },
      al: {
        kind: "alignment",
...
describe("broken-alignment: the three sites that were not driven", () => {
  ...
  test("(structural.ts:670) an edge names a sentence that is not a span of its own block", () => {
    const records = {
      b: block,
      t: tu,
      al: {
  ...
  test("(structural.ts:703) an edge targets a translation unit that is missing in its paper", () => {
    const records = {
      b: block,
      t: tu,
      al: {
  ...
  test("(structural.ts:718) an edge's target range runs past the end of the translation text", () => {
...
describe("impossible-date-order: the two later sites, distinguished by which pair they compare", () => {
  ...
  test("(structural.ts:1033) received is strictly after issue publication", () => {
    const records = {
      p: {
        kind: "paper",
        id: "brownian-motion",
  ...
  test("(structural.ts:1050) issue publication is strictly after a later edition", () => {
    const records = {
      p: {
        kind: "paper",
        id: "brownian-motion",
...
describe("the four single sites", () => {
  test("(structural.ts:1146) germanLatex and englishLatex differ byte for byte", () => {
    const records = {
      e: {
        kind: "equation",
        id: "eq-1",
  ...
  test("(structural.ts:1240) a paper is complete while one of its blocks is still a draft", () => {
    const records = {
  ...
  test("(structural.ts:1325) a hero quote's anchor resolves to no edition record at all", () => {
    const records = {
      b: {
        kind: "source-block",
        id: "s1-p1",
  ...
  test("(structural.ts:1503) an edge's SOURCE span digest disagrees with the block text", () => {
...
describe("the two sites only the node-lane e2e reached", () => {
  test("(structural.ts:182) two source blocks in one paper declare the same sentence span id", () => {
    const records = {
      a: {
        kind: "source-block",
        id: "s1-p1",
  ...
  test("(structural.ts:1183) an aligned English equation differs by bytes from its German block", () => {
    const records = {
      g: {
        kind: "source-block",
        id: "eq-1",
