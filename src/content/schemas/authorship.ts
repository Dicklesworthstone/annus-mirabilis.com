...
export function validateAuthorshipEntry(
  ...
  if (!raw || typeof raw !== "object") {
    throw new AuthorshipValidationError(
      "invalid-authorship-entry",
      "Authorship entry must be an object.",
      path,
    );
  }
...
export function validateAuthorshipBlock(raw: unknown, path = "authorship"): AuthorshipBlock {
  if (!raw || typeof raw !== "object") {
    throw new AuthorshipValidationError(
      "invalid-authorship-block",
      "Authorship block must be an object.",
      path,
    );
  }
  ...
  if (!Array.isArray(o.draftedBy) || o.draftedBy.length === 0) {
    throw new AuthorshipValidationError(
      "missing-drafted-by",
      "Authorship block must contain at least one draftedBy entry.",
      `${path}.draftedBy`,
    );
  }

  const draftedBy = o.draftedBy.map((e, i) =>
    validateAuthorshipEntry(e, "author", `${path}.draftedBy[${i}]`),
  );
  const translatedBy = Array.isArray(o.translatedBy)
    ? o.translatedBy.map((e, i) =>
        validateAuthorshipEntry(e, "translator", `${path}.translatedBy[${i}]`),
      )
    : undefined;
...
export function authorshipOf(record: any): AuthorshipBlock {
  if (!record || typeof record !== "object") {
    throw new AuthorshipValidationError(
      "invalid-record",
      "Cannot derive authorship from non-object.",
    );
  }
  ...
  throw new AuthorshipValidationError(
    "unrecognized-record",
    "Record does not carry recognized authorship fields.",
  );
}
