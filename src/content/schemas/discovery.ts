...
export const DISCOVERY_PAPERS = [
  "light-quanta", "brownian-motion", "special-relativity", "mass-energy",
] as const;
...
export type DiscoveryExercise = Readonly<{
  ...
  domains: Readonly<Record<string, Readonly<{ min: number; max: number; scale: "linear" | "log" }>>>;
...
function record(input: unknown, path: string, keys: readonly string[], optional: readonly string[] = []): Record<string, unknown> {
...
function text(input: unknown, path: string, limit = 2400): string {
  if (typeof input !== "string" || !input.trim() || input.length > limit ||
      input !== input.normalize("NFC") || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(input))
...
function list<T>(input: unknown, path: string, min: number, max: number,
  parse: (item: unknown, path: string) => T): readonly T[] {
...
function citation(input: unknown, path: string): DiscoveryCitation {
  ...
  let parsed: URL;
  try { parsed = new URL(url); } catch { return fail(`${path}.url`, "Expected an absolute URL."); }
  if (parsed.protocol !== "https:" || parsed.username || parsed.password ||
      parsed.href !== url || /\s/u.test(url))
    fail(`${path}.url`, "Use a canonical HTTPS source URL without credentials or whitespace.");
  return Object.freeze({ label: text(r.label, `${path}.label`), url,
...
function lab(input: unknown, path: string): DiscoveryLab {
  ...
  return Object.freeze({ id: labId, title: text(r.title, `${path}.title`),
    task: text(r.task, `${path}.task`), observe: text(r.observe, `${path}.observe`) });
}
function alternative(input: unknown, path: string): DiscoveryAlternative {
  ...
  return Object.freeze({ id: id(r.id, `${path}.id`), title: text(r.title, `${path}.title`),
    retains: text(r.retains, `${path}.retains`), consequence: text(r.consequence, `${path}.consequence`),
    limitation: text(r.limitation, `${path}.limitation`) });
}
function stage(input: unknown, path: string): DiscoveryStage {
  const r = record(input, path, ["id", "title", "role", "dependsOn", "premises", "question",
    "overview", "qualifications", "reasoning", "alternatives", "labs", "paperLocator", "unavailable"]);
  const role = choice(r.role, `${path}.role`, ["question", "fork", "move", "consequence"] as const);
  const alternatives = list(r.alternatives, `${path}.alternatives`, role === "fork" ? 2 : 0, 4, alternative);
  unique(alternatives.map((a) => a.id), `${path}.alternatives`);
  ...
  unique(labs.map((l) => l.id), `${path}.labs`);
  return Object.freeze({ id: id(r.id, `${path}.id`), title: text(r.title, `${path}.title`), role,
    dependsOn: refs(r.dependsOn, `${path}.dependsOn`), premises: refs(r.premises, `${path}.premises`),
    question: text(r.question, `${path}.question`), overview: paragraphs(r.overview, `${path}.overview`),
    qualifications: paragraphs(r.qualifications, `${path}.qualifications`),
    reasoning: paragraphs(r.reasoning, `${path}.reasoning`), alternatives, labs,
    paperLocator: text(r.paperLocator, `${path}.paperLocator`), unavailable });
}

function exercise(input: unknown, path: string): DiscoveryExercise {
  const r = record(input, path, ["id", "stageId", "prompt", "declaredNames", "domains",
    "referenceSource", "tolerance", "workedExplanation"]);
  ...
  for (const name of declaredNames) {
    ...
    if (typeof d.min !== "number" || typeof d.max !== "number" || !Number.isFinite(d.min) ||
        !Number.isFinite(d.max) || d.min >= d.max || !Number.isFinite(d.max - d.min))
  ...
  const absolute = rawTolerance.absolute, relative = rawTolerance.relative;
  if (typeof absolute !== "number" || typeof relative !== "number" || !Number.isFinite(absolute) ||
      !Number.isFinite(relative) || absolute <= 0 || relative <= 0 || relative >= 1)
    fail(`${path}.tolerance`, "Declare positive finite absolute and relative tolerances; relative must be below one.");
  ...
  const points = Array.from({ length: 2 ** declaredNames.length }, (_, mask) =>
    Object.fromEntries(declaredNames.map((name, i) => [name,
      mask & (1 << i) ? domains[name]!.max : domains[name]!.min])));
  points.push(Object.fromEntries(declaredNames.map(name => {
    const d = domains[name]!;
    return [name, d.scale === "log" ? Math.exp((Math.log(d.min) + Math.log(d.max)) / 2) : d.min / 2 + d.max / 2];
  })));
  if (points.some(point => evaluate(parsed.expr, point).status !== "value"))
    fail(`${path}.referenceSource`, "The reference is undefined at a checked domain boundary or midpoint.");
  return Object.freeze({ id: id(r.id, `${path}.id`), stageId: id(r.stageId, `${path}.stageId`),
    prompt: text(r.prompt, `${path}.prompt`), declaredNames, domains: Object.freeze(domains),
    referenceSource, tolerance: Object.freeze({ absolute, relative }),
...
export function validateDiscoveryJourney(input: unknown, path = "journey"): DiscoveryJourney {
  const r = record(input, path, ["kind", "schemaVersion", "id", "paper", "revision", "reviewState",
    "title", "question", "introduction", "scope", "source", "shelf", "stages", "evidence", "conclusion"], ["exercises"]);
  ...
  if (r.reviewState !== "machine-draft") fail(`${path}.reviewState`, "This schema admits machine drafts only.");
  const revision = id(r.revision, `${path}.revision`);
  const shelf = list(r.shelf, `${path}.shelf`, 1, 24, (input, p): ShelfCard => {
    const c = record(input, p, ["id", "title", "latestYear", "availability", "admission",
      "proposition", "limitation", "citation"]);
    const latestYear = year(c.latestYear, `${p}.latestYear`);
    const availability = choice(c.availability, `${p}.availability`, ["available-by-1904", "admitted-1905"] as const);
    ...
    return Object.freeze({ id: cardId, title: text(c.title, `${p}.title`), latestYear, availability,
      admission: text(c.admission, `${p}.admission`), proposition: text(c.proposition, `${p}.proposition`),
      limitation: text(c.limitation, `${p}.limitation`), citation: citation(c.citation, `${p}.citation`) });
  });
  unique(shelf.map((c) => c.id), `${path}.shelf`);
  const stages = list(r.stages, `${path}.stages`, 3, 16, stage);
  unique(stages.map((s) => s.id), `${path}.stages`);
  if (stages.filter((s) => s.role === "move").length !== 1)
    fail(`${path}.stages`, "Name exactly one conceptual move.");
  if (!stages.some((s) => s.role === "fork")) fail(`${path}.stages`, "A journey needs a worked fork.");
  const prior = new Set<string>();
  const premiseIds = new Set(shelf.map((c) => c.id));
  for (const s of stages) {
    for (const dep of s.dependsOn)
      if (!prior.has(dep)) fail(`${path}.${s.id}`, `Dependency ${dep} has not been established; no forward or circular premises.`);
  ...
  const exercises = Object.hasOwn(r, "exercises") ? list(r.exercises, `${path}.exercises`, 2, 5, exercise) : undefined;
  if (exercises) {
    unique(exercises.map(e => e.id), `${path}.exercises`);
    for (const e of exercises) if (!prior.has(e.stageId)) fail(`${path}.${e.id}`, "Exercise references a missing guide stage.");
  }
  const evidence = list(r.evidence, `${path}.evidence`, 0, 8, (input, p): DiscoveryEvidence => {
    ...
    return Object.freeze({ id: id(e.id, `${p}.id`), year: evidenceYear,
      title: text(e.title, `${p}.title`), supports: text(e.supports, `${p}.supports`),
      limitation: text(e.limitation, `${p}.limitation`), citation: citation(e.citation, `${p}.citation`) });
  });
  unique([...shelf.map((c) => c.id), ...evidence.map((e) => e.id)], `${path}.source-identities`);
  return Object.freeze({ kind: "discovery-journey", schemaVersion: 1, id: journeyId, paper, revision,
    reviewState: "machine-draft", title: text(r.title, `${path}.title`),
    question: text(r.question, `${path}.question`), introduction: paragraphs(r.introduction, `${path}.introduction`),
    scope: paragraphs(r.scope, `${path}.scope`), source: citation(r.source, `${path}.source`),
    shelf, stages, evidence, conclusion: paragraphs(r.conclusion, `${path}.conclusion`),
    ...(exercises ? { exercises } : {}) });
}
