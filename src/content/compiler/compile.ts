...
export type PaperPayload = Readonly<{ schemaVersion: 1; paper: Paper; arguments: readonly Argument[]; foundations: readonly Foundation[]; citations: readonly Citation[] }>;
const routes = [
...
export function compileReadingContent(files: readonly Readonly<{ path: string; text: string }>[]) {
  ...
  for (const file of [...files].sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0)) {
    ...
    try {
      ...
      if (record.kind !== route.kind || record.id !== match[match.length - 1] || (record.kind === "argument" && record.paper !== match[1])) throw new ContentError("path-identity", file.path, "Record identity disagrees with its file path.");
  ...
  if (!diagnostics.some(d => d.severity === "error")) for (const paper of records.values()) if (paper.kind === "paper") {
    ...
    papers.push({ schemaVersion: 1, paper, arguments: args, foundations, citations });
  }
