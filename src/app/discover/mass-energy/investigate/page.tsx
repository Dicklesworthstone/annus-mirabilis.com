...
import { ARGUMENT_STEPS, WORKED_ARGUMENT, argumentStep } from "../../../../discovery/massEnergyArgument.ts";
import { DEFAULT_PREPARED_EXAMPLE } from "../../../../experiments/me01/session.ts";
import "./argument.css";

export const metadata: Metadata = {
  title: "Build the mass–energy argument",
  description: "Assemble the two-ledger derivation, keep its premises visible, and distinguish a mass–energy derivation from a circular consistency check.",
  robots: { index: false },
};
...
export default function MassEnergyArgumentPage() {
  const equations = Object.fromEntries(ARGUMENT_STEPS.map((card) => [card.id, <Formula key={card.id} latex={card.latex} tabIndex={0} />]));
  return (
    <article className="mass-energy-investigation" data-discovery-workbench="mass-energy" data-edition-status="explanatory-preview">
      ...
      <section className="reading" aria-labelledby="mass-energy-premises">
        ...
        <p>
          ...
          emitted in the rest frame. C denotes the additive offset in the stated identification
          H − E = K + C. Whether that offset changes is a premise to inspect, not an algebraic detail.
        </p>
        <p>
          <a href="/lab/sr-10/">Inspect the imported finite-light-complex transformation →</a>
          {" "}<a href="/lab/me-01/#two-ledgers-argument">Read the two-ledger equations and their source context →</a>
        </p>
      </section>
      ...
      <section className="reading" id="worked-argument" aria-labelledby="worked-argument-title">
        ...
        <p>
          The first useful subtraction determines a difference of frame-energy differences.
          The unchanged-offset premise then makes it a kinetic-energy decrease. The small-speed
      ...
      <section className="reading" aria-labelledby="mass-energy-transfer">
        ...
        <details>
          ...
          <p>
            The two balances and their subtraction survive. The combination K₀ − K₁ + (C₀ − C₁)
            is constrained, but K₀ − K₁ is not separately identified. An unspecified offset is
            not zero. In the laboratory below, relaxing the premise must not produce a fabricated
            numerical kinetic-energy loss.
          </p>
        </details>
        <details>
          <summary>Keep a finite observer speed. Is twice the kinetic-energy drop divided by v² the exact mass loss?</summary>
          <p>
            No. It is a finite-speed coefficient proxy. Identifying the mass loss uses its limit
            as v approaches zero. A zero-speed run alone gives zero kinetic-energy difference;
            dividing that result by zero is not the limit calculation.
          </p>
        ...
        <details>
          ...
          <p>
            The emitting body and the body-plus-radiation system are different systems. Energy
            that has left the body may remain inside a larger closed boundary. Do not transfer a
      ...
      <section id="test-the-accounts" aria-labelledby="test-the-accounts-title">
        <h2 id="test-the-accounts-title">Test consequences with the existing two-ledger instrument</h2>
        <p>
          ...
          ME-01 host-reference owner, not from the argument checker. A computed consequence does
          not independently verify its premises.
        </p>
        <TwoLedgersComparison example={DEFAULT_PREPARED_EXAMPLE} />
      </section>
      <footer className="reading actions">
        <a className="button" href="/papers/mass-energy/#arg-me-two-ledgers">Return to the explanatory paper preview →</a>
