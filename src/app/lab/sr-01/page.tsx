    ...
    <>
      <header className="page-intro">
        ...
        <p className="lead">
          ...
          time in half is Einstein&apos;s stated procedure for giving a time to something far
          away &mdash; a measurement, not an assumption about simultaneity.
        </p>
        <p>
      ...
      <section className="reading" id="clock-sync-argument">
        ...
        <p>
          ...
          frame where the procedure is applied &mdash; not because the speed of light was
          measured this way, but because the procedure was built to make it come out that way.
        </p>

        <h2>Section 2: the moving rod</h2>
        <p>
          Running the same procedure on the two ends of a moving rod gives two different leg
          times in the stationary frame: an outbound leg <code>r_AB / (c - u)</code> and a return
          leg <code>r_AB / (c + u)</code>, because the receiving end has moved during the
          light&apos;s travel. For observers moving with the rod, the criterion that held for the
          stationary observer fails.
        </p>
        ...
        <p>
          ...
          speed <var>v</var>, is found out of step by <code>vL/c&sup2;</code> in the platform
          frame &mdash; with the trailing clock ahead. Try the moving-pair preset above and
          predict the answer before revealing it.
        </p>

        <h2>Light is not the only way</h2>
        <p>
          The paper chooses light because it makes the assumption it is making visible, not
          because light is the only possible way to compare distant clocks. Slow clock transport
          is a coherent alternative: carry a clock, synchronized side by side with another, out to
          the distant station. It requires its own assumption &mdash; how a clock&apos;s rate
          depends on its motion &mdash; and agrees with the light convention only in the limit of
          vanishingly slow transport.
        </p>
