...
export const SR02_CAPTION = Object.freeze({
  ...
  r2: "A path across the boost has endpoint events with Δx = 0, so they are simultaneous in both frames and the length is unchanged. Then the magnet-frame electromotive force is vBℓ and the conductor-frame value is gamma vBℓ. The excess gamma − 1 is computed as gamma²β²/(gamma+1), never as a float64 ratio minus one.",
