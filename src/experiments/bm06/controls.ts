...
export const BM06_FIELDS: readonly { key: NumericKey; label: string; unit: string; power: number }[] = [
...
export function toBm06Draft(p: Bm06Parameters): Bm06Draft {
  const values = Object.fromEntries(BM06_FIELDS.map(f => [f.key, formatScaledDecimal(p[f.key], f.power)])) as Record<NumericKey, string>;
...
export function fromBm06Draft(draft: Bm06Draft): Bm06Parameters {
  const entries = BM06_FIELDS.map(f => {
    ...
    catch { throw new Error(`${f.label}: enter a representable finite decimal number in ${f.unit}.`); }
  });
