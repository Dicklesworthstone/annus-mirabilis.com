import { C_SI, fieldInvariants, lorentzForce, type Sr08Input, type Sr08Snapshot, type Vec3 } from "../../physics/reference/fields.ts";
...
export const SR08_FORCE_LEDGER_OUTPUTS = Object.freeze({
  ...
  forceMovingFromFourForce: contract("N", "three-force-moving-modern-oracle", "host:three-force-transform-v1"),
  forceTransformationResidual: contract("N", "three-force-covariance-residual", "host:three-force-transform-v1"),
  particleTimeJacobian: contract("1", "dt-prime-over-dt-along-particle", "host:three-force-transform-v1"),
  fieldInvariantEDotBMoving: contract("T V/m", "field-invariant-dot-moving", "fields.fieldInvariants"),
  fieldInvariantE2MinusC2B2Moving: contract("V^2/m^2", "field-invariant-difference-moving", "fields.fieldInvariants"),
});
...
const unpack = (result: ScientificResult): Vec3 | null => {
  if (result.status !== "value" || !(result.value instanceof Float64Array) || result.value.length !== 3 || !result.value.every(Number.isFinite)) return null;
...
export function forceLedgerOutputs(input: Sr08Input, snapshot: Sr08Snapshot): ScientificResult[] {
  const outside = (reason: string, domainKind: "physical" | "numerical"): ScientificResult[] =>
    (Object.keys(SR08_FORCE_LEDGER_OUTPUTS) as ForceLedgerId[]).map((id) => ({
      ...identity(id), status: "outside-domain", domainKind, reason,
  ...
  if (!E || !B || !Ep || !Bp || !u || !up || !F || !Fp)
    return outside("No finite, admitted field/velocity/force snapshot is available for this comparison.", "numerical");
  const oracle = transformThreeForce({ force: F, velocity: u, beta: input.boost / C_SI, c: C_SI });
  if (oracle.status !== "value") return outside(oracle.reason, oracle.domainKind === "physical" ? "physical" : "numerical");
  const invariants = fieldInvariants(Ep, Bp);
  const value = (id: ForceLedgerId, data: number | Float64Array): ScientificResult => ({ ...identity(id), status: "value", value: data });
  const outputs = [
    ...
    value("forceTransformationResidual", vector({ x: Fp.x - oracle.force.x, y: Fp.y - oracle.force.y, z: Fp.z - oracle.force.z })),
  ...
  for (const output of outputs) {
    ...
    const finite = typeof output.value === "number" ? Number.isFinite(output.value) : output.value instanceof Float64Array && output.value.every(Number.isFinite);
    if (!finite) return outside("The force breakdown or invariant comparison exceeds the finite numerical range.", "numerical");
  }
  return outputs;
}
