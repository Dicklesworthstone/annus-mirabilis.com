...
export function readVector3(item: PublishedResult | undefined): readonly [number, number, number] | null {
  if (!item || item.status !== "value" || typeof item.value !== "object" || item.value === null) return null;
  const data = item.value;
  if (!("length" in data) || data.length !== 3 || !("at" in data) || typeof data.at !== "function") return null;
...
export function readScalar(item: PublishedResult | undefined): number | null {
  return item?.status === "value" && typeof item.value === "number" && Number.isFinite(item.value) ? item.value : null;
}
