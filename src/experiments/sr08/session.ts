...
export function snapshotOutputs(p: Sr08Parameters): ScientificResult[] {
  ...
  return [
    snap.electricFieldStationary, snap.electricFieldMoving,
    snap.magneticFieldStationary, snap.magneticFieldMoving,
    snap.fieldInvariantEDotB, snap.fieldInvariantE2MinusC2B2, snap.lorentzFactor,
    snap.chargeVelocityStationary, snap.chargeVelocityMoving,
    snap.transverseForceLaboratory, snap.transverseForceComoving,
...
function computationFailure(outputs: readonly ScientificResult[]): string | null {
  for (const output of outputs) {
    ...
    if (output.status === "value") {
      ...
      if (!finite) return "These settings exceed the finite numerical range. Reduce field strength, charge, or speed.";
    }
  }
  return null;
}
...
export function createSr08Session(
  ...
  const store = createInstanceStore({
    experimentId: "sr-08", instanceId,
    initialParameters: initial.data as unknown as Parameters,
    parameterClasses: SR08_CLASSES,
    outputs: { ...SR08_OUTPUTS, ...SR08_FORCE_LEDGER_OUTPUTS },
    allowPartial: false,
  });
  const token = store.issue("setup-change");
  const published = store.publish({ ...token, stepIndex: 0, simulationTime: 0, final: true, outputs: initialOutputs });
  if (!published.accepted) throw new Error(`Invalid prepared SR-08 publication: ${published.reason}`);
  ...
  return Object.freeze({
    ...
    acceptedParameters(): Sr08Parameters {
      return (store.getSnapshot().accepted?.parameters ?? initial.data) as unknown as Sr08Parameters;
    },
    apply(input: unknown) {
      ...
      if (failure) return { kind: "refused" as const, refusal: {
        ...makeRefusal("invalid-parameter", { parameterIds: ["boost", "testCharge"] }),
        message: `${failure} The last accepted calculation is unchanged.`,
      } };
      const previous = store.getSnapshot().accepted!.parameters as unknown as Sr08Parameters;
      ...
      for (const key of Object.keys(next) as (keyof Sr08Parameters)[]) {
        ...
        const target = SR08_CLASSES[key] === "observer" ? observer : SR08_CLASSES[key] === "presentation" ? presentation : setup;
        target[key] = next[key];
      }
      if (![setup, observer, presentation].some((patch) => Object.keys(patch).length))
        return { kind: "accepted" as const, data: store.getSnapshot().requested! };
      ...
      const decision = store.publish({
        ...request, stepIndex: 0, simulationTime: 0, final: true, outputs,
      });
