...
export const ME03_CAPTION = Object.freeze({
  ...
  r3: "Einstein's 1905 paper draws a conditional conclusion: a body that emits energy L loses mass L/c², radium salts might put this to the test, and radiation carries inertia from the emitting to the absorbing body. In the modern four-momentum lens, two opposite pulses of total energy L have invariant mass L/c², so the system inside the boundary keeps its mass while the light stays inside. Einstein's 1906 box argument (it credits Poincaré 1900) reaches the inertia of radiation from the rule that the centre of mass of an isolated system does not move.",
});
