...
export function validateComparisonParameters(parameters: ComparisonParameters): void {
  if (!parameters || ![Object.prototype, null].includes(Object.getPrototypeOf(parameters)) ||
      Reflect.ownKeys(parameters).length > 256) throw new TypeError("Expected bounded parameter data.");
  for (const key of Reflect.ownKeys(parameters)) {
    ...
    if (!(typeof value === "boolean" || (typeof value === "string" && value.length <= 4096) ||
...
export function pinBaseline(snapshot: ComparisonSnapshot, identity: ComparisonIdentity,
  outputIds: readonly string[]): Baseline {
  if (!snapshot.final || !Number.isSafeInteger(snapshot.snapshotVersion) || snapshot.snapshotVersion < 1 ||
      !Number.isSafeInteger(snapshot.actionIndex) || snapshot.actionIndex < 0 ||
  ...
  for (const key of ["modelVersion", "streamVersion", "allocationId", "constantSetId", "sourceDigest", "executionLabel"] as const) {
  ...
  const outputs: [string, ComparisonValue][] = outputIds.map((id) => {
    ...
    if (matches.length !== 1 || !output || ![id, output.status, output.unit, output.semanticKind].every(nonempty))
      throw new TypeError(`Missing or duplicate accepted output: ${id}.`);
    if (output.status === "value" && (typeof output.value !== "number" || !Number.isFinite(output.value)))
      throw new TypeError(`Comparison output ${id} must be a finite scalar, not an array.`);
    return [id, Object.freeze({ status: output.status, unit: output.unit, semanticKind: output.semanticKind,
      value: output.status === "value" ? output.value as number : null,
      reason: typeof output.reason === "string" ? output.reason : output.status === "value" ? "" : output.status })];
  });
  return Object.freeze({
    experimentId: snapshot.experimentId, instanceId: snapshot.instanceId, runId: snapshot.runId,
    snapshotVersion: snapshot.snapshotVersion, actionIndex: snapshot.actionIndex,
    acceptedInputRevision: snapshot.revisions.input,
    revisions: Object.freeze({ ...snapshot.revisions }),
    parameters: Object.freeze({ ...snapshot.parameters }), identity: Object.freeze({ ...identity }),
