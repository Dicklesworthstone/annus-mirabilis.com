...
const identity = {
  ...
  streamVersion: "normal-1",
  allocationId: "allocation-1",
  constantSetId: "constants-1",
  sourceDigest: "source:sha256:example",
  artifactDigest: null,
  executionLabel: "host-calculation",
};
const input = (label, command = "setup-change", comparable = true) => ({
  label,
  command,
  comparable,
  unit: "",
...
const contract = {
  ...
  inputs: {
    ...
    seed: input("Seed", "setup-change", false),
    interval: input("Observation interval", "measurement-change"),
    frame: input("Frame", "observer-change"),
    estimator: input("Estimator", "estimator-change"),
...
test("pinning detaches scalar data and provenance from later mutation", () => {
  const s = snapshot(),
    meta = { ...identity },
    baseline = pin(s, meta);
  s.parameters.radius = 99;
  s.outputs[0].value = 99;
  s.revisions.input = 99;
  meta.sourceDigest = "changed";
  assert.equal(baseline.parameters.radius, 1);
  assert.equal(baseline.outputs.spread.value, 4);
  assert.equal(baseline.acceptedInputRevision, 1);
  assert.equal(baseline.identity.sourceDigest, identity.sourceDigest);
  assert.ok(Object.isFrozen(baseline.outputs.spread));
  assert.ok(Object.isFrozen(baseline.identity));
});
...
test("nonnumeric statuses stay typed with their owner explanation", () => {
  ...
  assert.equal(baseline.outputs.spread.value, null);
  assert.equal(baseline.outputs.spread.reason, "Zero elapsed time");
  const result = compareBaselines(baseline, variant(), contract);
  assert.equal(result.kind, "accepted");
  assert.equal(result.rows[0].ratio.status, "not-applicable");
});
test("missing, duplicate, nonfinite or array scalar outputs are refused", () => {
  ...
  for (const outputs of [
    ...
    [{ ...s.outputs[0], value: Infinity }],
    [{ ...s.outputs[0], value: new Float64Array([4]) }],
  ])
    assert.throws(() => pin({ ...s, outputs }));
});
...
test("a second input change is blocked with a re-baseline repair", () => {
  ...
  assert.equal(result.code, "second-input-change");
  assert.match(result.message, /pin a new baseline/u);
});
...
test("a new baseline deliberately permits a different next change", () => {
  ...
  const current = variant(
    { radius: 2, viscosity: 2 },
    { runId: "run-3", snapshotVersion: 3, actionIndex: 3 },
  );
...
test("parameter accessors never execute at the boundary", () => {
  ...
  assert.equal(singleVariationLock(pin().parameters, p, contract).kind, "refused");
...
test("physical setup changes branch the run; an unchanged identity is refused", () => {
  assert.equal(
    compareBaselines(pin(), variant({ radius: 2 }, { runId: "run-1" }), contract).code,
    "run-not-branched",
  );
});
for (const [parameter, value, command] of [
  ["interval", 4, "measurement-change"],
  ["frame", 0.6, "observer-change"],
  ["estimator", "mean-square", "estimator-change"],
]) {
  test(`${command} retains the run instead of simulating a different world`, () => {
    assert.equal(
      compareBaselines(pin(), variant({ [parameter]: value }), contract).code,
      "run-not-preserved",
    );
    const result = compareBaselines(
      pin(),
      variant({ [parameter]: value }, { runId: "run-1" }),
      contract,
    );
    assert.equal(result.kind, "accepted");
    assert.equal(result.variation.command, command);
  });
}
test("zero baseline ratios and numeric overflow are explicitly not applicable", () => {
  ...
  assert.equal(result.rows[0].ratio.status, "not-applicable");
  assert.equal(result.rows[0].difference.value, 4);
  s.outputs[0].value = Number.MIN_VALUE;
  assert.equal(
    compareBaselines(pin(s), variant(), contract).rows[0].ratio.status,
    "not-applicable",
  );
});
test("ratios and differences use accepted values rather than a model recomputation", () => {
  ...
  assert.equal(result.rows[0].ratio.value, 0.75);
  assert.equal(result.rows[0].difference.value, -1);
...
test("presentation changes make no scientific claim", () => {
  const a = pin(),
    b = variant({ zoom: 2 }, { runId: "run-1" });
  assert.match(
    comparisonStatement(a, b, contract, compareBaselines(a, b, contract)),
    /^Only the view changed/u,
  );
});
test("a stale or mixed snapshot is not a valid measurement variant", () => {
  ...
  assert.equal(
    compareBaselines(
      pin(),
      variant({ interval: 4 }, { runId: "run-1", snapshotVersion: 1 }),
      contract,
    ).code,
    "mixed-snapshot",
  );
