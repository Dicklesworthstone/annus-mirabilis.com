...
} from "./singleVariationLock.ts";

export type ComparisonNumber = Readonly<
  { status: "value"; value: number } | { status: "not-applicable"; reason: string }
>;
...
export type ComparisonResult = Readonly<
  | {
      kind: "accepted";
      variation: Extract<VariationDecision, { kind: "accepted" }>;
...
function arithmetic(
  ...
  return Number.isFinite(value)
    ? { status: "value", value }
    : na("The comparison exceeds the numeric range.");
}
...
export function compareBaselines(
  ...
  const refuse = (code: string, detail: string): ComparisonResult => ({
    kind: "refused",
    code,
    message: `${detail} The difference would not isolate one change. Keep the baseline or pin a new one.`,
  });
  if (
    baseline.experimentId !== variant.experimentId ||
    baseline.experimentId !== contract.experimentId
  )
  ...
  for (const key of ["modelVersion", "streamVersion", "allocationId", "constantSetId"] as const) {
    if (baseline.identity[key] !== variant.identity[key])
      return refuse("model-mismatch", `The ${key} changed.`);
  }
  for (const key of ["sourceDigest", "artifactDigest", "executionLabel"] as const) {
    if (baseline.identity[key] !== variant.identity[key])
      return refuse("engine-mismatch", `The ${key} changed without a declared equivalence.`);
  }
  ...
  if (
    variation.command &&
    ["measurement-change", "observer-change", "estimator-change"].includes(variation.command) &&
    !sameRun
  )
    return refuse(
      "run-not-preserved",
      "A change of measurement, observer, or estimator must preserve the recorded run.",
    );
  ...
  if (
    sameRun &&
    (variant.snapshotVersion < baseline.snapshotVersion ||
      variant.actionIndex < baseline.actionIndex)
  )
    return refuse("stale-variant", "The variant precedes the pinned result.");
  if (
    sameRun &&
    variation.changedInput !== null &&
    variant.snapshotVersion === baseline.snapshotVersion
  )
  ...
  for (const { id } of contract.outputs) {
    ...
    rows.push(
      Object.freeze({
        ...
        baseline: a,
        variant: b,
        ratio: arithmetic(a, b, "ratio"),
        difference: arithmetic(a, b, "difference"),
      }),
    );
  }
