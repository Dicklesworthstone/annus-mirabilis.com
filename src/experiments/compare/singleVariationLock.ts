...
export type ComparisonOutput = Readonly<{
  id: string;
  label: string;
  displayUnit: string;
  displayFactor: number;
...
export type VariationDecision = Readonly<
  ...
  | {
      kind: "refused";
      code: "invalid-input" | "second-input-change" | "input-locked";
      message: string;
    }
>;

/** Compare complete canonical parameter records. Form controls are never the enforcement boundary. */
export function singleVariationLock(
  baseline: ComparisonParameters,
  requested: ComparisonParameters,
  contract: ComparisonContract,
): VariationDecision {
  try {
    validateComparisonParameters(baseline);
    validateComparisonParameters(requested);
  } catch {
    return {
      kind: "refused",
      code: "invalid-input",
      message: "Use finite, complete canonical settings.",
    };
  }
  const keys = Object.keys(baseline).sort(),
    declared = Object.keys(contract.inputs).sort();
  if (
    keys.length !== declared.length ||
    keys.some((key, i) => key !== declared[i]) ||
    Object.keys(requested).length !== keys.length ||
    keys.some(
      (key) => !Object.hasOwn(requested, key) || typeof requested[key] !== typeof baseline[key],
    )
  )
    return {
      kind: "refused",
      code: "invalid-input",
      message: "Every independent input must have one declared comparison role.",
    };
  const changed = keys.filter((key) => !Object.is(baseline[key], requested[key]));
  if (changed.length > 1)
    return {
      kind: "refused",
      code: "second-input-change",
  ...
  if (!input?.comparable)
    return {
      kind: "refused",
      code: "input-locked",
      message: `${input?.label ?? key} is held fixed in this comparison. Pin a new baseline to change the question.`,
    };
