...
export const LQ03_CAPTION = Object.freeze({
  ...
  r3: "Einstein wrote L for the speed of light and ρ_{ν} for the density per unit frequency, with Planck's constants α and β, β = 4.866 × 10^{−11} (the exponent of α on the plate is read as −56 on a single witness, where the N he states needs −57). He gave the divergence no name, the textbook name for it being Ehrenfest's of 1911, and he names no one for the classical law: Rayleigh had argued for a density growing as ν^{2}T at long wavelengths in June 1900, and Jeans's correction of the coefficient came in 1905. Wien's law dates from 1896 and Planck's formula from October 1900. The value N = 6.17 × 10^{23} is Planck's own; Einstein's point in Section 2 is that it follows from the long-wavelength limit alone, without Planck's theory of resonators.",
});
