...
const draft = () =>
  Object.fromEntries(Object.entries(AVOGADRO_DEFAULTS).map(([k, v]) => [k, String(v)]));

test("default and boundary parameters round-trip with a frozen accepted copy", () => {
  for (const key of Object.keys(AVOGADRO_FIELDS))
    for (const value of [AVOGADRO_FIELDS[key].min, AVOGADRO_FIELDS[key].max]) {
      ...
      assert.notEqual(decoded.parameters, p);
      assert.ok(Object.isFrozen(decoded.parameters));
    }
});
...
test("unrecognized, missing, fractional and out-of-bound parameters refuse", () => {
  for (const input of [
    ...
    [],
    {},
    { ...AVOGADRO_DEFAULTS, unknown: 1 },
    { ...AVOGADRO_DEFAULTS, coordinateCount: 1.2 },
    { ...AVOGADRO_DEFAULTS, coefficient: 1.5 },
    { ...AVOGADRO_DEFAULTS, radiusKnown: 0.5 },
    { ...AVOGADRO_DEFAULTS, independentModel: 0.1 },
    { ...AVOGADRO_DEFAULTS, temperature: Infinity },
    { ...AVOGADRO_DEFAULTS, temperature: "293" },
  ]) {
