...
export const AVOGADRO_FIELDS: Readonly<Record<AvogadroKey, Readonly<{
  ...
  independentModel: { label: "Independent, error-free increments admitted: 1 yes, 0 no", min: 0, max: 1 },
  ...
  molarConcentration: { label: "Solute molar concentration (mol/m³ of solution)", min: 0, max: 10000 },
...
export type Validation = Readonly<{ kind: "accepted"; parameters: AvogadroParameters }> |
...
export function validateAvogadroParameters(input: unknown): Validation {
  if (typeof input !== "object" || input === null || Array.isArray(input)) return refused("Settings must be a parameter record.");
  const record = input as Record<string, unknown>;
  if (Object.keys(record).length !== keys.length || Object.keys(record).some((key) => !Object.hasOwn(AVOGADRO_FIELDS, key))) {
    return refused("Settings must contain exactly the supported parameters.");
  }
  const parameters = {} as Record<AvogadroKey, number>;
  for (const key of keys) {
    const n = record[key]; const field = AVOGADRO_FIELDS[key];
    if (typeof n !== "number" || !Number.isFinite(n) || n < field.min || n > field.max) return refused(`${field.label}: enter a finite number from ${field.min} to ${field.max}. No value was clamped.`);
    parameters[key] = n;
  }
  if (!Number.isSafeInteger(parameters.coordinateCount)) return refused("The independent coordinate count must be an integer.");
  if (![0, 1].includes(parameters.radiusKnown) || ![0, 1].includes(parameters.independentModel)) return refused("Choose yes or no for the radius and observation assumptions.");
  if (![1, 2.5].includes(parameters.coefficient)) return refused("Choose the original coefficient 1 or the corrected coefficient 2.5.");
...
export function parseAvogadroDraft(draft: Readonly<Record<AvogadroKey, string>>): Validation {
  ...
  for (const key of keys) {
    ...
    if (!text || text.length > 64 || !decimal.test(text)) return refused(`${AVOGADRO_FIELDS[key].label}: enter a decimal number, not an empty field or expression.`);
...
export function encodeAvogadroParameters(input: AvogadroParameters): string {
  ...
  if (checked.kind !== "accepted") throw new Error(checked.reason);
...
export function decodeAvogadroParameters(search: string): Validation {
  ...
  if (!query.has("av") && !keys.some((key) => query.has(key))) return validateAvogadroParameters(AVOGADRO_DEFAULTS);
  if (query.getAll("av").length !== 1 || query.get("av") !== "1") return refused("This comparison bookmark has an unsupported or ambiguous version.");
  const draft = {} as Record<AvogadroKey, string>;
  for (const key of keys) {
    if (query.getAll(key).length !== 1) return refused("A comparison bookmark must contain each parameter exactly once.");
