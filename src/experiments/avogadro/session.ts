...
import {
  ...
  MODERN_AVOGADRO,
} from "../../physics/reference/radiation/avogadro.ts";
...
const outside = (reason: string): ResultPayload =>
  Object.freeze({
    status: "outside-domain",
    condition: "inference-not-admitted",
    domainKind: "model",
    reason,
...
export function evaluateAvogadro(input: unknown) {
  ...
  output.push(
    scalar("radiationNumber", "mol^-1", "radiation", {
      status: "value",
      value: radiation.avogadroConstant / p.alphaScale,
    }),
  );
  output.push(
    scalar("definedNumber", "mol^-1", "radiation", { status: "value", value: MODERN_AVOGADRO }),
  );

  const summary = estimateSummaryStatistics({
    meanSquareDisplacement: p.meanSquareUm2 * 1e-12,
    observationInterval: p.observationSeconds,
    independentCoordinateCount: p.coordinateCount,
  });
  let brownian: ResultPayload = outside(
    "The supplied summary is not admitted by the inference owner.",
  );
  let product: ResultPayload = brownian;
  if (p.independentModel === 0) {
    brownian = outside(
      "Noise, blur, drift, overlap or censoring need their own observation model. No independent-increment interval is supplied.",
    );
    product = brownian;
    ...
    const family = identifiabilityFamily(
      {
        D: summary.data.dHat,
        T: p.temperature,
        eta: p.viscosityMpaS * 1e-3,
        radiusRange: [p.radiusUm * 1e-6, p.radiusUm * 2e-6],
        synthetic: false,
      },
      constants,
    );
    ...
    if (p.radiusKnown === 0) {
      brownian = {
        ...
        compatibleFamily:
          "a N = RT/(6 pi eta D). Doubling the radius halves the compatible molecular number.",
      ...
      if (interval.kind === "accepted") {
        ...
        if (inverse.kind === "accepted")
          brownian = {
            status: "value",
            value: inverse.data.estimate,
            uncertainty: Object.freeze({
              kind: "statistical-interval",
              lower: inverse.data.interval.lower,
              upper: inverse.data.interval.upper,
              coverage: inverse.data.interval.coverage,
              sampleSize: p.coordinateCount,
              method:
                "Independent Gaussian coordinate increments, known zero drift; exact auxiliary inputs; conditional 95% chi-square interval.",
            }),
          };
  ...
  const companion = inferMolecularDimensions({
    ...
    viscosity: p.viscosityMpaS * 1e-3,
    diffusion: p.soluteDiffusionUm2S * 1e-12,
    molarConcentration: p.molarConcentration,
    specificViscosity: p.specificViscosity,
    gasConstant: R,
    viscosityCoefficient: p.coefficient === 1 ? 1 : 2.5,
  });
  output.push(
    companion.radius,
    companion.molecularNumber,
    companion.radiusTimesMolecularNumber,
    companion.volumeFraction,
  );
  return Object.freeze({
...
export function createAvogadroSession(
  ...
  const classes: Record<string, ParameterClass> = Object.fromEntries(
    Object.keys(AVOGADRO_DEFAULTS).map((key) => [key, "input"]),
  );
  const outputs: Record<string, OutputContract> = Object.fromEntries(
    initial.outputs.map((r) => [
      r.quantityId,
      {
        unit: r.unit,
  ...
  return Object.freeze({
    ...
    apply(input: unknown) {
      ...
      // The diff below reads keys that come from Object.entries, so they arrive as `string`. The
      // two sides of this union are object types with no index signature, and indexing them with a
      // string is what TS7053 refuses. Annotating the binding as a read-only bag says what the diff
      // actually does - compare values by identity, key by key - without asserting anything about
      // which keys exist, which a cast at the index would have done silently.
      const previous: Readonly<Record<string, unknown>> =
        store.getSnapshot().accepted?.parameters ?? initial.parameters;
      // Read through a Map rather than indexing `previous` by a string key. The two
      // arms of that union have different known keys, so `previous[key]` is an
      // implicit any and `tsc` refuses it - which stopped `next build` outright for
      // everyone, because next typechecks and the export never ran. A Map keeps the
      // comparison exact without an `as` cast papering over the union.
      const previousByKey = new Map<string, unknown>(Object.entries(previous));
      const changed = Object.fromEntries(
        Object.entries(evaluated.parameters).filter(
          ([key, value]) => !Object.is(previousByKey.get(key), value),
        ),
      );
      if (Object.keys(changed).length) {
        const decision = store.publish({
          ...store.issue("setup-change", changed),
          stepIndex: 0,
          simulationTime: 0,
          final: true,
