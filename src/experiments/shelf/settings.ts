...
import { shelfPath, validateShelfInput, type ShelfId, type ShelfMode } from "./definition.ts";
...
export function decodeShelfSettings(id: ShelfId, mode: ShelfMode, search: string): ShelfSettingsLink {
  const invalid = (): ShelfSettingsLink => ({ kind: "invalid", message:
  ...
  try {
    ...
    if (Object.keys(record).length !== 4 || record.version !== 1 || record.instrument !== id || record.mode !== mode) return invalid();
    const checked = validateShelfInput(id, mode, record.parameters);
    return checked.kind === "accepted" ? { kind: "settings", parameters: checked.parameters } : invalid();
  } catch {
    return invalid();
  }
}
