...
function evaluate(id: ShelfId, mode: ShelfMode, p: Parameters): ShelfEvaluation {
  ...
  const add = (
    ...
    const payload: ScientificResult =
      item.status === "value" && (typeof item.value !== "number" || !Number.isFinite(item.value))
        ? {
            quantityId: key,
            unit,
            semanticKind,
            ownerId: "shelf-optics",
            status: "outside-domain",
            condition: "nonrepresentable-output",
            domainKind: "numerical",
            reason:
              "These settings exceed the finite numerical representation. Reduce their magnitudes.",
  ...
  if (id === "shelf-michelson-morley") {
    for (const contraction of [false, true]) {
      ...
      const result = michelsonMorleyFringeShift(
        mode === "1904"
          ...
          : {
              ...
              length: n("length"),
              wavelength: n("wavelengthNm") * 1e-9,
              contraction,
      ...
      add(
        ...
        model,
        "Perpendicular-arm round trip",
        result.timePerpendicular,
        timeUnit,
        "time",
      );
      add(
    ...
    for (const hypothesis of hypotheses) {
      const result = fizeauFringeShift({
        waterPathPerBeam: n("waterPathPerBeam"),
        wavelength: n("wavelengthNm") * 1e-9,
        refractiveIndex: n("refractiveIndex"),
        dragHypothesis: hypothesis,
        reversal: p.reversal === true,
        ...(mode === "1904"
          ? { waterSpeedFractionOfC: n("waterSpeedFractionOfC") }
          : { waterSpeed: n("waterSpeed"), constantSet: "modern-si-2019" }),
      });
      const speedUnit = mode === "1904" ? "c" : "m/s";
      const canReturn = [result.speedAlongFlow, result.speedAgainstFlow].every(
        (item) => item.status !== "value" || (typeof item.value === "number" && item.value > 0),
      );
      ...
      const returning = (item: ScientificResult): ScientificResult =>
        !canReturn && item.status === "value"
          ? {
              ...
              status: "outside-domain",
              condition: "beam-cannot-return",
              domainKind: "model",
              reason:
                "A beam cannot return against this flow under this drag hypothesis. Reduce the flow magnitude.",
              boundary: {
                parameterId: mode === "1904" ? "waterSpeedFractionOfC" : "waterSpeed",
                value: 0,
              },
            }
          : item;
      ...
      add(
        `${hypothesis}-leading`,
        hypothesis,
        "Small-flow approximation",
        returning(result.fringeShiftFirstOrder),
        "dimensionless",
        "fringe-shift",
      );
      add(
        `${hypothesis}-delay`,
        hypothesis,
        "Difference of beam times",
        returning(result.timeDifference),
        mode === "1904" ? "m/c" : "s",
        "time",
      );
      add(
        `${hypothesis}-along`,
        hypothesis,
        "Speed along the flow",
        result.speedAlongFlow,
        speedUnit,
        "speed",
      );
      add(
        `${hypothesis}-against`,
        hypothesis,
        "Return speed against the flow",
        result.speedAgainstFlow,
        speedUnit,
        "speed",
      );
    }
    ...
    if (mode === "full") {
      const later = relativisticDraggedSpeed({
        waterSpeed: n("waterSpeed"),
        refractiveIndex: n("refractiveIndex"),
        constantSet: "modern-si-2019",
      });
      add(
        "laterSpeed",
        "Later derivation (Laue 1907)",
        "Speed along the flow",
        later.draggedSpeed,
        "m/s",
        "speed",
      ...
      add(
        "laterIncrement",
        "Later derivation (Laue 1907)",
        "Increment over still water",
        later.velocityIncrement,
        "m/s",
        "speed",
      );
    }
    ...
    for (const map of ["galilean", "lorentz"] as const) {
      for (const direction of ["forward", "backward"] as const) {
        ...
        const result = waveEquationResidual({
          map,
          beta: direction === "forward" ? n("beta") : -n("beta"),
          wavenumber: n("wavenumber"),
          ...(mode === "full" ? { constantSet: "modern-si-2019" } : {}),
        });
        add(
          `${map}-${direction}`,
          map,
          `${direction === "forward" ? "Forward" : "Backward"} wave: normalised residual`,
          result.relativeResidual,
          "dimensionless",
          "residual",
        );
        add(
          `${map}-${direction}-absolute`,
          map,
          `${direction === "forward" ? "Forward" : "Backward"} wave: maximum residual`,
          result.maxResidual,
          "1/m2",
          "operator-residual",
        );
...
export function evaluateShelf(
  ...
  if (checked.kind !== "accepted")
    throw new ExperimentRuntimeError("parameters-rejected", checked.message, "shelf");
  return mode === "1904"
    ? withMode1904Guard(() => evaluate(id, mode, checked.parameters))
...
export function createShelfSession(
  ...
  const classes: Readonly<Record<string, ParameterClass>> = Object.freeze(
    Object.fromEntries(shelfFields(id, mode).map((field) => [field.id, field.parameterClass])),
  );
  const contracts: Readonly<Record<string, OutputContract>> = Object.freeze(
    Object.fromEntries(
      initial.readings.map((reading) => [
        reading.id,
        Object.freeze({
          unit: reading.unit,
          semanticKind: reading.semanticKind,
          ownerId: "shelf-optics",
  ...
  const store = createInstanceStore({
    experimentId: id,
    instanceId,
    initialParameters: checked.parameters,
  ...
  const publish = (token: RequestToken, outputs: readonly ScientificResult[]) => {
    ...
    if (!decision.accepted)
      throw new ExperimentRuntimeError(
        "publication-refused",
        `Shelf publication refused: ${decision.reason}`,
        "shelf",
      );
  };
  ...
  return Object.freeze({
    ...
    apply(input: unknown) {
      ...
      for (const [key, value] of Object.entries(next)) {
        ...
        const target =
          classes[key] === "presentation"
            ? presentation
            : classes[key] === "observer"
              ? observer
              : setup;
        target[key] = value;
      }
      ...
      if (Object.keys(presentation).length)
        token = store.issue("presentation-change", presentation);
      if (!token)
        throw new ExperimentRuntimeError(
          "no-command-for-change",
          "Changed shelf settings produced no command.",
          "shelf",
        );
