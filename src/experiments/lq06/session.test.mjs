...
test("modern and historical computations use their declared constants", () => {
  ...
  assert.notEqual(value(modern, "effectiveIndependentCount"), value(printed, "effectiveIndependentCount"));
});
...
test("invalid or nonrepresentable requests leave accepted and requested identities untouched", () => {
  ...
  for(const patch of [{constantSetId:"not-real"}, {radiationEnergy:Number.MAX_VALUE}, {frequency:Number.MIN_VALUE}]) {
...
test("a getter patch never runs and cannot alter accepted state", () => {
  ...
  let calls = 0;
  const before = session.getSnapshot();
  assert.equal(session.apply({get frequency(){calls++;return 1;}}).kind,"refused");
...
test("stale serialized output cannot impersonate historical results", () => {
  ...
  const session = createLq06Session("prepared", {
    sourceDigest:"stale-fixture", parameters:p,
    results:evaluateLq06(LQ06_DEFAULTS).map(o=>JSON.stringify(o)),stepIndex:0,simulationTime:0,
  });
...
test("server snapshot is stable and instances stay independent", () => {
  const a=createLq06Session("a"), b=createLq06Session("b");
  const server=a.getServerSnapshot(), before=b.getSnapshot();
