...
export type ShelfMetric = Readonly<{ label: string; unit: string; value: number; quantityId: string; ownerId: string }>;
export type ShelfRow = Readonly<{ modelId: string; label: string; metrics: readonly ShelfMetric[] }>;
...
export function shelfSnapshot(report: ShelfReport, revision = 0): ShelfSnapshot {
  if (!Number.isSafeInteger(revision) || revision < 0) throw new Error("Invalid calculation revision.");
  ...
  if (report.rows.length < 2 || new Set(report.rows.map((row) => row.modelId)).size !== report.rows.length) {
    throw new Error("A comparison requires distinct model rows.");
  }
  const copyMetric = (metric: ShelfMetric): ShelfMetric => {
    if (!metric.label || !metric.unit || !metric.quantityId || !metric.ownerId || !Number.isFinite(metric.value)) {
      throw new Error("The owner did not provide a finite, labeled result.");
    }
    return Object.freeze({ ...metric });
  };
  const rows = report.rows.map((row) => {
    if (!row.modelId || !row.label || row.metrics.filter((metric) => metric.label === report.primaryMetric).length !== 1) {
      throw new Error("Every model needs one named primary observable.");
    }
    return Object.freeze({ ...row, metrics: Object.freeze(row.metrics.map(copyMetric)) });
  });
  return Object.freeze({ revision, report: Object.freeze({
    ...report, parameters: parsed.parameters, rows: Object.freeze(rows),
...
export function applyShelfParameters(current: ShelfSnapshot, raw: unknown, evaluate: ShelfEvaluator): ShelfApply {
  ...
  try {
    ...
    if (JSON.stringify(report.parameters) !== JSON.stringify(parsed.parameters)) {
      return { kind: "refused", message: "The calculation returned settings different from the request. The previous calculation is unchanged." };
    }
    return { kind: "accepted", snapshot: shelfSnapshot(report, current.revision + 1) };
  } catch {
    return { kind: "refused", message: "The reference calculation could not produce a complete finite comparison. The previous calculation is unchanged." };
  }
}
...
export function primaryShelfMetrics(report: ShelfReport) {
  return report.rows.map((row) => {
    ...
    if (!metric) throw new Error("Missing primary observable.");
