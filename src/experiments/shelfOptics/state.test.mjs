...
import { SHELF_IDS, SHELF_DEFINITIONS, isShelfId, parseShelfParameters, shelfDraft } from "./definition.ts";
...
function report(parameters, value = 0) {
  return {
    ...
    rows: ["model-a", "model-b"].map((modelId) => ({
      ...
      metrics: [{ label: "Observable", unit: "1", value, quantityId: "test-value", ownerId: "test-owner" }],
    })),
    later: [],
  };
}
...
for (const id of SHELF_IDS) {
  ...
  for (const field of definition.fields) {
    test(`${id}: ${field.key} includes both declared bounds and rejects values beyond them`, () => {
      for (const bound of [field.min, field.max]) {
        assert.equal(parseShelfParameters(id, { ...definition.defaults, [field.key]: bound }).kind, "parameters");
      }
      for (const bound of [field.min - Math.max(1, Math.abs(field.min)), field.max + Math.max(1, Math.abs(field.max))]) {
        assert.equal(parseShelfParameters(id, { ...definition.defaults, [field.key]: bound }).kind, "refused");
      }
    });
    test(`${id}: ${field.key} cannot be absent, inherited, empty or coerced`, () => {
      for (const value of [undefined, null, "", "  ", true, false, NaN, Infinity, -Infinity, "NaN", "Infinity", "0x10", "0b11", "1,000", "1e999", {}, [], { valueOf: () => 1 }]) {
        assert.equal(parseShelfParameters(id, { ...definition.defaults, [field.key]: value }).kind, "refused", String(value));
      }
      const { [field.key]: inherited, ...own } = definition.defaults;
      assert.equal(parseShelfParameters(id, Object.assign(Object.create({ [field.key]: inherited }), own)).kind, "refused");
    });
  }
  for (const field of definition.switches) {
    test(`${id}: ${field.key} accepts booleans only, not strings or absent flags`, () => {
      for (const value of ["false", "true", 0, 1, null, undefined]) {
        assert.equal(parseShelfParameters(id, { ...definition.defaults, [field.key]: value }).kind, "refused");
      }
      for (const value of [true, false]) {
        assert.equal(parseShelfParameters(id, { ...definition.defaults, [field.key]: value }).kind, "parameters");
      }
    });
  }
  test(`${id}: refuse foreign instrument settings and unknown fields atomically`, () => {
    ...
    let calls = 0;
    const evaluate = () => { calls += 1; throw new Error("Should not run"); };
    for (const input of [null, [], "bad", {}, { ...definition.defaults, instrumentId: "other" }, { ...definition.defaults, unexpected: 1 }]) {
...
test("numeric input includes finite scientific notation and signed zero", () => {
  ...
  const result = parseShelfParameters(id, { armLength: "+1.1e1", wavelength: " 5.5E-7 ", beta: "-0" });
  assert.equal(result.kind, "parameters");
...
test("copy/freeze prevents aliases between instances or evaluator-owned arrays", () => {
  ...
  source.later.push({ label: "late", unit: "1", value: 99, ownerId: "test-owner", quantityId: "late" });
  ...
  assert.throws(() => { first.report.rows[0].metrics[0].value = 7; });
});

for (const [name, broken] of [
  ["exception", () => { throw new Error("owner failure"); }],
...
test("revision overflow is refused rather than publishing an ambiguous identity", () => {
  const current = shelfSnapshot(report(SHELF_DEFINITIONS["shelf-fizeau"].defaults), Number.MAX_SAFE_INTEGER);
  assert.equal(applyShelfParameters(current, current.report.parameters, report).kind, "refused");
  for (const revision of [-1, 0.5, NaN, Infinity]) assert.throws(() => shelfSnapshot(current.report, revision));
});
