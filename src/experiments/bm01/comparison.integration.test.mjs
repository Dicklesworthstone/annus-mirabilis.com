...
const parameters = { ...BM01_DEFAULTS, T: 290.15, eta: 1.35e-3, M: 64, h: .1, H: 4,
  seed: "18446744073709551615" };
const sourceDigest = `source:sha256:${"a".repeat(64)}`; // Fixture protocol identity, not a production digest.
const unwrap = (result) => { assert.equal(result.kind, "accepted"); return result.data; };
const scalar = (outputs, id) => {
  const value = outputs.find((output) => output.quantityId === id);
  assert.equal(value?.status, "value"); assert.equal(typeof value.value, "number"); return value.value;
};
function prepared(p, evaluation) {
  return { parameters: p, sourceDigest, results: evaluation.outputs.map(encodeResult),
    stepIndex: evaluation.stepIndex, simulationTime: evaluation.simulationTime };
}
async function examples() {
  ...
  return { schemaVersion: 1, baseline, doubledRadius: prepared(doubled, unwrap(measureBm01(other, doubled, false))),
    allocationId: recording.allocationId, streamVersion: recording.normalVersion };
}
function pinned(id, example, identity) {
  const session = createBm01Session(id, example, () => { throw new Error("Static projection started a worker"); });
  return pinBaseline(session.getServerSnapshot().accepted, identity, BM01_COMPARISON.outputs.map((output) => output.id));
}

for (const key of ["a", "eta", "T"]) {
  test(`the real owner propagates a doubled ${key} into diffusivity and both RMS readouts`, async () => {
    const example = await examples(), identity = bm01ComparisonIdentity(example);
    ...
    const result = compareBaselines(pinned("baseline", example.baseline, identity),
      pinned("variant", prepared(p, evaluated), identity), BM01_COMPARISON);
    assert.equal(result.kind, "accepted");
    const expectedD = key === "T" ? 2 : .5;
    for (const [id, expected] of [["diffusionCoefficient", expectedD], ["rmsDisplacement1d", Math.sqrt(expectedD)], ["sampleRms", Math.sqrt(expectedD)]]) {
      const ratio = result.rows.find((row) => row.id === id)?.ratio;
      assert.equal(ratio?.status, "value");
      assert.ok(withinTolerance(ratio.value, expected, { relative: 1e-12 }).ok, `${id}: ${ratio.value}`);
    }
    ...
    if (key !== "T") {
      ...
      assert.equal(withinTolerance(actual, .5, { relative: 1e-12 }).ok, false);
    }
  });
}
test("remeasurement consumes no draws and leaves every latent coordinate bitwise intact", async () => {
  const recording = unwrap(await createBm01Recording(parameters, { yieldControl: async () => {} }));
  const before = recording.values.slice(), draws = recording.draws;
  ...
  assert.ok(withinTolerance(scalar(second.outputs, "rmsDisplacement1d") / scalar(first.outputs, "rmsDisplacement1d"), 2, { relative: 1e-12 }).ok);
});
test("zero time stays a typed apparent-speed limit and off-grid time is refused", async () => {
  ...
  assert.equal(zero.outputs.find((output) => output.quantityId === "modelApparentSpeed").status, "not-applicable");
  assert.equal(measureBm01(recording, { ...parameters, interval: .15 }, true).kind, "refused");
});
...
test("the real session, scheduler, codec and owner preserve a warm recording across a comparison", async () => {
  ...
  function channel() {
    ...
    const host = createBm01Host((message) => {
      replies.push(message); onMessage(structuredClone(message));
    }, sourceDigest);
    return {
      send(message) { void host.receive(structuredClone(message)).catch((error) => { failures.push(error); onError(); }); },
      listen(message, error) { onMessage = message; onError = error; queueMicrotask(() => host.hello());
        return () => { onMessage = () => {}; onError = () => {}; }; },
      dispose() { disposals++; host.dispose(); },
    };
  }
  ...
  async function complete(action) {
    ...
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => { unsubscribe(); reject(new Error("Comparison did not finish")); }, 10000);
      const unsubscribe = comparison.subscribe(() => {
        if (!comparison.getSnapshot().pending) { clearTimeout(timer); unsubscribe(); resolve(); }
      });
    });
  ...
  try {
    ...
    const plot = (s) => s.outputs.find((output) => output.quantityId === "plotSampleRms").value.copy();
    ...
    assert.equal(creates, 1); assert.deepEqual(failures, []);
  } finally { comparison.disconnect(); }
