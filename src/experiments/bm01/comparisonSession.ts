...
import { BM01_COMPARISON, bm01ComparisonIdentity, type PreparedBm01Comparison, verifyBm01Comparison } from "./comparison.ts";
import { createBm01Session } from "./session.ts";

/** Composes the existing session; no comparison-specific worker or numerical owner. */
export function createBm01ComparisonSession(instanceId: string, example: PreparedBm01Comparison,
  ...
  const staticVariant = createBm01Session(`${instanceId}-worked-variant`, example.doubledRadius, () => {
  ...
  return createControlledComparison(session, {
    contract: BM01_COMPARISON, identity: bm01ComparisonIdentity(example), baseline, variant,
