...
export function encodeBm01Settings(p: Bm01Parameters): string {
  if (validateBm01Parameters(p).kind !== "accepted")
    throw new TypeError("Cannot share invalid tracer settings.");
  const q = new URLSearchParams({ tracers: "1" });
  for (const k of Object.keys(BM01_DEFAULTS) as (keyof Bm01Parameters)[]) q.set(k, String(p[k]));
  return `?${q}`;
}
export function decodeBm01Settings(
  search: string,
):
  | { kind: "absent" }
  ...
  const q = new URLSearchParams(search),
    keys = Object.keys(BM01_DEFAULTS);
  if (
    q.get("tracers") !== "1" ||
    q.size !== keys.length + 1 ||
    [...q.keys()].some((k) => k !== "tracers" && !keys.includes(k))
  )
    return invalid();
  ...
  try {
    for (const k of keys) {
      ...
      const v = q.get(k)!;
      p[k] = k === "seed" || k === "statistic" ? v : parseScaledDecimal(v, 0);
    }
  } catch {
    return invalid();
  }
  const result = validateBm01Parameters(p);
