import { embedInstrument, isEmbeddableId, type EmbeddableId } from "./catalogue.ts";
...
export const DEFAULT_EMBED_OPTIONS: EmbedOptions = Object.freeze({
  theme: "system", detail: "overview", motion: "system",
});
const allowed = {
  theme: ["system", "light", "dark"],
  detail: ["overview", "full", "steps"],
...
export function decodeEmbedOptions(search: string): EmbedDecode {
  ...
  for (const key of query.keys()) {
    if (!keys.includes(key)) return invalid("This embed does not accept experiment settings, tapes, or other URL fields. Open the full laboratory to restore an experiment.");
    if (query.getAll(key).length !== 1) return invalid("The embed link repeats a presentation option.");
  }
...
export function embedPath(id: EmbeddableId, options: EmbedOptions = DEFAULT_EMBED_OPTIONS): string {
  if (!isEmbeddableId(id)) throw new TypeError("No admitted adapter exists for this instrument.");
  // Runtime callers do not get to smuggle additional object properties into a public URL.
  const query = new URLSearchParams({ embed: EMBED_VERSION, theme: options.theme, detail: options.detail, motion: options.motion });
  const parsed = decodeEmbedOptions(query.toString());
  if (parsed.kind !== "options") throw new TypeError(parsed.message);
 ...
 */
export function embedMarkup(id: EmbeddableId, options: EmbedOptions = DEFAULT_EMBED_OPTIONS, height = 900): string {
  const instrument = embedInstrument(id);
  if (!instrument) throw new TypeError("Unknown embeddable instrument.");
  if (!Number.isInteger(height) || height < 400 || height > 2000)
    throw new TypeError("Embed height must be a whole number from 400 to 2000 pixels.");
