...
export const SR01_CAPTION = Object.freeze({
  ...
  r1: "Einstein's procedure: a signal leaves clock A at t_{A}, reflects at the distant clock B, and returns to A at t′_{A}. The remote event is assigned the time (t_{A} + t′_{A})/2 by definition, and the round-trip speed 2AB/(t′_{A} − t_{A}) equals c in the frame where the procedure runs. A pair of clocks synchronized this way in their own rest frame, but moving at v as judged from a platform, is found out of step there by vL/c², with the trailing clock ahead.",
  r2: "Start from the assumption that light travels at the same speed c in every direction and every inertial frame. A flash leaves A at t_{A}; it must reach B after covering the distance AB, so it arrives at t_{A} + AB/c. The paper does not measure that one-way time. It chooses to call it t_{B}, which is exactly the halfway point between t_{A} and the return time t′_{A}, since the return leg covers the same distance at the same speed. Doing this for a rod's two ends while the rod itself moves (section 2) gives two different leg times, r_{AB}/(c − u) going out and r_{AB}/(c + u) coming back, because the receiving end has moved during the light's travel.",
