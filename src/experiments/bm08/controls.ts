...
const powers: Partial<Record<keyof Bm08Parameters, number>> = { D: 12, sigma: 6, flowDrift: 6, stageDrift: 6, coverage: 2 };
export function toCameraDraft(p: Bm08Parameters): CameraDraft {
  return Object.fromEntries((Object.keys(BM08_DEFAULTS) as (keyof Bm08Parameters)[]).map(k => [k, typeof p[k] === "number" ? formatScaledDecimal(p[k] as number, powers[k] ?? 0) : String(p[k])])) as CameraDraft;
}
