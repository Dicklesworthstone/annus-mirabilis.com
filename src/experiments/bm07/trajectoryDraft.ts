...
export const EMPTY_TRAJECTORY_DRAFT: TrajectoryDraft = Object.freeze({
  timeUnit: "", positionUnit: "", micrometresPerPixel: "",
  estimator: "drift-centered", coveragePercent: "95",
  independentIsotropic: false, commonDriftAndDiffusion: false,
  localizationNanometres: "", exposureMilliseconds: "", censored: "unknown",
  radiusIndependent: false, temperatureKelvin: "", viscosityMillipascalSeconds: "", radiusMicrometres: "",
});

function numeric(text: string, label: string, scale: number, allowZero = false): number {
  if (!/^[+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?$/.test(text.trim())) {
    throw new Error(`${label} needs an explicit ${allowZero ? "nonnegative" : "positive"} decimal value.`);
  }
  const raw = Number(text), value = raw * scale;
...
export function readTrajectoryDraft(draft: TrajectoryDraft): Readonly<{
  ...
  if (!["s", "ms"].includes(draft.timeUnit) || !["m", "um", "nm", "px"].includes(draft.positionUnit)) {
    throw new Error("Choose both time and position units; they are never inferred from magnitudes.");
  }
  if (!["independent-increment-known-zero-drift", "drift-centered", "maximum-likelihood-centered"].includes(draft.estimator)) {
    throw new Error("Choose a registered estimator.");
  }
  if (!["unknown", "yes", "no"].includes(draft.censored)) throw new Error("Declare the observation selection status.");
  const percent = numeric(draft.coveragePercent, "Coverage", 1);
  if (percent < 50 || percent > 99.9) throw new Error("Choose interval coverage from 50% through 99.9%.");
  ...
  const coverage = Math.min(.999, percent / 100);
  ...
  return Object.freeze({
    ...
    assumptions: Object.freeze({
      ...
      localizationStd: draft.localizationNanometres.trim() === "" ? null : numeric(draft.localizationNanometres, "Localization error", 1e-9, true),
      exposureTime: draft.exposureMilliseconds.trim() === "" ? null : numeric(draft.exposureMilliseconds, "Exposure time", 1e-3, true),
      censored: draft.censored === "unknown" ? null : draft.censored === "yes",
      independentRadius: draft.radiusIndependent ? Object.freeze({
