...
export const KITCHEN_COLUMNS = Object.freeze([
  "schema_version", "kind", "object_id", "frame_time_s", "x_px", "y_px", "lost",
  "loss_reason", "point_status", "exclusion_reason", "calibration_id", "identity_decision",
] as const);
...
export const KITCHEN_METADATA_KEYS = Object.freeze([
  "source_width_px", "source_height_px", "working_scale", "rotation_degrees", "pixel_aspect_ratio",
...
export type KitchenPoint = Readonly<{
  ...
  objectId: string; time: number; x: number | null; y: number | null;
  status: PointStatus; lossReason: "" | "edge" | "focus" | "occluded";
  exclusionReason: string; calibrationId: string;
  identityDecision: "" | "reacquired-same" | "new-object";
}>;
export type KitchenDocument = Readonly<{
  schemaVersion: 2; metadata: KitchenMetadata; points: readonly KitchenPoint[];
  notes: readonly string[];
}>;
export const KITCHEN_LIMITS = Object.freeze({
  bytes: 2 * 1024 * 1024, rows: 20000, cell: 2048, duration: 600,
  coordinates: 100000, edgeLossShare: .2, anisotropy: .02, stationaryClicks: 10,
});
export class KitchenInputError extends TypeError {
  ...
  constructor(row: number, field: string, message: string) {
    ...
    this.name = "KitchenInputError"; this.row = row; this.field = field;
  }
}
...
export function intervalMetadata(raw: string, field: string): readonly [number, number] | null {
  ...
  const a = kitchenNumber(m[1]!.trim(), field), b = kitchenNumber(m[2]!.trim(), field);
  if (!(a > 0 && b >= a)) throw new KitchenInputError(0, field, "bounds must be positive and ordered.");
...
export function validateKitchenMetadata(input: Record<string, string>): KitchenMetadata {
  ...
  const numeric = (key: MetadataKey, min: number, max: number, nullable = false) => {
    ...
    if (n < min || n > max) throw new KitchenInputError(0, key, `use a number from ${min} to ${max}.`);
    m[key] = String(n);
  };
  numeric("source_width_px", 1, 4096); numeric("source_height_px", 1, 4096);
  for (const k of ["source_width_px", "source_height_px"] as const) if (!Number.isInteger(Number(m[k]))) throw new KitchenInputError(0, k, "use a whole number of pixels.");
  numeric("working_scale", 1e-6, 1); numeric("pixel_aspect_ratio", .01, 100, true);
  for (const k of ["pixels_per_um_x", "pixels_per_um_y"] as const) numeric(k, 1e-6, 1e6, true);
  for (const k of ["pixels_per_um_x_uncertainty", "pixels_per_um_y_uncertainty"] as const) numeric(k, 0, 1e6, true);
  numeric("frame_rate_hz", .1, 10000); numeric("temperature_k", 1, 2000, true);
  numeric("viscosity_mpa_s", 1e-6, 1e9, true); numeric("radius_um", 1e-6, 1e6, true);
  numeric("radius_interval_coverage", .001, 1, true); numeric("exposure_s", 0, 2, true);
  const choice = (k: MetadataKey, values: readonly string[]) => { if (!values.includes(m[k])) throw new KitchenInputError(0, k, `choose ${values.join(", ")}.`); };
  choice("rotation_degrees", ["0", "90", "180", "270"]); choice("calibration_axes", ["x", "y", "both"]);
  choice("calibration_method", ["micrometer", "hair", "other"]); choice("declared_interval_s", ["0.5", "1", "2"]);
  ...
  for (const [k, point] of [["temperature_interval_k", "temperature_k"], ["radius_interval_um", "radius_um"], ["calibration_interval_um", ""]] as const) {
    ...
    if (range && point && m[point] && !(range[0] <= Number(m[point]) && Number(m[point]) <= range[1]))
      throw new KitchenInputError(0, k, "the range must contain the declared point value.");
    if (range) m[k] = `[${range.join(",")}]`;
  }
  if (m.exposure_s && Number(m.exposure_s) > Number(m.declared_interval_s)) throw new KitchenInputError(0, "exposure_s", "overlapping exposures are not supported.");
  if (m.viscosity_mpa_s && !m.viscosity_source.trim()) throw new KitchenInputError(0, "viscosity_source", "name the source or the approximation you chose.");
