import { type ExecutionOutcomeId, executionOutcomeRegistry } from "../../results/outcomes.ts";
...
export type AcquiredVideoFrame = Readonly<{
...
function outcome(
  code: Exclude<ExecutionOutcomeId, "budget-exhausted">,
  message: string,
): CaptureFailure {
  return {
    kind: "outcome",
...
export function createVideoFrameReader(
  video: VideoPort,
  fps: number,
  draw: (geometry: VideoGeometry) => void,
  ...
  let accepted: AcquiredVideoFrame | null = null,
    imageValid = true;
  const callbacks =
  ...
  function cancel(code: "cancelled" | "superseded" = "cancelled") {
    ...
    pending?.(
      outcome(
        code,
        code === "cancelled"
          ? "Frame reading stopped. Existing annotations are unchanged."
          : "A newer frame request replaced this one.",
      ),
    );
    pending = null;
  }
  ...
  function acquire(target: number, action: () => void): Promise<CaptureResult<AcquiredVideoFrame>> {
    ...
    if (disposed)
      return Promise.resolve(
        outcome(
          "cancelled",
          "This video was released. Export annotations before starting another session.",
        ),
      );
    if (reads >= limits.frameReads)
      return Promise.resolve(
        captureBudget(
          "Frame-read budget reached. Export the annotations before starting a new clip.",
          reads + 1,
          limits.frameReads,
        ),
      );
    ...
    return new Promise((resolve) => {
      ...
      const geometry = (): CaptureResult<VideoGeometry> => {
        if (
          accepted &&
          (video.videoWidth !== accepted.geometry.width ||
            video.videoHeight !== accepted.geometry.height)
        )
          return captureRefusal(
            "Video dimensions changed. Export this session and recalibrate a separate clip; old annotations are unchanged.",
            "geometry",
          );
      ...
      const metadata = () => {
        ...
        if (g.kind !== "ready") finish(g);
          ...
          video.videoWidth > limits.sourceEdge ||
          video.videoHeight > limits.sourceEdge ||
          video.duration > limits.durationSeconds
        )
          finish(
            captureRefusal(
              "The decoded video exceeds this session's size or duration limit.",
              "video",
            ),
          );
      };
      const error = () =>
        finish(
          outcome(
            ...
            "The browser could not decode this local video. Try another supported encoding or import coordinates as CSV.",
          ),
        );
      const capture = (time: number, counter: number | null) => {
        ...
        try {
          ...
          finish(
            outcome(
              "context-lost",
              "The frame could not be drawn. No observation was added; previous annotations are preserved.",
            ),
          );
          return true;
        }
      ...
      const fallback = () => {
        if (!callbacks) capture(video.currentTime, null);
      };
      ...
      timer = setTimer(
        () =>
          finish(
            captureBudget(
              "Reading one frame exceeded the time budget. No click was added. Retry or use a shorter/lower-resolution clip.",
              limits.stepMilliseconds + 1,
              limits.stepMilliseconds,
            ),
          ),
        limits.stepMilliseconds,
      );
  ...
  return {
    ...
    async open(file: Blob): Promise<CaptureResult<AcquiredVideoFrame>> {
      if (url || disposed)
        return captureRefusal("Start a new capture session before choosing another video.", "file");
      const checked = checkVideoFile(file.size);
      if (checked.kind !== "ready") return checked;
      if (file.size > limits.fileBytes)
        return captureRefusal(
          "The local file exceeds the configured byte limit.",
          "fileBytes",
          file.size,
          limits.fileBytes,
        );
      try {
        ...
        return outcome(
          "environment-unsupported",
          "Local video URLs are unavailable. Use the CSV import instead.",
        );
      }
    ...
    read(time: number): Promise<CaptureResult<AcquiredVideoFrame>> {
      if (!accepted || !url || disposed)
        return Promise.resolve(outcome("cancelled", "Open a local video before reading a frame."));
      if (!Number.isFinite(time) || time < 0 || time >= accepted.geometry.duration)
        return Promise.resolve(
          captureRefusal(
            "Choose a time at or after zero and before the end of this video.",
            "time",
          ),
        );
