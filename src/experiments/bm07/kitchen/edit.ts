...
export function setKitchenExclusion(
  ...
  if (point.kind !== "particle" || !["measured", "excluded"].includes(point.status))
    throw new TypeError(
      "Only measured or already excluded particle rows can be relabeled here. Lost and interpolated rows retain their provenance.",
    );
  if (reason !== null && !reason.trim())
    throw new TypeError("Record why this observation is excluded.");
  const revised: KitchenPoint = {
    ...point,
    status: reason === null ? "measured" : "excluded",
    exclusionReason: reason?.trim() ?? "",
  };
  // Read/write through the same bounded contract as imports. No parallel validation grammar.
  return parseKitchenCsv(
    exportKitchenCsv({
      ...document,
      points: document.points.map((p, i) => (i === index ? revised : p)),
    }),
  );
}
...
export function kitchenInputDraft(document: KitchenDocument): KitchenInputDraft {
  return Object.freeze(
    Object.fromEntries(KITCHEN_EDITABLE_INPUTS.map((k) => [k, document.metadata[k]])),
  ) as KitchenInputDraft;
}
/** Observation origin, timestamps, pixel coordinates and identity decisions cannot be rewritten by this form. */
export function reviseKitchenInputs(
  document: KitchenDocument,
  input: KitchenInputDraft,
): KitchenDocument {
  const keys = Reflect.ownKeys(input);
  if (
    keys.length !== KITCHEN_EDITABLE_INPUTS.length ||
    keys.some(
      (k) => typeof k !== "string" || !(KITCHEN_EDITABLE_INPUTS as readonly string[]).includes(k),
    )
  )
  ...
  return parseKitchenCsv(
    exportKitchenCsv({ ...document, metadata: { ...document.metadata, ...input } }),
  );
}
