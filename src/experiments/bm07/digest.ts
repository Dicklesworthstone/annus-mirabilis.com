// Pure TypeScript SHA-256 implementation (browser & runtime safe)
...
function sha256Bytes(data: Uint8Array): string {
  ...
  const k = (byteLen + 1 + 8) % 64;
  const padLen = k === 0 ? 0 : 64 - k;
...
export function observationDigest(increments: Float64Array, extra = ""): string {
  const incBytes = new Uint8Array(increments.buffer, increments.byteOffset, increments.byteLength);
  let combined = incBytes;
  if (extra.length > 0) {
    const extraBytes = new TextEncoder().encode(extra);
    combined = new Uint8Array(incBytes.length + extraBytes.length);
    combined.set(incBytes, 0);
    combined.set(extraBytes, incBytes.length);
  }
  return `sha256:${sha256Bytes(combined)}`;
}
