...
export function decodeMe01Settings(search: string): Me01PermalinkResult {
  try {
    const query = settingsQuery(search, [
      "v",
      "phi",
      "L",
      "premise",
    ...
    const checked = validateMe01Parameters({
      ...
      offsetDisplay: settingChoice(
        query,
        "offsets",
        ["symbolic", "offsets"],
        ME01_DEFAULTS.offsetDisplay,
      ),
      step: settingChoice(
        query,
        "step",
      ...
      cancelInternalEnergies: settingBoolean(
        query,
        "internal",
        ME01_DEFAULTS.cancelInternalEnergies,
      ),
      cancelAdditiveConstant: settingBoolean(
        query,
        "constant",
        ME01_DEFAULTS.cancelAdditiveConstant,
      ),
    });
    if (checked.kind !== "accepted")
      throw new Error("The link settings are outside the model domain and could not be loaded.");
    return { kind: "settings", parameters: checked.data };
  } catch (error) {
    return {
      kind: "invalid",
      message: error instanceof Error ? error.message : "Invalid two-ledger settings link.",
    };
  }
}
...
export function encodeMe01Settings(p: Me01Parameters): string {
  ...
  const values = {
    v: p.frameSpeed,
    phi: p.emissionAngle,
    L: p.emittedEnergyRestFrame,
    premise: p.premise,
    notation: p.notation,
    offsets: p.offsetDisplay,
    step: p.step,
    angle: p.cancelAngleFactors ? "1" : "0",
    internal: p.cancelInternalEnergies ? "1" : "0",
    constant: p.cancelAdditiveConstant ? "1" : "0",
  };
  const defaults = {
    v: ME01_DEFAULTS.frameSpeed,
    phi: ME01_DEFAULTS.emissionAngle,
    L: ME01_DEFAULTS.emittedEnergyRestFrame,
    premise: ME01_DEFAULTS.premise,
    notation: ME01_DEFAULTS.notation,
    offsets: ME01_DEFAULTS.offsetDisplay,
    step: ME01_DEFAULTS.step,
    angle: "1",
    internal: "1",
    constant: "1",
  };
  ...
  if (decodeMe01Settings(search).kind === "invalid")
    throw new Error("Cannot share invalid two-ledger parameters.");
  return search;
}
