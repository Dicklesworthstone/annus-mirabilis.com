 ...
 */
export function validateToleranceSpec(spec: ToleranceSpec, reference: number): readonly ToleranceIssue[] {
  const issues: ToleranceIssue[] = [];
  if (spec.absolute !== undefined && !isValidAbsolute(spec.absolute)) {
    issues.push({ code: "invalid-number", message: `absolute tolerance ${spec.absolute} must be a finite number >= 0.` });
  }
  if (spec.relative !== undefined && !isValidRelative(spec.relative)) {
    issues.push({ code: "invalid-number", message: `relative tolerance ${spec.relative} must be a finite number in [0, 1).` });
  }
  ...
  if (!anyInvalid && absolute === 0 && relative === 0) {
    issues.push({
      ...
      message: "Neither absolute nor relative tolerance is positive; use compareBitwise for an exact comparison instead.",
    });
  }
 ...
 */
export function validateToleranceSpecAcross(spec: ToleranceSpec, references: readonly number[]): readonly ToleranceIssue[] {
  const issues: ToleranceIssue[] = [];
  ...
  for (const reference of references) for (const issue of validateToleranceSpec(spec, reference)) record(issue);
...
export type ToleranceVerdictKind = "within" | "outside" | "invalid-spec" | "nonfinite-actual" | "nonfinite-reference";
...
function verdict(ok: boolean, kind: ToleranceVerdictKind, diff: number, allowed: number, issues: readonly ToleranceIssue[] = []): ToleranceVerdict {
 ...
 */
export function withinTolerance(actual: number, reference: number, spec: ToleranceSpec): ToleranceVerdict {
  if (!Number.isFinite(reference)) return verdict(false, "nonfinite-reference", Number.NaN, Number.NaN);
  ...
  const scale = relativeTo === "larger" ? Math.max(Math.abs(actual), Math.abs(reference)) : Math.abs(reference);
...
function compareArraysBitwise(actual: ArrayLike<number | bigint>, expected: ArrayLike<number | bigint>): BitwiseVerdict {
  if (actual.length !== expected.length) {
    return Object.freeze({ ok: false, kind: "length-mismatch" as const, detail: `lengths differ: ${actual.length} vs ${expected.length}` });
  }
  for (let i = 0; i < actual.length; i++) {
    if (!bitwiseEqualScalar(actual[i]!, expected[i]!)) {
      return Object.freeze({ ok: false, kind: "mismatch" as const, detail: `first differing index: ${i} (${String(actual[i])} vs ${String(expected[i])})` });
    }
  }
  return Object.freeze({ ok: true, kind: "match" as const, detail: `identical across ${actual.length} elements` });
}
...
export function compareBitwise(actual: unknown, expected: unknown): BitwiseVerdict {
  if (typeof actual === "number" && typeof expected === "number") {
    ...
    return Object.freeze({
      ...
      detail: equal ? "identical IEEE-754 bit patterns" : `bit patterns differ: 0x${float64Bits(actual).toString(16)} vs 0x${float64Bits(expected).toString(16)}`,
    });
  }
  if (typeof actual === "bigint" && typeof expected === "bigint") {
    const equal = actual === expected;
    return Object.freeze({ ok: equal, kind: equal ? ("match" as const) : ("mismatch" as const), detail: equal ? "identical bigint value" : `bigint values differ: ${actual} vs ${expected}` });
  }
  if (typeof actual === "string" && typeof expected === "string") {
    const equal = actual === expected;
    return Object.freeze({ ok: equal, kind: equal ? ("match" as const) : ("mismatch" as const), detail: equal ? "identical string" : "strings differ" });
  }
  if (isNumericArrayLike(actual) && isNumericArrayLike(expected)) {
    return compareArraysBitwise(actual, expected);
  }
  return Object.freeze({ ok: false, kind: "type-mismatch" as const, detail: `unsupported or mismatched types: ${describeBitwiseType(actual)} vs ${describeBitwiseType(expected)}` });
}
 ...
 */
export function classifyWithTolerance(value: number, band: ToleranceBand = {}): ClassificationVerdict {
  if (!Number.isFinite(value)) throw new TypeError("classifyWithTolerance requires a finite value.");
  ...
  const sign: SignClassification =
    value === 0 && allowed === 0 ? "zero" : allowed > 0 && Math.abs(value) <= allowed ? "indeterminate" : value > 0 ? "positive" : "negative";
