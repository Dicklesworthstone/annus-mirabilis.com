...
function shift(text: string, power: number): string {
  if (!Number.isSafeInteger(power) || Math.abs(power) > 308)
    throw new RangeError("Unsupported decimal unit scale.");
  if (text.length > 128 || !/^[+-]?(?:\d+\.?\d*|\.\d+)(?:e[+-]?\d+)?$/i.test(text))
  ...
  if (!Number.isSafeInteger(adjusted))
    throw new RangeError("The exponent is outside the supported range.");
