...
export function validateSessionRecord(raw: unknown, path = "SessionRecord"): SessionRecord {
  ...
  const forbiddenPersonKeys = ["participantId", "userId", "personId", "participantName", "learnerId", "userLevel", "placementScore"];
  ...
  if (typeof raw.sessionId !== "string" || !raw.sessionId.trim()) {
    throw new SessionValidationError("missing-session-id", "sessionId is required", `${path}.sessionId`);
  }
  ...
  if (typeof raw.argumentId !== "string" || !raw.argumentId.trim()) {
    throw new SessionValidationError("missing-argument-id", "argumentId is required", `${path}.argumentId`);
  }

  if (typeof raw.facilitator !== "string" || !raw.facilitator.trim()) {
    throw new SessionValidationError("missing-facilitator", "facilitator is required", `${path}.facilitator`);
  }
  ...
  if (typeof raw.buildCommit !== "string" || !raw.buildCommit.trim()) {
    throw new SessionValidationError("missing-build-commit", "buildCommit is required", `${path}.buildCommit`);
  }
  ...
  const changes: AccomplishmentChange[] = raw.accomplishmentChanges.map((c, i) => {
    if (!isRecord(c)) {
      throw new SessionValidationError("invalid-change-record", "Change entry must be an object", `${path}.accomplishmentChanges[${i}]`);
    }
    if (typeof c.timestamp !== "string" || !c.timestamp.trim()) {
      throw new SessionValidationError("missing-timestamp", "timestamp is required", `${path}.accomplishmentChanges[${i}].timestamp`);
    }
    if (!ACCOMPLISHMENTS.includes(c.from as Accomplishment)) {
      throw new SessionValidationError("invalid-from-accomplishment", `Invalid from accomplishment "${c.from}"`, `${path}.accomplishmentChanges[${i}].from`);
    }
    if (!ACCOMPLISHMENTS.includes(c.to as Accomplishment)) {
      throw new SessionValidationError("invalid-to-accomplishment", `Invalid to accomplishment "${c.to}"`, `${path}.accomplishmentChanges[${i}].to`);
    }
    return {
  ...
  if (typeof raw.outcomeReached !== "boolean") {
    throw new SessionValidationError("missing-outcome-reached", "outcomeReached boolean is required", `${path}.outcomeReached`);
  }

  const supportLadder = validateSupportLadderUsage(raw.supportLadder, `${path}.supportLadder`);

  if (!Array.isArray(raw.stumblingPoints)) {
    throw new SessionValidationError("missing-stumbling-points", "stumblingPoints array is required", `${path}.stumblingPoints`);
  }

  const stumblingPoints: StumblingPointRecord[] = raw.stumblingPoints.map((sp, i) => {
    if (!isRecord(sp)) {
      throw new SessionValidationError("invalid-stumbling-point", "Stumbling point must be an object", `${path}.stumblingPoints[${i}]`);
    }
    ...
    if (typeof sp.target !== "string" || !sp.target.trim()) {
      throw new SessionValidationError("missing-target", "target is required", `${path}.stumblingPoints[${i}].target`);
    }
    if (typeof sp.observation !== "string") {
      throw new SessionValidationError("missing-observation", "observation string is required", `${path}.stumblingPoints[${i}].observation`);
    }
    return {
  ...
  return {
    ...
    supportLadder,
    stumblingPoints,
    ...(Array.isArray(raw.stopRuleObservations) ? { stopRuleObservations: raw.stopRuleObservations } : {}),
  };
}

export function validateSupportLadderUsage(raw: unknown, path = "SupportLadderUsage"): SupportLadderUsage {
  if (!isRecord(raw)) {
    throw new SessionValidationError("invalid-record", "SupportLadderUsage must be an object", path);
  }

  if (!Array.isArray(raw.rungsUsed)) {
    throw new SessionValidationError("missing-rungs-used", "rungsUsed array is required", `${path}.rungsUsed`);
  }
  ...
  if (!Array.isArray(raw.rungOrder)) {
    throw new SessionValidationError("missing-rung-order", "rungOrder array is required", `${path}.rungOrder`);
  }
  ...
  if (typeof raw.wentStraightToExplanation !== "boolean") {
    throw new SessionValidationError("missing-went-straight", "wentStraightToExplanation boolean is required", `${path}.wentStraightToExplanation`);
  }
  ...
  if (typeof raw.transferCaseResolved !== "boolean") {
    throw new SessionValidationError("missing-transfer-case", "transferCaseResolved boolean is required", `${path}.transferCaseResolved`);
  }
...
export function validateSupportDefaultChange(raw: unknown, path = "SupportDefaultChangeRecord"): SupportDefaultChangeRecord {
  if (!isRecord(raw)) {
    throw new SessionValidationError("invalid-record", "SupportDefaultChangeRecord must be an object", path);
  }
  ...
  for (let i = 0; i < raw.justifyingRoundIds.length; i++) {
    ...
    if (typeof id !== "string" || !id.trim()) {
      throw new SessionValidationError("invalid-round-id", "Round ID must be a non-empty string", `${path}.justifyingRoundIds[${i}]`);
    }
  }

  if (typeof raw.changeDate !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(raw.changeDate)) {
    throw new SessionValidationError("invalid-change-date", "changeDate must be YYYY-MM-DD", `${path}.changeDate`);
  }

  if (typeof raw.rationale !== "string" || !raw.rationale.trim()) {
    throw new SessionValidationError("missing-rationale", "rationale is required", `${path}.rationale`);
  }
