...
export function chapterFixture() {
  ...
  return {
    paper: { id: "brownian-motion", title: "Motion and measurement", germanTitle: "Über die Bewegung",
      citation: "source", description: "An explanation", sourceNotice: "Source review is pending.",
    ...
    arguments: [{ id: "arg-bm-observable", paper: "brownian-motion", section: "s4", title: "Watch the spread",
      question: "What should we measure?", recap: "Mean square is useful.", citations: ["source"],
      premises: ["Independent steps"], limitations: ["Not instantaneous speed"], experiments: ["bm-01", "bm-08"],
      help: { why: "squares", missingStep: "squares", example: "squares" },
      readings: { overview: [p("A cloud spreads.")], full: [p("The signed mean may vanish."),
    ...
    foundations: [{ id: "squares", title: "Mean squares", question: "Why square?", summary: "Retain spread.",
      prerequisites: ["arithmetic"], explanation: [p("Squares do not cancel.")],
      example: [p("The mean of -1 and 1 is zero; their mean square is one.")],
      stoppingPoint: "No model of collisions is needed.", citations: ["source"] },
      { id: "arithmetic", title: "Arithmetic", question: "How to take a mean?", summary: "Add and divide.",
        prerequisites: [], explanation: [p("Add the values.")], example: [p("One plus one is two.")],
        stoppingPoint: "This defines the mean.", citations: ["source"] }],
    citations: [{ id: "source", title: "Original source", locator: "Section 4, page 559", url: "https://example.org/source" }],
    equations: [],
    examples: [{ instrumentId: "bm-01", sourceDigest: `source:sha256:${"d".repeat(64)}`,
      parameters: { seed: "18446744073709551615", M: 400 },
      rows: [{ quantity: "sampleMean", value: "-1e-7", unit: "m", status: "value" },
        { quantity: "apparentSpeed", value: "not applicable", unit: "m/s", status: "not-applicable" }], omittedArrays: 2 }],
    identity: { buildDigest: "a".repeat(64), contentRevision: "b".repeat(64), translationRevision: null,
      generatedAt: "2026-09-17T12:00:00Z", releaseId: null },
    assets: { mathCss: ".katex{font-size:1em}", printCss: "@media print{p{orphans:2}}", notices: ["Fixture license notice"] },
    budget: { rawBytes: 2097152, gzipBytes: 700000 },
  };
}
// Structural fixture only: the production adapter calls KaTeX, not this renderer.
export const fixtureMath = () => '<span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><msup><mi>x</mi><mn>2</mn></msup></math></span><span class="katex-html" aria-hidden="true">x²</span></span>';
...
test("one chapter retains every reading, with full explanation visible without JS", () => {
  const { html } = build();
  for (const text of ["A cloud spreads.", "The signed mean may vanish.", "Square the distances.", "A modern qualification."])
...
test("foundation closure includes transitive prerequisites and exact local links", () => {
  ...
  assert.deepEqual(collectChapterFoundations(input.arguments, input.foundations).map((f) => f.id), ["arithmetic", "squares"]);
...
test("source notices, provenance, German language and missing translation are explicit", () => {
  const { html } = build();
  for (const text of ["Source review is pending.", "lang=\"de\"", "Section 4, page 559",
    "No reviewed translation", "a".repeat(64), "b".repeat(64), "Fixture license notice"])
...
test("missing citations fail and executable citation URLs are refused", () => {
  const input = chapterFixture(); input.citations = [];
  assert.throws(() => build(input), /Missing offline citation/);
  const bad = chapterFixture(); bad.citations[0].url = "javascript:alert(1)";
  assert.throws(() => build(bad), /HTTPS/);
});
test("credentials in source links are refused", () => {
  const input = chapterFixture(); input.citations[0].url = "https://user:pass@example.org/";
  assert.throws(() => build(input), /credentials/);
});
test("no stored data is inspected and no active remote resources occur", () => {
  const { html } = build();
  assert.doesNotMatch(html, /localStorage|sessionStorage|indexedDB|fetch\(|XMLHttpRequest|sendBeacon|new Worker|<canvas|<iframe|<img/);
...
test("the same input yields byte-identical output and a changed reading changes the path", () => {
  ...
  const changed = chapterFixture(); changed.arguments[0].readings.steps[0].items.push("A new step.");
...
test("size limits fail with the chapter name and contributor breakdown", () => {
  const input = chapterFixture(); input.budget.rawBytes = 1;
  assert.throws(() => build(input), /brownian-motion\/s4 exceeds budget.*stylesAndFonts/);
  const compressed = chapterFixture(); compressed.budget.gzipBytes = 1;
  assert.throws(() => build(compressed), /gzip bytes/);
});
test("byte budget equality is accepted", () => {
  const reference = build(), input = chapterFixture();
...
test("static examples preserve exact seeds, canonical readouts and typed absence", () => {
  const { html } = build();
  for (const text of ["18446744073709551615", "-1e-7", "not-applicable", "Static host-calculated", "2 array outputs"])
    assert.ok(html.includes(text));
  assert.match(html, /bm-08.*not included/s);
});
test("styles cannot load external resources or escape their element", () => {
  for (const css of ['@import "https://example.org/x.css";', 'a{background:url(/remote.png)}', '</style><script>bad()</script>']) {
    const input = chapterFixture(); input.assets.mathCss = css;
    assert.throws(() => build(input), /Offline styles/);
  }
});
test("embedded math fonts are permitted", () => {
  const input = chapterFixture(); input.assets.mathCss = '@font-face{src:url(data:font/woff2;base64,d09GMg==)}';
  assert.doesNotThrow(() => build(input));
});
test("math must be static HTML plus MathML", () => {
  for (const html of ['<span>x</span>', '<span class="katex"><math></math><script>bad()</script></span>',
...
test("unknown block kinds require a deliberate renderer", () => {
  const input = chapterFixture(); input.arguments[0].readings.full.push({ kind: "raw-html", text: "raw" });
...
test("malformed revision identities and invalid content ids are refused", () => {
  const input = chapterFixture(); input.identity.contentRevision = "unknown";
  assert.throws(() => build(input), /revision identity/);
  const bad = chapterFixture(); bad.section.id = "../../escape";
