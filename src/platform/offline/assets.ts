...
export async function inlineKatexFonts(
  ...
  if (faces.length === 0 || faces.length > 64) throw new Error("KaTeX font-face coverage is missing or excessive.");
  let embedded = css;
  let total = 0;
  for (const [rule] of faces) {
    ...
    if (bytes.byteLength < 4 || bytes.byteLength > 256_000 || total > 1_048_576 ||
...
export async function loadOfflineAssets(root: string) {
  ...
  const [projectLicense, mathLicense] = await Promise.all([
    readFile(resolve(root, "LICENSE"), "utf8"), readFile(resolve(katex, "LICENSE"), "utf8"),
  ]);
  return Object.freeze({ mathCss, printCss, notices: Object.freeze([
