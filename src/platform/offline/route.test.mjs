...
import { GET, generateStaticParams, dynamic, dynamicParams } from "../../app/offline/[paper]/[file]/route.ts";
...
test("static route downloads exact admitted bytes, refuses retained files, and rejects tampering", async () => {
  const originalCwd = process.cwd(), originalProfile = process.env.AM_RELEASE_PROFILE;
  ...
  const entry = { paper: "brownian-motion", section: "s4", title: "Section 4",
    sha256, contentRevision: "b".repeat(64), bytes: Buffer.byteLength(html), gzipBytes: 50,
  ...
  const get = (values = params) => GET(new Request(`https://example.test${entry.path}`), { params: Promise.resolve(values) });
  try {
    process.chdir(root); process.env.AM_RELEASE_PROFILE = "scaffold";
    await publishOfflineChapters(root, manifest, [{ entry, html }]);
    assert.equal(dynamic, "force-static"); assert.equal(dynamicParams, false);
    ...
    assert.equal(response.headers.get("Content-Disposition"), 'attachment; filename="brownian-motion-s4.html"');
