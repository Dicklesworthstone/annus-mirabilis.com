...
import { OFFLINE_CHAPTER_VERSION, type OfflineChapterEntry, offlineDigest } from "./chapter.ts";
...
export function parseOfflineManifest(value: unknown): OfflineManifest {
  ...
  if (
    manifest.schemaVersion !== OFFLINE_CHAPTER_VERSION ||
    typeof manifest.buildDigest !== "string" ||
    !HASH.test(manifest.buildDigest) ||
    !Array.isArray(manifest.chapters) ||
    manifest.chapters.length > 2048 ||
    !["scaffold", "preview", "launch"].includes(manifest.profile)
  )
    throw new TypeError("Invalid offline manifest.");
  const seen = new Set<string>();
  for (const entry of manifest.chapters) {
    if (
      !entry ||
      typeof entry !== "object" ||
      typeof entry.paper !== "string" ||
      !ID.test(entry.paper) ||
      typeof entry.section !== "string" ||
      !ID.test(entry.section) ||
      typeof entry.title !== "string" ||
      !entry.title.trim() ||
      entry.title.length > 2000 ||
      !HASH.test(entry.sha256) ||
      !HASH.test(entry.contentRevision) ||
      !Number.isSafeInteger(entry.bytes) ||
      entry.bytes < 1 ||
      entry.bytes > 16_777_216 ||
      !Number.isSafeInteger(entry.gzipBytes) ||
      entry.gzipBytes < 1 ||
      entry.path !== `/offline/${entry.paper}/${entry.section}-${entry.sha256}.html` ||
      seen.has(`${entry.paper}/${entry.section}`)
    )
      throw new TypeError("Invalid or duplicate offline chapter descriptor.");
...
export async function publishOfflineChapters(
  root: string,
  ...
  for (const entry of manifest.chapters) {
    ...
    if (
      !file ||
      offlineDigest(file.html) !== entry.sha256 ||
      Buffer.byteLength(file.html) !== entry.bytes
    )
    ...
    try {
      await writeFile(path, file.html, { flag: "wx" });
    } catch (error) {
...
export async function loadOfflineManifest(
  root = process.cwd(),
  profile = offlineProfile(process.env.AM_RELEASE_PROFILE),
): Promise<OfflineManifest | null> {
  let text: string;
  try {
    text = await readFile(resolve(root, "generated/offline/index.json"), "utf8");
  } catch (error) {
  ...
  const current = JSON.parse(
    await readFile(resolve(root, "generated/content/index.json"), "utf8"),
  ) as { buildDigest: string };
  if (manifest.buildDigest !== current.buildDigest || manifest.profile !== profile)
    throw new Error(
      "Offline chapters are stale for the current content or profile. Run prepare:offline.",
    );
  return manifest;
}
