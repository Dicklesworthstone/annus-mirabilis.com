...
export function validateSearchManifest(input: unknown): SearchManifest {
  ...
  if (data.schemaVersion !== SEARCH_VERSION || typeof data.buildDigest !== "string" ||
      !/^[a-f0-9]{64}$/u.test(data.buildDigest) ||
      !["scaffold", "preview", "launch"].includes(String(data.profile)) ||
      !Array.isArray(data.shards) || data.shards.length > SEARCH_LIMITS.shards)
  ...
  const shards = data.shards.map((input): SearchShardDescriptor => {
    ...
    if (typeof shard.sha256 !== "string" || !/^[a-f0-9]{64}$/u.test(shard.sha256) ||
        shard.path !== `/search/s-${shard.sha256}.json` || paths.has(shard.path))
    ...
    return Object.freeze({ path: shard.path, sha256: shard.sha256, bytes: shard.bytes,
      gzipBytes: shard.gzipBytes, documents: shard.documents });
  });
  if (shards.reduce((sum, shard) => sum + shard.documents, 0) !== data.totalDocuments ||
  ...
  return Object.freeze({ schemaVersion: SEARCH_VERSION, buildDigest: data.buildDigest,
    profile: data.profile as SearchManifest["profile"], totalDocuments: data.totalDocuments,
    shards: Object.freeze(shards) });
}

export function parseSearchShard(input: unknown, expectedCount: number) {
  const data = record(input);
  if (data.schemaVersion !== SEARCH_VERSION || !Array.isArray(data.documents) ||
      data.documents.length !== expectedCount || data.documents.length > SEARCH_LIMITS.documents)
