import { createSearchEngine, SEARCH_LIMITS, type SearchAlias, type SearchDocument } from "./core.ts";
...
export function createSearchLoader(fetcher: typeof fetch = (...args) => globalThis.fetch(...args)) {
  ...
  async function fetchAll(): Promise<LoadedSearch> {
    ...
    try {
      const response = await fetcher("/search/index-manifest.json", {
        cache: "no-cache", credentials: "omit", redirect: "error", signal: controller.signal,
      });
      const manifest = validateSearchManifest(json(await readSearchResponse(response, SEARCH_LIMITS.manifestBytes)));
      const documents: SearchDocument[] = [], aliases: SearchAlias[] = [];
      for (const descriptor of manifest.shards) {
        const response = await fetcher(descriptor.path, {
          cache: "force-cache", credentials: "omit", redirect: "error", signal: controller.signal,
        });
        const bytes = await readSearchResponse(response, descriptor.bytes);
        if (bytes.byteLength !== descriptor.bytes || await searchBytesDigest(bytes) !== descriptor.sha256)
      ...
      return Object.freeze({ engine: createSearchEngine(documents, aliases), buildDigest: manifest.buildDigest,
        profile: manifest.profile, papers: Object.freeze([...new Set(documents.map((doc) => doc.paper))].sort()) });
    } finally {
  ...
  return Object.freeze({
    load(): Promise<LoadedSearch> {
      ...
      if (!pending) {
        pending = fetchAll().then((result) => { loaded = result; return result; });
