...
const documents = [
  { id: "a", type: "argument", paper: "brownian-motion", title: "RMS displacement", text: "The particles wander",
    terms: ["λ_x"], route: "/papers/brownian-motion/", anchor: "arg-bm-observable", face: "reading",
    section: "s5", lang: "en", scopeLabel: "Authored preview" },
  { id: "b", type: "instrument", paper: "special-relativity", title: "Clock synchronization", text: "Compare distant clocks",
    terms: ["sr-01"], route: "/lab/sr-01/", anchor: "", face: "", section: "", lang: "en", scopeLabel: "Laboratory preview" },
];
const bundle = () => packageSearchIndex(documents, [{ phrase: "clocks disagree", target: "b", label: "search aid" }], "b".repeat(64), "scaffold");
function network() {
  const pack = bundle(), calls = [];
  const files = new Map([["/search/index-manifest.json", pack.manifestText], ...pack.files.map((f) => [f.descriptor.path, f.text])]);
  let online = true;
  const fetcher = async (path, options) => {
    calls.push({ path, options });
    if (!online) throw new Error("Offline");
    return files.has(path) ? new Response(files.get(path)) : new Response("Not found", { status: 404 });
  };
  return { pack, calls, files, fetcher, offline() { online = false; } };
}

test("loader construction fetches nothing; loading is deduplicated and local queries work offline", async () => {
  const n = network(), loader = createSearchLoader(n.fetcher);
  ...
  assert.ok(n.calls.every(({ options }) => options.credentials === "omit" && options.redirect === "error"));
...
test("failed partial downloads never become an offline cache and can be retried", async () => {
  const n = network(), loader = createSearchLoader(n.fetcher), broken = n.pack.files[1];
...
test("truncated and oversized shards are refused before parsing", async () => {
  for (const change of [(s) => s.slice(1), (s) => `${s} `]) {
    const n = network(), shard = n.pack.files[0];
...
test("a missing shard is an error, not an empty search result", async () => {
  const n = network(); n.files.delete(n.pack.files[0].descriptor.path);
...
test("external paths and path traversal are rejected before any shard request", async () => {
  for (const path of ["https://evil.test/search.json", "//evil.test/search.json", "/search/../secret", "/search/%2e%2e/secret"]) {
    const n = network(), manifest = JSON.parse(n.pack.manifestText);
...
test("invalid manifests reject versions, profiles, duplicate shards and inconsistent totals", () => {
  for (const patch of [{ schemaVersion: 2 }, { buildDigest: "bad" }, { profile: "preveiw" },
    { totalDocuments: NaN }, { totalDocuments: 3 }, { totalDocuments: SEARCH_LIMITS.documents + 1 }]) {
...
test("manifest size budgets include each shard and the complete transfer", () => {
  ...
  const many = Array.from({ length: 9 }, (_, i) => {
    ...
    return { path: `/search/s-${sha}.json`, sha256: sha, bytes: SEARCH_LIMITS.shardBytes,
      gzipBytes: 100, documents: 1 };
  });
  assert.throws(() => validateSearchManifest({ ...bundle().manifest, totalDocuments: 9, shards: many }));
});
test("stream reading is bounded even without Content-Length", async () => {
  let cancelled = false;
  const stream = new ReadableStream({
    start(controller) { controller.enqueue(new Uint8Array(10)); controller.enqueue(new Uint8Array(10)); },
...
test("shard document counts, versions, aliases and unsafe document links are validated", () => {
  ...
  assert.throws(() => parseSearchShard({ ...data, aliases: [{ phrase: "bad", target: "missing", label: "search aid" }] }, 1));
