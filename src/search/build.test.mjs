...
import { aliasesForDocuments, assertSearchCoverage, documentsFromCompiled, searchProfile } from "./documents.ts";
import { packageSearchIndex } from "./build.ts";

const paragraph = (text) => ({ kind: "paragraph", text });
function corpus() {
  const argument = {
    id: "arg-bm-observable", section: "s5", title: "A measurable displacement", question: "How far does it wander?",
    recap: "Four times the time gives twice the coordinate RMS.", review: "draft",
    premises: ["Independent steps"], limitations: ["No molecular collisions"], experiments: ["bm-01"],
    readings: { overview: [paragraph("Particles spread")], full: [paragraph("Look at the mean square displacement")],
      steps: [{ kind: "formula", spoken: "lambda x equals square root of two D t", latex: "lambda_x" }],
      margin: [paragraph("A modern lens")] },
  };
  return {
    papers: [{ schemaVersion: 1, paper: { id: "brownian-motion", title: "Brownian motion", germanTitle: "Bewegung von Teilchen",
      description: "An explanation, not a reviewed transcription.", status: "explanation-preview",
      sections: [{ id: "s5", title: "The observable", arguments: [argument.id] }] }, arguments: [argument],
      equations: [{ id: "eq-model-spread", argument: argument.id, title: "RMS displacement", spoken: "Root mean square",
        explanation: "What a coordinate spread measures", review: "draft", notation: "modern-pedagogical",
        tree: {}, assumptions: ["Positive diffusion coefficient"], notes: [{ title: "Observation time", explanation: "A fixed interval" }] }] }],
    foundations: [{ id: "mean-square", title: "Mean squares", question: "Why square?", summary: "Left and right do not cancel",
      review: "draft", explanation: [paragraph("Squaring displacement")], example: [{ kind: "steps", items: ["Take two steps", "Square them"] }], stoppingPoint: "Return to the argument" }],
    instruments: [{ id: "bm-01", title: "Tracer ensemble", question: "Follow the particles", status: "registered" },
...
const documents = () => {
  const c = corpus();
  return documentsFromCompiled(c.papers, c.foundations, c.instruments, "scaffold", { "eq-model-spread": "λ_x = sqrt(2 D t)" });
};
...
test("compiled equation spellings and instrument argument links are searchable", () => {
  const docs = documents(), engine = createSearchEngine(docs, aliasesForDocuments(docs));
  assert.equal(engine.search("λₓ", { type: "equation" })[0].document.id, "equation:eq-model-spread");
  assert.equal(engine.search("measurable displacement", { type: "instrument" })[0].document.id, "instrument:bm-01");
  assert.equal(engine.search("how far does it wander")[0].document.id, "argument:arg-bm-observable");
...
test("dangling argument/equation anchors and unknown registered instrument mappings fail", () => {
  ...
  const d = corpus(); d.papers[0].equations[0].argument = "missing";
  assert.throws(() => documentsFromCompiled(d.papers, [], [], "scaffold"), /rendered argument/u);
  assert.throws(() => documentsFromCompiled([], [], [{ id: "new-instrument", title: "New", status: "registered" }], "scaffold"));
});
test("shard hashes, canonical byte counts, gzip measurements and totals agree", () => {
  const docs = documents(), bundle = packageSearchIndex(docs, aliasesForDocuments(docs), digest, "scaffold");
...
test("identical compiled input produces identical bytes despite object/list insertion order", () => {
  const docs = documents(), aliases = aliasesForDocuments(docs);
...
function noise(length) {
  ...
  for (let i = 0; i < length; i++) { x ^= x << 13; x ^= x >>> 17; x ^= x << 5; text += alphabet[(x >>> 0) % alphabet.length]; }
  return text;
}
test("oversized paper shards split by type and then size without losing documents", () => {
  const base = documents()[0];
  const many = Array.from({ length: 5 }, (_, i) => ({ ...base, id: `record-${i}`, type: i < 3 ? "argument" : "equation",
    text: noise(95000).split("").reverse().join("") + i }));
  const bundle = packageSearchIndex(many, [], digest, "scaffold");
  assert.ok(bundle.files.length > 1);
  assert.equal(bundle.files.reduce((n, f) => n + f.descriptor.documents, 0), many.length);
  assert.ok(bundle.files.every((file) => file.descriptor.gzipBytes <= SEARCH_LIMITS.gzipShardBytes));
});
