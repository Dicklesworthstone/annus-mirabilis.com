...
for (const [name, forms] of [
  ...
  [
    "micro units and decimal comma",
    ["0,8 Mikron", "0.8 µ", "0.8 μ", "0.8 µm", "0.8 μm", "0.8 um", "0.8 micron"],
  ],
  ...
  test(name, () => {
    for (const form of forms)
      assert.equal(normalizeSearchText(form), normalizeSearchText(forms[0]), form);
  });

test("normalization leaves the original document and visible German excerpt intact", () => {
  const source = doc("arg-bm-printed", "Printed distance", {
    text: "0,8 Mikron; daß Wärme",
    lang: "de",
  });
...
test("all five plan queries reach an existing fixture target first", () => {
  const docs = [
    doc("rms", "Root mean square displacement"),
  ...
  for (const [query, id] of [
    ["how far does it wander", "rms"],
    ["clocks disagree", "clocks"],
    ["βν", "quantum"],
    ["0.8 μ", "printed"],
    ["Besso", "besso"],
  ]) {
...
test("ambiguous symbols preserve scoped meanings instead of joining papers", () => {
  const engine = createSearchEngine([
    doc("wien", "Wien constant", {
      terms: ["β"],
      paper: "light-quanta",
      scopeLabel: "Wien constant",
    }),
    doc("lorentz", "Lorentz factor", {
      terms: ["β"],
      paper: "special-relativity",
      scopeLabel: "Lorentz factor (printed beta)",
    }),
    doc("viscosity", "Viscosity", {
      terms: ["k"],
      scopeLabel: "Viscosity (Einstein's k), not Boltzmann's constant",
    }),
  ]);
  assert.deepEqual(
    new Set(engine.search("beta").map((h) => h.document.scopeLabel)),
...
test("prefix matching supports German compounds but not one-letter symbols", () => {
  const engine = createSearchEngine([
    doc("a", "Elektrodynamik", { text: "Lichtgeschwindigkeit" }),
    doc("b", "Kinetics"),
  ]);
...
test("modern aliases are labeled, resolve only admitted ids and do not mutate results", () => {
  ...
  assert.throws(() =>
    createSearchEngine([], [{ phrase: "photon", target: "missing", label: "modern term" }]),
  );
});
...
test("input ordering cannot change tie-breaking and result limits are bounded", () => {
  const records = Array.from({ length: 40 }, (_, i) =>
    doc(`id-${String(i).padStart(2, "0")}`, "Diffusion"),
  );
...
test("types filter without searching hidden documents", () => {
  const engine = createSearchEngine([
    doc("eq", "Diffusion", { type: "equation" }),
    doc("arg", "Diffusion"),
  ]);
  assert.deepEqual(
    engine.search("diffusion", { type: "equation" }).map((h) => h.document.id),
    ["eq"],
  );
});
...
test("local result URLs preserve the actual anchor and face", () => {
  assert.equal(
    searchResultHref(doc("arg-bm-observable", "Observable")),
    "/papers/brownian-motion/?view=reading#arg-bm-observable",
  );
  assert.equal(
    searchResultHref(
      doc("beta", "Beta", { route: "/notation/", anchor: "lq.beta.wien", face: "" }),
    ),
...
test("duplicate ids, malformed records, and oversized content fail loudly", () => {
  ...
  assert.throws(() =>
    validateSearchDocument(
      doc("id", "title", { text: "x".repeat(SEARCH_LIMITS.documentCharacters + 1) }),
    ),
  );
});
