...
export async function readSearchManifest(
  ...
  const bytes = await boundedFile(
    resolve(root, "generated/search/index-manifest.json"),
    SEARCH_LIMITS.manifestBytes,
  );
  const manifest = validateSearchManifest(JSON.parse(bytes.toString("utf8")));
  const content = JSON.parse(
    (await boundedFile(resolve(root, "generated/content/index.json"), 2 * 1024 * 1024)).toString(
      "utf8",
    ),
  );
  if (content.buildDigest !== manifest.buildDigest || manifest.profile !== expectedProfile)
    throw new Error(
      "Search is stale for this content build or release profile. Run prepare:content.",
    );
...
export async function readCurrentSearchShard(
  name: string,
  root = process.cwd(),
): Promise<string | null> {
