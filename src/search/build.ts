...
import {
  createSearchEngine, SEARCH_LIMITS, SEARCH_VERSION,
  type SearchAlias, type SearchDocument,
} from "./core.ts";
import { type SearchProfile, searchProfile } from "./documents.ts";

export type SearchShardDescriptor = Readonly<{
  path: string; sha256: string; bytes: number; gzipBytes: number; documents: number;
}>;
...
export function packageSearchIndex(
  ...
  if (!/^[a-f0-9]{64}$/u.test(buildDigest)) throw new TypeError("Search requires the compiled build digest.");
  ...
  function emit(records: readonly SearchDocument[], splitByType: boolean): void {
    ...
    const shardAliases = aliases.filter((alias) => ids.has(alias.target))
      .sort((a, b) => {
        const left = `${a.phrase}\0${a.target}\0${a.label}`, right = `${b.phrase}\0${b.target}\0${b.label}`;
        return left < right ? -1 : left > right ? 1 : 0;
      });
    // Reconstruct keys in fixed order; caller object insertion order cannot affect a digest.
    const canonical = records.map((d) => ({
      id: d.id, type: d.type, paper: d.paper, section: d.section, lang: d.lang,
      title: d.title, text: d.text, terms: [...d.terms].sort(), route: d.route,
      anchor: d.anchor, face: d.face, scopeLabel: d.scopeLabel,
    }));
    const text = `${JSON.stringify({ schemaVersion: SEARCH_VERSION, documents: canonical,
      aliases: shardAliases.map((a) => ({ phrase: a.phrase, target: a.target, label: a.label })) })}\n`;
    const bytes = Buffer.byteLength(text), gzipBytes = gzipSync(text, { level: 9 }).byteLength;
    if (bytes > SEARCH_LIMITS.shardBytes || gzipBytes > SEARCH_LIMITS.gzipShardBytes) {
      if (records.length <= 1) throw new RangeError("A single search document exceeds the shard budget.");
      const types = [...new Set(records.map((d) => d.type))].sort();
      if (splitByType && types.length > 1) {
        for (const type of types) emit(records.filter((d) => d.type === type), false);
      } else {
    ...
    files.push({ descriptor: { path: `/search/s-${sha256}.json`, sha256, bytes, gzipBytes,
      documents: records.length }, text });
  }
  ...
  for (const paper of [...new Set(sorted.map((d) => d.paper))].sort())
    emit(sorted.filter((document) => document.paper === paper), true);
  if (files.length > SEARCH_LIMITS.shards ||
  ...
  const manifest: SearchManifest = { schemaVersion: SEARCH_VERSION, buildDigest, profile,
    totalDocuments: documents.length, shards: files.map((file) => file.descriptor) };
