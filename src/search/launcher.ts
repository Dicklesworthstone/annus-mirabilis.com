...
export function mountSearchLauncher(anchor: HTMLAnchorElement, status: HTMLElement): () => void {
  ...
  async function open() {
    ...
    loading = true;
    status.textContent = "Opening search…";
    try {
      const { openCommandPalette } = await import("./CommandPalette.ts");
      if (disposed) return;
      closePalette = openCommandPalette({
        onClose: () => {
          closePalette = null;
        },
      });
      ...
      if (!disposed)
        status.textContent = "Search could not open. The papers and outlines remain available.";
    } finally {
      loading = false;
    }
  }
  ...
  const frame = requestAnimationFrame(() => {
    if (
      ...
      window.location.hash.length > 512 ||
    ...
    try {
      ...
      /* A malformed URL fragment does not interfere with reading. */
    }
  });
  return () => {
    disposed = true;
