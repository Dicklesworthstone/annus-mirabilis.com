...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), "annus-search-")), digest = "c".repeat(64);
  const document = { id: "trial", type: "instrument", paper: "brownian-motion", section: "", lang: "en",
    title: "Tracer trial", text: "Synthetic preview", terms: [], route: "/lab/bm-01/", anchor: "", face: "",
  ...
  await writeFile(join(root, "generated/content/index.json"), JSON.stringify({ buildDigest: digest }));
  await writeFile(join(root, "generated/search/index-manifest.json"), bundle.manifestText);
  for (const file of bundle.files) await writeFile(join(root, "generated/search", file.descriptor.path.slice(8)), file.text);
...
test("retained old shards never become public merely because files still exist", async () => {
  const f = await fixture(), old = `s-${"d".repeat(64)}.json`;
...
test("a stale content build or modified shard refuses serving", async () => {
  ...
  await writeFile(join(f.root, "generated/content/index.json"), JSON.stringify({ buildDigest: "e".repeat(64) }));
...
test("untrusted dynamic route names cannot traverse generated directories", async () => {
  const f = await fixture();
  for (const name of ["../content/index.json", "%2e%2e/secret", "index-manifest.json", "s-abc.json", "/etc/passwd"]) {
