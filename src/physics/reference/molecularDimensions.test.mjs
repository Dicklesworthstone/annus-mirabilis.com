...
test("joint inversion satisfies both independently evaluated forward equations", () => {
  for (const viscosityCoefficient of [1, 2.5]) {
    ...
    const a = value(s.radius);
    const N = value(s.molecularNumber);
    close((p.gasConstant * p.temperature) / (6 * Math.PI * p.viscosity * a * N), p.diffusion);
    close(
      viscosityCoefficient * ((4 * Math.PI) / 3) * p.molarConcentration * N * a ** 3,
      p.specificViscosity,
    );
    ...
    for (const r of Object.values(s)) {
      ...
      assert.equal(r.ownerId, "molecular-dimensions");
    }
  }
});
...
test("radius and N are recovered without a target Avogadro number or Boltzmann constant", () => {
  for (const a of [1e-10, 3e-10, 1e-9])
    for (const N of [2e23, 6e23, 1e24]) {
      const p = {
        ...input,
        diffusion:
          (input.gasConstant * input.temperature) / (6 * Math.PI * input.viscosity * a * N),
        molarConcentration: 0.1,
        specificViscosity: 2.5 * ((4 * Math.PI) / 3) * 0.1 * N * a ** 3,
      };
      const s = inferMolecularDimensions(p);
      close(value(s.radius), a);
      close(value(s.molecularNumber), N);
    }
});
...
test("missing viscosity information exposes the compatible family, not a fitted target", () => {
  ...
  assert.equal(s.radius.status, "underdetermined");
  assert.equal(s.molecularNumber.status, "underdetermined");
  value(s.radiusTimesMolecularNumber);
  assert.equal(value(s.volumeFraction), 0);
...
test("inconsistent zero pairs and concentrated solutions refuse explicitly", () => {
  for (const patch of [
    ...
    { specificViscosity: 0 },
    { specificViscosity: 0.126 },
  ]) {
    for (const r of Object.values(inferMolecularDimensions({ ...input, ...patch }))) {
      assert.equal(r.status, "outside-domain");
      assert.equal(r.domainKind, "model");
    }
  }
  ...
  for (const key of Object.keys(input).filter((k) => k !== "viscosityCoefficient")) {
    for (const n of [-1, NaN, Infinity, -Infinity, "2", undefined]) {
      for (const r of Object.values(inferMolecularDimensions({ ...input, [key]: n }))) {
        assert.equal(r.status, "outside-domain");
        assert.equal(r.domainKind, "input");
      }
    }
  }
  for (const viscosityCoefficient of [0, 2, 3, NaN, "2.5"]) {
    assert.equal(
      inferMolecularDimensions({ ...input, viscosityCoefficient }).radius.status,
      "outside-domain",
    );
  }
});
  ...
  for (const patch of [
    ...
    for (const r of Object.values(s)) {
      if (r.status === "value") assert.ok(Number.isFinite(r.value) && r.value > 0);
        ...
        assert.equal(r.status, "outside-domain");
        assert.equal(r.domainKind, "numerical");
      }
    }
  }
});
