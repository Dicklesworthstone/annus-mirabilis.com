...
import {
  type Assessment, type DeclaredInterval, type Estimate, type StatisticalInterval,
  combinedMolecularNumberInterval, estimatorInterval, identifiabilityFamily,
} from "../inference.ts";
import { cameraMoments } from "./observation.ts";

const invalid = (requirements: string): Assessment<never> => ({
  kind: "refused",
  refusal: makeRefusal("invalid-parameter", { capabilityId: "diffusion.inference" },
    { details: { requirements } }),
});
const positive = (value: number) => Number.isFinite(value) && value > 0;

/** The same confidence interval for D maps to an entire band, not a unique N. */
export function diffusionCompatibleBand(input: {
  estimate: Estimate; T: number; eta: number; alpha: number;
  radiusRange: readonly [number, number]; synthetic: boolean;
}, constants: ConstantSet) {
  const interval = estimatorInterval(input.estimate, input.alpha);
  if (interval.kind !== "accepted") return interval;
  const conditions = { T: input.T, eta: input.eta, radiusRange: input.radiusRange,
  ...
  return { kind: "accepted" as const, data: Object.freeze({
    ...center.data, interval: interval.data,
    lowerNumbers: lower.data.numbers, upperNumbers: upper.data.numbers,
  }) };
}

export type IndependentMeasurement = Readonly<{
  value: number; interval: DeclaredInterval; provenance: string;
}>;
 ...
 */
export function conditionalMolecularNumberInterval(input: {
  estimate: Estimate; T: number; eta: number; alpha: number; synthetic: boolean;
...
}, constants: ConstantSet): Assessment<StatisticalInterval> {
  if (!positive(input.alpha) || input.alpha >= 1)
    return invalid("Choose a total error probability strictly between zero and one.");
  if (!input.radius) return { kind: "no-value", status: "underdetermined",
    reason: "Diffusion fixes a radius–number product. Add an independently established radius interval before asking for a molecular-number interval." };
  ...
  for (const value of measurements) {
    if (!value || typeof value.provenance !== "string" || !value.provenance.trim() || value.provenance.length > 512)
      return invalid("Every added measurement needs a provenance description of at most 512 characters.");
    const interval = value.interval;
    if (!positive(value.value) || !interval || !positive(interval.lower) || !positive(interval.upper) ||
        interval.lower > value.value || interval.upper < value.value ||
        !Number.isFinite(interval.coverage) || interval.coverage > 1 || interval.coverage <= 0)
      return invalid("Positive ordered measurement bounds must contain their point value and have a declared coverage.");
    if (interval.coverage < 1 - alphaD) return { kind: "no-value", status: "not-applicable",
  ...
  return combinedMolecularNumberInterval({
    estimate: input.estimate, alphaD, synthetic: input.synthetic,
    T: input.temperature?.value ?? input.T, eta: input.viscosity?.value ?? input.eta,
    a: input.radius.value, radiusProvenance: input.radiusProvenance,
    inputs: { a: input.radius.interval,
 ...
 */
export function cameraCompatibleFamily(input: { variance: number; dt: number; exposure: number }): Assessment<Readonly<{
  maximumDiffusion: number; maximumNoiseVariance: number;
  diffusion: Float64Array; noiseVariance: Float64Array; covariance: Float64Array;
}>> {
  const { variance, dt, exposure } = input;
  if (!positive(variance) || !positive(dt) || !Number.isFinite(exposure) || exposure < 0 || exposure > dt)
    return invalid("Use a positive coordinate variance and frame spacing, with known uniform exposure between zero and that spacing.");
  ...
  const diffusion = new Float64Array(41), noiseVariance = new Float64Array(41), covariance = new Float64Array(41);
  for (let i = 0; i < diffusion.length; i++) {
    ...
    const D = maximumDiffusion * fraction, sigma2 = maximumNoiseVariance * (1 - fraction);
    ...
    diffusion[i] = D; noiseVariance[i] = sigma2; covariance[i] = predicted.data.covariance;
  }
  return { kind: "accepted", data: Object.freeze({ maximumDiffusion, maximumNoiseVariance,
    diffusion, noiseVariance, covariance }) };
}
...
export function twoIntervalCameraEstimate(input: {
  firstVariance: number; secondVariance: number; firstDt: number; secondDt: number; exposure: number;
  ...
  if (![firstVariance, secondVariance, firstDt, secondDt].every(positive) ||
      !Number.isFinite(exposure) || exposure < 0 || exposure > Math.min(firstDt, secondDt) || secondDt <= firstDt)
    return invalid("Use positive moments at two increasing spacings and the same known exposure duration, no longer than either spacing.");
  const D = ((secondVariance - firstVariance) / 2) / (secondDt - firstDt);
  const sigma2 = firstVariance / 2 - D * (firstDt - exposure / 3);
  if (![D, sigma2].every(Number.isFinite)) return invalid("The two-interval inverse is not representable.");
