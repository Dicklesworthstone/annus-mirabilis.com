...
export type PhotoelectricObservation = Readonly<{
  row: number;
  frequencyTHz: number;
  stoppingV: number;
  sigmaV?: number;
...
export type FitOptions = Readonly<{
  ...
  offset:
    ...
    | Readonly<{ kind: "known"; volts: number; sigmaV: number }>;
}>;
...
export type PhotoelectricDataFit =
  | Readonly<{
      status: "value";
      fit: LineEstimate;
...
function missing(
  reason: string,
  unit: string,
  status: "underdetermined" | "outside-domain" = "underdetermined",
): InferredQuantity {
...
export function analyzePhotoelectricData(
  ...
  if (
    !reference.constantSetId ||
    ![reference.elementaryCharge, reference.planckConstant, reference.speedOfLight].every(
      (x) => Number.isFinite(x) && x > 0,
    )
  ) {
    throw new PhotoelectricDataError(
      "photoelectric-reference-invalid",
      "The reference constants must come from a declared positive finite calibration.",
    );
  }
  if (options.weighting !== "equal" && options.weighting !== "declared-sigma")
    throw new PhotoelectricDataError("photoelectric-weighting-unknown", "Unknown weighting model.");
  if (options.offset.kind !== "unknown" && options.offset.kind !== "known")
    throw new PhotoelectricDataError("photoelectric-offset-model-unknown", "Unknown offset model.");
  if (
    options.offset.kind === "known" &&
    (!Number.isFinite(options.offset.volts) ||
      Math.abs(options.offset.volts) > 1e4 ||
      !Number.isFinite(options.offset.sigmaV) ||
      options.offset.sigmaV < 0 ||
      options.offset.sigmaV > 1e4)
  )
    throw new PhotoelectricDataError(
      "photoelectric-offset-out-of-range",
      "Use an offset within ±10000 V and a nonnegative standard uncertainty up to 10000 V.",
    );
  if (rows.length < 3 || rows.length > 1000)
    throw new PhotoelectricDataError("photoelectric-row-count", "Select 3 to 1000 observations.");
  const seen = new Set<number>();
  for (const row of rows) {
    if (!Number.isInteger(row.row) || row.row < 1 || seen.has(row.row))
      throw new PhotoelectricDataError(
    ...
    seen.add(row.row);
    if (
      !Number.isFinite(row.frequencyTHz) ||
      row.frequencyTHz < 1e-6 ||
      row.frequencyTHz > 1e6 ||
      !Number.isFinite(row.stoppingV) ||
      Math.abs(row.stoppingV) > 1e4
    )
      throw new PhotoelectricDataError(
    ...
    if (
      row.sigmaV !== undefined &&
      (!Number.isFinite(row.sigmaV) || row.sigmaV < 1e-9 || row.sigmaV > 1e4)
    )
      throw new PhotoelectricDataError(
        "photoelectric-sigma-invalid",
        "Invalid voltage standard uncertainty.",
      );
    if (options.weighting === "declared-sigma" && row.sigmaV === undefined)
      throw new PhotoelectricDataError(
  ...
  if (
    withinTolerance(hi, lo, {
      absolute: 64 * Number.EPSILON,
    ...
    return Object.freeze({
      ...
      reason:
        "The selected frequencies are identical or numerically unresolved. Repeated measurements at one frequency cannot separate slope from intercept.",
      usedRows,
    });
  }
  const fit = fitLine(
    rows.map((r) => ({
      x: r.frequencyTHz,
      y: r.stoppingV,
      ...(r.sigmaV === undefined ? {} : { sigma: r.sigmaV }),
    })),
    options.weighting === "declared-sigma",
  );
  const slopeError = Math.sqrt(fit.slopeVariance);
  const warnings: string[] = [];
  if (options.weighting === "equal" && rows.some((r) => r.sigmaV !== undefined))
    warnings.push("The sigma_V column is retained but ignored in the selected equal-weight fit.");
  if (fit.residualStandardDeviation === 0)
    warnings.push(
      "The entered rows lie exactly on a line. Zero residual scatter is not evidence of exact measurement or absent systematic error.",
    );
  if (fit.reducedChiSquare !== null)
    warnings.push(
  ...
  if (hi - lo < 0.01 * Math.max(Math.abs(lo), Math.abs(hi)))
    warnings.push(
      "The frequency span is narrow. The intercept and threshold are long extrapolations even when the fitted line looks precise.",
    );
  ...
  if (fit.slope <= 0) {
    const reason =
      "The fitted slope is not positive. These rows do not yield a physically positive h under this photoelectric model; the empirical line is still shown.";
    ...
    planckEstimate = quantity(
      (reference.elementaryCharge * fit.slope) / 1e12,
      (reference.elementaryCharge * slopeError) / 1e12,
      "J s",
    );
    if (slopeError >= fit.slope)
      warnings.push(
        "The slope is not separated from zero at the one-standard-error scale. A ratio-based threshold is unresolved here.",
      );
    if (options.offset.kind === "unknown") {
      const reason =
        "An unknown common voltage offset and the surface escape work enter the same intercept. The frequency sweep alone cannot identify either the work function or its physical threshold.";
      ...
      if (phi < 0) {
        const reason =
          "The calibrated intercept implies negative escape work. Check voltage sign, calibration and the applicability of the model instead of clamping it to zero.";
        workFunction = missing(reason, "eV", "outside-domain");
        threshold = missing(reason, "THz", "outside-domain");
      } else {
        ...
        const thresholdError =
          Math.sqrt(
            fit.centerVariance +
              options.offset.sigmaV ** 2 +
              (delta / fit.slope) ** 2 * fit.slopeVariance,
          ) / fit.slope;
        threshold =
          slopeError >= fit.slope
            ? missing(
                "The fitted slope is too close to zero for a stable linearized threshold uncertainty.",
                "THz",
              )
            : quantity(phi / fit.slope, thresholdError, "THz");
        if (phiError >= phi)
          warnings.push(
            "The work-function estimate is not separated from zero at one standard error; threshold uncertainty is a local linear approximation, not a confidence bound.",
          );
      }
    }
  }
  ...
  if (![referenceSlopeVPerTHz, relativeSlopeDifference].every(Number.isFinite))
    throw new PhotoelectricDataError(
      "photoelectric-reference-comparison-nonfinite",
      "Reference comparison is not numerically representable.",
    );
  for (const q of [planckEstimate, workFunction, threshold])
    if (q.status === "value" && ![q.value, q.standardError].every(Number.isFinite))
      throw new PhotoelectricDataError(
  ...
  return Object.freeze({
    ...
    workFunction,
    threshold,
    referenceSlopeVPerTHz,
    relativeSlopeDifference,
