...
import { type Assessment, invertToMolecularNumber, type StatisticalInterval } from "../inference.ts";
...
const range = (v: unknown): v is PositiveRange => Array.isArray(v) && v.length === 2 &&
  positive(v[0]) && positive(v[1]) && v[0] <= v[1];
const invalid = (requirements: string): Assessment<never> => ({ kind: "refused", refusal:
  makeRefusal("invalid-parameter", { capabilityId: "diffusion.inference" }, { details: { requirements } }) });
const unavailable = (reason: string): Assessment<never> => ({ kind: "no-value", status: "not-applicable", reason });
 ...
 */
export function allocateJointInputCoverage(target: number, jointInputCoverage: number | null):
  Assessment<Readonly<{ alphaCamera: number; cameraCoverage: number; coverageLowerBound: number }>> {
  ...
  if (jointInputCoverage === null)
    return unavailable("No joint input coverage is declared. The range envelope is a sensitivity analysis, not a combined confidence interval.");
  ...
  if (alphaCamera < 1e-8)
    return unavailable("The declared joint input coverage must exceed the requested total coverage and leave at least 1e-8 camera error probability. Supply stronger input coverage or request lower total coverage; no range is silently relabeled.");
  const cameraCoverage = 1 - alphaCamera;
  return { kind: "accepted", data: Object.freeze({ alphaCamera, cameraCoverage,
    coverageLowerBound: Math.min(jointInputCoverage, 1 - (alphaCamera + (1 - jointInputCoverage))) }) };
}
...
export function cameraMolecularInputEnvelope(
  ...
  if (!diffusion || !box || !positive(box.nominalScale) ||
      ...
      box.nominalScale < box.scale[0] || box.nominalScale > box.scale[1])
    return invalid("Provide positive, ordered input ranges and a scale range containing its nominal value; declare whether the radius shares the same ruler.");
  if (diffusion.lower === 0)
    return { kind: "no-value", status: "underdetermined", reason:
  ...
  if (!positive(diffusion.lower) || !positive(diffusion.upper) || diffusion.lower > diffusion.upper)
    return invalid("Use an admitted positive camera interval. An empty or negative physical confidence set cannot become a positive molecular interval.");
  const invert = (T: number, eta: number, a: number) => invertToMolecularNumber({
    dHat: diffusion.lower, interval: diffusion, T, eta, a,
    radiusProvenance: "independently-declared", synthetic,
  }, constants);
  ...
  if (!(box.gasConstant[0] <= R && R <= box.gasConstant[1]))
    return invalid("The gas-constant range must contain the value from the selected constant set. Changing its provenance requires a different set, not a range edit.");
  const exponent = box.radiusCalibration === "same-axis" ? -3 : -2;
  const lo = lower.data.interval.lower * (box.gasConstant[0] / R) *
    (box.scale[1] / box.nominalScale) ** exponent;
  const hi = upper.data.interval.upper * (box.gasConstant[1] / R) *
    (box.scale[0] / box.nominalScale) ** exponent;
  if (!positive(lo) || !positive(hi) || lo > hi)
    return invalid("The input envelope is outside the supported numerical range. No endpoint has been clipped or replaced.");
  return { kind: "accepted", data: Object.freeze({ lower: lo, upper: hi, scaleExponent: exponent }) };
}
