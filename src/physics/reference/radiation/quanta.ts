 ...
 */
export function meanQuantumEnergyWien(T: number, set: ConstantSet): MeanQuantumEnergyWienResult {
 ...
 * <epsilon> = k_B * T * (gamma(4, xMax) - gamma(4, xMin)) / (gamma(3, xMax) - gamma(3, xMin)).
 * Labeled "editorial-variant", not the historical §6 full-spectrum result.
 */
export function bandLimitedMeanQuantumEnergyWien(
  T: number,
  xMin: number,
  xMax: number,
  set: ConstantSet,
  ...
  if (!Number.isFinite(xMin) || !Number.isFinite(xMax) || xMin < 0 || xMax <= xMin) {
    return {
      ...
      domainKind: "input",
      reason: `Dimensionless frequency bounds must be finite and satisfy 0 <= xMin < xMax (got xMin=${xMin}, xMax=${xMax}).`,
    };
  }
  ...
  const meanEnergyJoules = kB * T * ratio;
  const meanEnergyEv = meanEnergyJoules / JOULES_PER_EV;

  // Fraction of total Wien energy and count below boundary x0
  const energyShareBelowBoundary = gamma4(wienBoundaryX) / 6; // Gamma(4) = 6
  const countShareBelowBoundary = gamma3(wienBoundaryX) / 2; // Gamma(3) = 2
