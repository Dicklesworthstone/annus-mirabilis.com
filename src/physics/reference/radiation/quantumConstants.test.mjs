...
for (const value of [NaN, Infinity, -Infinity, 0, -1]) {
  test(`nonpositive or nonfinite thermal value ${value} is refused`, () => {
    assert.throws(() => quantumConstantsSI({ ...modern, values: { ...modern.values, thermal: value } }, readers), ConstantSetError);
  });
  test(`nonpositive or nonfinite action value ${value} is refused`, () => {
    assert.throws(() => quantumConstantsSI({ ...modern, values: { ...modern.values, planckConstant: value } }, readers), ConstantSetError);
  });
  test(`nonpositive or nonfinite historical beta ${value} is refused`, () => {
    assert.throws(() => quantumConstantsSI({ ...printed, values: { ...printed.values, wienConstantBeta: value } }, readers), ConstantSetError);
  });
}
...
test("underflow during the CGS conversion is rejected", () => {
  assert.throws(() => thermalConstantSI(fixture("tiny-fixture", "erg/(mol K)", Number.MIN_VALUE, 1), readers), ConstantSetError);
});
test("overflow deriving the historical action coefficient is rejected", () => {
  assert.throws(() => quantumConstantsSI(fixture(printed.id, "J/(mol K)", Number.MAX_VALUE, NaN, Number.MAX_VALUE), readers), ConstantSetError);
});
...
test("refuses a thermal constant that is not representable (quantumConstants.ts:20)", () => {
  const err = refusalFor(() => thermalConstantSI(fixture("modern-si-2019", "J/K", 0, 6.626e-34), readers));
...
test("refuses a value tagged with another constant set (quantumConstants.ts:38)", () => {
  const set = fixture("modern-si-2019", "J/K", 1.380649e-23, 6.62607015e-34);
  const foreign = { ...readers, thermal: () => ({ setId: "einstein-1905-brownian-printed", value: 1.4e-23 }) };
...
test("refuses an unsupported thermal-constant unit (quantumConstants.ts:49)", () => {
  const err = refusalFor(() => thermalConstantSI(fixture("modern-si-2019", "cal/K", 1.4e-23, 6.6e-34), readers));
...
test("refuses an unsupported action-constant unit (quantumConstants.ts:81)", () => {
  const set = fixture("modern-si-2019", "J/K", 1.380649e-23, 6.62607015e-34);
  set.entries = set.entries.map((e) => (e.quantityId === "planckConstant" ? { ...e, unit: "eV s" } : e));
  const err = refusalFor(() => quantumConstantsSI(set, readers));
