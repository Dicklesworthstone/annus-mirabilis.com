...
import {
  ...
  validateOccupancySettings, OCCUPANCY_CHECKS, MAX_OCCUPANCY_POINTS,
...
test("moved public owners retain numeric and rational input behavior", () => {
  assert.deepEqual(binomialInside(4, 0.5).terms.map((t) => t.exactProbability.numerator), [1n, 4n, 6n, 4n, 1n]);
...
test("all admitted settings normalize and preserve means but change fluctuations", () => {
  for (let n = 1; n <= MAX_OCCUPANCY_POINTS; n++) {
    for (let quarters = 0; quarters <= 4; quarters++) {
      const r = compareOccupancyModels({ n, quarters }), f = quarters / 4;
      for (const model of [r.independent, r.locked]) {
        close(model.probabilities.reduce((a, b) => a + b, 0), 1);
      ...
      assert.equal(distinguishOccupancy({ n, quarters }, OCCUPANCY_CHECKS).status,
...
test("the full-case entropy is a log constraint weight, including the impossible edge", () => {
  ...
  for (const key of ["independent", "locked"]) {
    assert.equal(compareOccupancyModels({ n: 4, quarters: 0 })[key].entropyChange.kind, "zero-probability");
    assert.equal(compareOccupancyModels({ n: 4, quarters: 4 })[key].entropyChange.value, 0);
  }
});

for (const bad of [null, [], {}, { n: 4 }, { n: 0, quarters: 2 }, { n: 13, quarters: 2 },
  { n: 4.5, quarters: 2 }, { n: NaN, quarters: 2 }, { n: Infinity, quarters: 2 },
  { n: "4", quarters: 2 }, { n: 4, quarters: -1 }, { n: 4, quarters: 5 },
  { n: 4, quarters: 0.5 }, { n: 4, quarters: "2" }, { ...settings, verdict: "independent" }]) {
  test(`refuse invalid settings ${JSON.stringify(bad)}`, () => assert.throws(() => compareOccupancyModels(bad)));
}

test("settings accessors, prototypes and sparse check lists fail without executing code", () => {
  let read = false;
  const input = { get n() { read = true; return 4; }, quarters: 2 };
...
test("boundary contradictions reject BOTH candidates, not the inconvenient one", () => {
  assert.equal(compareOccupancyEvidence({ n: 4, quarters: 0 }, [0, 1, 0, 0, 0]).status, "neither-possible");
  assert.equal(compareOccupancyEvidence({ n: 4, quarters: 4 }, [1, 0, 0, 0, 0]).status, "neither-possible");
...
test("tiny positive likelihoods remain possible, even when exponentiating would underflow", () => {
  const counts = Array(13).fill(0); counts[12] = 10000;
...
for (const bad of [null, [], [0, 0, 0, 0, 0], [1, 2], [1, -1, 0, 0, 0], [1, 0.5, 0, 0, 0],
  ["1", 0, 0, 0, 0], [Infinity, 0, 0, 0, 0], [NaN, 0, 0, 0, 0], [10001, 0, 0, 0, 0], Array(5)]) {
  test(`refuse invalid evidence ${JSON.stringify(bad)}`, () => assert.throws(() => compareOccupancyEvidence(settings, bad)));
}

test("evidence accessors do not execute and accepted data has no mutable aliases", () => {
  let read = false;
  const accessor = [0, 0, 0, 0, 0];
  Object.defineProperty(accessor, "0", { get() { read = true; return 1; } });
  assert.throws(() => compareOccupancyEvidence(settings, accessor));
  assert.equal(read, false);
  const input = [1, 4, 6, 4, 1], result = compareOccupancyEvidence(settings, input);
  input[0] = 999;
