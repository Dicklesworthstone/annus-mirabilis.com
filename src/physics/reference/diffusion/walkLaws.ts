...
export const WALK_KERNELS = Object.freeze({
  coin: Object.freeze({
    stepKernel: 0,
    drawsPerStep: 1,
    fourthMomentFactor: 1,
    excessKurtosis: -2,
  }),
  uniform: Object.freeze({
    stepKernel: 1,
    drawsPerStep: 1,
    fourthMomentFactor: 1.8,
    excessKurtosis: -1.2,
  }),
  gaussian: Object.freeze({
    stepKernel: 3,
    drawsPerStep: 2,
    fourthMomentFactor: 3,
    excessKurtosis: 0,
  }),
});
const identity = (
  ...
  unit: string,
  owner: string,
  semanticKind = "declared-step-model",
) => ({ quantityId, unit, semanticKind, ownerId: `diffusion.${owner}` });
function outside(
  quantityId: string,
  unit: string,
  owner: string,
  ...
  return {
    ...identity(quantityId, unit, owner),
    ...
    boundary: {
      alternativeModel:
        "Use independent symmetric steps with finite variance, or explicitly include drift or a different transport law.",
    },
  };
}
function value(
  ...
  if (!Number.isFinite(v) || (positive && v <= 0))
    return {
      ...
      reason: "The result is outside the representable numerical range.",
      boundary: { alternativeModel: "Use a rescaled calculation." },
    };
  return { ...identity(quantityId, unit, owner), status: "value", value: v };
}
const invalid = (message: string): Computation<never> => ({
  kind: "refused",
  refusal: makeRefusal(
    "invalid-parameter",
    { capabilityId: "diffusion.walkLaws" },
    { details: { requirements: message } },
  ),
});
const numerical = (reason: string): Computation<never> => ({
  kind: "outcome",
  outcome: {
...
export function kernelMoments(kernel: StepKernel): Readonly<{
  ...
  fourthMoment: ScientificResult;
}> {
  const names = [
    ["mean", "stepMean", "m"],
    ["secondMoment", "stepSecondMoment", "m2"],
    ["variance", "stepVariance", "m2"],
    ["fourthMoment", "stepFourthMoment", "m4"],
  ] as const;
  const bad = (reason: string) =>
    Object.fromEntries(
      names.map(([key, id, unit]) => [
        key,
        outside(id, unit, "kernelMoments", "finite-moments", reason),
      ]),
    ) as ReturnType<typeof kernelMoments>;
  if (!kernel || typeof kernel !== "object") return bad("Declare a supported step kernel.");
  if (kernel.kind === "cauchy")
    return bad(
  ...
  if (kernel.kind === "biased-coin") {
    ...
    if (!Number.isFinite(p) || p < 0 || p > 1)
      return bad("The biased coin probability must lie between zero and one.");
    return {
      mean: value("stepMean", "m", "kernelMoments", (2 * p - 1) * s),
      secondMoment: value("stepSecondMoment", "m2", "kernelMoments", s * s, true),
      variance: value(
        "stepVariance",
        "m2",
        "kernelMoments",
      ...
      fourthMoment: value("stepFourthMoment", "m4", "kernelMoments", s ** 4, true),
    };
  }
  if (!Object.hasOwn(WALK_KERNELS, kernel.kind)) return bad("Declare a supported step kernel.");
  return {
    mean: value("stepMean", "m", "kernelMoments", 0),
    secondMoment: value("stepSecondMoment", "m2", "kernelMoments", s * s, true),
    variance: value("stepVariance", "m2", "kernelMoments", s * s, true),
    fourthMoment: value(
      "stepFourthMoment",
      "m4",
      "kernelMoments",
      WALK_KERNELS[kernel.kind].fourthMomentFactor * s ** 4,
      true,
    ),
  };
}
...
export function continuumLimit({
  stepScale,
  tau,
  scaling,
}: {
  stepScale: number;
  tau: number;
  scaling: "fixed-step" | "fixed-ratio";
}): ScientificResult {
  if (
    !Number.isFinite(stepScale) ||
    stepScale <= 0 ||
    !Number.isFinite(tau) ||
    tau <= 0 ||
    !["fixed-step", "fixed-ratio"].includes(scaling)
  )
    return outside(
      "continuumDiffusionCoefficient",
      "m2/s",
      "continuumLimit",
      "positive-scale-and-interval",
      "Use a positive finite scale and interval and declare the limiting procedure.",
    );
  if (scaling === "fixed-step")
    return outside(
      "continuumDiffusionCoefficient",
      "m2/s",
      "continuumLimit",
      "hold-variance-over-time-fixed",
...
export function dkwBound(W: number, alpha: number): Computation<number> {
  if (!isCount(W) || W < 1 || !Number.isFinite(alpha) || alpha <= 0 || alpha >= 1)
    return invalid("Use 1–10000 walkers and a probability strictly between zero and one.");
  return { kind: "accepted", data: Math.sqrt((Math.log(2) - Math.log(alpha)) / (2 * W)) };
}
export function kolmogorovDistanceToGaussian(
  ...
  if (
    ...
    return invalid("Use finite samples and positive variance.");
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    distance = Math.max(
      distance,
      Math.abs(cdf - i / sorted.length),
      Math.abs((i + 1) / sorted.length - cdf),
    );
  }
  return { kind: "accepted", data: distance };
}
export type CoinDistribution = Readonly<{
...
export function uniformSumDistribution(n: number, x: number): { cdf: number; pdf: number } {
  if (!isCount(n, 400) || n < 1 || !Number.isFinite(x))
    throw new RangeError("Uniform sums admit 1–400 steps and finite coordinates.");
...
export function kolmogorovShapeTerm(kernel: WalkKernel, n: number): Computation<ShapeTerm> {
  if (!Object.hasOwn(WALK_KERNELS, kernel) || !isCount(n) || n < 1)
    return invalid("Choose a supported symmetric kernel and 1–10000 steps.");
  if (kernel === "gaussian")
    return {
      kind: "accepted",
      data: { distance: 0, location: 0, method: "Gaussian sums are Gaussian.", resolution: 0 },
    };
  if (kernel === "coin") {
    const distribution = coinWalkDistribution(n, 1);
    if (distribution.kind !== "accepted") return distribution;
    let cdf = 0,
      distance = 0,
      location = 0;
    for (let k = 0; k <= n; k++) {
    ...
    return {
      ...
      data: {
        distance,
        location,
        method: "Binomial enumeration, both limits at every atom.",
        resolution: 0,
      },
    };
  }
  if (n > 400)
    return {
      ...
      outcome: {
        ...
        requested: { workUnits: n * n, allocationBytes: n * 16 },
        allowed: { workUnits: 400 * 400, allocationBytes: 400 * 16 },
        details: {
          reason:
            "The exact uniform-sum shape comparison is bounded to 400 steps. Choose at most 400 steps, or a different step law.",
        },
      },
    };
  const scale = Math.sqrt(n / 12),
    support = Math.sqrt(3 * n),
    end = Math.min(9, support);
  const evaluate = (z: number) => {
    const { cdf, pdf } = uniformSumDistribution(n, n / 2 - z * scale);
    return {
      error: standardNormalCdf(-z) - cdf,
      derivative: pdf * scale - Math.exp((-z * z) / 2) / Math.sqrt(2 * Math.PI),
    };
  };
  let distance = 0,
    location = 0;
  const consider = (z: number) => {
    const e = Math.abs(evaluate(z).error);
    if (e > distance) {
      distance = e;
      location = z;
    }
  ...
  const segments = Math.ceil(end / 0.05);
  for (let j = 1; j <= segments; j++) {
    const b = (end * j) / segments,
      fb = evaluate(b).derivative;
    consider(b);
    if (fa * fb < 0) {
      let lo = a,
        hi = b,
        flo = fa;
      for (let i = 0; i < 32 && hi - lo > 1e-10; i++) {
        const mid = (lo + hi) / 2,
          f = evaluate(mid).derivative;
        if (flo * f <= 0) hi = mid;
        else {
          lo = mid;
          flo = f;
  ...
  return {
    ...
    data: {
      distance,
      location,
      method: "Irwin–Hall cardinal-spline recurrence with bracketed density crossings.",
      resolution: 1e-10,
    },
  };
}
