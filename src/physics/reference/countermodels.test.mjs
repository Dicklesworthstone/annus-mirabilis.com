...
const value = (r) => {
  assert.equal(r.status, "value", r.reason);
  return r.value;
};
const close = (a, b, absolute = 1e-12) =>
  assert.ok(withinTolerance(a, b, { absolute, relative: 1e-12 }).ok, `${a} != ${b}`);
...
test("low-speed composition retains the cancellation-free residual", () => {
  ...
  assert.notEqual(
    value(lorentzCompositionResidual(10, 10)),
    20 - value(composeCollinear(10 / c, 10 / c)) * c,
  );
});
  ...
  for (const beta of [0.1, 0.6, 0.9])
    for (const event of events) {
      ...
      for (const key of ["t", "x", "y", "z"])
        close(ether[key], einstein[key], key === "t" ? 1e-12 : 1e-6);
    }
});
test("the ether route does not call the Einstein event map", () => {
  ...
  const body = source
    .split("export function lorentz1904EventMap")[1]
    .split("/** Closed owner registry")[0];
  assert.ok(body.includes("galileanMap"));
...
test("simultaneous endpoint measurements derive the moving rod length", () => {
  close(value(mappedRodLength(map("galilean"), 100, 0.6, c)), 100);
  for (const id of ["lorentz", "lorentz-ether"])
    close(value(mappedRodLength(map(id), 100, 0.6, c)), 80);
});
test("clock rates and collinear composition arise from each candidate's events", () => {
  for (const id of ["lorentz", "lorentz-ether"]) {
    close(value(mappedClockRate(map(id), 0.6, c)), 0.8);
    close(value(mappedWorldlineSpeed(map(id), 0.6 * c, -0.6, c)) / c, 15 / 17);
  }
});
test("an intentionally first-order local time fails the clock and composition checks", () => {
  const broken = (e, b, k) => ({
    status: "value",
    value: { ...e, x: value(gamma(b)) * (e.x - b * k * e.t), t: e.t - (b * e.x) / k },
  });
  assert.ok(!withinTolerance(value(mappedClockRate(broken, 0.6, c)), 0.8, { relative: 1e-12 }).ok);
  assert.ok(
    !withinTolerance(value(mappedWorldlineSpeed(broken, 0.6 * c, -0.6, c)) / c, 15 / 17, {
      relative: 1e-12,
    }).ok,
  );
});
test("nonfinite or luminal observer input returns a typed refusal, never numbers", () => {
  ...
  for (const id of ["galilean", "lorentz", "lorentz-ether"]) {
    assert.equal(
      countermodelEventMap(id, { t: NaN, x: 0, y: 0, z: 0 }, 0.6, c).status,
      "outside-domain",
    );
    assert.equal(
      countermodelEventMap(id, { t: 1, x: 0, y: 0, z: 0 }, 0.6, 0).status,
      "outside-domain",
    );
  }
});
