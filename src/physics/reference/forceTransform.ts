...
const outside = (
  domainKind: "input" | "physical" | "numerical",
  reason: string,
): ForceTransformResult => Object.freeze({ status: "outside-domain", domainKind, reason });

export function transformThreeForce(
  input: Readonly<{
    force: ForceVector;
  ...
  if (![F.x, F.y, F.z, u.x, u.y, u.z, beta, c].every(Number.isFinite) || c <= 0)
    return outside(
      "input",
      "Force and velocity components must be finite, and c must be positive.",
    );
  if (Math.abs(beta) >= 1)
  ...
  if (!(inverseGamma > 0) || !(denominator > 0))
    return outside(
      "numerical",
      "The boost or particle time transformation is numerically unresolved.",
    );
