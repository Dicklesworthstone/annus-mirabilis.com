...
describe("notebook with the real shared storage layer", () => {
  test("saves and reloads only its registered namespace", () => {
    const f = fixture();
    f.values.set("am:settings:v1:detail", "2");
    createNotebookStore(f.storage).add(entry());
    const reopened = createNotebookStore(createNotebookStorage(f.context));
    reopened.open();
    expect(reopened.getSnapshot().document.entries).toEqual([entry()]);
    reopened.clearConfirmed();
    expect(f.values.get("am:settings:v1:detail")).toBe("2");
  ...
  test("an external clear is authoritative even after a quota failure left a fallback", () => {
    const f = fixture(),
      store = createNotebookStore(f.storage);
    store.add(entry());
    f.mode.quota = true;
    store.add(entry("n2"));
    expect(f.context.fallback.has(NOTEBOOK_KEY)).toBe(true);
    f.values.delete(NOTEBOOK_KEY);
    f.mode.quota = false;
  ...
  test("future versions are quarantined and are never overwritten by normal saving", () => {
    const f = fixture(),
      raw = JSON.stringify({ schemaVersion: 99, future: "preserve" });
    ...
    expect(
      listQuarantine(f.context).some((e) => e.originalKey === NOTEBOOK_KEY && e.rawValue === raw),
    ).toBe(true);
  ...
  test("an unregistered notebook namespace cannot be used", () => {
    const f = fixture();
    expect(() =>
      createNotebookStorage({ ...f.context, registry: createKeyRegistry([]) }),
    ).toThrow();
  });
});
