...
function record(input: unknown, fields: readonly string[]): Record<string, unknown> {
  ...
  if (
    ...
    fields.some((key) => !Object.hasOwn(Object.getOwnPropertyDescriptor(input, key) ?? {}, "value"))
  )
...
export function parseNotebookFrame(input: unknown): NotebookFrame {
  ...
  if (
    !NOTEBOOK_PAPERS.includes(f.paper as NotebookPaper) ||
    !NOTEBOOK_VIEWS.includes(f.view as NotebookFrame["view"]) ||
    ![0, 1, 2].includes(f.detail as number) ||
    !["paper", "modern"].includes(f.lens as string)
  )
...
export function parseNotebookEntry(input: unknown): NotebookEntry {
  ...
  if (
    ...
    !NOTEBOOK_KINDS.includes(e.kind as NotebookKind) ||
...
export function parseLastPlace(input: unknown): LastPlace {
  ...
  return Object.freeze({
    frame: parseNotebookFrame(p.frame),
    title: text(p.title, NOTEBOOK_LIMITS.title),
    recap: text(p.recap, NOTEBOOK_LIMITS.recap),
    recapKind: p.recapKind,
  });
}
...
export function notebookFrameHref(input: NotebookFrame): string {
  const frame = parseNotebookFrame(input),
