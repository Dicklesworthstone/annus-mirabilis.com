...
export function replayCompatibility(replay: ComparisonReplay, anchor: string, current: ReplayCatalogue) {
  const mismatches = (["modelVersion", "streamVersion", "allocationId", "constantSetId", "sourceDigest", "artifactDigest", "executionLabel"] as const)
  ...
  const changed = passage !== undefined && (passage.contentRevision !== replay.passage.contentRevision ||
  ...
  return Object.freeze({
    ...
    modelMessage: mismatches.length ? "The model has changed since you saved this; replaying starts a new identified run." :
      "Replaying uses the current matching model and creates a new identified run. Saved readouts stay separate.",
    passageMessage: !passage ? "The saved passage could not be resolved. Open the saved paper to find the current argument." :
      changed ? "The passage has been revised since this entry was saved. The link opens the current text." :
      replay.passage.contentRevision === null ? "No passage revision was available when this entry was saved." : "The saved passage revision matches this build.",
    aliasMessage: resolved.kind === "redirect" ? `The saved anchor was retired; ${resolved.redirect.note ?? "its successor is linked"}.` : "",
  });
}
