...
export interface NotebookStorage {
  ...
  write(document: NotebookDocument): Readonly<{ status: "ok" | "unavailable" | "quota" | "unregistered" }>;
  decode(raw: string): unknown;
  preserve(raw: string): void;
  discardFallback(): void;
}
const estimateBytes = (key: string, value: string) => (key.length + value.length) * 2;
const SESSION_MESSAGE = "Notes are available in this tab, but could not be saved on this device. Export them before leaving.";
const PROTECTED_MESSAGE = "A saved notebook could not be read by this version. Its original is preserved. New notes stay in this tab until you export them or explicitly clear the saved notebook.";
const CONFLICT_MESSAGE = "The saved notebook changed in another tab or was cleared. This tab's work is retained without overwriting that change. Export it before loading the saved notebook.";
...
export function createNotebookStore(storage: NotebookStorage) {
  ...
  let state: NotebookState = Object.freeze({ document: emptyNotebook(), persistence: "unopened", message: "", recoveryRaw: null });
  let baseRaw: string | null = null;
  const listeners = new Set<() => void>();
  function publish(next: NotebookState) {
    state = Object.freeze(next);
    for (const listener of listeners) {
      try { listener(); } catch { /* A detached view must not interrupt persistence or another subscriber. */ }
    }
  }
  function readSaved() {
  ...
  function open() {
    ...
    baseRaw = saved.text;
    if (saved.text === null) {
      publish({ ...state, persistence: saved.raw.status === "unavailable" ? "session-only" : "saved",
        message: saved.raw.status === "unavailable" ? SESSION_MESSAGE : "Notes stay on this device. Browser storage is not a permanent backup." });
      return;
    }
    try {
      ...
      publish({ document, persistence: "saved", message: "Notes stay on this device. Browser storage is not a permanent backup.", recoveryRaw: null });
    } catch {
      try { storage.preserve(saved.text); } catch { /* The opaque original also remains in memory. */ }
      publish({ ...state, persistence: "protected", message: PROTECTED_MESSAGE, recoveryRaw: saved.text });
    }
  }
  function commit(input: NotebookDocument, allowProtected = false): NotebookChange {
    ...
    try {
      ...
      if (estimateBytes(NOTEBOOK_KEY, JSON.stringify(document)) > storage.maxBytes)
        return { ok: false, message: "The notebook is full. Export it and remove entries before adding more; no notes were discarded." };
    } catch (error) {
      return { ok: false, message: error instanceof Error ? error.message : "Invalid notebook change." };
    }
    ...
    if (state.persistence === "conflict" ||
        (saved.raw.status !== "unavailable" && saved.text !== baseRaw &&
    ...
    publish({ document, persistence: outcome.status === "ok" ? "saved" : "session-only",
      message: outcome.status === "ok" ? "Saved on this device. Export important notes as a backup." : SESSION_MESSAGE,
  ...
  return Object.freeze({
    ...
    add(input: NotebookEntry): NotebookChange {
      ...
      try { entry = parseNotebookEntry(input); }
      catch (error) { return { ok: false, message: error instanceof Error ? error.message : "Invalid entry." }; }
    ...
    updateText(id: string, text: string): NotebookChange {
      ...
      return commit({ ...state.document, entries: state.document.entries.map((entry) => entry.id === id ? { ...entry, text } : entry) });
    },
    remove(id: string): NotebookChange {
      open();
      return commit({ ...state.document, entries: state.document.entries.filter((entry) => entry.id !== id) });
    },
    remember(input: LastPlace): NotebookChange {
      open();
      let place: LastPlace;
      try { place = parseLastPlace(input); }
      catch { return { ok: false, message: "This passage has no portable reading location or authored recap." }; }
    ...
    reloadConfirmed() {
      ...
      state = Object.freeze({ document: emptyNotebook(), persistence: "unopened", message: "", recoveryRaw: null });
      open();
    },
    checkForExternalChange() {
      if (state.persistence === "unopened") return;
      const saved = readSaved();
      if (saved.raw.status !== "unavailable" && saved.text !== baseRaw &&
          !(state.persistence === "session-only" && saved.text === JSON.stringify(state.document)))
