import { type NotebookDocument, parseNotebookDocument } from "./schema.ts";
...
export function mergeNotebook(
  ...
  for (const entry of imported.entries) {
    ...
    if (existing) {
      ...
      if (JSON.stringify(existing) !== JSON.stringify(entry))
        throw new TypeError(
          "An imported entry has the same id as different work already in this notebook. No entries were imported; keep both exports.",
        );
      duplicates++;
    } else {
      entries.push(entry);
  ...
  return Object.freeze({
    document: parseNotebookDocument({
      ...current,
      entries,
      lastPlace: current.lastPlace ?? imported.lastPlace,
    }),
    added,
    duplicates,
  });
