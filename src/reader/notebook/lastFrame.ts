import { NOTEBOOK_PAPERS, NOTEBOOK_VIEWS, parseLastPlace, parseNotebookFrame,
  type LastPlace, type NotebookFrame } from "./schema.ts";

/** Read navigation metadata and authored prose from the existing reader, never from note text. */
export function frameForPassage(passage: HTMLElement, location: Pick<Location, "pathname" | "search">,
  root: HTMLElement): NotebookFrame | null {
  ...
  const view = root.closest<HTMLElement>("[data-reader-root]")?.dataset.view ?? query.get("view") ?? "reading";
  const detailValue = document.documentElement.dataset.detail ?? query.get("detail") ?? "1";
  const detail = ({ overview: 0, full: 1, steps: 2 } as Record<string, number>)[detailValue] ?? Number(detailValue);
  try {
    return parseNotebookFrame({ paper, anchor: passage.id,
      view: NOTEBOOK_VIEWS.includes(view as NotebookFrame["view"]) ? view : "reading",
      detail, lens: document.documentElement.dataset.lens === "modern" || query.get("lens") === "modern" ? "modern" : "paper",
      open: "" });
  } catch { return null; }
}
...
export function authoredLastPlace(passage: HTMLElement, frame: NotebookFrame): LastPlace | null {
  ...
  try {
    return parseLastPlace({ frame, title, recap: recap || overview, recapKind: recap ? "authored-recap" : "overview" });
  } catch { return null; }
}
...
export function shouldUseCompactRecap(last: LastPlace, pathname: string, hash: string): boolean {
  ...
  let anchor: string;
  try { anchor = decodeURIComponent(hash.slice(1)); } catch { return true; }
