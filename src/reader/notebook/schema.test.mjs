...
export function document(entries = [entry()]) {
  return { ...emptyNotebook(), entries };
}

test("a parsed notebook is detached and deeply immutable", () => {
  const input = document(),
    parsed = parseNotebookDocument(input);
  input.entries[0].frame.detail = 2;
  input.entries[0].text = "changed";
  assert.equal(parsed.entries[0].frame.detail, 1);
  assert.equal(parsed.entries[0].text, "My question");
...
test("duplicate ids and oversized entry collections are refused", () => {
  ...
  assert.throws(() =>
    parseNotebookDocument(document(Array.from({ length: 101 }, (_, i) => entry({ id: `n-${i}` })))),
  );
});
test("bounded text accepts the exact limit and rejects one character more", () => {
  assert.equal(
    parseNotebookEntry(entry({ text: "x".repeat(NOTEBOOK_LIMITS.text) })).text.length,
    4000,
  );
...
test("frame URL injection, private extra fields, and arbitrary routes are rejected", () => {
  for (const patch of [
    { paper: "//evil.test" },
    { anchor: "x?note=secret" },
    { open: "foundation:x&notes=secret" },
...
test("only the supported portable clarification is admitted", () => {
  assert.equal(
    parseNotebookFrame({ ...frame, open: "foundation:probability" }).open,
...
test("last-place records retain authored recap or labeled overview, not a fabricated summary", () => {
  for (const recapKind of ["authored-recap", "overview"]) {
    const doc = parseNotebookDocument({
      ...document(),
      lastPlace: { frame, title: "The observable", recap: "An authored recap.", recapKind },
    });
  ...
  assert.throws(() =>
    parseNotebookDocument({
      ...document(),
      lastPlace: { frame, title: "T", recap: "R", recapKind: "generated" },
    }),
  );
});
