...
import { type NotebookReplayEntry, parseNotebookEntry } from "./schema.ts";

export async function makeReplayEntry(
  input: Readonly<{
  ...
  return parseNotebookEntry({
    ...
    createdAt,
    kind: "replay",
    title: "My controlled Brownian comparison",
    text: input.notes,
    frame: {
...
export async function saveComparisonReplay(
  store: NotebookStore,
  input: Parameters<typeof makeReplayEntry>[0],
) {
