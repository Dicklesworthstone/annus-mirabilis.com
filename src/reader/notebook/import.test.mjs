...
const entry = (id, text = "A question") => ({ id, kind: "note", frame, title: "The observable", text, createdAt: "2026-09-17T14:00:00.000Z" });
const document = (entries = []) => ({ ...emptyNotebook(), entries });
const place = { frame, title: "The observable", recap: "The squares add.", recapKind: "authored-recap" };
...
test("exported entries merge locally and identical ids are deduplicated", () => {
  ...
  assert.equal(merged.added, 1); assert.equal(merged.duplicates, 1);
  assert.deepEqual(merged.document.entries.map((e) => e.id), ["a", "b"]);
});
...
test("different text at an existing id is rejected rather than overwritten", () => {
  ...
  assert.throws(() => mergeNotebook(original, document([entry("b"), entry("a", "Other private work")])));
  assert.equal(original.entries.length, 1); assert.equal(original.entries[0].text, "A question");
});
...
test("future or unknown shapes are refused by the same persisted-document validator", () => {
  for (const input of [{ schemaVersion: 99 }, { ...document(), unexpected: "hidden data" }, null, []])
...
test("confirmed import uses persistence limits and preserves memory on storage failure", () => {
  let raw = null; let quota = false;
  const store = createNotebookStore({ maxBytes: 64000,
    read: () => raw === null ? { status: "missing" } : { status: "ok", value: raw },
    write: (doc) => { if (quota) return { status: "quota" }; raw = JSON.stringify(doc); return { status: "ok" }; },
    decode: JSON.parse, preserve() {}, discardFallback() {} });
  store.add(entry("existing")); quota = true;
  ...
  quota = false; store.retry(); assert.equal(JSON.parse(raw).entries.length, 2);
});
test("oversized merged document is refused before writing", () => {
  let writes = 0;
  const store = createNotebookStore({ maxBytes: 1000, read: () => ({ status: "missing" }),
    write() { writes++; return { status: "ok" }; }, decode: JSON.parse, preserve() {}, discardFallback() {} });
  assert.equal(store.importConfirmed(document([entry("a", "x".repeat(1000))])).ok, false);
  assert.equal(writes, 0); assert.equal(store.getSnapshot().document.entries.length, 0);
});
