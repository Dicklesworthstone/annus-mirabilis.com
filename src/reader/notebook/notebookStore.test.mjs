...
const entry = (id = "n1", text = "Private question") => ({
  id,
  text,
  kind: "note",
  frame,
...
function backend(raw = null) {
  ...
  return {
    ...
    write(document) {
      this.writes++;
      if (this.mode !== "ok") return { status: this.mode === "blocked" ? "unavailable" : "quota" };
      value = JSON.stringify(document);
      return { status: "ok" };
    },
    decode: JSON.parse,
    preserve(raw) {
      this.preserved.push(raw);
    },
    discardFallback() {},
    raw: () => value,
    replace: (text) => {
...
test("add, reload, edit and remove use a single document", () => {
  ...
  const next = createNotebookStore(io);
  next.open();
  assert.equal(next.getSnapshot().document.entries[0].text, "Private question");
  next.updateText("n1", "Revised question");
  assert.equal(JSON.parse(io.raw()).entries[0].text, "Revised question");
  next.remove("n1");
  assert.deepEqual(JSON.parse(io.raw()).entries, []);
});
for (const mode of ["blocked", "quota"])
  test(`${mode} keeps all new session notes, even when old saved bytes remain`, () => {
    const io = backend(),
      store = createNotebookStore(io);
    store.add(entry());
    const old = io.raw();
...
for (const raw of [
  "{broken",
  JSON.stringify({ ...emptyNotebook(), schemaVersion: 9 }),
  JSON.stringify({ ...emptyNotebook(), extra: true }),
])
  test(`unreadable original is protected: ${raw.slice(0, 20)}`, () => {
    const io = backend(raw),
      store = createNotebookStore(io);
    store.open();
    store.add(entry());
    ...
    assert.deepEqual(io.preserved, [raw]);
    store.clearConfirmed();
    assert.deepEqual(JSON.parse(io.raw()), emptyNotebook());
  });
...
test("a second tab's write is never silently overwritten", () => {
  ...
  assert.equal(second.getSnapshot().document.entries[0].id, "first");
});
...
test("remembering a frame is idempotent, exported and clearable without changing settings", () => {
  ...
  const next = createNotebookStore(io);
  next.open();
  assert.deepEqual(next.getSnapshot().document.lastPlace, place);
  next.forgetPlace();
  assert.equal(next.getSnapshot().document.lastPlace, null);
  next.clearConfirmed();
  assert.deepEqual(JSON.parse(io.raw()), emptyNotebook());
});
test("clear failure is visibly session-only, never claimed as a persistent deletion", () => {
  ...
  store.clearConfirmed();
  assert.equal(store.getSnapshot().persistence, "session-only");
  assert.equal(JSON.parse(io.raw()).entries.length, 1);
});
test("subscriber failures and unsubscribe do not break saving", () => {
  const store = createNotebookStore(backend());
  let calls = 0;
  const off = store.subscribe(() => {
    calls++;
  });
  store.subscribe(() => {
    throw Error("view detached");
  });
  store.add(entry());
  off();
  const before = calls;
...
test("JSON export round trips exactly, including Unicode and negative-looking text", () => {
  const store = createNotebookStore(backend());
  store.add(entry("n1", "λₓ\n=HYPERLINK('private')"));
  store.remember(place);
  assert.deepEqual(
    JSON.parse(exportNotebookJson(store.getSnapshot().document)),
    store.getSnapshot().document,
  );
});
test("readable export escapes hostile content and makes no executable or remote asset", () => {
  const store = createNotebookStore(backend());
  store.add(entry("n1", '<img src="https://evil.test">\n<script>alert(1)</script>'));
  const html = exportNotebookHtml(store.getSnapshot().document);
  assert.ok(html.includes("&lt;script&gt;"));
  assert.ok(!html.includes("<script"));
  assert.ok(!html.includes("<img"));
  assert.match(html, /default-src 'none'/);
  assert.match(html, /https:\/\/annus-mirabilis.com\/papers\/brownian-motion\/#arg-bm-observable/);
});
