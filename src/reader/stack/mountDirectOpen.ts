...
import { deserializeStackState, openClarification, resolveOpenParam, serializeStackState } from "./history.ts";
import { EMPTY_STACK_STATE, topFrame, type StackFrame } from "./stackStore.ts";

const DIALOG_ATTR = "data-instrument-clarification-dialog";
const MOUNT_ATTR = "data-instrument-clarification-mount";
type Mounted = {root: Root; dialog: HTMLDialogElement; frame: StackFrame; onCancel: (e: Event) => void};
...
function ensureDialog(doc: Document) {
  ...
  if (!dialog) {
    dialog = doc.createElement("dialog"); dialog.setAttribute(DIALOG_ATTR, "");
    const mount = doc.createElement("div"); mount.setAttribute(MOUNT_ATTR, "");
    dialog.appendChild(mount); doc.body.appendChild(dialog);
  }
...
export function closeDirectOpenDialog(doc: Document, returnFocus = false): void {
  ...
  if (returnFocus) doc.defaultView?.requestAnimationFrame(() => {
  ...
  queueMicrotask(() => {
    ...
    mounted.delete(doc); current.root.unmount();
  });
}
...
export function openFromTrigger(doc: Document, trigger: HTMLElement, raw: string): boolean {
  ...
  const anchor = trigger.closest<HTMLElement>(".reader-passage")?.id ?? decodeAnchor(win.location.hash);
  if (!trigger.id) trigger.id = `clarification-trigger-${++triggerNumber}`;
  const outcome = openClarification(mounted.get(doc)?.dialog.open
    ? deserializeStackState(previous?.annusClarification?.stack) ?? EMPTY_STACK_STATE : EMPTY_STACK_STATE, {
    kind: raw.slice(0, raw.indexOf(":")), id: raw.slice(raw.indexOf(":") + 1),
    question: trigger.closest(".reader-passage")?.querySelector(".passage-question")?.textContent ?? "Why does this step follow?",
    returnTo: {anchor,face:reader?.dataset.view ?? "reading",detail:Number(doc.documentElement.dataset.detail ?? 1),
      perspective:params.get("lens"),notation:params.get("notation"),unitLayer:params.get("units"),
      selectionId:trigger.dataset.selectionId ?? null,formId:null,triggerId:trigger.id,
      scrollFraction:trigger.getBoundingClientRect().top / Math.max(1,win.innerHeight),lab:null},
  });
  if (outcome.status !== "descended") return false;
  const url = new URL(win.location.href); url.searchParams.set("open",raw); if (anchor) url.hash = anchor;
  // Preserve the reader's independent history keys and every unrelated query parameter.
  const state = {...previous,annusClarification:{stack:serializeStackState(outcome.state),pushed:true}};
...
function decodeAnchor(hash: string): string {
  try { const value = decodeURIComponent(hash.replace(/^#/,"")); return /^[a-zA-Z][a-zA-Z0-9._-]*$/.test(value) ? value : ""; }
  catch { return ""; }
}
...
export function openFromSearch(doc: Document, search: string): boolean {
  ...
  if (!resolved.definition.descends) {
    ...
    doc.getElementById(anchorId)?.scrollIntoView(); return true;
  }
  const renderKind = resolved.definition.render, win = doc.defaultView;
  if (!renderKind || !win) return false;
  const saved = topFrame(deserializeStackState(win.history.state?.annusClarification?.stack) ?? EMPTY_STACK_STATE);
  const frame: StackFrame = saved?.clarification.kind === resolved.kind && saved.clarification.id === resolved.rawId ? saved : {
    anchor:decodeAnchor(win.location.hash),face:doc.documentElement.dataset.view ?? "reading",
    detail:Number(doc.documentElement.dataset.detail ?? 1),perspective:params.get("lens"),
    notation:params.get("notation"),unitLayer:params.get("units"),
    selectionId:null,formId:null,clarification:{kind:resolved.kind,id:resolved.rawId},title:resolved.definition.title(resolved.parsedId),
    question:"Opened from a direct link.",triggerId:"",scrollFraction:0.15,lab:null,
  };
  ...
  const onReturn = () => {
    ...
    if (pushed) {
      ...
      win.addEventListener("popstate",()=>win.requestAnimationFrame(()=>restore(doc,frame)),{once:true});
      win.history.back();
    } else {
      const url = new URL(win.location.href); url.searchParams.delete("open");
      win.history.replaceState({...win.history.state,annusClarification:null},"",url);
      restore(doc,frame);
    }
  };
  const onCancel = (e: Event) => {e.preventDefault();onReturn();};
  mounted.set(doc,{root,dialog,frame,onCancel}); dialog.addEventListener("cancel",onCancel);
  const child = renderKind({parsed:resolved.parsedId,instanceId:`${resolved.kind}:${resolved.rawId}`});
  flushSync(()=>root.render(createElement("div",null,createElement(Compass,{frame,onReturn}),child)));
