...
export type ReaderRegistry = Readonly<{
  paperId: string;
...
export function parseReaderLocation(
  ...
  const view = single("view"),
    detail = parseDetail(single("detail")) ?? parseDetail(stored) ?? 1;
  let rawAnchor = "";
  try {
    rawAnchor = decodeURIComponent(hash.replace(/^#/, ""));
  } catch {
  ...
  return Object.freeze({
    detail,
    view: FACES.includes(view as Face) ? (view as Face) : "reading",
    lens: single("lens") === "modern",
    anchor,
    frames: Object.freeze(
      registry.foundations.includes(foundationId)
        ? [{ foundationId, triggerId: "", relativeY: 0 }]
        : [],
    ),
  });
}
export function openFoundation(
  ...
  if (!registry.foundations.includes(frame.foundationId) || !Number.isFinite(frame.relativeY))
    throw new TypeError("Unknown or malformed clarification.");
  const frames = [...state.frames];
  if (frames.length === MAX_CLARIFICATION_DEPTH)
    frames[MAX_CLARIFICATION_DEPTH - 1] = Object.freeze({ ...frame });
...
export function passageHref(
  registry: ReaderRegistry,
  state: Pick<ReaderState, "view" | "detail" | "lens" | "anchor">,
): string {
  if (
    !/^[a-z][a-z0-9-]*$/.test(registry.paperId) ||
    !registry.anchors.includes(state.anchor) ||
    !FACES.includes(state.view) ||
    ![0, 1, 2].includes(state.detail)
  )
    throw new TypeError("Invalid passage location.");
...
export function readerHref(registry: ReaderRegistry, state: ReaderState): string {
  ...
  if (frame) url.searchParams.set("open", `foundation:${frame.foundationId}`);
...
export function restoreReaderState(input: unknown, registry: ReaderRegistry): ReaderState | null {
  ...
  if (
    !FACES.includes(s.view) ||
    ![0, 1, 2].includes(s.detail) ||
    typeof s.lens !== "boolean" ||
    !registry.anchors.includes(s.anchor) ||
    !Array.isArray(s.frames) ||
    s.frames.length > MAX_CLARIFICATION_DEPTH
  )
    return null;
  if (
    s.frames.some(
      (f) =>
        !f ||
        !registry.foundations.includes(f.foundationId) ||
        typeof f.triggerId !== "string" ||
        f.triggerId.length > 200 ||
        !Number.isFinite(f.relativeY),
    )
  )
    return null;
  return Object.freeze({
