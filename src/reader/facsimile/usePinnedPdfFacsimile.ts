 ...
 * Source path: src/components/patents/usePinnedPdfFacsimile.ts
 * Pinned commit: da11ff475902728fd8dd1d9db9f3af37c16ec8a5
 * License: MIT License (with OpenAI/Anthropic Rider)
 * Preserved license text: /LICENSE
 *
 * Modifications:
 * - Decoupled patent-specific comments to support general historical facsimiles.
 * - Enforced dynamic loading of pdfjs-dist and lifecycle cancellation safety.
 */
...
export function usePinnedPdfFacsimile({
  ...
  useEffect(() => {
    ...
    const loadDocument = async () => {
      try {
        // PDF.js reaches browser Worker/Canvas APIs and must only be loaded dynamically on client.
