/** Build-time adapter only. The browser receives prepared values, never a second physics model. */
...
function scalar(result: ScientificResult): EntranceScalar {
...
export function prepareMassEnergyScenario(id: "fast" | "slow"): MassEnergyEntranceScenario {
  ...
  const inputs = Object.freeze({ emittedEnergyRestFrame: 10, frameSpeed: beta, emissionAngle: 0,
    offsetDisplay: "symbolic" as const, notation: "modern" as const });
  ...
  const bodyEnergies = [ledger.restBodyBefore, ledger.restBodyAfter,
    ledger.movingBodyBefore, ledger.movingBodyAfter].map(result => {
  ...
  return Object.freeze({
    id, beta, speedLabel: id === "fast" ? "60 percent of light speed" : "1 percent of light speed",
    restPulse: scalar(atRest.pulse1Moving), restLight: scalar(ledger.restBalanceLight),
    movingPulse1: scalar(ledger.pulse1Moving), movingPulse2: scalar(ledger.pulse2Moving),
    movingLight: scalar(ledger.movingBalanceLight), subtraction: scalar(ledger.subtractionDifference),
    kinetic: scalar(ledger.kineticEnergyDifference), relaxedKinetic: relaxed.kineticEnergyDifference,
    quadratic: scalar(coefficient.quadraticApproximation), discrepancy: scalar(coefficient.quadraticDiscrepancy),
