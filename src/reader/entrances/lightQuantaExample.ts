...
export function tokenExample(input: TokenSetup): TokenExampleResult {
  if (!input || typeof input.locked !== "boolean" || !Number.isSafeInteger(input.tokens)
    || input.tokens < 1 || input.tokens > (input.locked ? 10 : 4)
    || ![2, 3].includes(input.parts)) {
    return { kind: "refused", message: "Use one to four independent tokens, or up to ten together, in two or three equal parts." };
  }
  ...
  if (count.status !== "value") return { kind: "refused", message: "The counting owner could not enumerate this example." };
  ...
  const arrangements = Array.from({ length: count.totalConfigurations }, (_, index) => {
    ...
    return Object.freeze({ parts: Object.freeze(positions), allLeft: positions.every(part => part === 0) });
  });
  const parameters = { ...LQ05_DEFAULTS, n: setup.tokens, f: 1 / setup.parts,
    locked: setup.locked, view: "enumeration" as const };
  return { kind: "ready", example: Object.freeze({ setup, arrangements: Object.freeze(arrangements),
    total: count.totalConfigurations, favorable: count.favorableConfigurations, probability,
