...
import { encodeSr01Settings } from "../../experiments/sr01/permalink.ts";
...
export function clockExample(input: ClockReadings): ClockExampleResult {
  if (!input || !Number.isFinite(input.departure) || !Number.isFinite(input.reception)
    || Math.abs(input.departure) > 1e6 || Math.abs(input.reception) > 1e6) {
    return { kind: "refused", message: "Use finite clock readings between minus one million and one million seconds." };
  }
  ...
  if (elapsed <= 0) return { kind: "refused", message: "The return must be later than the departure. The accepted example has not changed." };
  ...
  const result = synchronizationRound({ emissionTimeA: readings.departure,
    receptionTimeA: readings.reception, separationLs });
  if (result.status !== "value" || !Number.isFinite(result.value.assignedRemoteTime)) {
    return { kind: "refused", message: "The clock owner cannot represent this round trip. The accepted example has not changed." };
  }
  const parameters = { ...SR01_DEFAULTS, stationSeparationLs: separationLs,
    emissionTimeA: readings.departure, clockOffsetB: 0, rodBeta: 0,
    pairSeparationLs: separationLs, pairBeta: 0, frameBeta: 0 };
  return { kind: "ready", example: Object.freeze({ readings, elapsed, separationLs,
...
export function parseClockDraft(departure: string, reception: string): ClockExampleResult {
  ...
  if (![departure, reception].every(text => text.length <= 64 && decimal.test(text.trim()))) {
    return { kind: "refused", message: "Enter two complete decimal clock readings. Blank or partly numeric text is not a reading." };
  }
