 ...
 * Source path: src/components/patents/visuals/three/ThreeStudioScene.ts
 * Pinned commit: da11ff475902728fd8dd1d9db9f3af37c16ec8a5
 * License: MIT License (with OpenAI/Anthropic Rider)
 * Preserved license text: /LICENSE
 *
 * Modifications:
 * - Decoupled patent-specific branding to support Annus Mirabilis historical physics simulations.
 * - Inlined deterministic createLcg pseudo-random generator.
 * - Preserved ACES Filmic tone mapping, PCF shadows, dynamic orbit controls, and lifecycle disposal.
 */

function createLcg(initialSeed: number) {
...
export function createThreeStudioScene(opts: StudioOptions): StudioContext {
  ...
  const onPointerMove = (e: PointerEvent) => {
    ...
    if (activePointers.size === 1) {
      ...
      const pt1 = pts[1];
      if (pt0 && pt1) {
        const dist = Math.hypot(pt0.x - pt1.x, pt0.y - pt1.y);
        if (initialPinchDist > 0 && dist > 0) {
          const pinchFactor = initialPinchDist / dist;
          targetSpherical.radius = Math.max(
            minRadius,
  ...
  const onPointerUp = (e: PointerEvent) => {
    ...
    if (activePointers.size === 0) {
      ...
      const p1 = pair[1];
      if (p0 && p1) {
        initialPinchDist = Math.hypot(p0.x - p1.x, p0.y - p1.y);
        initialPinchRadius = targetSpherical.radius;
        prevPinchMidX = (p0.x + p1.x) / 2;
        prevPinchMidY = (p0.y + p1.y) / 2;
      }
