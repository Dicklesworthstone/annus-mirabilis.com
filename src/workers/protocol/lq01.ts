import { LQ01_MODEL, LQ01_OUTPUTS } from "../../experiments/lq01/definition.ts";
import {
  decodeOutcome,
  decodeRefusal,
  decodeResultBatch,
...
import type { Lq01Evaluation } from "../operations/lq01.ts";
import { LabProtocolError } from "./bm06.ts";

export { LabProtocolError } from "./bm06.ts";

export const LQ01_PROTOCOL = "lq01-host-v1";

export type LabRequest = Readonly<{
  messageKind: "request";
  protocolVersion: typeof LQ01_PROTOCOL;
  sourceDigest: string;
  token: RequestToken;
}>;
...
export type LabResponse = Readonly<{
  ...
  sourceDigest: string;
  token: RequestToken;
  result: Computation<Lq01Evaluation>;
}>;
...
export function decodeLabToken(input: unknown): RequestToken {
  ...
  if (o.experimentId !== "lq-01") fail("Unknown instrument.");
  text(o.instanceId);
  text(o.runId);
      ...
      const outputs = decodeResultBatch(
        { revisions: token.revisions, outputs: data.outputs },
        {
          ...
          statuses: Object.fromEntries(
            Object.entries(LQ01_OUTPUTS).map(([id, c]) => [id, c.statuses]),
          ),
        },
      ).outputs;
      for (const output of outputs) {
        const c = LQ01_OUTPUTS[output.quantityId];
        ...
        if (output.status === "value") {
          if (output.quantityId === "screenIntensity") {
            if (!(output.value instanceof Float64Array) || output.value.length !== 101) {
