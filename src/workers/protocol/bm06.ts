...
export type LabRequest = Readonly<{ messageKind: "request"; protocolVersion: typeof BM06_PROTOCOL; sourceDigest: string; token: RequestToken }>;
export type LabCancel = Readonly<{ messageKind: "cancel"; protocolVersion: typeof BM06_PROTOCOL; instanceId: string; actionIndex: number }>;
export type LabResponse = Readonly<{ messageKind: "result"; protocolVersion: typeof BM06_PROTOCOL; sourceDigest: string; token: RequestToken; result: Computation<Bm06Evaluation> }>;
export type LabHello = Readonly<{ messageKind: "hello"; protocolVersion: typeof BM06_PROTOCOL; sourceDigest: string; ownerKind: "host-reference"; modelId: string }>;
export class LabProtocolError extends TypeError {
  readonly code: "protocol-mismatch" | "artifact-mismatch" | "malformed-response";
  constructor(code: LabProtocolError["code"], reason: string) { super(reason); this.name = "LabProtocolError"; this.code = code; }
}
...
function version(input: unknown): void {
  if (input !== BM06_PROTOCOL) throw new LabProtocolError("protocol-mismatch", "Unsupported laboratory protocol.");
}
...
export function decodeLabToken(input: unknown): RequestToken {
  ...
  if (validateBm06Parameters(o.parameters).kind !== "accepted") fail("Invalid complete parameter record.");
...
export function decodeLabHello(input: unknown, expectedDigest: string): LabHello {
  ...
  version(o.protocolVersion); digest(o.sourceDigest, expectedDigest);
  if (o.messageKind !== "hello" || o.ownerKind !== "host-reference" || o.modelId !== BM06_MODEL.id) fail("Unregistered calculation owner.");
...
export function decodeLabResponse(input: unknown, expected: RequestToken, expectedDigest: string): LabResponse {
  try {
    ...
    if (r?.kind === "accepted") {
      ...
      const requiredSteps = token.parameters.gridEnabled && token.parameters.t !== 0 ? token.parameters.steps : 0;
      if (data.stepIndex !== requiredSteps || data.simulationTime !== token.parameters.t) fail("Response evaluates a different time or step count.");
      const outputs = decodeResultBatch({ revisions: token.revisions, outputs: data.outputs }, {
        ...
        statuses: Object.fromEntries(Object.entries(BM06_OUTPUTS).map(([id, c]) => [id, c.statuses])),
      }).outputs;
      for (const output of outputs) {
        ...
        if (output.status === "value") {
          const length = ["positionCoordinate1d", "probabilityDensity"].includes(output.quantityId) ? 81
            : ["comparisonTimes", "comparisonRms"].includes(output.quantityId) ? 3
            : ["gridDensity", "cellMasses", "cellProbabilities"].includes(output.quantityId) ? token.parameters.n : null;
        ...
        if (output.quantityId === "probabilityDensity" && (output.status === "analytic-limit") !== (token.parameters.t === 0)) fail("Density representation does not match elapsed time.");
        if (["gridDensity", "cellMasses", "cellProbabilities", "maxCellMassDifference", "wallContact", "stabilityRatio", "gridTimeStep"].includes(output.quantityId) && (output.status === "value") !== token.parameters.gridEnabled) fail("Grid output does not match the requested mode.");
      }
...
export function labHello(sourceDigest: string): LabHello {
  ...
  return { messageKind: "hello", protocolVersion: BM06_PROTOCOL, sourceDigest, ownerKind: "host-reference", modelId: BM06_MODEL.id };
}
