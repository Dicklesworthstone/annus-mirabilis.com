import { BM01_MODEL, BM01_OUTPUTS, bm01Layout, type Bm01Parameters } from "../../experiments/bm01/definition.ts";
...
export type LabHello = Readonly<{ messageKind: "hello"; protocolVersion: typeof BM01_PROTOCOL; sourceDigest: string; ownerKind: "host-reference"; modelId: string }>;
...
function version(input: unknown): void {
  if (input !== BM01_PROTOCOL) throw new LabProtocolError("protocol-mismatch", "Unsupported laboratory protocol.");
}
...
export function decodeLabToken(input: unknown): RequestToken {
  ...
  if (validateBm01Parameters(o.parameters).kind !== "accepted") fail("Invalid complete parameter record.");
...
export function decodeLabHello(input: unknown, expectedDigest: string): LabHello {
  ...
  version(o.protocolVersion); digest(o.sourceDigest, expectedDigest);
  if (o.messageKind !== "hello" || o.ownerKind !== "host-reference" || o.modelId !== BM01_MODEL.id) fail("Unregistered calculation owner.");
...
export function decodeLabResponse(input: unknown, expected: RequestToken, expectedDigest: string): LabResponse {
  try {
    ...
    if (r?.kind === "accepted") {
      ...
      if (data.stepIndex !== requiredSteps || data.simulationTime !== token.parameters.H) fail("Response evaluates a different time or step count.");
      const outputs = decodeResultBatch({ revisions: token.revisions, outputs: data.outputs }, {
        ...
        statuses: Object.fromEntries(Object.entries(BM01_OUTPUTS).map(([id, c]) => [id, c.statuses])),
      }).outputs;
      for (const output of outputs) {
        ...
        if (["modelApparentSpeed", "sampledApparentSpeed"].includes(output.quantityId) && (output.status === "not-applicable") !== (p.interval === 0)) fail("Apparent speed is undefined at the starting point.");
      }
...
export function labHello(sourceDigest: string): LabHello {
  ...
  return { messageKind: "hello", protocolVersion: BM01_PROTOCOL, sourceDigest, ownerKind: "host-reference", modelId: BM01_MODEL.id };
}
