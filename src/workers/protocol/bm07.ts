import { INFERENCE_GRID_STEPS, INFERENCE_GRID_DT } from "../../physics/reference/inference/synthetic.ts";
import { BM07_MODEL, BM07_OUTPUTS, bm07Layout, type Bm07Parameters } from "../../experiments/bm07/definition.ts";
...
export type LabRequest = Readonly<{ messageKind: "request"; protocolVersion: typeof BM07_PROTOCOL; sourceDigest: string; token: RequestToken }>;
export type LabCancel = Readonly<{ messageKind: "cancel"; protocolVersion: typeof BM07_PROTOCOL; instanceId: string; actionIndex: number }>;
export type LabResponse = Readonly<{ messageKind: "result"; protocolVersion: typeof BM07_PROTOCOL; sourceDigest: string; token: RequestToken; result: Computation<Bm07Evaluation> }>;
export type LabHello = Readonly<{ messageKind: "hello"; protocolVersion: typeof BM07_PROTOCOL; sourceDigest: string; ownerKind: "host-reference"; modelId: string }>;
...
function version(input: unknown): void {
  if (input !== BM07_PROTOCOL) throw new LabProtocolError("protocol-mismatch", "Unsupported laboratory protocol.");
}
...
export function decodeLabToken(input: unknown): RequestToken {
  ...
  if (validateBm07Parameters(o.parameters).kind !== "accepted") fail("Invalid complete parameter record.");
...
export function decodeLabHello(input: unknown, expectedDigest: string): LabHello {
  ...
  version(o.protocolVersion); digest(o.sourceDigest, expectedDigest);
  if (o.messageKind !== "hello" || o.ownerKind !== "host-reference" || o.modelId !== BM07_MODEL.id) fail("Unregistered calculation owner.");
...
export function decodeLabResponse(input: unknown, expected: RequestToken, expectedDigest: string): LabResponse {
  try {
    ...
    if (r?.kind === "accepted") {
      ...
      if (data.stepIndex !== requiredSteps || data.simulationTime !== INFERENCE_GRID_STEPS * INFERENCE_GRID_DT) fail("Response evaluates a different time or step count.");
      const outputs = decodeResultBatch({ revisions: token.revisions, outputs: data.outputs }, {
        ...
        statuses: Object.fromEntries(Object.entries(BM07_OUTPUTS).map(([id, c]) => [id, c.statuses])),
      }).outputs;
      for (const output of outputs) {
        ...
        if (output.quantityId === "sampleCount" && (output.status !== "value" || output.value !== p.M)) fail("The sample count does not describe these observations.");
        if (output.quantityId === "degreesOfFreedom" && (output.status !== "value" || output.value !== p.d * (p.estimator === "independent-increment-known-zero-drift" ? p.M : p.M - 1))) fail("Wrong estimator degrees of freedom.");
        if (!p.radiusKnown && ["avogadroNumberEstimate", "molecularInterval", "conditionalInterval"].includes(output.quantityId) && output.status !== "underdetermined") fail("A molecular number was identified without an independent radius.");
        if (output.quantityId === "observationTimes" && output.status === "value") {
          const times = output.value as Float64Array;
          if (!times.every((t, i) => t === i * p.dt)) fail("The observation times do not match the accepted measurement.");
        }
...
export function labHello(sourceDigest: string): LabHello {
  ...
  return { messageKind: "hello", protocolVersion: BM07_PROTOCOL, sourceDigest, ownerKind: "host-reference", modelId: BM07_MODEL.id };
}
