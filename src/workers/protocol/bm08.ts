...
import { CAMERA_GRID_DT, CAMERA_GRID_STEPS } from "../../physics/reference/inference/camera.ts";
...
export type LabRequest = Readonly<{
  messageKind: "request";
  protocolVersion: typeof BM08_PROTOCOL;
  sourceDigest: string;
  token: RequestToken;
}>;
...
export type LabResponse = Readonly<{
  ...
  sourceDigest: string;
  token: RequestToken;
  result: Computation<Bm08Evaluation>;
}>;
...
export function decodeLabHello(input: unknown, expectedDigest: string): LabHello {
  ...
  version(o.protocolVersion);
  digest(o.sourceDigest, expectedDigest);
  if (o.messageKind !== "hello" || o.ownerKind !== "host-reference" || o.modelId !== BM08_MODEL.id)
  ...
  try {
    ...
    if (r?.kind === "accepted") {
      ...
      if (
        ...
        data.simulationTime !== CAMERA_GRID_STEPS * CAMERA_GRID_DT
      )
      ...
      const outputs = decodeResultBatch(
        { revisions: token.revisions, outputs: data.outputs },
        {
          ...
          statuses: Object.fromEntries(
            Object.entries(BM08_OUTPUTS).map(([id, c]) => [id, c.statuses]),
          ),
        },
      ).outputs;
      for (const output of outputs) {
        ...
        if (
          output.quantityId === "pairCount" &&
          (output.status !== "value" || output.value !== Math.floor((p.M + 1) / 2))
        )
          fail("Wrong pair count.");
        if (
          output.quantityId === "pairDegrees" &&
          (output.status !== "value" || output.value !== p.d * (Math.floor((p.M + 1) / 2) - 1))
        )
          fail("Wrong pair degrees of freedom.");
        if (
          output.quantityId === "modelDiffusion" &&
          (output.status !== "value" || output.value !== p.D)
        )
          fail("The generating scale does not match this request.");
        if (
          output.quantityId === "times" &&
          output.status === "value" &&
          !(output.value as Float64Array).every((t, i) => t === i * p.dt)
        )
          fail("The exposure-start times do not match this measurement.");
        if (
          output.quantityId === "naiveInterval" &&
          (p.sigma > 0 || p.exposure > 0 || p.flowDrift + p.stageDrift !== 0) &&
          output.status !== "not-applicable"
        )
          fail("An invalid naive interval was admitted.");
        if (
          output.quantityId === "centeredInterval" &&
          (p.sigma > 0 || p.exposure > 0) &&
          output.status !== "not-applicable"
        )
          fail("An invalid centered interval was admitted.");
      }
