import { BM05_MODEL, BM05_OUTPUTS, bm05Layout, type Bm05Parameters } from "../../experiments/bm05/definition.ts";
...
export type LabRequest = Readonly<{ messageKind: "request"; protocolVersion: typeof BM05_PROTOCOL; sourceDigest: string; token: RequestToken }>;
export type LabCancel = Readonly<{ messageKind: "cancel"; protocolVersion: typeof BM05_PROTOCOL; instanceId: string; actionIndex: number }>;
export type LabResponse = Readonly<{ messageKind: "result"; protocolVersion: typeof BM05_PROTOCOL; sourceDigest: string; token: RequestToken; result: Computation<Bm05Evaluation> }>;
export type LabHello = Readonly<{ messageKind: "hello"; protocolVersion: typeof BM05_PROTOCOL; sourceDigest: string; ownerKind: "host-reference"; modelId: string }>;
...
function version(input: unknown): void {
  if (input !== BM05_PROTOCOL) throw new LabProtocolError("protocol-mismatch", "Unsupported laboratory protocol.");
}
...
export function decodeLabToken(input: unknown): RequestToken {
  ...
  if (validateBm05Parameters(o.parameters).kind !== "accepted") fail("Invalid complete parameter record.");
...
export function decodeLabHello(input: unknown, expectedDigest: string): LabHello {
  ...
  version(o.protocolVersion); digest(o.sourceDigest, expectedDigest);
  if (o.messageKind !== "hello" || o.ownerKind !== "host-reference" || o.modelId !== BM05_MODEL.id) fail("Unregistered calculation owner.");
...
export function decodeLabResponse(input: unknown, expected: RequestToken, expectedDigest: string): LabResponse {
  try {
    ...
    if (r?.kind === "accepted") {
      ...
      if (data.stepIndex !== requiredSteps || data.simulationTime !== Number(token.parameters.runSteps) * Number(token.parameters.tau)) fail("Response evaluates a different time or step count.");
      const outputs = decodeResultBatch({ revisions: token.revisions, outputs: data.outputs }, {
        ...
        statuses: Object.fromEntries(Object.entries(BM05_OUTPUTS).map(([id, c]) => [id, c.statuses])),
      }).outputs;
      for (const output of outputs) {
        ...
        if (["sumKurtosis", "kolmogorovDistance", "shapeTerm", "agreementBound", "withinBound"].includes(output.quantityId) && output.status !== (p.n === 0 ? "not-applicable" : "value")) fail("The zero-step comparison status is inconsistent.");
        if (["coinPositions", "coinProbabilities", "coinNumerators", "coinDenominator"].includes(output.quantityId) && output.status !== (p.kernel === "coin" && p.n <= 16 ? "value" : "not-applicable")) fail("Exact coin support status is inconsistent.");
        if (output.quantityId === "biasedDiffusion" && output.status !== (p.bias === .5 ? "value" : "outside-domain")) fail("Biased diffusion status is inconsistent.");
      }
...
export function labHello(sourceDigest: string): LabHello {
  ...
  return { messageKind: "hello", protocolVersion: BM05_PROTOCOL, sourceDigest, ownerKind: "host-reference", modelId: BM05_MODEL.id };
}
