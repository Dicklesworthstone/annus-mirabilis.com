...
import { stokesEinsteinD, rmsDisplacement, intervalProbability, gaussianPropagator, moments } from "../../physics/reference/diffusion/distributions.ts";
import { ftcs1d, ftcsAdvance, ftcsAnalyticComparison, type Computation } from "../../physics/reference/diffusion/ftcs.ts";
...
export type Bm06Evaluation = Readonly<{ outputs: readonly ScientificResult[]; stepIndex: number; simulationTime: number }>;
...
const gridKeys = ["gridDensity", "cellMasses", "cellProbabilities", "maxCellMassDifference", "wallContact", "stabilityRatio", "gridTimeStep"] as const;
...
function refused(parameterIds: readonly string[], requirements: string): Computation<never> {
  return { kind: "refused", refusal: makeRefusal("invalid-parameter", { parameterIds }, { details: { requirements } }) };
}
function value(quantityId: string, value: number | Float64Array): ScientificResult {
  ...
  return decodeResult({ quantityId, unit: c.unit, semanticKind: c.semanticKind, ownerId: c.ownerId, status: "value", value });
}
function scalar(result: ScientificResult): number {
  if (result.status !== "value" || typeof result.value !== "number") throw new RangeError("The requested model quantity has no representable scalar value.");
  return result.value;
}
/** All model numbers and sampled curves are assembled here, never in the React view. */
export async function evaluateBm06(input: unknown, control: EvaluationControl = {}): Promise<Computation<Bm06Evaluation>> {
  ...
  if (!Number.isSafeInteger(chunkSteps) || chunkSteps < 1 || chunkSteps > 1024) return refused(["chunkSteps"], "Use 1–1024 scientific steps per chunk.");
  const cancelled = control.cancelled ?? (() => false);
  const yieldControl = control.yieldControl ?? (() => new Promise<void>(resolve => setTimeout(resolve, 0)));
  ...
  if (workUnits > BM06_BUDGET.workUnits || allocationBytes > BM06_BUDGET.allocationBytes) return {
    kind: "outcome", outcome: { outcome: "budget-exhausted", ...executionOutcomeRegistry["budget-exhausted"], requested: { workUnits, allocationBytes }, allowed: BM06_BUDGET },
  };
  try {
    ...
    const density = p.t === 0 ? gaussianPropagator(0, 0, D).result
      : value("probabilityDensity", Float64Array.from(xs, x => scalar(gaussianPropagator(x, p.t, D).result)));
    const times = new Float64Array([1, 10, 60]);
    const outputs: ScientificResult[] = [
      diffusivity, rms, moments(1, D, p.t).marginal.result, probability,
      value("positionCoordinate1d", xs), density, value("comparisonTimes", times),
      value("comparisonRms", Float64Array.from(times, t => scalar(rmsDisplacement(D, t).result))),
    ];
    let stepIndex = 0;
    if (p.gridEnabled) {
      ...
      if (initial.kind !== "accepted") {
        if (initial.kind === "refused" && initial.refusal.code === "ftcs-unstable") {
          const stepRepair = initial.refusal.rankedRepairs.find(r => r.action?.parameterId === "dt")?.action;
          if (stepRepair && typeof stepRepair.value === "number") {
            ...
            return { kind: "refused", refusal: { ...initial.refusal, rankedRepairs: [
              ...(Number.isSafeInteger(requiredSteps) && requiredSteps <= 4_000_000 ? [{ label: `Use ${requiredSteps} time steps for this elapsed time.`, action: { parameterId: "steps", value: requiredSteps } }] : []),
      ...
      if (cancelled()) return outcome("cancelled");
      const comparison = ftcsAnalyticComparison({ field, dx: p.dx, t: p.t, D, startCell: Math.floor(p.n / 2) });
      if (comparison.kind !== "accepted") return comparison;
      outputs.push(value("gridDensity", field), value("cellMasses", comparison.data.cellMasses),
        value("cellProbabilities", comparison.data.cellProbabilities), comparison.data.maxCellMassDifference,
        value("wallContact", Number(comparison.data.wallContact)), value("stabilityRatio", p.t === 0 ? 0 : initial.data.stabilityRatio),
        value("gridTimeStep", p.t === 0 ? 0 : dt));
    } else for (const quantityId of gridKeys) {
      const c = BM06_OUTPUTS[quantityId]!;
      outputs.push({ quantityId, unit: c.unit, semanticKind: c.semanticKind, ownerId: c.ownerId, status: "not-applicable", reason: "The optional numerical grid is switched off." });
    }
