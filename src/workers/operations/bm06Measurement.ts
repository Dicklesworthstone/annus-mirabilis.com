...
export function remeasureBm06(
  previous: Bm06Parameters,
  data: Bm06Evaluation,
  requested: Bm06Parameters,
): Computation<Bm06Evaluation> | null {
  if (
    Object.keys(previous).some(
      (k) =>
        k !== "lower" &&
        k !== "upper" &&
        previous[k as keyof Bm06Parameters] !== requested[k as keyof Bm06Parameters],
    )
  )
    return null;
  ...
  return {
    ...
    data: {
      ...data,
      outputs: data.outputs.map((o) => (o.quantityId === "intervalProbability" ? probability : o)),
    },
  };
}
