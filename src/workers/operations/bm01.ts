...
export function measureBm01(recording: TracerRecording, p: Bm01Parameters, reused: boolean): Computation<Bm01Evaluation> {
 ...
 try {
  ...
  const outputs:ScientificResult[]=[number("temperature",p.T),number("viscosity",p.eta),number("particleRadius",p.a),number("observationInterval",p.interval),number("boltzmannConstant",constantSet.entries.find(e=>e.quantityId==="boltzmannConstant")!.value),number("diffusionCoefficient",D),number("rmsDisplacement1d",sigma),number("modelSecondMoment",value(model.total.result)),number("modelMeanNorm",value(model.meanRadius.result)),number("modelRmsNorm",value(model.rmsRadius.result)),number("tracerPositions",positions)];
