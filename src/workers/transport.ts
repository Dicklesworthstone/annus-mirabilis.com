 ...
 * Source path: src/physics/transport.ts
 * Pinned commit: da11ff475902728fd8dd1d9db9f3af37c16ec8a5
 * License: MIT License (with OpenAI/Anthropic Rider)
 * Preserved license text: /LICENSE
 *
 * Modifications:
 * - Disabled shared-memory transport mode and SharedArrayBuffer pool paths (throws typed SharedMemoryDisabledError).
 * - Generalized PatentTransport to ExperimentTransport (retaining backwards compatibility alias).
 * - Preserved transferable buffer paths, copy fallback, BoundedBufferPool, and TransportWorkerSupervisor.
 */
...
export interface TransportTapeFrame {
  ...
  telemetry: UniversalPhysicsTelemetry;
}
 ...
 * Attempting to force 'shared-memory' throws SharedMemoryDisabledError.
 */
export function forceTransportMode(mode: TransportBufferMode | null): void {
  if (mode === "shared-memory") {
    throw new SharedMemoryDisabledError("Cannot force disabled shared-memory mode.");
  }
  forcedMode = mode;
}
 ...
 * SharedArrayBuffer is always disabled in this runtime.
 */
export function probeTransportCapabilities(injected?: {
...
export interface BodyPoseSample {
  ...
  readonly angularVelocity?: readonly [number, number, number] | undefined;
  readonly tick: number;
}
...
// ----------------------------------------------------------------------------
// 5. ExperimentTransport & TransportBus
// ----------------------------------------------------------------------------

export class ExperimentTransport {
  ...
  private pool?: BoundedBufferPool | undefined;

  constructor(experimentId: string, tickS = 1 / 60) {
...
export class TransportBus {
  private transports = new Map<string, ExperimentTransport>();

  get(experimentId: string): ExperimentTransport {
    let t = this.transports.get(experimentId);
    if (!t) {
      t = new ExperimentTransport(experimentId);
      this.transports.set(experimentId, t);
    }
    return t;
  }
}
