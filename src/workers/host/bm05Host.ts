import { decodeLabRequest, labHello, BM05_PROTOCOL, type LabRequest, type LabResponse, type LabHello } from "../protocol/bm05.ts";
...
export function createBm05Host(send:(message:LabResponse|LabHello)=>void,sourceDigest:string) {
 ...
 async function receive(input:unknown):Promise<void> {
  ...
  const p=message.token.parameters as Bm05Parameters;let result;
  try {
   const reused=cache!==null && cache.runId===message.token.runId && (Object.keys(BM05_CLASSES) as (keyof Bm05Parameters)[]).every(k=>BM05_CLASSES[k]!=="input" || Object.is(p[k],cache!.parameters[k]));
   const options={cancelled:()=>cancelled||stopped};
   const recording=reused?{kind:"accepted" as const,data:cache!.recording}:await createBm05Recording(p,options);
   if(recording.kind!=="accepted")result=recording;
   else {result=await measureBm05(recording.data,p,reused,options);if(result.kind==="accepted" && !cancelled && !stopped)cache={runId:message.token.runId,parameters:p,recording:recording.data};}
  ...
  active=null;if(!stopped)send({messageKind:"result",protocolVersion:BM05_PROTOCOL,sourceDigest,token:message.token,result});
 }
