import { decodeLabRequest, labHello, BM01_PROTOCOL, type LabRequest, type LabResponse, type LabHello } from "../protocol/bm01.ts";
...
export function createBm01Host(send: (message: LabResponse | LabHello) => void, sourceDigest: string) {
 let active: LabRequest | null=null, stopped=false, cancelled=false, instanceId: string | null=null;
 let cache: { runId: string; parameters: Bm01Parameters; recording: TracerRecording } | null=null;
 async function receive(input: unknown): Promise<void> {
  ...
  let result;
  try {
   const reused=cache!==null && cache.runId===message.token.runId && (Object.keys(BM01_CLASSES) as (keyof Bm01Parameters)[]).every(k=>BM01_CLASSES[k]!=="input" || Object.is(p[k],cache!.parameters[k]));
   const recording=reused ? {kind:"accepted" as const,data:cache!.recording} : await createBm01Recording(p,{cancelled:()=>cancelled||stopped});
   if(recording.kind!=="accepted")result=recording;
   else {
    result=measureBm01(recording.data,p,reused);
    if(result.kind==="accepted" && !cancelled && !stopped)cache={runId:message.token.runId,parameters:p,recording:recording.data};
   }
  ...
  active=null;if(!stopped)send({messageKind:"result",protocolVersion:BM01_PROTOCOL,sourceDigest,token:message.token,result});
 }
