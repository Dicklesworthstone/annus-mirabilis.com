...
import {
  ...
  type LabResponse,
  labHello,
} from "../protocol/bm08.ts";
export function createBm08Host(
  ...
  async function receive(input: unknown): Promise<void> {
    ...
    if (instanceId !== null && instanceId !== message.token.instanceId)
      throw new TypeError("A camera owner cannot serve another instance.");
      ...
      if (recording.kind !== "accepted") result = recording;
        ...
        if (frames.kind !== "accepted") result = frames;
          ...
          if (result.kind === "accepted") {
            const outputs = result.data.outputs.map((o) =>
              o.status === "value" && o.quantityId === "measurementDraws"
                ? { ...o, value: reobserved ? 0 : frames.data.measurementDraws }
                : o.status === "value" && o.quantityId === "reusedObservation"
                  ? { ...o, value: Number(reobserved) }
                  : o,
            );
            result = { kind: "accepted" as const, data: { ...result.data, outputs } };
            if (!cancelled && !stopped)
              cache = {
                runId: message.token.runId,
                parameters: p,
