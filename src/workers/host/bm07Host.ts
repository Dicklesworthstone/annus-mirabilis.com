import { decodeLabRequest, labHello, BM07_PROTOCOL, type LabRequest, type LabResponse, type LabHello } from "../protocol/bm07.ts";
...
export function createBm07Host(send:(message:LabResponse|LabHello)=>void,sourceDigest:string) {
 let active:LabRequest|null=null,stopped=false,cancelled=false,instanceId:string|null=null;
 let cache:{runId:string;parameters:Bm07Parameters;recording:InferenceRecording}|null=null;
 async function receive(input:unknown):Promise<void> {
  ...
  if(instanceId!==null && instanceId!==message.token.instanceId)throw new TypeError("A recording owner cannot serve another instance.");
  instanceId=message.token.instanceId;active=message;cancelled=false;
  const p=message.token.parameters as Bm07Parameters;let result;
  try {
   const reused=cache!==null && cache.runId===message.token.runId && (Object.keys(BM07_CLASSES) as (keyof Bm07Parameters)[]).every(k=>BM07_CLASSES[k]!=="input" || Object.is(p[k],cache!.parameters[k]));
   const options={cancelled:()=>cancelled||stopped};
   const recording=reused?{kind:"accepted" as const,data:cache!.recording}:await createBm07Recording(p,options);
   if(recording.kind!=="accepted")result=recording;
   else {result=await measureBm07(recording.data,p,reused,options);if(result.kind==="accepted" && !cancelled && !stopped)cache={runId:message.token.runId,parameters:p,recording:recording.data};}
  ...
  active=null;if(!stopped)send({messageKind:"result",protocolVersion:BM07_PROTOCOL,sourceDigest,token:message.token,result});
 }
