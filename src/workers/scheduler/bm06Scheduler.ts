import { BM06_PROTOCOL, decodeLabHello, decodeLabResponse, LabProtocolError, validateSourceDigest } from "../protocol/bm06.ts";
...
export function createBm06Scheduler(store: Store, factory: () => WorkerChannel, sourceDigest: string, report: (event: SchedulerEvent) => void = () => {}) {
  ...
  function dispatch(): void {
    ...
    try {
      channel.send({ messageKind: "request", protocolVersion: BM06_PROTOCOL, sourceDigest, token: active });
  ...
  function connect(): void {
    ...
    try {
      ...
      unsubscribe = channel.listen(message => {
        ...
        try {
          if (!ready) { decodeLabHello(message, sourceDigest); ready = true; dispatch(); return; }
          if (!active) return;
          const response = decodeLabResponse(message, active, sourceDigest);
          const token = active;
          let decision;
  ...
  return Object.freeze({
    request(token: RequestToken): void {
      ...
      if (active && !cancelSent && channel) {
        try { channel.send({ messageKind: "cancel", protocolVersion: BM06_PROTOCOL, instanceId: active.instanceId, actionIndex: active.actionIndex }); cancelSent = true; }
        catch { fail("transport-error"); }
      }
      dispatch();
    },
