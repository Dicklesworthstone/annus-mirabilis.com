...
export function createHostScheduler(
  ...
  let active: RequestToken | null = null,
    pending: RequestToken | null = null;
  let ready = false,
    disposed = false,
    cancelSent = false;
  let epoch = 0,
  ...
  function notify(kind: SchedulerEvent["kind"], token: RequestToken): void {
    try {
      report({ kind, actionIndex: token.actionIndex });
    } catch {
      /* Diagnostics cannot change publication. */
    }
  }
  function close(): void {
    epoch++;
        ...
        (message) => {
          ...
          try {
            ...
            if (response.result.kind === "accepted")
              decision = store.publish({ ...response.token, ...response.result.data, final: true });
            ...
            fail(error instanceof LabProtocolError ? error.code : "malformed-response");
          }
        },
        () => {
