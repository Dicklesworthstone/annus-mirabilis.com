...
export function inferenceValue(snapshot: AcceptedSnapshot, id: string, factor = 1): string {
  const value = result(snapshot, id),
    e = inferenceText;
  ...
  const reason =
    value.status === "underdetermined"
      ? value.compatibleFamily
      : "reason" in value
        ? value.reason
        : "No scalar value for this question.";
...
export function inferenceInterval(snapshot: AcceptedSnapshot, id: string, factor = 1): string {
  ...
  if (values.length !== 2)
    throw new TypeError("An inference interval requires exactly two endpoints.");
...
export function renderInferenceFamily(snapshot: AcceptedSnapshot, band = false): string {
  ...
  for (let i = 0; i < radii.length; i++) {
    if (
      ![radii.at(i), lower.at(i), numbers.at(i), upper.at(i)].every(
        (v) => Number.isFinite(v) && v > 0,
      ) ||
      lower.at(i) > numbers.at(i) ||
      numbers.at(i) > upper.at(i) ||
      (i > 0 && radii.at(i) <= radii.at(i - 1))
    )
  ...
  const points = (values: typeof numbers) =>
    Array.from({ length: radii.length }, (_, i) => `${x(radii.at(i))},${y(values.at(i))}`).join(
      " ",
    );
  const ticks = indices
    .map(
    ...
    .join("");
  const bounds = band
    ? `<polyline class="comparison-curve" stroke-dasharray="5 4" points="${points(lower)}"/><polyline class="comparison-curve" stroke-dasharray="5 4" points="${points(upper)}"/>`
    : "";
...
export function renderCameraMoments(snapshot: AcceptedSnapshot, showModel = true): string {
  const rows = [
    ["Increment variance", "sampleVariance", "expectedVariance", "sdVariance"],
    ["Adjacent-increment covariance", "sampleCovariance", "expectedCovariance", "sdCovariance"],
  ] as const;
...
export function renderCompatibleLine(snapshot: AcceptedSnapshot): string {
  const diffusion = array(snapshot, "familyDiffusion"),
    noise = array(snapshot, "familyNoise"),
    covariance = array(snapshot, "familyCovariance");
  if (diffusion.length !== noise.length || noise.length !== covariance.length)
    throw new TypeError("Incomplete camera family.");
  const maxD = scalar(snapshot, "maximumDiffusion"),
