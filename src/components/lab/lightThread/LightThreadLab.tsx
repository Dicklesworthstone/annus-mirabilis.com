...
function draftOf(p: LightThreadParameters): Record<keyof LightThreadParameters, string> {
  return {
    ...
    pulseEnergyJ: String(p.pulseEnergyJ),
    beta: String(p.beta),
    angleDeg: String(p.angleDeg),
  };
}
        ...
        <tbody>
          {ids.map((id) => (
            <tr key={id}>
              ...
              <td>
                <Reading snapshot={snapshot} id={id} />
              </td>
...
export function LightThreadLab() {
  ...
  useEffect(() => {
    function restore() {
      ...
      const state = query
        ? decodeLightThreadParameters(query)
        : { kind: "accepted" as const, parameters: LIGHT_THREAD_DEFAULTS };
