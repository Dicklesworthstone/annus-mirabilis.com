      ...
      <figure>
        <svg viewBox="0 0 640 300" role="img" aria-labelledby={`${id}-plot-title ${id}-plot-desc`}>
          ...
          <desc id={`${id}-plot-desc`}>
            Positions {view.start} through {view.end} of {view.total}. Time spans{" "}
            {number(view.timeRange[0])} to {number(view.timeRange[1])} seconds; position spans{" "}
            {number(view.coordinateRange[0])} to {number(view.coordinateRange[1])} metres. Dots are
            recorded samples, with no inferred connecting path. The same samples appear in the
            table.
          </desc>
          ...
          <g fill="currentColor" fontSize="12">
            <text x="85" y="270">
              {number(view.timeRange[0])}
            </text>
            <text x="610" y="270" textAnchor="end">
              {number(view.timeRange[1])}
            </text>
            <text x="350" y="292" textAnchor="middle">
              Time (s)
            </text>
            <text x="80" y="30" textAnchor="end">
              {number(view.coordinateRange[1])}
            </text>
            <text x="80" y="250" textAnchor="end">
              {number(view.coordinateRange[0])}
            </text>
            <text x="12" y="145" transform="rotate(-90 12 145)" textAnchor="middle">
