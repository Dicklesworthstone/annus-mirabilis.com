...
import { kitchenNumber, type KitchenPoint } from "../../../experiments/bm07/kitchen/schema.ts";
import { appendVideoPoint, calibrateVideoAxis, captureRefusal, sourcePoint, startCapture,
  videoCaptureJson, videoDocument, type CaptureFailure, type Mark, type VideoCapture } from "../../../experiments/bm07/kitchen/videoCapture.ts";
import { createVideoFrameReader, type AcquiredVideoFrame } from "../../../experiments/bm07/kitchen/videoFrames.ts";
import "./videoTracker.css";

/** Acquisition only. The existing worker owns all diffusion/noise inference. */
export function VideoTracker({ disabled = false, onAnalyze }: {
  disabled?: boolean;
  onAnalyze: (csv: string) => void | Promise<void>;
}) {
  const id = useId(), video = useRef<HTMLVideoElement>(null), canvas = useRef<HTMLCanvasElement>(null);
  const reader = useRef<ReturnType<typeof createVideoFrameReader> | null>(null), generation = useRef(0);
  const downloads = useRef(new Set<string>()), fileInput = useRef<HTMLInputElement>(null);
  const [ready, setReady] = useState(false), [file, setFile] = useState<File | null>(null);
  const [capture, setCapture] = useState<VideoCapture | null>(null), [frame, setFrame] = useState<AcquiredVideoFrame | null>(null);
  const [busy, setBusy] = useState(false), [released, setReleased] = useState(false), [discard, setDiscard] = useState(false);
  const [failure, setFailure] = useState<CaptureFailure | null>(null), [notice, setNotice] = useState("");
  const [fps, setFps] = useState(""), [interval, setInterval] = useState("1"), [target, setTarget] = useState("0");
  const [mark, setMark] = useState<Mark>("x-a"), [x, setX] = useState("0"), [y, setY] = useState("0");
  const [distance, setDistance] = useState(""), [exposure, setExposure] = useState(""), [aspect, setAspect] = useState("");
  const [particle, setParticle] = useState("particle-1"), [identity, setIdentity] = useState<KitchenPoint["identityDecision"]>("");
  ...
  const pointReady = !locked && !released && !!frame && !!capture &&
    !(failure?.kind === "outcome" && failure.outcome.outcome === "context-lost");
  const counts = capture?.points.length ?? 0;
  function report(error: unknown) {
    setFailure(captureRefusal(error instanceof Error ? error.message : "The local operation could not finish. Existing observations are unchanged.", "capture"));
  }
  function release() {
    generation.current++; reader.current?.close(); reader.current = null;
    setBusy(false); setReleased(true); setNotice("The video resource was released. The captured annotations remain available for export and analysis.");
  }
  async function open() {
    if (locked || capture || !file || !video.current || !canvas.current) return;
    setFailure(null); setNotice("");
    let rate: number;
    try { rate = kitchenNumber(fps, "confirmed frame rate"); }
    catch (error) { report(error); return; }
    if (rate < 0.1 || rate > 10000) { setFailure(captureRefusal("Confirm a nominal rate from 0.1 to 10000 Hz.", "fps")); return; }
    ...
    try {
      ...
      if (result.kind !== "ready") { setFailure(result); reader.current = null; return; }
      const model = startCapture(result.value.geometry, rate, interval);
      if (model.kind !== "ready") { setFailure(model); local.close(); reader.current = null; return; }
      setCapture(model.value); setFrame(result.value); setX("0"); setY("0");
      setNotice("Video opened locally. Calibrate a measured axis before tracking; repeat clicks on one stationary feature to characterize click scatter.");
    } catch (error) { if (generation.current === ticket) { local.close(); reader.current = null; report(error); } }
  ...
  async function read(time: number) {
    ...
    try {
      ...
      if (generation.current !== ticket) return;
      if (result.kind === "ready") { setFrame(result.value); setTarget(String(time)); setNotice("Frame accepted. Coordinates will use its actual captured time, not the requested time."); }
      else setFailure(result);
    } catch (error) { if (generation.current === ticket) report(error); }
    finally { if (generation.current === ticket) setBusy(false); }
  }
  function record(lost = false) {
    if (!pointReady || !capture || !frame) return;
    if (mark === "particle" && !calibrated) { setFailure(captureRefusal("Calibrate at least one axis before recording particle positions.", "calibration")); return; }
    try {
      const result = appendVideoPoint(capture, frame.frame, mark, lost ? 0 : kitchenNumber(x, "x coordinate"),
        lost ? 0 : kitchenNumber(y, "y coordinate"), particle, lost ? loss : "", mark === "particle" && !lost ? identity : "");
      if (result.kind !== "ready") { setFailure(result); return; }
      setCapture(result.value); setIdentity(""); setFailure(null); setNotice(`Recorded observation ${result.value.points.length} at ${frame.frame.time} seconds. No position was interpolated.`);
  ...
  function calibrate(axis: "x" | "y") {
    ...
    try {
      ...
      else { setCapture(result.value); setFailure(null); setNotice(`Axis ${axis} calibrated from repeated mark clicks. This calibration is fixed once tracking begins.`); }
    } catch (error) { report(error); }
  }
  function key(event: KeyboardEvent<HTMLCanvasElement>) {
    if (!pointReady || !capture || !["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(event.key)) return;
    event.preventDefault();
    const step = event.shiftKey ? 10 : 1;
    const nx = Number(x) + (event.key === "ArrowLeft" ? -step : event.key === "ArrowRight" ? step : 0);
  ...
  function csv() {
    ...
    if (result.kind !== "ready") { setFailure(result); return null; }
  ...
  function download(kind: "csv" | "json") {
    ...
    try {
      ...
      const link = document.createElement("a"); link.href = url; link.download = `video-annotations.${kind}`; link.click();
  ...
  async function analyze() {
    try {
      const text = csv();
      if (text) { await onAnalyze(text); setNotice("Observations sent to the local analyzer below. Check its accepted-result or refusal message. Later capture edits do not change that submitted snapshot."); }
    } catch (error) { report(error); }
  }
  function newSession() {
    if (!discard || locked) return;
    release(); setCapture(null); setFrame(null); setFile(null); setDiscard(false); setReleased(false);
    setExposure(""); setAspect(""); setDistance(""); setTarget("0"); setMark("x-a"); setFailure(null); setNotice("");
    if (fileInput.current) fileInput.current.value = "";
    if (canvas.current) { canvas.current.width = 1; canvas.current.height = 1; }
  }
  return <section className="video-tracker" id={`${id}-local-video`} aria-labelledby={`${id}-title`} data-local-video>
    <h3 id={`${id}-title`}>Track a particle in a local video</h3>
    <p>No video or frame pixels are uploaded or included in exports. Nothing is saved automatically. Keep the original recording and download annotations before leaving this page.</p>
    <p className="fine">Limits: 256 MiB, 10 minutes, 4096 pixels per source edge, a 1920-pixel working canvas, 2000 requested frame reads, and 2 seconds per read. Internal decoder work is not observable or counted. Unsupported videos can still be measured elsewhere and imported as CSV.</p>
    ...
    <video ref={video} muted playsInline preload="none" className="video-decoder" aria-label="Local video decoder view" />
    <div className="video-frame-wrap">
      <canvas ref={canvas} width={1} height={1} tabIndex={pointReady ? 0 : -1} role="img" aria-label="Captured video frame. Choose coordinates by pointer or arrow keys, then use Record point. Typed coordinates are also available."
        onKeyDown={key} onClick={event => {
          if (!pointReady || !capture) return;
          const rect = event.currentTarget.getBoundingClientRect();
          const point = sourcePoint(capture.geometry, event.clientX - rect.left, event.clientY - rect.top, rect.width, rect.height);
          if (point.kind === "ready") { setX(String(point.value.x)); setY(String(point.value.y)); }
          else setFailure(point);
        }} />
      {capture && frame && Number.isFinite(Number(x)) && Number.isFinite(Number(y)) && <svg viewBox={`0 0 ${capture.geometry.width} ${capture.geometry.height}`} aria-hidden="true">
        <path d={`M ${Number(x)-8} ${y} h 16 M ${x} ${Number(y)-8} v 16`} stroke="white" strokeWidth="4" />
        <path d={`M ${Number(x)-8} ${y} h 16 M ${x} ${Number(y)-8} v 16`} stroke="black" strokeWidth="2" />
      </svg>}
    </div>
    {frame && <p data-captured-frame>Frame {frame.frame.stamp.frameId}: actual time {frame.frame.time} s; requested {frame.frame.stamp.requestedTime} s. Timing: {frame.frame.stamp.timingSource}. Presentation counter: {frame.frame.stamp.presentedFrames ?? "unavailable — inferred timing"}. {frame.reads} frame reads.</p>}
    ...
    <fieldset disabled={locked || !capture}>
      ...
      <div className="actions"><button type="button" disabled={!counts} onClick={() => download("json")}>Download raw annotations JSON</button>
        ...
        <button type="button" disabled={!tracking || !calibrated} onClick={() => void analyze()}>Analyze captured observations</button></div>
      <p className="fine">The raw JSON is an annotation recovery record, not a video file or a supported session-replay import. The CSV is the supported analysis interchange.</p>
    </fieldset>
    <p role="status" aria-live="polite" data-video-status>{busy ? "Reading a local frame. The previous image and annotations are retained; new clicks are disabled." : notice}</p>
    {failure && <p role="alert" data-video-failure={failure.kind === "refused" ? failure.refusal.code : failure.outcome.outcome}>{failure.message}</p>}
    {capture && <details><summary>{counts} observations (last 30 shown; downloads contain all)</summary><div className="video-observation-scroll"><table><thead><tr><th>Kind / label</th><th>Actual time</th><th>Requested time</th><th>X</th><th>Y</th><th>Status / identity</th></tr></thead><tbody>
      {capture.points.slice(-30).map((p,i) => <tr key={counts-30+i}><td>{p.kind} / {p.objectId}</td><td>{p.time}</td><td>{p.capture?.requestedTime}</td><td>{p.x ?? "—"}</td><td>{p.y ?? "—"}</td><td>{p.status} {p.lossReason} {p.identityDecision}</td></tr>)}
    </tbody></table></div></details>}
    {capture && <fieldset disabled={locked}><legend>Start a separate recording</legend><label><input type="checkbox" checked={discard} onChange={e => setDiscard(e.target.checked)} />I have exported these annotations or choose to discard this capture session.</label><button type="button" disabled={!discard} onClick={newSession}>Start new capture session</button><p>Previously accepted analyzer results below are independent and are not cleared by starting a new capture.</p></fieldset>}
  </section>;
}
