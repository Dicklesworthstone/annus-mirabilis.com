 ...
 * Source path: src/components/ui/ColorizedEquation.tsx
 * Pinned commit: da11ff475902728fd8dd1d9db9f3af37c16ec8a5
 * License: MIT License (with OpenAI/Anthropic Rider)
 * Preserved license text: /LICENSE
 *
 * Modifications:
 * - Marked as legacy (@deprecated); prohibited from being imported in src/app, src/reader, or content routes.
 * - Decoupled from patent physics hook; telemetry can be passed via optional prop or defaults to empty.
 * - Retained keyboard navigation, color-blind mode, term highlighting, and token lookup seams.
 */
...
import { COLOR_STYLES, type ColorVariant, prepareInteractiveLatex } from "../colorPalette.ts";
import { LatexRenderer, TextWithLatex } from "../render/LatexRenderer.tsx";
import { formatEquationTelemetryValue } from "../valueFormatting.ts";
import type {
  ColorizedEquation as ColorizedEquationModel,
  EquationVariable,
  SentenceFragment,
} from "./equationTypes.ts";
...
function Maximize2Icon({ className = "w-3.5 h-3.5" }: { readonly className?: string }) {
  return (
    <svg
      className={className}
      fill="none"
...
function Minimize2Icon({ className = "w-3.5 h-3.5" }: { readonly className?: string }) {
  return (
    <svg
      className={className}
      fill="none"
...
function RotateCcwIcon({ className = "w-3.5 h-3.5" }: { readonly className?: string }) {
  return (
    <svg
      className={className}
      fill="none"
...
function SparklesIcon({ className = "w-3.5 h-3.5" }: { readonly className?: string }) {
  return (
    <svg
      className={className}
      fill="none"
...
function ZapIcon({ className = "w-3 h-3" }: { readonly className?: string }) {
  return (
    <svg
      className={className}
      fill="none"
...
export interface PhysicsTelemetryItem {
  readonly label: string;
  readonly value: number | string;
  readonly unit: string;
}

export interface ColorizedEquationProps {
  readonly equation: ColorizedEquationModel;
  readonly initialActiveVariableId?: string;
  readonly defaultExpanded?: boolean;
  readonly className?: string;
  readonly showLiveTelemetry?: boolean;
  readonly telemetry?: {
    readonly params?: Readonly<Record<string, number>>;
    readonly metrics?: readonly PhysicsTelemetryItem[];
  };
}
...
function liveTelemetryValueFor(
  ...
  metrics: readonly PhysicsTelemetryItem[] | undefined,
): string | null {
  if (!activeVar) return null;

  const telemetryKey = activeVar.telemetryKey;
  if (telemetryKey && params && params[telemetryKey] !== undefined) {
  ...
  if (!metricLabel || !metrics) return null;
...
function formulaVariableIdForKey(
  ...
  if (key === "Home") return variables[0]?.id ?? null;
  if (key === "End") return variables[variables.length - 1]?.id ?? null;
  ...
  return variables[nextIndex]?.id ?? null;
}
...
function synchronizeActiveFormulaTerms(
  ...
  for (let i = 0; i < activeElements.length; i++) {
    ...
    if (el) el.classList.remove("eq-term-active");
  }
  ...
  for (let i = 0; i < targets.length; i++) {
    const el = targets[i];
    if (el) el.classList.add("eq-term-active");
  }
}
...
interface EquationHeaderProps {
  ...
  readonly onToggleColorBlindMode: () => void;
...
interface FormulaVariableChipProps {
  readonly variable: EquationVariable;
  readonly isSelected: boolean;
  readonly onSelectVariable: SelectVariable;
}
...
interface EquationFormulaSurfaceProps {
  readonly equation: ColorizedEquationModel;
  readonly activeVarId: string | null;
  readonly onSelectVariable: SelectVariable;
  readonly onFormulaMouseLeave: () => void;
...
interface PlainEnglishFragmentProps {
  readonly fragment: SentenceFragment;
    ...
    <div className="rounded-2xl p-4 sm:p-5 bg-parchment-100/50 dark:bg-ink-900/50 border border-parchment-300 dark:border-ink-800 space-y-2">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[10px] font-mono uppercase tracking-widest text-amber-800 dark:text-amber-400 font-bold flex items-center gap-1.5">
          <ZapIcon className="w-3 h-3 text-amber-600" />
...
interface ActiveVariableInspectorProps {
  readonly activeVar: EquationVariable | undefined;
  readonly showLiveTelemetry: boolean;
  readonly liveTelemetryValue: string | null;
}
      ...
      {showLiveTelemetry && liveTelemetryValue !== null && (
        <div className="flex items-center justify-between pt-2 border-t border-black/10 dark:border-white/10 text-xs font-mono">
          <div className="flex items-center gap-1.5 text-amber-900 dark:text-amber-300 font-bold">
            <ActivityIcon className="w-3.5 h-3.5 motion-safe:animate-pulse text-amber-600" />
...
interface PedagogicalInsightProps {
  ...
  readonly historicalSignificance?: string | undefined;
}
...
function PedagogicalInsight({
  ...
  return (
    <div className="rounded-xl border border-amber-200/80 dark:border-amber-900/40 bg-amber-50/50 dark:bg-amber-950/20 p-3.5 sm:p-4 text-xs font-sans text-ink-800 dark:text-parchment-200 leading-relaxed flex items-start gap-2.5">
      <InfoIcon className="w-4 h-4 text-amber-700 dark:text-amber-400 shrink-0 mt-0.5" />
      <div>
        <span className="font-serif font-bold text-ink-950 dark:text-parchment-100 block mb-0.5">
          Physical Principle &amp; Insight
        </span>
        <p>
...
/**
 * @deprecated Legacy component from donor repository.
 * Prohibited from being imported in src/app, src/reader, or content routes.
 * Enforced by src/testing/legacyImportBoundary.test.ts.
 */
export function ColorizedEquation({
  ...
  const liveTelemetryValue = useMemo(
    () => liveTelemetryValueFor(activeVar, telemetry?.params, telemetry?.metrics),
    [activeVar, telemetry?.params, telemetry?.metrics],
  );
  ...
  return (
    <div
      ...
      data-entity-id={equation.entityId}
