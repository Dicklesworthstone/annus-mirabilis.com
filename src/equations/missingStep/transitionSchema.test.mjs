...
test("the authored chain exposes four transitions with one marked move", () => {
  ...
  assert.equal(data.transitions.length, 4);
  assert.equal(data.chain.steps.filter((s) => s.isMove).length, 1);
  assert.equal(data.review, "draft");
  assert.ok(Object.isFrozen(data));
});
for (const [name, change, code] of [
  [
    "unknown highlight",
    (d) => (d.transitions[2].changedSubexpressionIds = ["notThere"]),
    "subexpression",
  ],
  [
    "highlight disappears in destination",
    (d) => {
      d.chain.steps[2].to.args[1].opId = "gone";
      d.chain.steps[3].from.args[1].opId = "gone";
    },
    "subexpression",
  ],
  [
    "unchanged highlighted subtree",
    (d) => {
      d.transitions[0].changedSubexpressionIds = ["A"];
      d.chain.steps[0].changedSubexpressionIds = ["A"];
    },
    "subexpression",
  ],
  ["chain gap", (d) => (d.chain.steps[1].from.opId = "different"), "continuity"],
  [
    "mismatched local symbol binding",
    (d) => (d.chain.steps[0].from.argument.base.args[0].quantityId = "stepB"),
    "expression",
  ],
  ...
  [
    "invented source anchor",
    (d) => (d.sourceLink = "/papers/brownian-motion/s4/#s4-p999"),
    "source",
  ],
  ...
  [
    "later dynamics in premise",
    (d) => (d.premiseNotice += " momentum relaxation time"),
    "anachronism",
  ],
  [
    "hostile symbol",
    (d) => (d.chain.steps[0].from.argument.base.args[0].termId = "<script>"),
    "schema",
  ],
])
  test(name, () => {
    ...
    change(d);
    assert.throws(() => parse(d), new RegExp(`missing-step-${code}`));
  });
test("expansion policy requires evidence and cannot relabel another initial case", () => {
  assert.throws(
    () =>
      parseMissingStepAllowlist({
        schemaVersion: 1,
        chains: [{ chainId: "another", initialCase: true, evidenceIds: [] }],
      }),
    /expansion-evidence/,
  );
  assert.deepEqual(
    parseMissingStepAllowlist({
      schemaVersion: 1,
      chains: [{ chainId: "another", evidenceIds: ["review-record-1"] }],
    }),
    ["another"],
  );
});
test("two-step table enumerates rather than typing totals", () => {
  ...
  assert.equal(e.rows.length, 4);
  assert.equal(e.mean, 0);
  assert.equal(e.meanSquare, 2);
  assert.equal(e.meanCrossTerm, 0);
  assert.equal(e.meanAbsolute, 1);
...
test("three and eight steps generalize the finite worked case", () => {
  for (const n of [3, 8]) {
    const e = enumerateSignedSteps(n);
    assert.equal(e.rows.length, 2 ** n);
    assert.equal(e.meanSquare, n);
    assert.equal(e.meanCrossTerm, 0);
  }
});
test("zero means alone do not eliminate cross terms", () => {
  const same = enumerateSignedSteps(2, "same-direction"),
    opposite = enumerateSignedSteps(2, "opposite-direction");
  assert.equal(same.mean, 0);
  assert.equal(same.meanSquare, 4);
  assert.equal(same.meanCrossTerm, 2);
  assert.equal(opposite.mean, 0);
  assert.equal(opposite.meanSquare, 0);
  assert.equal(opposite.meanCrossTerm, -2);
});
test("unbounded enumerations and unregistered alternatives are refused", () => {
  ...
  assert.throws(() => enumerateSignedSteps(3, "same-direction"));
  assert.throws(() => enumerateSignedSteps(2, "magic"));
});

test("finite worked check rejects a planted incorrect coefficient, never a proof certificate", async () => {
  const { checkWorkedTransitions } = await import("./workedCheck.ts");
  checkWorkedTransitions(parse(input));
  const d = structuredClone(input);
  d.chain.steps[0].to.argument.args[1].args[0].value = "3";
  d.chain.steps[1].from.argument.args[1].args[0].value = "3";
