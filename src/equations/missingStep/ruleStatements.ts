...
export function missingStepRuleStatement(rule: RuleApplication): string {
  if (rule.kind === "registered-identity") {
    if (rule.params.identityId === "linearity-of-average")
      return "Linearity of averaging: average the sum by adding the averages. Independence is not required for this move.";
    if (rule.params.identityId === "independent-zero-mean-product-vanishes")
      return "For independent, zero-mean quantities, the mean of their product is the product of their means, hence zero.";
  }
