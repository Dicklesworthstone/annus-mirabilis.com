...
import { type ExactScale, exactScale, record } from "../ast.ts";
import { checkDimensions } from "../dimensions.ts";
import { type EquationRecord, parseEquationRecord } from "../record.ts";
import { type TeachingPaper, teachingProfile } from "../teachingProfiles.ts";
import { equalityResidual, equalPolynomials, sumPolynomials } from "./exactPolynomial.ts";
 ...
 */
export function checkLinearCertificate(
  input: unknown,
  source: readonly EquationRecord[],
): CheckedLinearCertificate {
  ...
  for (const candidate of raw.premises) {
    ...
    id(p.id);
    list(p.equations, 8);
    if (premiseIds.has(p.id) || !["assumption", "definition"].includes(String(p.kind)))
      fail("Invalid or duplicate premise.");
    premiseIds.add(p.id);
    const names = p.equations.map((key) => {
      ...
      if (
        eq.tree.kind !== "relation" ||
        eq.tree.operator !== (p.kind === "definition" ? "define" : "=")
      )
  ...
  for (const candidate of raw.steps) {
    ...
    id(step.id);
    id(step.equation);
    list(step.combination, 16);
    if (stepIds.has(step.id) || premiseIds.has(step.id)) fail("Duplicate step identity.");
    stepIds.add(step.id);
    if (available.has(step.equation))
      fail("A conclusion cannot already be an assumed or derived equation.");
    const target = equation(step.equation);
    if (target.tree.kind !== "relation" || target.tree.operator !== "=")
      fail("Conclusions must be exact equalities.");
    ...
    const combination = step.combination.map((term) => {
      ...
      if (!requirements || used.has(item.equation))
        fail("Unknown, future, self-referential, or repeated justification.");
      used.add(item.equation);
      const eq = equation(item.equation);
      const dimension = checkDimensions(eq.tree, profile.quantities);
      if (
        dimension.status !== "consistent" ||
        !sameDimension(dimension.dimension, targetDimension.dimension)
      )
    ...
    const derived = sumPolynomials(
      combination.map((term) => ({
    ...
    if (!equalPolynomials(derived, equalityResidual(target.tree)))
      fail(`The combination does not establish ${target.id}.`);
  ...
  return freeze({
    id: raw.id,
    paper: raw.paper as TeachingPaper,
    premises,
    steps,
    check: "exact-polynomial-elimination" as const,
    requirements,
  });
}
