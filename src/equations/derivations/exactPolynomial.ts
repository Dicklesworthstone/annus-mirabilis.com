import { add, divide, multiply, rational, type Rational } from "../../content/dimensions/rational.ts";
import { record, type Expression } from "../ast.ts";
...
export class PolynomialRefusal extends Error {
  constructor(message: string) { super(message); this.name = "PolynomialRefusal"; }
}
function refuse(message: string): never { throw new PolynomialRefusal(message); }
...
function put(result: Map<string, Rational>, key: string, value: Rational): void {
  ...
  if (next.num === 0n) result.delete(key); else result.set(key, next);
...
function literal(text: unknown): Rational {
  ...
  return exponent >= 0 ? rational(numerator * 10n ** BigInt(exponent))
...
export function sumPolynomials(terms: readonly Readonly<{ polynomial: Polynomial; coefficient: Rational }>[]): Polynomial {
  ...
  for (const term of terms) {
    ...
    for (const [key, value] of term.polynomial) put(result, key, bounded(multiply(value, term.coefficient)));
  }
  return result;
}
export function equalPolynomials(left: Polynomial, right: Polynomial): boolean {
  return left.size === right.size && [...left].every(([key, a]) => {
...
export function exactPolynomial(expression: Expression): Polynomial {
  ...
  function visit(raw: Expression, depth: number): Polynomial {
    ...
    const kind = raw && typeof raw === "object" ? Object.getOwnPropertyDescriptor(raw, "kind")?.value : null;
    switch (kind) {
      case "number": {
      ...
      case "symbol": {
        ...
        if (typeof value.quantityId !== "string" || !/^[a-z][A-Za-z0-9-]{0,99}$/.test(value.quantityId))
        ...
        return scale.num === 0n ? new Map() : new Map([[JSON.stringify([value.quantityId]), scale]]);
      }
      case "group": case "negate": {
        const value = record(raw, "polynomial.unary", ["kind", "argument"], ["opId"]);
        return sumPolynomials([{ polynomial: visit(value.argument as Expression, depth + 1),
      ...
      case "sum": case "product": {
        ...
        if (kind === "sum") return sumPolynomials(operands.map(polynomial => ({ polynomial, coefficient: rational(1n) })));
      ...
      case "power": {
        ...
        if (power.den !== 1 || !Number.isSafeInteger(power.num) || Number(power.num) < 0 || Number(power.num) > 16)
      ...
      case "quotient": {
        const value = record(raw, "polynomial.quotient", ["kind", "numerator", "denominator"], ["opId"]);
        ...
        return sumPolynomials([{ polynomial: numerator, coefficient: divide(rational(1n), constant) }]);
      }
      default: return refuse("This operation is outside exact polynomial algebra; no proof was inferred.");
    }
  }
