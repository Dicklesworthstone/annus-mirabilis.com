...
export function assessLowSpeed(proof: Dependencies, selection: LowSpeedSelection) {
  ...
  if (![2,4,6].includes(selection.order) || new Set(selection.selected).size !== selection.selected.length ||
...
export function decodeLowSpeedSetup(proof: Dependencies, search: string):
  { kind: "absent" } | { kind: "invalid"; message: string } | {kind:"setup"; selection:LowSpeedSelection} {
  const invalid = () => ({kind:"invalid" as const,message:"This low-speed link is unsupported or ambiguous. The complete all-premise explanation remains selected."});
  if (search.length > 8192) return invalid();
  const query = new URLSearchParams(search);
  if (!["mel","mel-order","mel-off"].some(key => query.has(key))) return {kind:"absent"};
  if (query.getAll("mel").length !== 1 || query.get("mel") !== "1" ||
      query.getAll("mel-order").length > 1 || query.getAll("mel-off").length > 1) return invalid();
  const order = query.get("mel-order") ?? "2";
  if (!["2","4","6"].includes(order)) return invalid();
  const raw = query.get("mel-off"), omitted = raw === null ? [] : raw.split(",");
  const all = proof.premises.map(p => p.id);
  if (new Set(omitted).size !== omitted.length || omitted.some(id => !all.includes(id))) return invalid();
  return {kind:"setup",selection:{selected:all.filter(id => !omitted.includes(id)),order:Number(order) as LowSpeedOrder}};
}
