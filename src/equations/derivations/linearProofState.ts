...
export function assessLinearCertificate(
  proof: CheckedLinearCertificate,
...
export function proofSetupHref(
  proof: CheckedLinearCertificate,
  selected: readonly string[],
): string {
...
export function decodeProofSetup(
  proof: CheckedLinearCertificate,
  search: string,
):
  | { kind: "absent" }
  | { kind: "invalid"; message: string }
  | { kind: "setup"; selected: readonly string[] } {
  const invalid = () => ({
    kind: "invalid" as const,
    message:
      "This derivation link is unsupported or ambiguous. All stated premises remain selected.",
  });
  if (search.length > 8192) return invalid();
  const query = new URLSearchParams(search);
  if (!query.has("mep") && !query.has("mep-off")) return { kind: "absent" };
  if (
    query.getAll("mep").length !== 1 ||
    query.get("mep") !== "1" ||
    query.getAll("mep-off").length > 1
  )
    return invalid();
