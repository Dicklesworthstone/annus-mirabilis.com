...
function formula(latex: string, spoken: string): LowSpeedFormula {
  ...
  return {
    latex,
    spoken,
    html: renderToString(latex, { ...options, output: "html" }),
...
export function renderLowSpeedProof(
  checked: LowSpeedCertificate,
  equations: readonly CompiledEquation[],
): Omit<LowSpeedProofView, "sourceDigest"> {
  ...
  return {
    certificate,
    equations: used,
    approximations: ([2, 4, 6] as const).map((order) => {
      const terms = certificate.coefficients
        .slice(0, order + 1)
        .flatMap((coefficient, k) =>
          coefficient === "0" ? [] : [`${fraction(coefficient)}${k ? `\\beta^{${k}}` : ""}`],
        );
      return {
        order,
        omittedPower: order + 2,
        omittedCoefficient: certificate.coefficients[order + 2]!,
        formula: formula(
          `\\gamma-1\\approx ${terms.join("+")}`,
          `Keep powers through ${order} in the dimensionless speed ratio beta, which means v divided by c. This is an approximation, not a finite-speed equality.`,
        ),
      };
    }),
    ...
    limit: formula(
      String.raw`\lim_{\beta\to0}\frac{2(\gamma-1)}{\beta^2}=` + fraction(certificate.limit.limit),
