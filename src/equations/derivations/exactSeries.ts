import {
  add, dimension, divide, formatRational, isDimensionless, multiply, parseRational, sameDimension,
  type Rational, rational, subtract,
...
export class SeriesRefusal extends Error {
  constructor(message: string) { super(message); this.name = "SeriesRefusal"; }
}
const fail = (message: string): never => { throw new SeriesRefusal(message); };
...
export function exactSeriesAtZero(request: SeriesRequest): TaylorJet {
  ...
  if (!variable || variable.numerator === variable.denominator ||
      !Object.hasOwn(registry, variable.numerator) || !Object.hasOwn(registry, variable.denominator))
    fail("Declare a ratio of two distinct registered quantities.");
  if (!sameDimension(dimension(registry[variable.numerator]!.dimension),
  ...
  function bounded(value: Rational): Rational {
    if (++work > SERIES_LIMITS.work || value.num.toString(2).length > SERIES_LIMITS.bits ||
  ...
  const scalar = (a: Rational): Rational[] => Array.from({ length: order + 1 }, (_, k) => k ? ZERO : a);
  ...
  function power(base: readonly Rational[], exponent: Rational): Rational[] {
    ...
    const u = [...base]; u[0] = ZERO;
    let term = scalar(ONE), value = scalar(ONE), binomial = ONE;
    for (let k = 1; k <= order; k++) {
      term = product(term, u);
      binomial = times(binomial, over(subtract(exponent, rational(BigInt(k - 1))), rational(BigInt(k))));
      value = sum(value, term.map(coefficient => times(coefficient, binomial)));
    }
    return value;
  }
  function visit(node: Expression): Rational[] {
    ...
    if (node.kind === "quotient" && node.numerator.kind === "symbol" &&
        node.denominator.kind === "symbol" && unitScale(node.numerator) && unitScale(node.denominator) &&
        node.numerator.quantityId === variable.numerator && node.denominator.quantityId === variable.denominator) {
      const value = scalar(ZERO); if (order >= 1) value[1] = ONE; return value;
    }
    switch (node.kind) {
      case "number": return scalar(exactPolynomial(node).get("[]") ?? ZERO);
      case "group": return visit(node.argument);
      case "negate": return visit(node.argument).map(value => times(rational(-1n), value));
      case "sum": return node.args.map(visit).reduce(sum, scalar(ZERO));
      case "product": return node.args.map(visit).reduce(product, scalar(ONE));
      case "quotient": return quotient(visit(node.numerator), visit(node.denominator));
      case "root": return power(visit(node.radicand), rational(1n, BigInt(node.degree)));
      case "power": return power(visit(node.base), rational(BigInt(node.exponent.num), BigInt(node.exponent.den)));
      default: return fail("Unsupported operation or unbound quantity; no local series was inferred.");
    }
  }
  const coefficients = visit(expression).map(formatRational);
  return Object.freeze({ order, coefficients: Object.freeze(coefficients),
    variable: Object.freeze({ ...variable }), regularity: "real-analytic-near-zero",
    scope: "local-series-not-finite-error-bound" });
}
...
export function monomialQuotientLimit(request: SeriesRequest, power: number) {
  ...
  return Object.freeze({ status: "analytic-limit" as const, limit: jet.coefficients[power]!,
    quotientAtZero: "not-applicable" as const, removedPower: power,
    order: jet.order - power, coefficients: Object.freeze(jet.coefficients.slice(power)),
    variable: jet.variable, scope: jet.scope });
}
