...
import { handleStepKeyDown, parseDerivationStepParam } from "./stepFocus.ts";

describe("am-eq-derivation-renderer-9gd7: stepFocus and keyboard navigation", () => {
  test("ArrowDown moves focus to next step", () => {
    let focusedIndex = 0;
    const handled = handleStepKeyDown({ key: "ArrowDown", preventDefault: () => {} } as any, 0, 5, {
      onFocusStep: (idx) => {
        focusedIndex = idx;
      },
      onToggleExpand: () => {},
  ...
  test("ArrowUp moves focus to previous step bounded at zero", () => {
    let focusedIndex = 2;
    const handled = handleStepKeyDown({ key: "ArrowUp", preventDefault: () => {} } as any, 2, 5, {
      onFocusStep: (idx) => {
        focusedIndex = idx;
      },
      onToggleExpand: () => {},
  ...
  test("Enter triggers toggle expand callback", () => {
    let expandedIndex = -1;
    const handled = handleStepKeyDown({ key: "Enter", preventDefault: () => {} } as any, 3, 5, {
      onFocusStep: () => {},
      onToggleExpand: (idx) => {
        expandedIndex = idx;
      },
  ...
  test("Escape triggers exit focus mode", () => {
    let exited = false;
    const handled = handleStepKeyDown({ key: "Escape", preventDefault: () => {} } as any, 2, 5, {
      onFocusStep: () => {},
      onToggleExpand: () => {},
      onExitFocusMode: () => {
        exited = true;
  ...
  test("Unrelated keys return false and are inert", () => {
    const handled = handleStepKeyDown({ key: "Tab", preventDefault: () => {} } as any, 1, 5, {
      onFocusStep: () => {},
      onToggleExpand: () => {},
      onExitFocusMode: () => {},
    });
