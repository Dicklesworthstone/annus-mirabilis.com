...
describe("record refusal throw sites (am-muyh)", () => {
  const fixturePath = resolve(process.cwd(), "content/equations/brownian-motion/eq-model-bm-rms.json");
  ...
  test("refusal (record.ts:38): equation-invalid rejects invalid equation records", () => {
    ...
    const accepted = parseEquationRecord(validRecord, "content/equations/brownian-motion/eq-model-bm-rms.json");
