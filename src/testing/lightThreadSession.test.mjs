...
import { createLightThreadSession, LIGHT_THREAD_OWNERS } from "../experiments/lightThread/session.ts";
import { LIGHT_THREAD_DEFAULTS, LIGHT_THREAD_QUANTITIES, evaluateLightThread } from "../physics/reference/lightThread.ts";
...
const close = (a, b) => assert.ok(Math.abs(a - b) <= 2e-10 * Math.max(Math.abs(a), Math.abs(b), Number.MIN_VALUE));
...
test("production wiring uses both existing wave owners and the complete output contract", () => {
  ...
  for (const beta of [-0.999999, -0.6, 0, 0.6, 0.999999]) {
    for (const angleDeg of [0, 90, 180]) {
      ...
      close(evaluated.snapshot.values.quantumRatioMoving, evaluated.snapshot.values.quantumRatioStationary);
      ...
      for (const id of Object.keys(LIGHT_THREAD_QUANTITIES)) close(value(snapshot, id), evaluated.snapshot.values[id]);
    }
  }
});
...
test("observer changes preserve the physical run and source pulse; setup changes create a run", () => {
  ...
  assert.equal(session.apply({ ...LIGHT_THREAD_DEFAULTS, beta: -0.6, pulseEnergyJ: 2 }).kind, "accepted");
...
test("refused, mixed, repeated and independent-instance updates cannot mix snapshots", () => {
  ...
  close(value(accepted(left), "quantumRatioMoving"), value(accepted(left), "quantumRatioStationary"));
});
