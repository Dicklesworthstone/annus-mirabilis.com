...
import { buildMassEnergyElimination, MASS_ENERGY_ELIMINATION } from "../equations/derivations/massEnergyElimination.ts";
import { assessLinearCertificate, decodeProofSetup, proofSetupHref } from "../equations/derivations/linearProofState.ts";
...
test("the five-step bridge checks the real canonical equations, not hand-copied formulas", () => {
  ...
  assert.deepEqual(proof.certificate.steps.map(s => s.id), ["subtract", "regroup", "substitute-offset", "cancel-offset", "name-difference"]);
  assert.equal(proof.steps.filter(s => s.move).length, 1);
  assert.equal(proof.steps.find(s => s.move).id, "substitute-offset");
  for (const step of proof.certificate.steps) assert.ok(equations.some(e => e.id === step.equation));
  const offset = equations.find(e => e.id === "eq-model-me-offset-substitution");
  const copies = walk(offset.tree).filter(n => n.kind === "symbol" && n.quantityId === "additiveEnergyConstant");
...
for (let mask = 0; mask < 16; mask++) {
  test(`premise subset ${mask}: share/restore and support are the same dependency projection`, () => {
    ...
    assert.deepEqual(state.map(s => s.status), [twoAccounts, twoAccounts,
      twoAccounts && selected.includes("offset"), twoAccounts && selected.includes("offset"),
      twoAccounts && selected.includes("offset") && selected.includes("notation")].map(b => b ? "supported" : "blocked"));
...
for (const query of ["?mep=2", "?mep=1&mep=1", "?mep-off=offset", "?mep=1&mep-off=offset,offset", "?mep=1&mep-off=constructor", "?mep=1&mep-off=", "?mep=1&mep-off=offset&mep-off=moving", "?mep=1&mep-off=<script>", "?mep=" + "1".repeat(9000)]) {
...
test("unrelated reader settings do not become derivation data", () => {
  assert.deepEqual(decodeProofSetup(proof.certificate, "?detail=2&note=private"), { kind: "absent" });
  const restored = decodeProofSetup(proof.certificate, "?detail=2&note=private&mep=1&mep-off=offset");
...
test("changing an after-offset quantity leaves a remainder and invalidates the certificate", () => {
  ...
  for (const node of walk(after.tree)) if (node.kind === "symbol" && node.quantityId === "additiveEnergyConstant") node.quantityId = "bodyEnergyRestBefore";
...
test("the authored changed-offset arithmetic is a counterexample to automatic cancellation", () => {
  const before = { frameDifference: 6, offset: 2 }, after = { frameDifference: 5.5, offset: 3 };
  const accountDrop = before.frameDifference - after.frameDifference;
  const kineticDrop = (before.frameDifference - before.offset) - (after.frameDifference - after.offset);
  assert.equal(accountDrop, 0.5); assert.equal(kineticDrop, 1.5);
...
test("browser state module has no runtime imports of the parser, kernel or renderer", async () => {
  const source = await readFile(new URL("../equations/derivations/linearProofState.ts", import.meta.url), "utf8");
  assert.equal(source.split("\n").filter(line => line.startsWith("import ") && !line.startsWith("import type ")).length, 0);
});
