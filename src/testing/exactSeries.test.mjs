...
function stamp(raw) {
  ...
  function visit(value) {
    ...
    if (value && typeof value === "object") {
      ...
      else if (clone.kind && !["number", "constant"].includes(clone.kind))
        clone.opId = `${equationId}.op.n${next++}`;
      return clone;
    }
    return value;
  }
...
const request = (expression, order = 8) => ({
...
test("integer powers and geometric inversion work at nonunit constants", () => {
  ...
  assert.deepEqual(jet(div(num(1), sum(num(2), neg(x()))), 4), [
    "1/2",
    "1/4",
    "1/8",
    "1/16",
...
test("products use convolution rather than multiplying corresponding coefficients", () => {
  assert.deepEqual(jet(mul(sum(num(1), x()), sum(num(1), neg(x()))), 4), [
    "1",
    "0",
    "-1",
    "0",
  ...
  assert.deepEqual(jet(mul(pow(gamma(), 2), sum(num(1), neg(pow(x(), 2)))), 8), [
    "1",
    ...Array(8).fill("0"),
  ]);
});
...
test("arbitrarily small nonzero lower-order residues defeat a removable limit", () => {
  assert.throws(
    () => monomialQuotientLimit(request(sum(gamma(), num(-1), num("1e-100"))), 2),
    /lower-order/,
  );
  assert.throws(
    () => monomialQuotientLimit(request(sum(gamma(), num(-1), mul(num("1e-100"), x()))), 2),
    /lower-order/,
  );
});
...
test("declared variable matches canonical ratio identities, not display glyph or node id", () => {
  ...
  const aliasRegistry = {
    ...registry,
    impostor: { ...registry.frameSpeed, id: "impostor", glyph: "v" },
  };
...
test("the output and input expressions cannot be changed by consumers", () => {
  const source = request(gamma());
  const copy = structuredClone(source.expression);
...
for (const order of [-1, 13, NaN, Infinity, 2.5])
  test(`invalid requested order ${order} is rejected`, () => {
    assert.throws(() => jet(gamma(), order));
  });
for (const power of [0, -1, 9, NaN, 1.5])
  test(`unsupported removable division power ${power} is rejected`, () => {
...
for (const [label, expression] of [
  ...
  test(`refuse ${label}`, () => {
    assert.throws(() => jet(expression));
  });

test("malformed source getters are refused without evaluation", () => {
  const source = request(gamma());
  let accessed = false;
  Object.defineProperty(source.expression, "kind", {
    enumerable: true,
    get() {
      accessed = true;
      return "quotient";
...
test("the expansion variable itself must be dimensionless, even if the expression is a constant", () => {
  assert.throws(
    () =>
      exactSeriesAtZero({
        ...request(num(1)),
        variable: { numerator: "frameSpeed", denominator: "emittedEnergyRestFrame" },
      }),
    /dimensionless/,
  );
});
test("variable accessors are refused without execution", () => {
  let accessed = false;
  const variable = { denominator: "speedOfLight" };
  Object.defineProperty(variable, "numerator", {
    enumerable: true,
    get() {
      accessed = true;
      return "frameSpeed";
  ...
  assert.throws(() => exactSeriesAtZero({ ...request(gamma()), variable }));
  assert.equal(accessed, false);
});
