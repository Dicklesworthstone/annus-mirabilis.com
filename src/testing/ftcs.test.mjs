...
test("FTCS golden frames use old-field values and the documented no-flux operator", () => {
  ...
  assert.deepEqual(
    [...output.values],
    [0, 0, 1, 0, 0, 0, 0.25, 0.5, 0.25, 0, 0.0625, 0.25, 0.375, 0.25, 0.0625],
  );
  assert.equal(output.stepCount, 2);
  assert.equal(output.elapsedTime, 0.5);
...
test("the exact stability boundary is admitted and any excess refused", () => {
  ...
  assert.deepEqual(
    refusal.rankedRepairs.map((r) => r.action.parameterId),
    ["dt", "dx", "D"],
  );
  for (const repair of refusal.rankedRepairs)
    accepted(ftcs1d({ ...p, dt: 0.5000001, [repair.action.parameterId]: repair.action.value }));
});
test("all profiles preserve mass and the maximum principle", () => {
  for (const profile of [0, 1, 2]) {
    ...
    const result = accepted(
      ftcs1d({ n, frames: 101, stepsPerFrame: 5, D: 1, dx, dt: 0.019, profile }),
    );
    const initial = result.values.slice(0, n);
    const mass = sum(initial) * dx,
      max = Math.max(...initial);
...
test("zero diffusivity leaves every frame unchanged", () => {
  ...
  for (let frame = 1; frame < p.frames; frame++)
    assert.deepEqual(
      result.values.slice(frame * p.n, (frame + 1) * p.n),
      result.values.slice(0, p.n),
    );
});
test("constant field is a fixed point; continuation is chunk-independent and immutable", () => {
  ...
  assert.deepEqual(direct, chunked);
...
test("work and memory limits are software outcomes, not physics refusals", () => {
  for (const budget of [
    { workUnits: 0, allocationBytes: 10000 },
    { workUnits: 10000, allocationBytes: 0 },
  ]) {
    const result = ftcs1d(p, budget);
    assert.equal(result.kind, "outcome");
    assert.equal(result.outcome.outcome, "budget-exhausted");
    decodeOutcome(result.outcome);
  }
  const result = ftcs1d(
    { ...p, n: 1000, frames: 10000 },
    { workUnits: 1e12, allocationBytes: 1e12 },
  );
  assert.equal(result.kind, "outcome");
  assert.deepEqual(result.outcome.allowed, REFERENCE_FTCS_BUDGET);
  decodeOutcome(result.outcome);
});
...
test("cell-mass error decreases under grid refinement at fixed physical time", () => {
  ...
  decodeResult(coarse.maxCellMassDifference);
  decodeResult(fine.maxCellMassDifference);
  assert.ok(fine.maxCellMassDifference.value < coarse.maxCellMassDifference.value / 3);
  near(sum(coarse.cellMasses), 1);
  near(sum(fine.cellMasses), 1);
  assert.equal(coarse.wallContact, false);
  assert.equal(fine.wallContact, false);
});
test("comparison identifies wall contact rather than pretending box and free space coincide", () => {
  ...
  const zero = accepted(
    ftcsAnalyticComparison({ field: initial.values, dx: 1, t: 0, D: 1, startCell: 2 }),
  );
...
test("all dimensional repairs remain stable after binary64 boundary rounding", () => {
  for (const D of [4.294395645549615e-13, 1e-9, Math.PI, 1.123e-30])
    for (const dx of [1e-4, Math.PI / 100, 0.1, 1e-8]) {
