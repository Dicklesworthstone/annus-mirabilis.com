...
for (const [paper, sectionCount, prefix, labCount] of [
  ...
  test(`${paper}: every new section has a resolvable canonical reader and sitemap entry`, async () => {
    ...
    assert.deepEqual(
      sections.map((p) => p.section),
      Array.from({ length: sectionCount }, (_, i) => `s${i}`),
    );
    ...
    assert.deepEqual(await resolvePaperRoute({ paperId: paper, section: "s99" }), {
      ok: false,
  ...
  test(`${paper}: the rendered Try it action reaches every laboratory, not just secondary references`, async () => {
    ...
    for (const argument of payload.arguments) {
      ...
      for (const target of [actions.why, actions.missingStep, actions.example])
        assert.ok(foundations.has(target));
    }
    assert.deepEqual(
      [...targets].sort(),
      Array.from({ length: labCount }, (_, i) => `${prefix}-${String(i + 1).padStart(2, "0")}`),
    );
  });
}

test("a bibliographic key remains a citation identity, not an accidental fifth paper route", async () => {
  assert.deepEqual(await resolvePaperRoute({ paperId: "ap-17-891" }), {
    ok: false,
    code: "bibliographic-key",
  });
  assert.deepEqual(await resolvePaperRoute({ paperId: "molecular-dimensions" }), {
    ok: false,
    code: "unknown-paper",
  });
});
