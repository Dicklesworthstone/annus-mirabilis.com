...
async function accepted(patch: Record<string, string> = {}): Promise<KitchenAccepted> {
  ...
  const session = createKitchenSession("uncertainty-ui", () => ({
  ...
  try {
    const done = new Promise<KitchenAccepted>((resolve, reject) => {
      const timer = setTimeout(() => { off(); reject(new Error(session.getSnapshot().message || "No accepted analysis")); }, 3000);
      const off = session.subscribe(() => {
        const a = session.getSnapshot().accepted;
        if (a) { clearTimeout(timer); off(); resolve(a); }
      });
    });
...
test("a complete accepted uncertainty result renders with its assumptions and separate intervals without hydration", async () => {
  const a = await accepted(), shell = document.createElement("div");
  ...
  expect(shell.querySelector("[data-kitchen-combined-coverage]")?.textContent).toContain("At least 95%");
...
test("unknown coverage does not borrow the original conditional interval's probability", async () => {
  ...
  expect(shell.querySelector("[data-kitchen-combined]")?.textContent).toContain("sensitivity analysis");
...
test("range and coverage controls have labels and a static explanation instead of made-up defaults", async () => {
  const a = await accepted({ physical_input_coverage: "" }), shell = document.createElement("div");
  shell.innerHTML = renderToStaticMarkup(<KitchenInputs accepted={a} busy={false} revise={() => {}} onError={() => {}} />);
  for (const name of ["pixels_per_um_x_interval", "pixels_per_um_y_interval", "radius_scale_axis", "viscosity_interval_mpa_s", "gas_constant_interval", "physical_input_coverage", "physical_input_provenance"]) {
    const control = shell.querySelector<HTMLInputElement | HTMLSelectElement>(`[name="${name}"]`)!;
    expect(control).not.toBeNull();
    expect([...shell.querySelectorAll("label")].some(label => label.htmlFor === control.id)).toBe(true);
  }
...
test("editing and applying a relationship cannot mutate the already accepted result or download", async () => {
  const a = await accepted(), receipt = kitchenAnalysisJson(a, source);
  const container = createContainer(), root = createRoot(container);
  const revisions: KitchenDocument[] = [], errors: string[] = [];
  try {
    await act(async () => { root.render(<>
      <KitchenInputs accepted={a} busy={false} revise={d => revisions.push(d)} onError={e => errors.push(e)} />
      <KitchenResults accepted={a} />
    </>); });
    const select = container.querySelector<HTMLSelectElement>('[name="radius_scale_axis"]')!;
    await act(async () => { select.value = "x"; select.dispatchEvent(new Event("change", { bubbles: true })); });
    expect(revisions.length).toBe(0);
    expect(kitchenAnalysisJson(a, source)).toBe(receipt);
    expect(container.querySelector("[data-calibration-dependence]")?.textContent).toContain("power -2");
    await act(async () => { container.querySelector("form")!.dispatchEvent(new Event("submit", { bubbles: true, cancelable: true })); });
    expect(errors).toEqual([]); expect(revisions.length).toBe(1);
