...
import {
  ...
  exactPolynomial,
  PolynomialRefusal,
} from "../equations/derivations/exactPolynomial.ts";
import {
  assessLinearCertificate,
  checkLinearCertificate,
} from "../equations/derivations/linearCertificate.ts";
...
const catalog = await Promise.all(
  (await readdir(dir))
    ...
    .map(async (path) => JSON.parse(await readFile(new URL(path, dir), "utf8"))),
);
...
const tree = {
  kind: "relation",
  operator: "define",
  opId: `${definitionId}.op.definition`,
  left: symbol("kineticEnergyDifference", `${definitionId}.t.delta`),
  right: sum(
    symbol("kineticEnergyBefore", `${definitionId}.t.before`),
    negative(symbol("kineticEnergyAfter", `${definitionId}.t.after`)),
  ),
};
const definition = {
  ...catalog[0],
  ...
  notes: walk(tree)
    ...
    .map((n) => ({
      nodeId: nodeId(n),
      title: "Fixture term",
      explanation: "A definition, not an extra physical premise.",
  ...
  return {
    ...
    premises: [
      ...
      { id: "moving", kind: "assumption", equations: [equationId("moving-ledger")] },
      {
        id: "offset",
        kind: "assumption",
        equations: [equationId("offset-before"), equationId("offset-after")],
      },
    ...
    steps: [
      {
        id: "subtract",
        equation: equationId("ledger-subtraction"),
        combination: [ref("moving-ledger"), ref("rest-ledger", -1)],
      },
      {
        id: "cancel",
        equation: equationId("exact-drop"),
        combination: [
          ref("ledger-subtraction"),
          ref("offset-before", -1),
          ref("offset-after"),
          { equation: definitionId, coefficient: { num: 1, den: 1 } },
        ],
      },
    ],
  };
...
test("expand, factor, collect and reorder preserve canonical quantities rather than occurrence ids", () => {
  assert.ok(
    same(
      product(a, sum(b, num(-1))),
      sum(product(b, symbol("emittedEnergyRestFrame", "elsewhere")), negative(a)),
    ),
  );
  assert.ok(same(power(sum(a, b), 2), sum(power(a, 2), product(num(2), a, b), power(b, 2))));
  assert.ok(
    !same(symbol("bodyEnergyRestBefore", "sameGlyph"), symbol("bodyEnergyRestAfter", "sameGlyph")),
  );
...
test("cycles and accessors cannot execute unbounded or reader-provided code", () => {
  const cycle = { kind: "group" };
  cycle.argument = cycle;
  assert.throws(() => exactPolynomial(cycle), /budget/);
  let invoked = false;
  const node = { kind: "symbol", termId: "x" };
  Object.defineProperty(node, "quantityId", {
    enumerable: true,
    get() {
      invoked = true;
      return "x";
  ...
  assert.throws(() => exactPolynomial(node));
...
test("both real mass-energy elimination steps have exact certificates and transitive dependencies", () => {
  ...
  assert.deepEqual(checked.requirements, [
    { step: "subtract", premises: ["rest", "moving"] },
    { step: "cancel", premises: ["rest", "moving", "offset", "notation"] },
  ]);
  assert.deepEqual(
    assessLinearCertificate(checked, ["rest", "moving", "offset", "notation"]).map((s) => s.status),
    ["supported", "supported"],
  );
...
test("without the same-offset premise, conservation still supports the weaker result only", () => {
  ...
  for (const name of ["rest", "moving", "notation"]) {
    const status = assessLinearCertificate(
      checked,
      ["rest", "moving", "offset", "notation"].filter((id) => id !== name),
    );
    assert.equal(status[1].status, "blocked");
    assert.ok(status[1].missing.includes(name));
  }
  assert.equal(
    assessLinearCertificate(checked, []).every((s) => s.status === "blocked"),
    true,
  );
});
...
test("catalog identities, dimensions, paper scope and definition meaning stay closed", () => {
  ...
  const copy = structuredClone(equations);
  copy[0].paper = "brownian-motion";
  assert.throws(() => checkLinearCertificate(proof(), copy));
  const input = proof();
  input.paper = "constructor";
  assert.throws(() => checkLinearCertificate(input, equations));
  assert.throws(() =>
    equalityResidual({ kind: "relation", operator: "approx", left: num(1), right: num(1) }),
  );
  const checked = checkLinearCertificate(proof(), equations);
