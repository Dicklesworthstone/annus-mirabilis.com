...
test("Brownian DoD Item 1: complete paper text inventory", async () => {
  ...
  assert.deepEqual(sectionIds, ["s4", "s5"]);
  ...
  suiteLogger.log({
    ...
    message: "Verified Item 1: §4 and §5 authored; §§1-3, intro, and closing absent.",
    extra: {
...
test("Brownian DoD Item 3: explanations and foundations inventory", async () => {
  ...
  suiteLogger.log({
    ...
    outcome: "passed",
    durationMs: Date.now() - startTime,
    message: "Verified Item 3: 6 arguments for §§4-5 and >=18 foundation records present.",
    extra: {
      item: "3. Explanations",
