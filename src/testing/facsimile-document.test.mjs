...
test("rejects unsafe URLs, mismatched identity, unknown decisions and invalid pin metadata", () => {
  ...
  for (const mutate of mutations) {
    const c = config();
    mutate(c);
    refusesWith(() => project(c), "facsimile-data-invalid", "(document.ts:51)");
  }
  ...
  refusesWith(() => project(unparseable), "facsimile-data-invalid", "(document.ts:124)");
});
test("same-length wrong page window, stale extract map and out-of-parent pages are rejected", () => {
  ...
  refusesWith(() => project(missingAnchor), "facsimile-data-invalid", "(document.ts:46)");
  const mutations = [
...
test("inventory identity, page ranges, missing locators, duplicate ids and unsafe aliases fail closed", () => {
  ...
  for (const mutate of mutations) {
    const i = inventory();
    mutate(i);
    refusesWith(() => project(config(), i), "facsimile-data-invalid", "(document.ts:51)");
  }
});
...
test("a frozen id with a printed letter label is admitted, and a malformed id is still refused", () => {
  ...
  for (const id of ["Eq-A", "eq A", "eq-<A>", "eq-A\n", "", `eq-${"A".repeat(160)}`]) {
    const bad = inventory();
    bad.units[1].id = id;
    refusesWith(() => project(config(), bad), "facsimile-data-invalid", "(document.ts:51)");
  }
});
