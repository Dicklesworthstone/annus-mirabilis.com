...
import { fromWalkDraft, toWalkDraft } from "../experiments/bm05/controls.ts";
...
test("walk display units roundtrip without changing the underlying trial inputs", () => {
  for (const kernel of ["coin", "uniform", "gaussian"])
    for (const stepRms of [0.5e-6, 1.234567890123456e-9, 1e-40, 1e40]) {
      ...
      d.n = "43";
      const q = fromWalkDraft(d);
      assert.equal(q.stepRms, p.stepRms);
      assert.equal(q.seed, p.seed);
      assert.equal(q.n, 43);
    }
});
test("control parsing rejects invalid step counts, unsigned seeds and unsupported sampled laws", () => {
  for (const patch of [
    ...
    assert.throws(() => fromWalkDraft({ ...toWalkDraft(BM05_DEFAULTS), ...patch }));
...
test("accepted SI settings links retain the exact seed and all step parameters", () => {
  for (const kernel of ["coin", "uniform", "gaussian"]) {
    const p = { ...BM05_DEFAULTS, kernel, n: 64, seed: "9007199254740993" };
    const link = encodeBm05Settings(p),
      decoded = decodeBm05Settings(link);
    assert.equal(decoded.kind, "settings");
    assert.deepEqual(decoded.parameters, p);
  }
  assert.equal(decodeBm05Settings("").kind, "absent");
  assert.equal(decodeBm05Settings("?").kind, "absent");
});
...
test("parameter decoding does not evaluate accessors or silently drop unknown fields", () => {
  ...
  assert.equal(validateBm05Parameters(p).kind, "refused");
  assert.equal(touched, false);
  assert.equal(validateBm05Parameters({ ...BM05_DEFAULTS, hidden: 1 }).kind, "refused");
  assert.equal(validateBm05Parameters({ ...BM05_DEFAULTS, seed: 1905 }).kind, "refused");
});
