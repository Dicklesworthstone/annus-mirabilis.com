...
import { type PreparedLightInvestigation, evaluateLightInvestigation } from "../discovery/lightQuanta/investigation.ts";
...
const equations = { entropy: <span>entropy equation</span>, counting: <span>counting equation</span>, match: <span>matching equation</span>, emission: <span>emission equation</span> };
...
describe("Connected light investigation: real stores and server-rendered reading", () => {
  ...
  test("publishes a separate explanatory workbench, not a reviewed journey", () => {
    ...
    const placeholder = renderToStaticMarkup(<JourneyInPreparation {...DISCOVERY_PAPERS["light-quanta"]} />);
  ...
  test("outside-domain views preserve the gate and show no stale inferred count", () => {
    ...
    const altered: PreparedLightInvestigation = {...example,parameters,results:evaluateLightInvestigation(parameters).map(encodeResult)};
    const rendered=renderToStaticMarkup(<LightQuantaInvestigation example={altered} equations={equations}/>);
    expect(rendered).toContain("data-inference-blocked");
    expect(rendered).toContain('data-quantity-id="effectiveIndependentCount" data-result-status="outside-domain"');
    expect(rendered).toContain('data-quantity-id="independentProbability" data-result-status="value"');
  });
  test("two placements never reuse form IDs or share accepted run identities", () => {
    const rendered=renderToStaticMarkup(<><LightQuantaInvestigation example={example} equations={equations}/><LightQuantaInvestigation example={example} equations={equations}/></>);
    const ids=[...rendered.matchAll(/<(?:input|textarea)[^>]* id="([^"]+)"/g)].map(m=>m[1]);
    expect(ids.length).toBe(22);expect(new Set(ids).size).toBe(ids.length);
