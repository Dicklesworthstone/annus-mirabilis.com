...
beforeEach(installDom); afterEach(uninstallDom);
const states = (container: HTMLElement) => [...container.querySelectorAll("[data-linear-step]")].map(e => e.getAttribute("data-step-state"));
...
test("omitting the offset retains the first two steps and blocks only the dependent conclusions", async () => {
  ...
  try {
    await act(async () => { root.render(<LinearProofExplorer proof={proof} restoreSettings={false} />); });
    const before = [...container.querySelectorAll("[data-proof-equation]")].map(e => e.innerHTML);
    await act(async () => { container.querySelector<HTMLInputElement>('[data-proof-premise="offset"]')!.click(); });
    expect(states(container)).toEqual(["supported", "supported", "blocked", "blocked", "blocked"]);
    expect([...container.querySelectorAll("[data-proof-equation]")].map(e => e.innerHTML)).toEqual(before);
    expect(container.querySelector("[data-proof-status]")!.textContent).toContain("2 of 5");
    expect(container.querySelector("[data-proof-share]")!.getAttribute("href")).toContain("mep-off=offset");
    await act(async () => { container.querySelector<HTMLInputElement>('[data-proof-premise="offset"]')!.click(); });
    expect(states(container)).toEqual(Array(5).fill("supported"));
...
test("valid configuration links restore only the opted-in instance and IDs remain distinct", async () => {
  ...
  const container = createContainer(), root = createRoot(container);
  try {
    await act(async () => { root.render(<><LinearProofExplorer proof={proof} /><LinearProofExplorer proof={proof} restoreSettings={false} /></>); });
    const instances = container.querySelectorAll<HTMLElement>("[data-linear-proof]");
    expect(states(instances[0]!)).toEqual(["supported", "supported", "blocked", "blocked", "blocked"]);
...
test("an ambiguous shared configuration preserves the all-premise argument with a visible notice", async () => {
  ...
  const container = createContainer(), root = createRoot(container);
  try {
    await act(async () => { root.render(<LinearProofExplorer proof={proof} />); });
    expect(states(container)).toEqual(Array(5).fill("supported"));
...
test("compact and expanded views keep the same equation nodes, step order and conclusions", async () => {
  ...
  try {
    ...
    await act(async () => { select.value = "compact"; select.dispatchEvent(new Event("change", { bubbles: true })); });
    expect(container.querySelector("[data-linear-proof]")!.getAttribute("data-proof-mode")).toBe("compact");
    expect([...container.querySelectorAll<HTMLDetailsElement>(".linear-step-detail")].every(e => !e.open)).toBe(true);
    expect([...container.querySelectorAll("[data-proof-equation]")]).toEqual(before);
    expect(states(container)).toEqual(Array(5).fill("supported"));
    await act(async () => { select.value = "full"; select.dispatchEvent(new Event("change", { bubbles: true })); });
    expect([...container.querySelectorAll<HTMLDetailsElement>(".linear-step-detail")].every(e => e.open)).toBe(true);
  } finally { await act(async () => { root.unmount(); }); removeContainer(container); }
});
