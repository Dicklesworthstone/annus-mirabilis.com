...
import {
  ...
  tokenExample,
} from "../reader/entrances/lightQuantaExample.ts";

const record = JSON.parse(
  await readFile(
    new URL("../../content/arguments/light-quanta/entrance-light-quanta.json", import.meta.url),
    "utf8",
  ),
);
...
for (const example of TOKEN_WORKED_EXAMPLES) {
  ...
  test(`${tokens} tokens, ${parts} parts, ${locked ? "locked" : "independent"}: exhaustive table agrees with real reference owners`, () => {
    ...
    close(
      example.probability,
      locked
        ? lockedPositionsProbability(tokens, 1 / parts).value
        : independentPointsProbability(tokens, 1 / parts).value,
    );
  });
  test(`${tokens}/${parts}/${locked}: actual laboratory codec retains the exact counting setup`, () => {
    const decoded = decodeLq05Settings(
      new URL(example.labHref, "https://annus-mirabilis.com").search,
    );
...
for (const input of [
  null,
  {},
  { tokens: 0, parts: 2, locked: false },
  { tokens: 1.5, parts: 2, locked: false },
  { tokens: NaN, parts: 2, locked: false },
  { tokens: 1000000, parts: 2, locked: false },
  { tokens: 5, parts: 2, locked: false },
  { tokens: 11, parts: 2, locked: true },
  { tokens: 2, parts: 0, locked: false },
  { tokens: 2, parts: 4, locked: false },
...
test("results are immutable and independent of subsequent draft mutation", () => {
  ...
  assert.throws(() => {
    example.arrangements[0].parts[0] = 2;
  });
  assert.throws(() => {
    example.setup.tokens = 1;
  });
  assert.throws(() => {
    example.arrangements.push({});
  });
});
test("the real entrance validator admits the authored bridge without claiming review", () => {
  ...
  assert.equal(
    checked.helpEntries.some((e) =>
      /not evidence that light is made of dots/.test(e.clarification),
    ),
    true,
  );
});
test("a bridge cannot become an all-instrument list or omit its skill", () => {
  ...
  const noSkill = structuredClone(record);
  delete noSkill.bridge.newSkill;
