...
import { snapshotOutputs, createMe02Session } from "../experiments/me02/session.ts";
import { evaluateSubtraction, evaluateMe02 } from "../physics/reference/massEnergy.ts";
import { validateMe02Parameters } from "../experiments/me02/parameters.ts";

const close = (a, b, tolerance = 1e-12) => assert.ok(Math.abs(a-b) <= tolerance * Math.max(Math.abs(a), Math.abs(b), 1e-300), `${a} != ${b}`);

test("ME-01 permalinks include every accepted cancellation, step and offset setting", () => {
  const parameters = {...ME01_DEFAULTS, frameSpeed: -0.45, emittedEnergyRestFrame: 2.5, emissionAngle: 60,
    premise: "relaxed", notation: "modern", offsetDisplay: "offsets", step: "premise-kinetic",
    cancelAngleFactors: false, cancelInternalEnergies: false, cancelAdditiveConstant: false};
...
test("ME-02 exact settings survive permalink round trips", () => {
  const parameters = {...ME02_DEFAULTS, beta: 1e-8, emittedEnergy: 1e7, energyUnit: "erg", notation: "modern", speedAxis: "logarithmic", showNaive: true};
  const decoded = decodeMe02Settings(encodeMe02Settings(parameters));
  assert.equal(decoded.kind, "settings");
  assert.deepEqual(decoded.parameters, parameters);
});

for (const [decode, speed] of [[decodeMe01Settings, "v"], [decodeMe02Settings, "beta"]]) {
  for (const query of [`?${speed}=0.6garbage`, `?${speed}=1`, `?${speed}=0.6&${speed}=0.01`,
    `?${speed}=1e-999`, "?L=", "?L=Infinity", "?L=0", "?notation=unknown", "?unregistered=1"]) {
    test(`${speed}: refuse ambiguous or unrepresentable settings ${query}`, () => {
      assert.equal(decode(query).kind, "invalid");
    });
  }
  test(`${speed}: empty search is not a request to recalculate`, () => assert.equal(decode("").kind, "none"));
}
...
test("an erg input is converted to joules before the coefficient owner runs", () => {
  ...
  const smaller = snapshotOutputs({...ergs, emittedEnergy: 1}).find(o => o.quantityId === "kineticEnergyDifference");
...
test("input unit reinterpretation creates a new run; invalid converted energy preserves the accepted snapshot", () => {
  ...
  const result = session.apply({...ME02_DEFAULTS, emittedEnergy: Number.MIN_VALUE, energyUnit: "erg"});
