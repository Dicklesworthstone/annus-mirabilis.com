...
test("all presets roundtrip exactly through human display units", () => {
  for (const preset of Object.values(BM06_PRESETS))
    assert.deepEqual(fromBm06Draft(toBm06Draft(preset.parameters)), preset.parameters);
...
test("an unchanged physical field does not drift when the interval is edited", () => {
  ...
  const draft = toBm06Draft(parameters);
  draft.lower = "0";
  const changed = fromBm06Draft(draft);
  for (const key of Object.keys(parameters))
    if (key !== "lower") assert.equal(changed[key], parameters[key]);
});
test("decimal scaling roundtrips across binary64 exponents and unit powers", () => {
  for (const power of [-12, -6, 0, 3, 6, 12])
    for (const value of [
      0,
      1,
      -1,
      Number.MIN_VALUE,
      Number.MAX_VALUE,
      ...Array.from({ length: 601 }, (_, i) => Number(`1.234567890123456e${i - 300}`)),
    ]) {
...
test("nonfinite input, nonzero underflow and malformed decimals are refused", () => {
  for (const value of [
    ...
    assert.throws(() => parseScaledDecimal(value, 0));
