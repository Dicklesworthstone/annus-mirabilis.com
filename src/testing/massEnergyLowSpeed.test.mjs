...
import { assessLowSpeed, decodeLowSpeedSetup, lowSpeedHref } from "../equations/derivations/lowSpeedState.ts";
...
function annotated(record) {
  ...
  record.notes = walk(record.tree).flatMap(node => nodeId(node) ? [{nodeId: nodeId(node),
    title:"Test mutation", explanation:"A deliberately mutated equation, not published content.", foundation:"work-energy"}] : []);
  record.sentence = [{text:"This fixture tests rejection, not scientific evidence."}];
  record.bindings = [];
  return record;
}
function mutation(name, change) {
  const copy = structuredClone(records), eq = find(copy, name);
  change(eq); annotated(eq);
...
const ratio = () => ({kind:"quotient", opId:"temporary", numerator:{kind:"symbol",termId:"temporary",quantityId:"frameSpeed"},
  denominator:{kind:"symbol",termId:"temporary",quantityId:"speedOfLight"}});
const power = (base, num) => ({kind:"power",opId:"temporary",base,exponent:{num,den:1}});
const product = (...args) => ({kind:"product",opId:"temporary",args});
const sum = (...args) => ({kind:"sum",opId:"temporary",args});
const energy = () => ({kind:"symbol",termId:"temporary",quantityId:"emittedEnergyRestFrame"});
...
test("the kinetic identification carries all dependencies of the existing exact ledger proof", () => {
  ...
  for (const step of ["kinetic","divide"]) assert.deepEqual(proof.requirements.find(r=>r.step===step).premises,dependencies);
  assert.deepEqual(proof.requirements.find(r=>r.step==="identify").premises,[...dependencies,"inertia"]);
  assert.equal(proof.premises.find(p=>p.id==="inertia").explanation.includes("additional physical premise"),true);
});
...
for (const [label,name,change,error] of [
  ["wrong radical branch","lorentz-factor",e=>{e.tree.right.denominator.degree=4;},/positive reciprocal radical/],
  ["changed numerator","lorentz-factor",e=>{e.tree.right.numerator.value="2";},/positive reciprocal radical/],
  ["tenth-order perturbation beyond the jet","lorentz-factor",e=>{
    const r=e.tree.right.denominator.radicand;
    r.args.push(product(number("1e-100"),power(ratio(),10)));
  },/positive reciprocal radical/],
  ["quadratic assertion promoted to equality","quadratic-drop",e=>{e.tree.operator="=";},/approximation sign/],
  ["wrong quadratic coefficient","quadratic-drop",e=>{
    const value=walk(e.tree).find(n=>n.kind==="number"&&n.value==="0.5"); value.value="0.4";
  },/exactly the quadratic/],
  ["hidden tenth-order term in stated quadratic","quadratic-drop",e=>{
    e.tree.right=sum(e.tree.right,product(energy(),power(ratio(),10)));
  },/exactly the quadratic/],
  ["proxy multiplier changed","finite-speed-proxy",e=>{e.tree.right.numerator.args[0].value="3";},/coefficient role/],
  ["proxy definition promoted to approximation","finite-speed-proxy",e=>{e.tree.operator="approx";},/coefficient role/],
  ["mass coefficient doubled","mass-decrease",e=>{e.tree.right.numerator=product(number("2"),e.tree.right.numerator);},/coefficient role/],
  ["speed denominator rescaled","mass-decrease",e=>{e.tree.right.denominator.base.scale={num:2,den:1};},/coefficient role/],
  ["exact kinetic source changed","exact-drop",e=>{e.tree.right=product(number("2"),e.tree.right);},/./],
  ["offset premise changed","offset-after",e=>{e.tree.right.args[1].scale={num:2,den:1};},/./],
...
test("missing proof equations or a duplicated canonical identity cannot yield a certificate",()=>{
  for(const name of ["lorentz-factor","offset-before","raw-subtraction","quadratic-drop","finite-speed-proxy","mass-decrease"])
    assert.throws(()=>buildMassEnergyLowSpeed(records.filter(e=>e.id!==`eq-model-me-${name}`)));
  assert.throws(()=>buildMassEnergyLowSpeed([...records,structuredClone(find(records,"mass-decrease"))]));
});
...
for(let mask=0;mask<32;mask++) test(`premise subset ${mask}: mathematics and physical identification remain separate`,()=>{
  const selected=ids.filter((_,i)=>mask&(1<<i));
  for(const order of [2,4,6]) {
    const selection={selected,order}, assessment=assessLowSpeed(proof,selection);
    for(const result of assessment) {
      const required=proof.requirements.find(r=>r.step===result.id).premises;
      assert.deepEqual(result.missing,required.filter(id=>!selected.includes(id)));
    ...
    assert.equal(assessment.find(s=>s.id==="identify").status,selected.length===5?"supported":"blocked");
...
test("retained order changes presentation but not the conditional mass coefficient",()=>{
  ...
  for(const s of selections) assert.deepEqual(assessLowSpeed(proof,s),assessLowSpeed(proof,selections[0]));
  assert.equal(proof.limit.limit,"1");
});
for(const search of [
  "?mel=2","?mel=1&mel=1","?mel-order=2","?mel=1&mel-order=2&mel-order=4",
  "?mel=1&mel-order=02","?mel=1&mel-order=2junk","?mel=1&mel-order=8",
  "?mel=1&mel-off=","?mel=1&mel-off=unknown","?mel=1&mel-off=inertia,inertia",
  "?mel=1&mel-off=offset&mel-off=inertia","?mel=1&mel-off=__proto__","?mel=1&mel-off=%3Cscript%3E",
  "?mel=1&mel-off=rest,","?mel=1&mel-order=NaN",`?mel=1&note=${"x".repeat(8192)}`,
...
test("other reader namespaces and unknown query text never become proof evidence",()=>{
  ...
  assert.equal(assessLowSpeed(proof,restored.selection).find(s=>s.id==="identify").status,"blocked");
...
for(const selection of [{selected:["offset","offset"],order:2},{selected:["fake"],order:2},{selected:ids,order:8}])
  test(`unregistered selection ${JSON.stringify(selection)} cannot be assessed or shared`,()=>{
    assert.throws(()=>assessLowSpeed(proof,selection));assert.throws(()=>lowSpeedHref(proof,selection));
  });
...
test("the independent numerical owner separates finite-speed proxy from the derived analytic limit",()=>{
  for(const beta of [-0.6,0,0.6]) {
    ...
    if(beta===0) assert.equal(result.finiteSpeedProxy.status,"not-applicable");
      ...
      assert.notEqual(result.finiteSpeedProxy.value,result.limitingCoefficient.representation.value);
