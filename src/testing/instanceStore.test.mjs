...
const publication = (token, value = 1, stepIndex = 0, final = true) => ({
  ...token,
  stepIndex,
  simulationTime: stepIndex * 0.25,
  final,
...
test("placements of the same experiment have independent accepted and requested state", () => {
  const a = createInstanceStore(options("inline")),
    b = createInstanceStore(options("expanded"));
...
test("observer, measurement, and estimator commands retain the physical run and seed", () => {
  ...
  let last = first;
  for (const [command, patch, revision] of [
    ["observer-change", { observer: 0.2 }, "observer"],
    ["measurement-change", { observationInterval: 2 }, "measurement"],
    ["estimator-change", { estimator: "likelihood" }, "estimator"],
  ]) {
...
test("stale actions, superseded runs, fabricated actions and mixed revisions are discarded", () => {
  ...
  assert.equal(
    store.publish(publication({ ...second, actionIndex: second.actionIndex + 1 })).reason,
    "unissued-action",
  );
  assert.equal(
    store.publish(publication({ ...second, revisions: { ...second.revisions, input: 0 } })).reason,
    "mixed-revisions",
  );
  assert.equal(
    store.publish(publication({ ...second, parameters: { ...second.parameters, D: 100 } })).reason,
    "parameter-mismatch",
  );
  assert.equal(store.getSnapshot(), pending);
...
test("progress is strictly monotone within an action and final completion seals it", () => {
  ...
  assert.equal(
    store.refuse(token, makeRefusal("invalid-parameter", { parameterIds: ["D"] })).reason,
    "completed-action",
  );
});
test("published numeric arrays cannot be mutated through source, view or returned copies", () => {
  ...
  source[0] = 100;
  values.copy()[1] = 200;
  assert.equal(values.at(0), 1);
  assert.equal(values.at(1), 2);
  assert.throws(() => {
    values.length = 0;
  });
  assert.throws(() => {
    snapshot.parameters.D = 2;
  });
  assert.throws(() => {
    snapshot.revisions.input = 100;
  });
...
test("malformed or wrongly labeled output never replaces accepted science", () => {
  ...
  const before = store.getSnapshot();
  for (const bad of [NaN, Infinity, new Float64Array([NaN])])
    assert.equal(store.publish(publication(token, bad, 1)).accepted, false);
  for (const patch of [
    { unit: "m" },
    ...
    assert.equal(
      store.publish({ ...publication(token, 1, 1), outputs: [{ ...output(1), ...patch }] })
        .accepted,
      false,
    );
  assert.equal(
    store.publish({ ...publication(token, 1, 1), extra: "unregistered" }).accepted,
    false,
  );
  assert.equal(store.getSnapshot(), before);
});
test("real FTCS instability preserves the last valid field and separates requested parameters", () => {
  ...
  const repaired = store.issue("setup-change", {
    dt: unstable.refusal.rankedRepairs[0].action.value,
  });
...
test("software failure and pause preserve accepted data and invalidate late responses", () => {
  ...
  expectAccepted(
    store.fail(next, { outcome: "worker-crashed", ...executionOutcomeRegistry["worker-crashed"] }),
  );
