...
import { MASS_ENERGY_ENTRANCE_INITIAL as initial, reduceMassEnergyEntrance as reduce } from "../reader/entrances/massEnergyState.ts";
...
test("two accounts at 60 percent light speed use the real owners, not invented body energies", () => {
  ...
  assert.ok(fast.bodyEnergies.every(item => item.status === "symbolic" && !Object.hasOwn(item, "value")));
});
...
test("relaxing the premise retains arithmetic but refuses a numeric kinetic interpretation", () => {
  for (const scenario of [fast, slow]) {
    ...
    assert.deepEqual(scenario.relaxedKinetic.neededInformation, ["additive-constant-invariance-under-emission"]);
    assert.ok(Object.isFrozen(scenario));
    assert.ok(Object.isFrozen(scenario.bodyEnergies));
    assert.throws(() => { scenario.subtraction.value = 999; }, TypeError);
  }
...
test("both entrance links restore the actual energy, observer, and unit rather than default examples", () => {
  for (const scenario of [fast, slow]) {
    ...
    const coefficient = decodeMe02Settings(new URL(scenario.me02Href, "https://example.test").search);
    ...
    close(snapshotOutputs(coefficient.parameters).find(o => o.quantityId === "kineticEnergyDifference").value, scenario.subtraction.value);
  }
});
