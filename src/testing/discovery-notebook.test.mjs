...
import { emptyNotebook, validateNotebook, recordPrediction, recordObservation, encodeNotebook,
  decodeNotebook, createImportGuard, NOTE_TEXT_LIMIT, NOTE_ATTEMPT_LIMIT, NOTE_FILE_LIMIT,
} from "../discovery/notebook.ts";
const manifest = { paper: "mass-energy", revision: "draft-one", stages: [
  {id: "emit", title: "Emit", alternatives: []},
  {id: "subtract", title: "Subtract", alternatives: [{id: "assume", title: "Assume"}, {id: "compare", title: "Compare"}]},
  {id: "limit", title: "Take the limit", alternatives: []},
]};
const fresh = () => emptyNotebook(manifest);
const one = () => recordPrediction(fresh(), manifest, "subtract", "The unknowns might cancel.", "compare");
test("prediction snapshots never mutate previous notes", () => {
  const before = fresh(), after = recordPrediction(before, manifest, "emit", "No recoil.", null);
  assert.equal(before.entries.length, 0); assert.equal(after.entries[0].attempts[0].prediction, "No recoil.");
  for (const v of [after, after.entries, after.entries[0], after.entries[0].attempts, after.entries[0].attempts[0]]) assert.ok(Object.isFrozen(v));
});
...
test("canonical exports are independent of step visitation order", () => {
  ...
  const b = recordPrediction(recordPrediction(fresh(), manifest, "emit", "Symmetry.", null), manifest,
    "subtract", "The unknowns might cancel.", "compare");
  assert.equal(encodeNotebook(a, manifest), encodeNotebook(b, manifest));
});
const invalid = [
  ["wrong kind", x=>x.kind="experiment-tape"], ["wrong schema", x=>x.schemaVersion=2],
  ["wrong paper", x=>x.paper="light-quanta"], ["stale revision", x=>x.journeyRevision="draft-zero"],
  ...
  ["oversized prediction", x=>x.entries[0].attempts[0].prediction="a".repeat(NOTE_TEXT_LIMIT+1)],
  ["oversized observation", x=>x.entries[0].attempts[0].observation="a".repeat(NOTE_TEXT_LIMIT+1)],
  ...
  ["too many attempts", x=>x.entries[0].attempts=Array(NOTE_ATTEMPT_LIMIT+1).fill(x.entries[0].attempts[0])],
  ["empty prediction and branch", x=>{x.entries[0].attempts[0].prediction=" "; x.entries[0].attempts[0].alternativeId=null;}],
];
for (const [name, change] of invalid) test(`refuses ${name} without replacing the accepted notebook`, () => {
  const doc=one(), copy=JSON.parse(JSON.stringify(doc)); change(copy);
...
test("getter import cannot execute code", () => {
  const doc={...one()}; let calls=0;
  Object.defineProperty(doc,"entries",{enumerable:true,get(){calls++;return[];}});
  assert.throws(()=>validateNotebook(doc,manifest)); assert.equal(calls,0);
});
test("HTML-like notes remain inert text and survive interchange", () => {
  ...
  assert.equal(decodeNotebook(encodeNotebook(doc,manifest),manifest).entries[0].attempts[0].prediction,text);
});
...
test("malformed, null, array and oversized JSON fail",()=>{
  for (const raw of ["{", "null", "[]", '"text"', " ".repeat(NOTE_FILE_LIMIT+1)]) assert.throws(()=>decodeNotebook(raw,manifest));
});
test("out-of-range observations do not create a hidden attempt",()=>{
  for (const index of [-1,1,NaN,Infinity,.5]) assert.throws(()=>recordObservation(one(),manifest,"subtract",index,"x"));
  assert.throws(()=>recordObservation(fresh(),manifest,"emit",0,"x"));
});
test("bounded history keeps all accepted prior attempts",()=>{
  let doc=fresh();
  for(let i=0;i<NOTE_ATTEMPT_LIMIT;i++) doc=recordPrediction(doc,manifest,"emit",String(i),null);
  assert.throws(()=>recordPrediction(doc,manifest,"emit","overflow",null));
  assert.deepEqual(doc.entries[0].attempts.map(a=>a.prediction),Array.from({length:NOTE_ATTEMPT_LIMIT},(_,i)=>String(i)));
});
test("local edit, newer import and clear invalidate an in-flight file read",()=>{
  const guard=createImportGuard(); const first=guard.begin(); assert.ok(guard.current(first));
  guard.invalidate(); assert.equal(guard.current(first),false);
  const second=guard.begin(),third=guard.begin(); assert.equal(guard.current(second),false); assert.ok(guard.current(third));
  guard.invalidate(); assert.equal(guard.current(third),false);
});
