...
const states = (container: HTMLElement) =>
  [...container.querySelectorAll("[data-low-speed-step]")].map((e) =>
    e.getAttribute("data-low-speed-state"),
  );
const math = (container: HTMLElement) =>
  [...container.querySelectorAll("math")].map((e) => e.outerHTML);
...
async function toggle(container: HTMLElement, id: string) {
  await act(async () => {
    container.querySelector<HTMLInputElement>(`[data-low-speed-premise="${id}"]`)!.click();
  });
}
...
test("removing the inertia premise leaves the mathematical limit supported and the equations unchanged", async () => {
  await mounted(<LowSpeedExplorer proof={proof} restoreSettings={false} />, async (container) => {
    ...
    expect(container.querySelector("[data-low-speed-status]")?.textContent).toContain(
      "not established",
    );
    expect(container.querySelector("[data-low-speed-share]")?.getAttribute("href")).toContain(
      "mel-off=inertia",
    );
    await toggle(container, "inertia");
...
test("removing the offset premise blocks the kinetic interpretation but cannot erase the pure limit", async () => {
  await mounted(<LowSpeedExplorer proof={proof} restoreSettings={false} />, async (container) => {
    const before = math(container);
    await toggle(container, "offset");
    expect(states(container)).toEqual(["supported", "blocked", "blocked", "supported", "blocked"]);
    expect(math(container)).toEqual(before);
    expect(container.querySelector('[data-low-speed-step="kinetic"]')?.textContent).toContain(
      "same offset",
    );
  });
});
...
test("retained order changes the real formula and omitted term, never the limit or premises", async () => {
  await mounted(<LowSpeedExplorer proof={proof} restoreSettings={false} />, async (container) => {
    const before = math(container),
      select = container.querySelector<HTMLSelectElement>("[data-low-speed-order]")!;
    await act(async () => {
      select.value = "6";
...
test("shared settings open the disclosure, restore only the opted-in instance, and keep distinct controls", async () => {
  ...
  await mounted(
    <>
      <MassEnergyLowSpeed />
      <LowSpeedExplorer proof={proof} restoreSettings={false} />
    </>,
    async (container) => {
      ...
      expect(container.querySelector<HTMLDetailsElement>("[data-low-speed-disclosure]")!.open).toBe(
        true,
      );
      expect(instances[0]!.querySelector<HTMLSelectElement>("[data-low-speed-order]")!.value).toBe(
        "4",
      );
      expect(instances[1]!.querySelector<HTMLSelectElement>("[data-low-speed-order]")!.value).toBe(
        "2",
      );
...
test("an ambiguous shared order displays a notice without inventing a selected state", async () => {
  window.history.replaceState(
    {},
    "",
    "/papers/mass-energy/?mel=1&mel-order=2&mel-order=6&mel-off=offset",
  );
