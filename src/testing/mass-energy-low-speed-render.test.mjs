...
const generated=JSON.parse(await readFile(new URL("../generated/mass-energy-low-speed.json",import.meta.url),"utf8"));

test("generated low-speed content is the actual checked catalogue and real KaTeX rendering",()=>{
  const checked=buildMassEnergyLowSpeed(records), rendered=renderLowSpeedProof(checked,records.map(compileEquation));
  ...
  assert.deepEqual(view.approximations.map(a=>a.order),[2,4,6]);
  assert.deepEqual(view.approximations.map(a=>a.omittedCoefficient),["3/8","5/16","35/128"]);
  for(const formula of [...view.equations,...view.approximations.map(a=>a.formula),view.limit,view.factorization]) {
...
test("the renderer refuses a missing canonical source instead of emitting a partial proof",()=>{
  const checked=buildMassEnergyLowSpeed(records), compiled=records.map(compileEquation);
  for(const id of checked.equationIds) assert.throws(()=>renderLowSpeedProof(checked,compiled.filter(e=>e.id!==id)));
});
