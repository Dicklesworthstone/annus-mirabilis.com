...
import { toTracerDraft, fromTracerDraft } from '../experiments/bm01/controls.ts';
import { encodeBm01Settings, decodeBm01Settings } from '../experiments/bm01/permalink.ts';
...
test('tracer controls preserve SI physics and all 64 seed bits exactly',()=>{
 for(const seed of ['0','9007199254740992','9007199254740993','18446744073709551615']) {
  const p={...BM01_DEFAULTS,seed};assert.deepEqual(fromTracerDraft(toTracerDraft(p)),p);
  const draft=toTracerDraft(p);draft.interval='4';assert.deepEqual(fromTracerDraft(draft),{...p,interval:4});
 }
});
...
test('links refuse ambiguous, incomplete, nonfinite, and lossy seed data',()=>{
 ...
 for(const invalid of [q+'&seed=42',q+'&extra=1',q.replace('tracers=1','tracers=2'),q.replace('seed=1905','seed=01'),q.replace('seed=1905','seed=1e5'),q.replace('T=293.15','T=1e400'),q.replace('&H=10',''),'?'+ 'x'.repeat(4096)]) assert.equal(decodeBm01Settings(invalid).kind,'invalid',invalid);
...
test('typed controls refuse malformed numbers and do not clamp observation times',()=>{
 const d=toTracerDraft(BM01_DEFAULTS);d.T='abc';assert.throws(()=>fromTracerDraft(d));
 ...
 assert.equal(validateBm01Parameters({...BM01_DEFAULTS,interval:.015}).kind,'accepted'); // The recording owner supplies its grid-specific refusal.
});
