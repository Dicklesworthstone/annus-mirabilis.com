...
import { checkExerciseAnswer, deriveExerciseSeed, exerciseDefinitionKey, snapshotExercise } from '../discovery/exercises/answer.ts';

const part = () => ({id:'identity',prompt:'Rewrite this expression.',declaredNames:['x','pi'],domains:{x:{min:0,max:1}},referenceSource:'x^2',tolerance:{absolute:1e-10,relative:1e-10},workedExplanation:'Multiply x by x.'});
...
test('SHA-256 seed matches the independent Node crypto digest and retains all 64 bits',async()=>{
 ...
 assert.equal(await deriveExerciseSeed(key),createHash('sha256').update(key).digest().readBigUInt64BE(0).toString());
});
test('sampling identity is stable under declaration order, and changes with exercise id or reference',async()=>{
 const p=part();const key=exerciseDefinitionKey(p);
 assert.equal(key,exerciseDefinitionKey({...p,declaredNames:['pi','x']}));
 assert.notEqual(await deriveExerciseSeed(key),await deriveExerciseSeed(exerciseDefinitionKey({...p,id:'another'})));
...
test('Brownian rearrangement passes without a stored answer-string comparison',async()=>{
 const p={...part(),declaredNames:['D','t'],domains:{D:{min:1e-14,max:1e-10,scale:'log'},t:{min:.1,max:100,scale:'log'}},referenceSource:'2*sqrt(D*t)'};
...
test('an in-flight check retains the submitted definition despite caller mutation',async()=>{
 const p=part();const task=checkExerciseAnswer(p,'x*x');
 p.referenceSource='x';p.domains.x.max=2;p.declaredNames.push('evil');
...
for(const patch of [{declaredNames:['x','x']},{declaredNames:['x','y']},{declaredNames:['sqrt']},{domains:{x:{min:1,max:0}}},{tolerance:{}},{tolerance:{absolute:null,relative:0.1}},{referenceSource:'unbound'},{id:''},{workedExplanation:''}])
...
test('exercise data accessors are rejected without execution',()=>{
 const p=part();let called=false;
 Object.defineProperty(p,'referenceSource',{enumerable:true,get(){called=true;throw Error('called')}});
 assert.throws(()=>snapshotExercise(p));assert.equal(called,false);
});
test('constant questions need no invented variable range',async()=>{
 const p={...part(),referenceSource:'2+3',declaredNames:[],domains:{}};
 const result=await checkExerciseAnswer(p,'5');assert.equal(result.outcome.status,'equivalent');assert.equal(result.outcome.acceptedPointCount,1);
});
test('snapshot cannot be changed after validation',()=>{
 const p=snapshotExercise(part());assert.throws(()=>{p.domains.x.max=9});assert.throws(()=>p.declaredNames.push('new'));
});
