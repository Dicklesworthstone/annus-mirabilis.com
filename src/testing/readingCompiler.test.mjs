...
const f = (id, prerequisites = []) => ({
  ...
  summary: "Summary",
  review: "draft",
  explanation: [{ kind: "paragraph", text: "Explanation." }],
  example: [{ kind: "paragraph", text: "Example." }],
  prerequisites,
  stoppingPoint: "Count and compare.",
  citations: ["primary"],
});
const file = (r) => ({
  path: `${r.kind === "citation" ? "bibliography" : "foundations"}/${r.id}.json`,
  text: JSON.stringify(r),
});
test("JSON parsing preserves values but rejects duplicate escaped keys, malformed syntax and pollution", () => {
  const x = parseContentJson('{"a":[true,null,-1.25e2,"ü"]}');
  assert.deepEqual(x.a, [true, null, -125, "ü"]);
  for (const x of [
    '{"a":1,"\\u0061":2}',
    '{"__proto__":{}}',
    "[1,]",
    ...
    assert.throws(() => parseContentJson(x));
});
test("loader rejects non-NFC, oversized and deeply nested records", () => {
  assert.throws(() => parseContentJson('"e\u0301"'), /NFC/);
  assert.throws(() => parseContentJson(" ".repeat(524289)), /512 KiB/);
  assert.throws(() => parseContentJson(`${"[".repeat(42)}0${"]".repeat(42)}`), /nesting/);
});
test("content schemas are closed and cannot claim a reviewed source edition", () => {
  ...
  for (const changed of [
    ...
    { ...citation, extra: 1 },
    { ...f("a"), review: "reviewed" },
    { ...f("a"), title: "<script>x</script>" },
  ])
    assert.throws(() => validateReadingRecord(changed, "x"));
});
test("math allowlist refuses link, HTML, macro definitions and executable commands", () => {
  ...
  for (const tex of [
    ...
    "\\def\\x{z}",
    "\\includegraphics{a}",
    "\\begin{evil}x\\end{evil}",
  ])
    assert.throws(() => validateMath(tex));
});
test("compiler aggregates independent errors instead of admitting partial output", () => {
  const result = compileReadingContent([
    { path: "bad.yaml", text: "x" },
    file({ ...citation, url: "http://example.org" }),
    file(f("a", ["absent"])),
  ]);
  assert.equal(result.ok, false);
  assert.equal(result.papers.length, 0);
  assert.ok(result.diagnostics.filter((x) => x.severity === "error").length >= 3);
});
test("foundations resolve prerequisite references and reject cycles", () => {
  ...
  assert.ok(
    compileReadingContent([
      ...
      file(f("a", ["b"])),
      file(f("b", ["a"])),
    ]).diagnostics.some((d) => d.code === "prerequisite-cycle"),
  );
});
test("compiler rejects path/id mismatches and repeated file inputs", () => {
  assert.equal(
    compileReadingContent([{ ...file(citation), path: "bibliography/other.json" }]).ok,
    false,
  );
  assert.ok(
    compileReadingContent([file(citation), file(citation)]).diagnostics.some(
      (d) => d.code === "duplicate-path",
    ),
  );
});
test("a genuinely unrouted path still fails, including a misplaced quantities-like path (AGENTS.md: unrouted-content is load-bearing)", () => {
  assert.ok(
    compileReadingContent([{ path: "bad.yaml", text: "x" }]).diagnostics.some(
      (d) => d.code === "unrouted-content",
    ),
  );
  assert.ok(
    compileReadingContent([{ path: "quantitites/typo.yaml", text: "- id: x" }]).diagnostics.some(
      (d) => d.code === "unrouted-content",
    ),
  );
  assert.ok(
    compileReadingContent([
      { path: "quantities/constant-sets/modern.yaml", text: "- id: x" },
    ]).diagnostics.some((d) => d.code === "unrouted-content"),
  );
});
test("quantities/*.yaml routes through the quantity schema and validates", () => {
  const good = [
    {
      path: "quantities/generic.yaml",
      text: "- id: exampleLength\n  name: Example length\n  description: A test quantity.\n  dimensionStatus: declared\n  dimension: [1, 0, 0, 0, 0, 0]\n  mathematicalKind: scalar\n  frame: not-applicable\n",
    },
  ];
...
test("duplicate quantity ids across files are reported, not silently overwritten", () => {
  const files = [
    {
      path: "quantities/a.yaml",
      text: "- id: sharedLength\n  name: Shared length\n  description: First declaration.\n  dimensionStatus: declared\n  dimension: [1, 0, 0, 0, 0, 0]\n  mathematicalKind: scalar\n  frame: not-applicable\n",
    },
    {
      path: "quantities/b.yaml",
  ...
  assert.equal(result.ok, false);
  assert.ok(
    result.diagnostics.some((d) => d.code === "duplicate-id" && d.message.includes("sharedLength")),
  );
});
