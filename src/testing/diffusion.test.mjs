...
import { getConstantSet, createDeclaredConstantSet, constantValue, thermalConstant, assertSameSet, RESERVED_SET_IDS } from "../physics/reference/constants.ts";
import { erf, erfc } from "../physics/reference/special/erf.ts";
import { stokesEinsteinD, rmsDisplacement, apparentSpeed, gaussianPropagator, intervalProbability, radialPropagator2d, radialPropagator3d, moments, osmoticPressure } from "../physics/reference/diffusion/distributions.ts";
...
const near = (actual, expected, relative = 1e-12, absolute = 0) => assert.ok(Math.abs(actual - expected) <= Math.max(absolute, relative * Math.abs(expected)), `${actual} != ${expected}`);
const modern = getConstantSet("modern-si-2019");
const entry = (quantityId, exactDecimal, unit, dependsOn = []) => ({ quantityId, exactDecimal, unit, value: Number(exactDecimal), kind: "declared-scenario", evidentialRole: "measured-observation", provenance: "Explicit numerical test inputs from am-ref-diffusion-lr3; not a checked transcription.", dependsOn });
const draft = {
  id: "scenario-brownian-plan-inputs", era: 1905, provenance: "Numerical reconstruction of the plan's declared inputs; no historical verification claimed.", precisionNote: "Binary64 evaluation of declared decimal values.", gasConstantProvenance: "measured-without-counting-molecules",
  entries: [entry("molarGasConstant", "8.31", "J/(mol K)"), entry("avogadroConstant", "6e23", "1/mol")],
};
test("modern exact constants and their dependency remain in one immutable set", () => {
  ...
  assert.throws(() => { modern.entries[0].value = 1; });
...
test("unverified historical and reserved sets cannot masquerade as available", () => {
  for (const id of ["einstein-1905-brownian-printed", ...Object.keys(RESERVED_SET_IDS)]) assert.throws(() => getConstantSet(id), e => e.code === "constant-set-not-registered");
  assert.throws(() => getConstantSet("unknown"), e => e.code === "unknown-constant-set");
});
test("declared scenarios retain provenance and refuse circular illustrative inputs", () => {
  ...
  assert.throws(() => assertSameSet(thermalConstant(declared), thermalConstant(modern)), e => e.code === "constant-set-mismatch");
  assert.throws(() => createDeclaredConstantSet({ ...draft, id: "einstein-1905-brownian-printed" }));
  assert.throws(() => createDeclaredConstantSet({ ...draft, entries: [draft.entries[0], draft.entries[0]] }));
  assert.throws(() => createDeclaredConstantSet({ ...draft, entries: [{ ...draft.entries[0], exactDecimal: "9" }] }));
  assert.throws(() => createDeclaredConstantSet({ ...draft, gasConstantProvenance: "defined" }));
  const illustration = createDeclaredConstantSet({ ...draft, entries: [{ ...draft.entries[0], evidentialRole: "illustrative-computation", dependsOn: ["temperature", "viscosity", "particleRadius"] }] });
  assert.throws(() => constantValue(illustration, "molarGasConstant"), e => e.code === "illustrative-value-as-input");
});
const fixture = JSON.parse(readFileSync(new URL("../physics/reference/special/erf.table.json", import.meta.url), "utf8"));
test("independent erf table has the recorded content digest", () => {
  assert.equal(createHash("sha256").update(JSON.stringify(fixture.rows)).digest("hex"), fixture.rowsSha256);
});
for (const row of fixture.rows) test(`erf/erfc independent 80-digit fixture x=${row.x}`, () => {
...
test("erf utilities reject nonfinite arguments", () => {
  for (const x of [NaN, Infinity, -Infinity]) { assert.throws(() => erf(x)); assert.throws(() => erfc(x)); }
});
test("modern Brownian golden scenario and scaling identities", () => {
  ...
  near(rms, 9.267573194261391458407590e-7, 1e-14);
...
test("declared 1905-plan inputs reproduce the arithmetic without claiming transcription", () => {
  ...
  near(val(rmsDisplacement(val(D), 60)), 6.156364840e-6, 1e-10);
...
test("gas, nonpositive physical inputs, and nonfinite parameters are explicit non-values", () => {
  for (const bad of [0, -1, Infinity, NaN]) {
    assert.equal(stokesEinsteinD({ T: bad, eta: 0.001, a: 0.5e-6 }, modern).result.status, "outside-domain");
    assert.equal(stokesEinsteinD({ T: 293.15, eta: bad, a: 0.5e-6 }, modern).result.status, "outside-domain");
    assert.equal(stokesEinsteinD({ T: 293.15, eta: 0.001, a: bad }, modern).result.status, "outside-domain");
  }
...
test("point distributions include closed endpoints, not an infinitely tall Gaussian", () => {
  for (const [D, t] of [[1, 0], [0, 1], [0, 0]]) {
    for (const f of [gaussianPropagator, radialPropagator2d, radialPropagator3d]) {
      const result = f(0, t, D).result;
      assert.equal(result.status, "analytic-limit"); decodeResult(result);
    }
    for (const [lo, hi, expected] of [[-1, 1, 1], [0, 1, 1], [-1, 0, 1], [0, 0, 1], [0.5, 1, 0], [-2, -1, 0]]) assert.equal(val(intervalProbability(lo, hi, t, D)), expected);
  }
...
test("interval probability uses tail-safe and narrow-interval calculations", () => {
  ...
  const a = 8, b = 8 + 1e-10;
  near(val(intervalProbability(a, b, 1, 0.5)), Math.exp(-32) / Math.sqrt(2 * Math.PI) * (b - a), 1e-9);
...
test("1D, 2D radial, and 3D radial densities normalize independently", () => {
  near(integrate(x => val(gaussianPropagator(x, 1, 0.5)), -10, 10), 1, 1e-12);
  for (const [d, density] of [[2, radialPropagator2d], [3, radialPropagator3d]]) {
    const stats = moments(d, 0.5, 1);
    near(integrate(r => val(density(r, 1, 0.5)), 0, 10), 1, 1e-12);
    near(integrate(r => r * val(density(r, 1, 0.5)), 0, 10), val(stats.meanRadius), 1e-12);
    near(integrate(r => r * r * val(density(r, 1, 0.5)), 0, 10), val(stats.total), 1e-12);
