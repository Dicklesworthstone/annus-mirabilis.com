...
test("static HTML contains every complete worked table, the bridge and real routes", () => {
  ...
  try {
    ...
    expect(container.textContent).toContain(
      "Counting tokens is not evidence that light is made of dots.",
    );
...
test("changing the shared-choice model updates table, live total and exact lab handoff together", async () => {
  ...
  try {
    await act(async () => {
      root.render(<LightQuantaFirstEncounter record={record} />);
    });
    ...
    const next = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent === "Next arrangement",
    )!;
    await act(async () => {
      next.click();
    });
    expect(container.querySelector("svg")?.getAttribute("aria-label")).toContain(
      "Token 1 in the right part",
    );
