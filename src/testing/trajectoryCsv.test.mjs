...
import { parseTrajectoryCsv as parse, trajectorySiCsv, TrajectoryImportError, TRAJECTORY_LIMITS } from "../experiments/bm07/trajectoryCsv.ts";

const units = { time: "s", position: "um" };
const equal = (a, b) => assert.ok(Math.abs(a - b) <= Math.max(Math.abs(b), Number.MIN_VALUE) * 1e-12, `${a} != ${b}`);
const reject = (csv, fragment, calibration = units) => assert.throws(() => parse(csv, calibration), (e) => e instanceof TrajectoryImportError && e.message.includes(fragment));

test("1D import converts explicitly declared units and freezes accepted data", () => {
  const t = parse("time,x\n0,0\n1,2\n2,1", units);
  assert.equal(t.dimension, 1); assert.equal(t.dt, 1); assert.equal(t.incrementCount, 2);
  equal(t.increments[0], 2e-6); equal(t.increments[1], -1e-6);
  assert.equal(t.kind, "user-supplied-trajectory");
  for (const value of [t, t.units, t.points, t.points[0], t.points[0].coordinates, t.increments]) assert.equal(Object.isFrozen(value), true);
});
test("reordered columns, BOM, CRLF, decimal exponents and 3D are explicit", () => {
  const t = parse('\uFEFFz,track,time,y,x\r\n3,A,0,2,1\r\n6,A,5e2,4,2\r\n', { time: "ms", position: "nm" });
  assert.equal(t.dimension, 3); equal(t.dt, .5); equal(t.increments[2], 3e-9);
});
test("interleaved tracks never create cross-particle displacements", () => {
  const t = parse('track,time,x,y\na,0,0,0\nb,0,100,200\na,1,1,2\nb,1,103,204', units);
  assert.equal(t.trackCount, 2); assert.equal(t.incrementCount, 2);
...
test("pixel calibration is explicit and squared scaling is left to the owner", () => {
  ...
  const t = parse(text, { time: "ms", position: "px", micrometresPerPixel: .4 });
  equal(t.dt, .01); equal(t.increments[0], 1.2e-6);
});
for (const value of [0, -1, Infinity, NaN, Number.MIN_VALUE]) test(`reject invalid pixel calibration ${value}`, () => {
  assert.throws(() => parse('time,x\n0,0\n1,1', {time:"s",position:"px",micrometresPerPixel:value}), TrajectoryImportError);
});
for (const value of ["", "NaN", "Infinity", "0x10", "1_000", "=1+2", "1e309", "1e-999"]) test(`reject invalid numeric field ${JSON.stringify(value)}`, () => {
  assert.throws(() => parse(`time,x\n0,0\n1,${value}`, units), TrajectoryImportError);
});
test("do not sort duplicate or decreasing timestamps", () => {
  for (const times of [[0,0], [1,0]]) reject(`time,x\n${times[0]},0\n${times[1]},1`, "strictly increase");
});
test("irregular timing is retained for inspection but cannot receive a common dt", () => {
  const t = parse('time,x\n0,0\n1,1\n3,2', units);
  assert.equal(t.dt, null); assert.match(t.timingIssue, /sampling interval/); assert.equal(t.points.length, 3);
});
...
test("ordinary decimal time roundoff is not a false irregularity", () => {
  const t = parse('time,x\n0,0\n.1,1\n.2,2\n.3,3', units); equal(t.dt, .1);
});
test("unresolved absolute epochs are not declared exactly spaced", () => {
  const t = parse('time,x\n1000000000000,0\n1000000000001,1', units);
  assert.equal(t.dt, null); assert.match(t.timingIssue, /elapsed times/);
});
test("single-point tracks are never silently dropped", () => reject('track,time,x\na,0,0\nb,0,1\nb,1,2', "only one position"));
test("stationary coordinates and zero numeric exponents are not missing data", () => {
  const t = parse('time,x\n0,0e-999\n1,-0.000', units); assert.equal(t.increments[0], 0);
});
