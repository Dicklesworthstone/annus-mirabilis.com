...
} from "../physics/reference/radiation/entropy.ts";
...
for (const [name, compile] of [
  ...
  test(`${name}: the introduction and all nine sections compile without claiming a reviewed edition`, async () => {
    ...
    assert.deepEqual(
      used,
      Array.from({ length: 9 }, (_, i) => `lq-0${i + 1}`),
    );
  ...
  for (const [label, change, code] of [
    ...
    [
      "missing foundation",
      (a) => ({ ...a, help: { ...a.help, why: "not-a-foundation" } }),
...
test("the coefficient correspondence has a distinct heuristic role and rejects an invented role", () => {
  ...
  assert.throws(() =>
    validateReadingRecord(
      { ...record, meaning: { ...record.meaning, logicalRole: "empirically-proven" } },
      path,
    ),
  );
});
...
test("all laboratory links from the existing light-quanta pages land on a compiled section", async () => {
  ...
  const sections = new Set(
    result.papers.find((p) => p.paper.id === "light-quanta").paper.sections.map((s) => s.id),
  );
  for (let i = 1; i <= 9; i++) {
