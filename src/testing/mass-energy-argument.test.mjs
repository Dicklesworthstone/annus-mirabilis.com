...
import {
  ARGUMENT_STEPS, CONSISTENCY_ARGUMENT, WORKED_ARGUMENT, argumentStep, assessArgument,
  readArgumentOrder, encodeArgument, decodeArgument, argumentExport,
...
test("the reconstruction graph has unique, resolvable, acyclic dependencies", () => {
  ...
  for (const step of ARGUMENT_STEPS) {
    ...
    assert.ok(Object.isFrozen(step)); assert.ok(Object.isFrozen(step.requires));
  }
});
...
for (const omitted of WORKED_ARGUMENT) test(`omitting ${omitted} cannot complete the derivation`, () => {
  assert.notEqual(assessArgument(WORKED_ARGUMENT.filter(id => id !== omitted)).outcome, "inertia-derived");
});

test("independent premises and balances admit multiple topological orders", () => {
  const order = [...WORKED_ARGUMENT];
  const early = ["inertial-coefficient", "kinetic-offset", "unchanged-offset", "light-transform", "conservation", "opposite-pulses"];
...
test("a later premise does not retroactively justify an earlier operation", () => {
  const order = [...WORKED_ARGUMENT];
  order.splice(order.indexOf("conservation"), 1); order.push("conservation");
...
for (const raw of [null, {}, "inertia-loss", ["unknown"], ["conservation", "conservation"], [1], ["__proto__"], new Array(17).fill("conservation")]) {
  test(`invalid imported order is refused: ${JSON.stringify(raw)}`, () => assert.throws(() => assessArgument(raw)));
}

test("orders, assessments and the deck are immutable copies", () => {
  const raw = ["conservation"];
  const order = readArgumentOrder(raw); raw.push("opposite-pulses");
  assert.deepEqual(order, ["conservation"]);
  const a = assessArgument(order);
  for (const value of [ARGUMENT_STEPS, WORKED_ARGUMENT, order, a, a.steps, a.steps[0], a.steps[0].missing, a.available]) assert.ok(Object.isFrozen(value));
});
...
test("saved links preserve exactly the selection order, including a blocked order", () => {
  for (const order of [[], WORKED_ARGUMENT, CONSISTENCY_ARGUMENT, [...WORKED_ARGUMENT].reverse()]) {
    ...
    assert.equal(decoded.kind, "argument"); assert.deepEqual(decoded.order, order);
    assert.deepEqual(assessArgument(decoded.order), assessArgument(order));
  }
});

for (const query of ["?proof=2&steps=", "?proof=1", "?steps=conservation", "?proof=1&proof=1&steps=", "?proof=1&steps=&steps=conservation", "?proof=1&steps=unknown", "?proof=1&steps=conservation,conservation", "?proof=1&steps=conservation,", "?proof=1&steps=constructor", `?proof=1&steps=${"x".repeat(4096)}`]) {
  test(`malformed or unsupported shared state is explicit: ${query.slice(0, 70)}`, () => assert.equal(decodeArgument(query).kind, "invalid"));
}
...
test("JSON export recomputes feedback and preserves notes without executable content", () => {
  ...
  assert.equal(exported.version, 1); assert.equal(exported.note, note);
...
test("historical imports, model limits and sign conventions are explicit", () => {
  assert.match(argumentStep("light-transform").explanation, /not knowledge supplied by a 1904 shelf/);
  assert.match(argumentStep("small-speed").explanation, /not be presented as the exact/);
  assert.match(argumentStep("inertial-coefficient").explanation, /not division by v² at exactly zero/);
