...
const box = {
  ...
  physical_input_provenance: "Synthetic test premise: complete joint box, not empirical evidence.",
};
...
test("old CSV observations need no uncertainty fields and never acquire assumed coverage", () => {
  const d = parseKitchenCsv(kitchenFixture()),
    a = analyze(d);
...
test("a complete joint box allocates a fresh camera interval, not a relabeled conditional one", () => {
  ...
  near(a.uncertainty.cameraCoverage, 0.975);
  assert.ok(a.uncertainty.combinedCoverage >= 0.95);
  assert.ok(a.uncertainty.combinedCoverage <= a.uncertainty.inputCoverage);
  const conditional = value(a, "diffusionInterval"),
    allocated = value(a, "combinedSamplingInterval");
  assert.ok(allocated[0] < conditional[0]);
  assert.ok(allocated[1] > conditional[1]);
  const sensitivity = value(a, "molecularInputRange"),
    combined = value(a, "combinedMolecularInterval");
  assert.ok(combined[0] < sensitivity[0]);
  assert.ok(combined[1] > sensitivity[1]);
  assert.equal(a.numberMeaning, "synthetic-recovery");
  assert.equal(JSON.stringify(d), before);
});

for (const coverage of ["", ".9", ".95"])
  test(`joint coverage '${coverage}' retains range sensitivity without a confidence claim`, () => {
...
test("input ranges change neither the pair selection nor any existing point or conditional estimate", () => {
  const d = doc(),
    a = analyze(d),
    b = analyze(doc({ physical_input_coverage: "", pixels_per_um_x_interval: "[8,12]" }));
...
test("same-ruler radius introduces precisely one additional inverse scale factor", () => {
  ...
  near(value(shared, "molecularInputRange")[0], value(independent, "molecularInputRange")[0] * 0.9);
...
for (const patch of [
  ...
  { radius_scale_axis: "x", pixel_aspect_ratio: "" },
  { radius_provenance: "same-displacements" },
])
...
test("uncertainty of a derived axis is not inferred from a point aspect ratio", () => {
  const a = analyze(
    doc({ calibration_axes: "x", pixels_per_um_y: "", pixels_per_um_y_interval: "" }),
    { axis: "y" },
  );
  assert.equal(a.scaleSource, "derived");
  assert.equal(a.uncertainty.state, "unavailable");
...
test("gas-source choices retain modern consistency versus independent experimental interpretation", () => {
  ...
  const modern = analyze(doc({ gas_constant_interval: "", data_origin: "reader-supplied" }), {
    constantSet: "modern-si-2019",
  });
  assert.equal(modern.uncertainty.state, "combined");
...
for (const patch of [
  ...
  { physical_input_coverage: "2" },
  { pixels_per_um_x_interval: "[11,12]" },
  { viscosity_interval_mpa_s: "[1,0]" },
  { radius_scale_axis: "__proto__" },
  { gas_constant_interval: "[0,9]" },
])
  test(`malformed or unsupported declaration is refused at CSV admission: ${JSON.stringify(patch)}`, () =>
    assert.throws(() => doc(patch)));

for (const defect of ["edge", "excluded", "missing", "irregular"])
  test(`even one ${defect} defect withholds combined coverage`, () => {
    const d = doc(),
      points = d.points.map((p, i) =>
        i !== 0
          ? p
          : defect === "edge"
            ? { ...p, status: "lost", lossReason: "edge", x: null, y: null }
            : defect === "excluded"
              ? { ...p, status: "excluded", exclusionReason: "reader selected this point" }
              : defect === "irregular"
                ? { ...p, time: 0.01 }
                : p,
      );
...
test("zero-containing and empty camera sets cannot produce finite combined molecular bounds", () => {
  ...
  for (const amplitude of [6, 20]) {
    const points = d.points.map((p, i) =>
      p.kind === "stationary" ? { ...p, x: 300 + (i % 2 ? amplitude : -amplitude) } : p,
    );
...
test("the new declarations are editable and CSV-roundtrip without changing observations or provenance", () => {
  const d = doc(),
    draft = kitchenInputDraft(d);
  for (const key of KITCHEN_UNCERTAINTY_KEYS) assert.equal(draft[key], d.metadata[key]);
  const next = reviseKitchenInputs(d, {
    ...draft,
    physical_input_coverage: "",
    radius_scale_axis: "x",
  });
  assert.deepEqual(next.points, d.points);
  assert.deepEqual(parseKitchenCsv(exportKitchenCsv(next)), next);
...
async function localSession(t, transform = (x) => x) {
  ...
  const host = createKitchenHost((message) => {
    ...
    listener(transform(structuredClone(message)));
  }, source);
  ...
  const wait = (predicate) => {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        off();
        reject(new Error(session.getSnapshot().message || "timeout"));
      }, 3000);
  ...
  return {
    ...
    source,
    getRequest: () => request,
    getResponse: () => structuredClone(response),
  };
}

test("worker, immutable accepted snapshot and receipt retain the same uncertainty allocation", async (t) => {
  const f = await localSession(t);
  assert.equal(f.state.view.status, "accepted", f.state.message);
  const accepted = f.state.accepted,
    receiptText = kitchenAnalysisJson(accepted, f.source),
  ...
  assert.equal(
    receipt.results.find((o) => o.quantityId === "combinedMolecularInterval").status,
    "value",
  );
  ...
  const next = (await f.wait((s) => s.accepted !== accepted && s.view.status === "accepted"))
    .accepted;
  assert.equal(next.report.uncertainty.state, "sensitivity");
  assert.equal(next.sourceId, accepted.sourceId);
  assert.notEqual(next.documentDigest, accepted.documentDigest);
  assert.equal(next.snapshot.runId, accepted.snapshot.runId);
  assert.deepEqual(next.document.points, accepted.document.points);
  await f.session.submit("bad csv");
  await f.wait((s) => s.view.status === "refused");
  assert.equal(f.session.getSnapshot().accepted, next);
  assert.equal(kitchenAnalysisJson(accepted, f.source), receiptText);
});

test("the response decoder rejects unsupported coverage, lost evidence and malformed envelopes", async (t) => {
  const f = await localSession(t);
  assert.equal(f.state.view.status, "accepted");
  for (const mutate of [
    (a) => (a.uncertainty.combinedCoverage = 0.99),
    (a) => (a.uncertainty.cameraCoverage = 0.95),
    ...
    (a) =>
      (a.outputs.find((o) => o.quantityId === "combinedMolecularInterval").value =
        Float64Array.of(1)),
    (a) =>
      (a.outputs.find((o) => o.quantityId === "combinedMolecularInterval").value = Float64Array.of(
        5,
        2,
      )),
    (a) =>
      (a.outputs.find((o) => o.quantityId === "combinedSamplingInterval").value = Float64Array.of(
        0,
        2,
      )),
    ...
    const forged = f.getResponse();
    mutate(forged.result.analysis);
