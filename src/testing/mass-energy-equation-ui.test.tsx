...
import { CoefficientComparison, CoefficientLab } from "../components/lab/CoefficientLab.tsx";
...
test("reader disclosures contain every equation exactly once, scoped to its own argument", () => {
  ...
  expect(
    renderToStaticMarkup(<ArgumentEquations paperId="light-quanta" argumentId={args[0]!} />),
  ).toBe("");
});
...
test("the static normalized laboratory cannot print its values as SI and never hides the worked answer", () => {
  const shell = document.createElement("div");
  shell.innerHTML = renderToStaticMarkup(
    <CoefficientLab example={example} equations={equations} />,
  );
  ...
  expect(
    shell.querySelectorAll('[data-coefficient-equations] [data-value-kind="value"]').length,
  ).toBe(0);
});
...
test("standalone settings restore into the same accepted equation snapshot; embedded labs stay independent", async () => {
  ...
  await mounted(
    <>
      <CoefficientComparison example={example} equations={equations} />
      <CoefficientLab example={example} equations={equations} />
    </>,
    async (container) => {
      ...
      expect(labs[1]!.querySelector(dropSelector)?.getAttribute("data-value-kind")).toBe(
        "symbolic",
      );
      const identities = [...labs].map((lab) =>
        lab.querySelector("[data-equation-values]")?.getAttribute("data-instance-id"),
      );
...
test("term selection opens the exact prerequisite without requiring pointer-only math interaction", async () => {
  await mounted(
    <ArgumentEquations paperId="mass-energy" argumentId="arg-me-constant-premise" />,
    async (container) => {
      const chip = container.querySelector<HTMLButtonElement>(
        '[data-node-id="eq-model-me-exact-drop.t.kineticEnergyDifference"]',
      )!;
      ...
      const inspector = container.querySelector(
        '[data-inspector-node="eq-model-me-exact-drop.t.kineticEnergyDifference"]',
      );
      expect(inspector).not.toBeNull();
      expect(inspector?.textContent).toContain("before minus after");
      expect(inspector?.querySelector("a[data-foundation]")?.getAttribute("href")).toBe(
        "/foundations/work-energy/",
      );
    },
  );
});
