...
import { TestLogger, newRunIdentity, suiteLogPath, artifactsRoot } from "./logger.ts";
...
test("writes three events and reads them back as valid JSON lines in order", async () => {
  ...
  assert.deepEqual(events.map((e) => e.testId), ["a", "b", "c"]);
...
test("seed must be a canonical decimal string, never a JSON number", () => {
  const logger = freshLogger();
  assert.throws(() => logger.log({ testId: "seed-numeric", seed: 42 as unknown as string }), LogSchemaError);
});

test("seed at 2^64 is rejected and 2^53+1 is accepted and read back unchanged", async () => {
  const logger = freshLogger();
  assert.throws(() => logger.log({ testId: "seed-too-big", seed: "18446744073709551616" }), LogSchemaError);
...
test("seed boundary neighbourhood: 0, 2^53 neighbours, and 2^64-1 all accepted", async () => {
  const logger = freshLogger();
  const seeds = ["0", "9007199254740991", "9007199254740992", "9007199254740993", "18446744073709551615"];
  for (const seed of seeds) logger.log({ testId: `seed-${seed}`, seed });
  await logger.flush();
  const events = readLines(logger).map(parseLogLine);
  assert.deepEqual(events.map((e) => e.seed), seeds);
});
...
test("seed rejects invalid sign, whitespace, leading zeros, and overlong input", () => {
  ...
  for (const seed of ["-1", " 5", "5 ", "01", "007", "1".repeat(21), "9".repeat(30)]) {
    assert.throws(() => logger.log({ testId: `seed-bad-${seed}`, seed }), LogSchemaError, `expected rejection for ${JSON.stringify(seed)}`);
  }
});
...
test("a runId shaped like a log/tool run id is rejected with the identity-confusion message", () => {
  const logger = freshLogger();
  assert.throws(
    () => logger.log({ testId: "run-id-confusion", runId: "20260914T120000Z-0a1b2c3d", instanceId: "bm01-instance-1" }),
    (error: unknown) => error instanceof LogSchemaError && error.message.includes("experiment realization"),
  );
});
...
test("an experiment runId without instanceId or instrumentId is rejected", () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof LogSchemaError && error.message.includes("instanceId") && error.message.includes("instrumentId"),
  );
});
...
test("logRunId and an experiment runId coexist and both read back unchanged", async () => {
  ...
  logger.log({ testId: "run-id-and-log-run-id", runId: "bm01-instance-1/run/3", instanceId: "bm01-instance-1" });
  await logger.flush();
  const [event] = readLines(logger).map(parseLogLine);
...
test("a script event uses toolRunId, and a malformed toolRunId is rejected", async () => {
  ...
  scriptLogger.log({ testId: "chunk-0003", toolRunId: "20260914T110000Z-1f2e3d4c", extra: { status: "completed" } });
  ...
  assert.throws(() => scriptLogger.log({ testId: "chunk-0004", toolRunId: "run-7" }), LogSchemaError);
});

test("browser journey fields (lane, journey, step) are accepted", async () => {
  const logger = freshLogger();
  logger.log({ testId: "journey-step", journey: "brownian-slice", lane: "desktop", step: "enter-deep-passage" });
  await logger.flush();
  const [event] = readLines(logger).map(parseLogLine);
...
test("a statistical sampling-band event carries no top-level expected, and is read back unchanged", async () => {
  ...
  logger.log({
    ...
    extra: { statistic: "chi-square", observedValue: 12.4, lowerBound: 8.1, upperBound: 16.9, alpha: 0.05, allocationId: "a1", n: 200 },
  });
  await logger.flush();
  const [event] = readLines(logger).map(parseLogLine);
  assert.equal(event!.expected, undefined);
  assert.deepEqual(event!.extra, { statistic: "chi-square", observedValue: 12.4, lowerBound: 8.1, upperBound: 16.9, alpha: 0.05, allocationId: "a1", n: 200 });
});
...
test("the same statistical event given a top-level expected without comparisonKind is rejected", () => {
  ...
  assert.throws(
    () => logger.log({
      ...
      expected: 12,
      extra: { statistic: "chi-square", observedValue: 12.4, lowerBound: 8.1, upperBound: 16.9, alpha: 0.05 },
    }),
    LogSchemaError,
  );
});
...
test("extra.freeTextAnswer is rejected; an overlong extra string is rejected; small serializable extra is accepted", async () => {
  const logger = freshLogger();
  assert.throws(() => logger.log({ testId: "reject-free-text", extra: { freeTextAnswer: "hello" } }), LogSchemaError);
  assert.throws(() => logger.log({ testId: "reject-overlong", extra: { digest: "x".repeat(2001) } }), LogSchemaError);
  logger.log({ testId: "accept-small-extra", extra: { key: "ok", check: "concordance-scope", nested: { count: 3 } } });
  await logger.flush();
  const [event] = readLines(logger).map(parseLogLine);
...
test("a failing browser event without a screenshot and DOM snapshot is rejected", () => {
  ...
  assert.doesNotThrow(() =>
    logger.log({
      ...
      browser: "chromium",
      outcome: "failed",
      evidence: { screenshot: "artifacts/test-logs/x/evidence/e.png", dom: "artifacts/test-logs/x/evidence/e.html" },
    }),
  );
});
