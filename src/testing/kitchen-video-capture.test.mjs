...
import { KITCHEN_OPTIONS, KITCHEN_VIDEO_LIMITS } from "../experiments/bm07/kitchen/definition.ts";
import {
  appendVideoPoint,
  calibrateVideoAxis,
  checkVideoFile,
...
const frame = (time = 0, requested = time, counter = time + 1, id = time + 1) =>
  take(frameStamp(requested, time, counter, 30, id, 60));
    ...
    for (const delta of [-1, 0, 1])
      s = take(
        appendVideoPoint(
          ...
          frame(),
          `${axis}-${mark}`,
          axis === "x" ? (mark === "a" ? 100 : 300) + delta : 50,
...
function observations() {
  let s = calibrated();
  for (let n = 0; n < 20; n++)
    s = take(appendVideoPoint(s, frame(), "stationary", 200 + (n % 2 ? 1 : -1), 100));
  for (let n = 0; n < 20; n++)
    s = take(appendVideoPoint(s, frame(n), "particle", 100 + [0, 20, 10, 40][n % 4], 50));
  return s;
}
...
test("file admission uses injectable limits without allocating a large fixture", () => {
  ...
  assert.equal(r.refusal.code, "invalid-parameter");
  assert.equal(r.refusal.details.measured, 9);
...
test("bounded canvas coordinates round-trip independently on both axes", () => {
  ...
  assert.equal(a.canvasWidth, 1920);
  assert.ok(a.canvasHeight <= 1920);
  const b = take(sourcePoint(a, 192, 93, 384, 186));
  assert.equal(b.x, 2048);
  assert.equal(b.y, 1365.5);
...
for (const fps of [0, NaN, 10001])
  test(`invalid frame rate ${fps}`, () =>
    assert.equal(startCapture(g(), fps, "1").kind, "refused"));
test("sampling is a declaration, not a timestamp repair", () => {
  ...
  assert.equal(f.time, 1.2);
  assert.equal(f.stamp.requestedTime, 1);
...
test("new video CSV uses the real camera inference and never supplies guessed physical inputs", () => {
  ...
  assert.equal(a.counts.retainedPairs, 10);
  assert.equal(a.counts.stationary, 20);
  ...
  assert.equal(doc.metadata.radius_um, "");
  assert.equal(doc.metadata.viscosity_mpa_s, "");
  assert.equal(doc.metadata.temperature_k, "");
  assert.equal(doc.metadata.pixel_aspect_ratio, "");
...
test("an omitted exposure is not silently an instantaneous camera", () => {
  ...
  assert.equal(
    a.outputs.find((o) => o.quantityId === "diffusionInterval").status,
    "not-applicable",
  );
...
test("losing a particle preserves the row and requires an identity decision", () => {
  ...
  const d = take(videoDocument(s));
  assert.equal(d.points.at(-1).identityDecision, "new-object");
});
...
test("old classroom observations keep their twelve-column contract", () => {
  ...
  const csv = exportKitchenCsv(d);
  assert.ok(!csv.includes("requested_time_s"));
  assert.deepEqual(parseKitchenCsv(csv), d);
});
test("malformed optional stamp, counter and formula text are rejected", () => {
  ...
  for (const change of [
    (s) => s.replace(",frame-callback,1,1", ",frame-callback,,1"),
    (s) => s.replace(",frame-callback,1,1", ",declared-rate,1,1"),
    (s) => s.replace(",frame-callback,1,1", ",frame-callback,1,2001"),
    (s) => s.replace("requested_time_s", "arbitrary_payload"),
  ])
    assert.throws(() => parseKitchenCsv(change(csv)));
});
test("capture provenance cannot attach conflicting evidence to the same acquired frame", () => {
  ...
  const altered = {
    ...d,
    points: d.points.map((q) => (q === p ? { ...q, capture: { ...q.capture, frameId: 1 } } : q)),
  };
...
test("skipped compositor counts survive export and withhold interval coverage", () => {
  ...
  const changed = {
    ...
    points: d.points.map((p) =>
      p.time > 0
        ? { ...p, capture: { ...p.capture, presentedFrames: p.capture.presentedFrames + 2 } }
        : p,
    ),
  };
  ...
  assert.equal(
    analysis.outputs.find((o) => o.quantityId === "diffusionInterval").status,
    "not-applicable",
  );
});
...
test("an out-of-order frame is refused against the latest prior observation, not the first", () => {
  ...
  assert.equal(
    out.kind,
    "refused",
    "t=2 after t=3 must be refused; a forward scan compares against t=1 and accepts it",
  );
