...
describe("Waves Reference Physics Evaluator (am-ref-waves-r53)", () => {
  ...
  describe("Light Complex (Einstein §8)", () => {
    test("factors and adversarial countermodel comparison at beta = 0.6", () => {
      ...
      expect(cm0.factor).toBeCloseTo(0.2, 12); // q^2 / gamma = 0.25 / 1.25 = 0.2
      ...
      expect(cmPi.factor).toBeCloseTo(3.2, 12); // 4 / 1.25 = 3.2

      // 3. Ray transverse in moving frame (cos(theta) = beta = 0.6)
      ...
      expect(cmTP.factor).toBeCloseTo(0.512, 12); // 0.64 / 1.25 = 0.512

      // 4. Ray transverse in unprimed frame (theta = 90 deg) - Coincidence check
      ...
      expect(cm90.factor).toBeCloseTo(1.25, 12); // Coincides at 90 deg!
    });
