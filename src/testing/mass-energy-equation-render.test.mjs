...
test("all fifteen mass-energy equations render every selectable node and a single semantic MathML tree", () => {
  ...
  for (const raw of records) {
    ...
    for (const node of compiled.navigation) assert.ok(compiled.html.includes(
      `data-${node.kind === "term" ? "term" : "op"}="${node.id}"`), node.id);
    ...
    for (const term of compiled.terms) assert.deepEqual(term.quantity, MASS_ENERGY_QUANTITIES[term.quantityId]);
...
test("generated payloads remain paper-local and match the actual renderer", async () => {
  ...
  for (const raw of records) assert.deepEqual(mass.equations.find(e => e.id === raw.id), compileEquation(raw));
});

test("review and malicious marker claims fail before KaTeX can publish them", () => {
  for (const change of [e => e.review = "reviewed", e => e.tree.opId += ' onclick="x',
    e => e.title = '<img src=x onerror="x">', e => e.unitSystem = "normalized"]) {
    const raw = structuredClone(records[0]); change(raw);
