...
test("SSR retains all four explanations, both owner-generated worked tables, and real links", () => {
  ...
  try {
    ...
    const tables = Array.from(
      container.querySelectorAll("[data-clock-worked] [data-reflection-reading]"),
    );
    expect(tables.map((cell) => cell.textContent)).toEqual(["5", "25"]);
    expect(container.textContent).toContain("Nothing tells us yet");
    expect(container.textContent).toContain(
      "not a measurement of the separate one-way travel times",
    );
    expect(
      container.querySelector("[data-clock-accepted] [data-reflection-reading]")?.textContent,
    ).toBe("Not assigned before an agreement");
    expect(container.querySelector(".entrance-bridge")?.textContent).toContain(
      record.bridge.newSkill,
    );
    expect(container.querySelectorAll('.entrance-bridge a[href^="/"]').length).toBe(3);
    expect(
      Array.from(container.querySelectorAll("fieldset")).every((field) => field.disabled),
    ).toBe(true);
  } finally {
...
test("agreeing assigns the distant time; withdrawing the agreement does not invent a measured time", async () => {
  ...
  try {
    await act(async () => {
      root.render(<ClockFirstEncounter record={record} />);
    });
    ...
    expect(container.querySelector("[data-clock-announcement]")?.textContent).toContain(
      "under the equal-travel-time agreement",
    );
    ...
    expect(
      container.querySelectorAll("[data-clock-worked] [data-reflection-reading]")[0]?.textContent,
    ).toBe("5");
  } finally {
    await act(async () => {
      root.unmount();
...
test("two independently mounted encounters do not share agreement or draft state", async () => {
  ...
  try {
    await act(async () => {
      root.render(
        <>
          <ClockFirstEncounter record={record} />
          <ClockFirstEncounter record={record} />
        </>,
      );
    });
    ...
    const assignments = container.querySelectorAll(
      "[data-clock-accepted] [data-reflection-reading]",
    );
...
test("the actual standalone lab consumes the entrance URL and the embedded lab stays independent", async () => {
  ...
  try {
    await act(async () => {
      root.render(
        <>
          <ClockSyncLab restoreFromLocation />
          <ClockSyncLab />
        </>,
      );
    });
...
test("a malformed shared link preserves the actual prepared laboratory ledger", async () => {
  ...
  try {
    await act(async () => {
      root.render(<ClockSyncLab restoreFromLocation />);
    });
