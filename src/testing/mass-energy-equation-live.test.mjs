...
import {
  createMe02Session,
  packPrintedConversion,
  snapshotOutputs,
} from "../experiments/me02/session.ts";
...
const term = (quantityId) => ({
  termId: `eq-model-me-live.t.${quantityId}`,
  quantityId,
  quantity: MASS_ENERGY_QUANTITIES[quantityId],
  scale: { num: 1, den: 1 },
});

for (const energyUnit of ["joule", "erg"])
  test(`${energyUnit} inputs attach the real SI result and retain analytic meaning`, () => {
    const f = fixture(`live-${energyUnit}`, {
      energyUnit,
      emittedEnergy: energyUnit === "erg" ? 2e7 : 2,
    });
    ...
    assert.equal(drop.kind, "value");
    assert.equal(drop.text, "0.5");
    assert.equal(drop.unit, "J");
    assert.equal(drop.ownerId, "massEnergy.exactDifference");
    const limit = readTermValue(term("inertialMassDecrease"), initial);
    assert.equal(limit.kind, "status");
    assert.match(limit.text, /coefficient|limit/i);
    assert.equal(limit.ownerId, "massEnergy.limitingCoefficient");
    const signed = readTermValue(term("massChangeSigned"), initial);
    assert.equal(signed.kind, "value");
    assert.ok(Number(signed.text) < 0);
    assert.equal(signed.unit, "kg");
    assert.equal(
      f.session.apply({ ...f.session.acceptedParameters(), beta: 0.01 }).kind,
      "accepted",
    );
...
test("normalized results cannot masquerade as SI despite legacy output contract labels", () => {
  const f = fixture();
  assert.equal(
    f.slot().view.accepted.outputs.find((o) => o.quantityId === "kineticEnergyDifference").unit,
    "J",
  );
  for (const id of [
    ...
    assert.equal(value.kind, "symbolic");
    assert.equal(value.ownerId, null);
  ...
  for (const energyUnit of ["normalized", "joule"]) {
    ...
    s.view = {
      ...
      requested: {
        ...s.view.accepted,
        parameters: {
          ...s.view.accepted.parameters,
          energyUnit: energyUnit === "joule" ? "normalized" : "joule",
        },
      },
    };
    assert.equal(
      readTermValue(term("kineticEnergyDifference"), s).kind,
      energyUnit === "joule" ? "value" : "symbolic",
    );
...
test("zero speed keeps zero drop, inapplicable proxy and analytic coefficient separate", () => {
  ...
  assert.equal(drop.kind, "value");
  assert.equal(drop.text, "0");
  const proxy = readTermValue(term("finiteSpeedMassProxy"), f.slot());
  assert.equal(proxy.kind, "status");
  assert.match(proxy.text, /zero|0\/0|v = 0/i);
...
test("missing provenance and mismatched units or meanings never earn substitutions", () => {
  ...
  for (const patch of [{ unit: "erg" }, { semanticKind: "quadratic-kinetic-energy" }]) {
    ...
    slot.view = {
      ...
      accepted: {
        ...slot.view.accepted,
        outputs: slot.view.accepted.outputs.map((o) =>
