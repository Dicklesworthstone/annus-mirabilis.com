...
import { decodeFacsimileAvailability, FACSIMILE_WIRE_BYTES, facsimileMapPath, readFacsimileResponse } from "../reader/facsimile/wire.ts";
...
const document = projectFacsimileDocument("mass-energy", "ap-18-639", {
  ...
  units: [{ id: "s0-p6", kind: "paragraph", locators: [{ page: 640 }, { page: 641 }], destination: { editionBlockId: "de-example" } }],
});
const envelope = () => JSON.parse(JSON.stringify({ schemaVersion: 1, kind: "available", document }));
...
test("publication refusal crosses the same transport without leaking a source URL", () => {
  const result = decode({ schemaVersion: 1, kind: "unavailable", code: "facsimile-not-published", message: "Not admitted." });
...
test("schema, paper identity, local URL and hash cannot be substituted by a response", () => {
  const mutations = [x => x.schemaVersion = 2, x => x.kind = "maybe", x => x.document.paperId = "light-quanta",
    x => x.document.pdfUrl = "https://example.org/wrong.pdf", x => x.document.pdfUrl = "/private.pdf",
    x => x.document.key = "../secret", x => x.document.sha256 = "bad",
    x => x.document.originUrl = "javascript:alert(1)", x => x.document.originUrl = "https://user:secret@example.org/",
    x => x.document.inventoryStatus = {}, x => x.document.acquisitionDate = "yesterday"];
  for (const mutate of mutations) { const value = envelope(); mutate(value); assert.throws(() => decode(value)); }
});
test("noncontiguous, repeated, fractional and out-of-bounds page mappings are rejected", () => {
  const mutations = [x => x.pages = [], x => x.pages[0].pdfPage = 0, x => x.pages[1].printedPage = 700,
    x => x.units[0].pdfPages = [0], x => x.units[0].pdfPages = [2, 2], x => x.units[0].pdfPages = [3, 2],
    x => x.units[0].pdfPages = [2.5], x => x.units[0].pdfPages = [4], x => x.units[0].pdfPages = []];
  for (const mutate of mutations) { const value = envelope(); mutate(value.document); assert.throws(() => decode(value)); }
});
test("invalid unit identities and aliases cannot manufacture source anchors", () => {
  const mutations = [x => x.units[0].id = "bad id", x => x.units.push(x.units[0]),
    x => x.units[0].section = "../s0", x => x.units[0].aliases = ["facsimile-page-639"],
    x => x.units[0].aliases = Array(33).fill("id"), x => x.units[0].id = "facsimile-page-640"];
  for (const mutate of mutations) { const value = envelope(); mutate(value.document); assert.throws(() => decode(value)); }
});
test("source-map URL construction cannot escape the paper route", () => {
  ...
  for (const id of ["../private", "a?b", "a#b", "https://example.org"]) assert.throws(() => facsimileMapPath(id));
});
...
test("HTML fallbacks, failed requests and corrupt JSON remain transport failures", async () => {
  for (const response of [new Response("<html>404</html>"), new Response("{}", { status: 404 }),
...
test("oversized streamed bodies are cancelled even without a Content-Length header", async () => {
  ...
  const response = new Response(new ReadableStream({
    start(controller) { controller.enqueue(new Uint8Array(FACSIMILE_WIRE_BYTES + 1)); },
