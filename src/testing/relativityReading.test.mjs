...
import { alignedBoost, composeCollinear, contractedLength, dilatedInterval, dilationLossPerSecond, inverseBoost, transformEvent, transformVelocity } from "../physics/reference/kinematics.ts";
import { fieldInvariants, transformChargeCurrent, transformSI } from "../physics/reference/fields.ts";
import { aberration, dopplerFactor, lightComplexFactors, movingMirror } from "../physics/reference/waves.ts";
import { kineticEnergy, longitudinalMass, transverseMassComoving, transverseMassLaboratory } from "../physics/reference/electron.ts";
...
const approx = (actual, expected) => assert.ok(Number.isFinite(actual) && Math.abs(actual - expected) <= 1e-12 * Math.max(1, Math.abs(expected)), `${actual} != ${expected}`);
function value(result) { assert.equal(result.status, "value"); return result.value; }
...
for (const [name, compile] of [["synchronous", compileReadingContent], ["production", compileContent]]) {
  test(`${name}: both halves compile across all ten sections with all thirteen laboratories`, async () => {
    ...
    assert.deepEqual(payload.paper.sections.map(s => s.id), Array.from({ length: 11 }, (_, i) => `s${i}`));
    ...
    assert.deepEqual([...new Set(payload.arguments.flatMap(a => a.experiments))].sort(), Array.from({ length: 13 }, (_, i) => `sr-${String(i + 1).padStart(2, "0")}`));
    ...
    for (let section = 6; section <= 10; section++) {
      assert.ok(payload.arguments.some(a => a.section === `s${section}` && a.readings.full.some(b => b.kind === "formula")));
    }
  });
  for (const [label, change] of [
    ["invented instrument", a => ({ ...a, experiments: ["sr-99"] })],
    ["missing premise", a => ({ ...a, prerequisites: [{ id: "arg-sr-missing", edge: "premise" }] })],
    ["self-justifying field law", a => ({ ...a, prerequisites: [{ id: a.id, edge: "premise" }] })],
...
test("the normalized worked event and inverse agree with the actual Lorentz owner", () => {
  ...
  approx(transformed.t, 1.75); approx(transformed.x, -0.25);
...
test("the clock example keeps exact loss separate from the second-order form and Earth refusal", () => {
  ...
  approx(loss.exact, 0.2); approx(loss.printedSecondOrder, 0.18);
...
test("two forward speeds compose to 15/17 while axial and transverse light retain unit speed", () => {
  ...
  approx(transverse.ux, -0.6); approx(transverse.uy, 0.8);
...
test("field signs, inverse, and invariant checks agree without equating raw components", () => {
  ...
  approx(transformed.E.y, -0.75); approx(transformed.B.z, 1.25);
  const recovered = transformSI({ ...transformed, boost: -0.6, c: 1 });
  approx(recovered.E.y, 0); approx(recovered.B.z, 1);
  const a = fieldInvariants(input.E, input.B, 1);
  const b = fieldInvariants(transformed.E, transformed.B, 1);
  approx(a.e2MinusC2B2, b.e2MinusC2B2); approx(a.eDotB, b.eDotB);
});
...
test("Doppler and packet examples distinguish transverse frames and material contraction", () => {
  ...
  approx(factors.volumeFactor, 2); approx(factors.energyFactor, 0.5);
...
test("mirror examples separate frequency, intercepted power, pressure and no interception", () => {
  ...
  approx(mirror.frequencyRatio, 0.25); approx(mirror.incidentPower, 0.4);
  approx(mirror.radiationPressure, 0.5); approx(mirror.energyBalanceResidual, 0);
...
test("neutral current transforms without dividing by zero density", () => {
  ...
  approx(transformed.rho, -0.75); approx(transformed.J.x, 1.25);
  const restored = transformChargeCurrent({ rho: transformed.rho, J: transformed.J, boost: -0.6, c: 1 });
  approx(restored.rho, 0); approx(restored.J.x, 1);
});
...
test("every existing relativity lab's section link lands in the new reader", async () => {
  ...
  const sections = new Set(result.papers.find(p => p.paper.id === "special-relativity").paper.sections.map(s => s.id));
  for (let i = 1; i <= 13; i++) {
    const source = await readFile(new URL(`../app/lab/sr-${String(i).padStart(2, "0")}/page.tsx`, import.meta.url), "utf8");
    for (const match of source.matchAll(/\/papers\/special-relativity\/(?:#(s\d+)|(s\d+)\/)/g)) assert.ok(sections.has(match[1] ?? match[2]), match[0]);
  }
});
