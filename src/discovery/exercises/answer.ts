...
export function snapshotExercise(part: ExpressionExercisePart): ExpressionExercisePart {
  ...
  if (!Array.isArray(rawNames) || rawNames.length > 9 ||
      rawNames.some((n) => typeof n !== "string" || !/^[A-Za-z][A-Za-z0-9_]{0,47}$/.test(n) ||
  ...
  if (variables.some((n) => !declaredNames.includes(n)) ||
  ...
  const domains = Object.freeze(Object.fromEntries(variables.map((name) => {
    const domain = rawDomains[name]!;
    return [name, Object.freeze({ min: domain.min, max: domain.max, scale: domain.scale ?? "linear" })];
  })));
  ...
  if (Reflect.ownKeys(rawTolerance).some((k) => typeof k !== "string" ||
      !["absolute", "relative", "relativeTo"].includes(k) || !fields[k]?.enumerable ||
      !Object.hasOwn(fields[k]!, "value"))) throw new TypeError("Invalid exercise tolerance field.");
  if ((fields.absolute && typeof fields.absolute.value !== "number") ||
  ...
  if (!["reference", "larger"].includes(tolerance.relativeTo!) || validateToleranceSpec(tolerance, 1).length)
    throw new TypeError("The exercise tolerance is invalid.");
  const reference = parse(referenceSource, new Set(declaredNames));
  if (!reference.ok) throw new TypeError(`The reference expression is invalid: ${reference.message}`);
  return Object.freeze({ id, prompt, workedExplanation, referenceSource, declaredNames, domains, tolerance });
}

export function exerciseDefinitionKey(part: ExpressionExercisePart): string {
  const p = snapshotExercise(part);
  return JSON.stringify(["annus-exercise-sampling-v1", p.id, p.referenceSource,
    [...p.declaredNames].sort(), p.domains, p.tolerance]);
}
...
export async function deriveExerciseSeed(identity: string): Promise<string> {
  ...
  if (!globalThis.crypto?.subtle) throw new Error("Secure browser hashing is unavailable; no answer verdict was produced.");
...
export async function checkExerciseAnswer(part: ExpressionExercisePart, rawInput: string): Promise<AnswerVerdict> {
  try {
    const p = snapshotExercise(part);
    if (typeof rawInput !== "string" || rawInput.length > 200)
      return { kind: "parse-error", position: 200, message: "Use an expression of at most 200 characters." };
    const normalized = normalize(rawInput);
    if (!normalized.ok) return { kind: "parse-error", position: normalized.position, message: normalized.message };
    const reader = parse(normalized.text, new Set(p.declaredNames));
    if (!reader.ok) return { kind: "parse-error", position: reader.position, message: reader.message };
    ...
    return { kind: "checked", outcome: checkEquivalence(reader.expr, reference.expr, p.domains, p.tolerance, { seed }) };
  } catch (error) {
    return { kind: "checked", outcome: { status: "could-not-compare",
