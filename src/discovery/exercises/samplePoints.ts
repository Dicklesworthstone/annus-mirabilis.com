...
export function validateDomains(domains: Readonly<Record<string, Domain>>): readonly string[] {
  ...
  for (const name of names) {
    ...
    if (!Object.hasOwn(descriptor, "value") || !/^[A-Za-z][A-Za-z0-9_]{0,47}$/.test(name) ||
    ...
    if (Reflect.ownKeys(domain).some((k) => typeof k !== "string" ||
        !["min", "max", "scale"].includes(k) || !d[k]?.enumerable || !Object.hasOwn(d[k]!, "value")))
      throw new TypeError(`Unexpected field or accessor in ${name}'s domain.`);
    const min: unknown = d.min?.value, max: unknown = d.max?.value, scale: unknown = d.scale?.value;
    if (typeof min !== "number" || typeof max !== "number" ||
        !Number.isFinite(min) || !Number.isFinite(max) || min >= max ||
        (scale !== undefined && scale !== "linear" && scale !== "log") ||
        (scale === "log" && min <= 0))
      throw new RangeError(`${name} needs finite increasing bounds; logarithmic bounds must be positive.`);
  }
  return names;
}
...
export function mapUnitToDomain(u: number, domain: Domain): number {
  if (!Number.isFinite(u) || u < 0 || u > 1) throw new RangeError("Expected a unit-interval sample.");
  ...
  if (!Number.isFinite(value)) throw new RangeError("The domain cannot be sampled at this numeric scale.");
...
export function haltonPoints(domains: Readonly<Record<string, Domain>>, count: number): readonly SamplePoint[] {
  const names = validateDomains(domains);
  checkCount(count);
  return Object.freeze(Array.from({ length: count }, (_, i) => Object.freeze(Object.fromEntries(
    names.map((name, j) => [name, mapUnitToDomain(haltonValue(i + 1, HALTON_BASES[j]!), domains[name]!)]),
  ))));
}

export function philoxPoints(domains: Readonly<Record<string, Domain>>, count: number,
  seed: string | bigint = "0"): readonly SamplePoint[] {
  const names = [...validateDomains(domains)].sort();
  checkCount(count);
  const streams = names.map((_, tile) => createPhiloxStream({ seed, kernel: EXERCISE_STREAM.kernel, tile }));
  ...
  return Object.freeze(Array.from({ length: count }, () => Object.freeze(Object.fromEntries(
...
export function boundaryPoints(domains: Readonly<Record<string, Domain>>): readonly SamplePoint[] {
  const names = validateDomains(domains);
  const middle = Object.fromEntries(names.map((name) => [name, mapUnitToDomain(0.5, domains[name]!)]));
  return Object.freeze([Object.freeze(middle), ...names.flatMap((name) => [
