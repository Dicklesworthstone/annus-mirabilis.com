import {
  createImportGuard, decodeNotebook, emptyNotebook, encodeNotebook, recordObservation,
  recordPrediction, type DiscoveryNotebook, type NoteManifest,
} from "./notebook.ts";
...
export function createNotebookSession(manifest: NoteManifest, storage: NotebookStoragePort) {
  const server: NotebookView = Object.freeze({ document: emptyNotebook(manifest), ready: false,
    blocked: false, message: "Notes become available after this page loads.", retainedRaw: null, resetSequence: 0 });
  let view = server;
  ...
  function load() {
    ...
    if (expectedRaw !== null) {
      try {
        publish({ document: decodeNotebook(expectedRaw, manifest), ready: true, message: "Loaded this guide’s recorded notes." });
      } catch (error) {
        publish({ ready: true, blocked: true, retainedRaw: expectedRaw,
          message: `${error instanceof Error ? error.message : "Stored notes could not be read."} The original stored text was preserved.` });
      }
    } else publish({ ready: true, blocked: raw.status === "corrupt", message: raw.status === "unavailable"
      ? "Device storage is unavailable. Record notes in this tab and export them before leaving."
      : raw.status === "corrupt" ? "Storage reports corrupt data. Nothing has been overwritten."
  ...
  function unchanged(): boolean {
    ...
    if (raw.status === "corrupt" || (value !== expectedRaw && value !== attemptedRaw)) {
      imports.invalidate();
      publish({ blocked: true, retainedRaw: value, message:
  ...
  function persist(doc: DiscoveryNotebook, reset: boolean): void {
    ...
    try { result = storage.write(raw); } catch { result = { status: "unavailable" }; }
    if (result.status === "ok") expectedRaw = raw;
    publish({ document: doc, resetSequence: view.resetSequence + (reset ? 1 : 0),
      message: result.status === "ok" ? "Recorded notes saved on this device. Draft fields are not saved."
