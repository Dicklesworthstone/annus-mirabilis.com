...
export function encodeLightInvestigationSettings(input: LightInvestigationParameters, sourceDigest: string): string {
  const p = validateLightInvestigation(input);
  if (!DIGEST.test(sourceDigest)) throw new ExperimentRuntimeError("source-digest-invalid", "The shared calculation needs its source digest.");
  const q = new URLSearchParams({ investigation: "1", model: LIGHT_INVESTIGATION_MODEL, source: sourceDigest });
...
export function decodeLightInvestigationSettings(search: string): SharedLightInvestigation {
  const invalid = (): SharedLightInvestigation => ({ kind: "invalid", message: "This investigation link is incomplete or unsupported. The worked result is unchanged." });
  ...
  if (REQUIRED.some(key => q.getAll(key).length !== 1) ||
      [...q.keys()].some(key => !REQUIRED.includes(key)) ||
      q.get("investigation") !== "1" || q.get("model") !== LIGHT_INVESTIGATION_MODEL)
    return invalid();
  try {
    const sourceDigest = q.get("source") ?? "";
    if (!DIGEST.test(sourceDigest)) return invalid();
    const parameters = validateLightInvestigation(Object.fromEntries(KEYS.map(key =>
      [key, parseScaledDecimal(q.get(key) ?? "", 0)])));
...
function evidenceSnapshot(snapshot: AcceptedSnapshot) {
  const fail = (): never => { throw new ExperimentRuntimeError("evidence-snapshot-invalid", "Only a complete accepted investigation snapshot can be exported."); };
  if (snapshot.experimentId !== "light-quanta-investigation" || !snapshot.final ||
      !Number.isSafeInteger(snapshot.snapshotVersion) || snapshot.snapshotVersion < 1 ||
      !snapshot.instanceId || !snapshot.runId || snapshot.outputs.length !== Object.keys(LIGHT_INVESTIGATION_OUTPUTS).length)
    return fail();
  ...
  const results = snapshot.outputs.map(output => {
    ...
    if (!contract || seen.has(result.quantityId) || result.ownerId !== contract.ownerId ||
        result.unit !== contract.unit || result.semanticKind !== contract.semanticKind ||
        !contract.statuses.includes(result.status) ||
        (result.status === "value" && typeof result.value !== "number")) return fail();
    seen.add(result.quantityId);
    return JSON.parse(encodeResult(result)) as unknown;
  });
  return { instanceId: snapshot.instanceId, runId: snapshot.runId,
    snapshotVersion: snapshot.snapshotVersion, parameters, results };
}

export function exportLightInvestigationEvidence(baseline: AcceptedSnapshot, current: AcceptedSnapshot, sourceDigest: string): string {
  if (!DIGEST.test(sourceDigest)) throw new ExperimentRuntimeError("source-digest-invalid", "The exported calculation needs its source digest.");
  if (baseline.instanceId !== current.instanceId) throw new ExperimentRuntimeError("evidence-instance-mismatch", "Compare snapshots from the same investigation placement.");
  return `${JSON.stringify({ schemaVersion: 1, modelId: LIGHT_INVESTIGATION_MODEL,
    constantSetId: LIGHT_INVESTIGATION_CONSTANTS, sourceDigest,
    evidenceKind: "model-consequence", reviewStatus: "explanatory-preview",
    privateNotesIncluded: false, changedSettings: changedInvestigationInputs(baseline, current),
    baseline: evidenceSnapshot(baseline), current: evidenceSnapshot(current),
  }, null, 2)}\n`;
}
...
export function lightInvestigationCoefficientHref(snapshot: AcceptedSnapshot): string | null {
  ...
  if (!required.every(id => snapshot.outputs.some(r => r.quantityId === id && r.status === "value" && typeof r.value === "number"))) return null;
  ...
  return `/lab/lq-06/${encodeLq06Settings({ ...LQ06_DEFAULTS, radiationEnergy: energy.value,
    frequency: p.frequency, volumeRatio: p.volumeRatio, temperature: p.referenceTemperature,
    gasParticles: p.pointCount, constantSetId: LIGHT_INVESTIGATION_CONSTANTS })}`;
}
