...
export class NotebookError extends Error {
  constructor(message: string) { super(message); this.name = "NotebookError"; }
}
const fail = (message: string): never => { throw new NotebookError(message); };
export function noteManifest(journey: DiscoveryJourney): NoteManifest {
  return Object.freeze({ paper: journey.paper, revision: journey.revision,
    stages: Object.freeze(journey.stages.map((s) => Object.freeze({ id: s.id, title: s.title,
      alternatives: Object.freeze(s.alternatives.map((a) => Object.freeze({ id: a.id, title: a.title }))),
    }))),
  });
}
export function emptyNotebook(manifest: NoteManifest): DiscoveryNotebook {
  return Object.freeze({ kind: "discovery-notes", schemaVersion: 1, paper: manifest.paper,
    journeyRevision: manifest.revision, entries: Object.freeze([]) });
}
function record(input: unknown, fields: readonly string[]): Record<string, unknown> {
  if (!input || typeof input !== "object" || Array.isArray(input) ||
      ![Object.prototype, null].includes(Object.getPrototypeOf(input))) return fail("Expected a notes record.");
  const descriptors = Object.getOwnPropertyDescriptors(input);
  if (Reflect.ownKeys(input).length !== fields.length) fail("Unexpected or missing notes fields.");
  for (const key of fields) {
    const d = descriptors[key];
    if (!d?.enumerable || !Object.hasOwn(d, "value")) fail("Notes must contain plain data, not accessors.");
  }
  return input as Record<string, unknown>;
}
function array(input: unknown, maximum: number): readonly unknown[] {
  if (!Array.isArray(input) || input.length > maximum) return fail("Notes exceed the entry budget.");
  const copy = [];
  for (let i = 0; i < input.length; i++) {
    const d = Object.getOwnPropertyDescriptor(input, i);
    if (!d || !Object.hasOwn(d, "value")) return fail("Sparse notes and accessors are not admitted.");
    copy.push(d.value);
  }
  return copy;
}
function text(input: unknown): string {
  if (typeof input !== "string" || input.length > NOTE_TEXT_LIMIT ||
      /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(input))
...
function stage(manifest: NoteManifest, stageId: string) {
  return manifest.stages.find((s) => s.id === stageId) ?? fail("This step does not belong to this guide.");
}
...
export function validateNotebook(input: unknown, manifest: NoteManifest): DiscoveryNotebook {
  ...
  if (r.journeyRevision !== manifest.revision)
    fail("These notes use a different guide revision. Export them before replacing them; they were not relabeled.");
  ...
  const entries = array(r.entries, manifest.stages.length).map((raw): NoteEntry => {
    ...
    const attempts = array(e.attempts, NOTE_ATTEMPT_LIMIT).map((raw): NoteAttempt => {
      ...
      if (!prediction.trim() && a.alternativeId === null) fail("Record a prediction or a tentative alternative first.");
      return Object.freeze({ prediction, alternativeId: a.alternativeId as string | null, observation: text(a.observation) });
    });
  ...
  entries.sort((a, b) => manifest.stages.findIndex((s) => s.id === a.stageId) -
    manifest.stages.findIndex((s) => s.id === b.stageId));
  const result: DiscoveryNotebook = Object.freeze({ ...emptyNotebook(manifest), entries: Object.freeze(entries) });
...
export function decodeNotebook(raw: string, manifest: NoteManifest): DiscoveryNotebook {
  if (typeof raw !== "string" || raw.length > NOTE_FILE_LIMIT ||
      new TextEncoder().encode(raw).byteLength > NOTE_FILE_LIMIT) fail("The notes file is too large.");
  let value: unknown;
  try { value = JSON.parse(raw); } catch { return fail("The notes file is not valid JSON."); }
...
export function recordPrediction(doc: DiscoveryNotebook, manifest: NoteManifest,
  stageId: string, prediction: string, alternativeId: string | null): DiscoveryNotebook {
  ...
  if ((previous?.attempts.length ?? 0) >= NOTE_ATTEMPT_LIMIT)
    fail(`This step already has ${NOTE_ATTEMPT_LIMIT} attempts. Export your notes before starting over.`);
  const entry = { stageId, attempts: [...(previous?.attempts ?? []), { prediction, alternativeId, observation: "" }] };
  return validateNotebook({ ...checked, entries: [...checked.entries.filter((e) => e.stageId !== stageId), entry] }, manifest);
}
export function recordObservation(doc: DiscoveryNotebook, manifest: NoteManifest,
  stageId: string, attemptIndex: number, observation: string): DiscoveryNotebook {
  const checked = validateNotebook(doc, manifest);
  ...
  return validateNotebook({ ...checked, entries: checked.entries.map((e) => e.stageId !== stageId ? e : {
    ...e, attempts: e.attempts.map((a, i) => i !== attemptIndex ? a : { ...a, observation }),
  }) }, manifest);
}
