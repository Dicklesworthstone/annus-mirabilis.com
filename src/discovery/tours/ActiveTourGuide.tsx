...
export default function ActiveTourGuide({
  pathname,
  search,
  compact = false,
}: {
  ...
  function leave() {
    ...
    window.history.replaceState(
      ...
      "",
      leaveTourHref(url.pathname, url.search, url.hash),
    );
    setDismissed(true);
  }
  ...
  if (decoded.kind === "invalid") {
    ...
    return (
      <aside
        className="guided-tour-guide"
        aria-label="Guided reading link needs attention"
        data-tour-error
      >
  ...
  if (!tourMatchesPath(position, pathname)) {
    ...
    return (
      <aside
        className="guided-tour-guide"
        aria-label="Return to your guided reading stop"
        data-tour-detour
      >
  ...
  if (compact) {
    return (
      <nav
        className="guided-tour-guide guided-tour-actions"
        aria-label="Continue guided reading after this page"
      >
        <span>
          Stop {index + 1} of {tour.stops.length}: {stop.title}
        </span>
        {previous && <a href={tourDestination(previous)}>Previous stop</a>}
        {next ? (
          <a href={tourDestination(next)}>Next stop</a>
        ) : (
          <a href={`/tours/${tour.id}/#tour-finish`}>Finish this route</a>
        )}
  ...
  async function copyPosition() {
    ...
    const unavailable =
      "Copying is unavailable. Use the bookmark link below, or copy its address from your browser.";
    // No clipboard is not a refusal: the reader is told what to do instead, with no throw.
    ...
    try {
      ...
      setNotice(
        "Copied this reading position. It contains no notes, answers or experiment settings.",
      );
    } catch {
      setNotice(unavailable);
    }
  ...
  return (
    <aside
      ...
      data-guided-tour={tour.id}
      data-tour-stop={stop.id}
    >
      <p className="eyebrow">
        Guided reading · Stop {index + 1} of {tour.stops.length}
      </p>
      ...
      <details>
        ...
        <div className="guided-tour-actions">
          ...
          <button type="button" onClick={copyPosition}>
            Copy reading-position link
          </button>
          {first && (
            <a href={tourDestination(tourPosition(tour, first))}>Restart from the first stop</a>
          )}
          <button type="button" onClick={leave}>
            Leave tour, keep this page
          </button>
        </div>
